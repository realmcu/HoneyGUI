...
Use :cpp:any:`gui_simple_img_create` to create a simple image widget. If ``src_mode_type`` is ``IMG_SRC_MEMADDR``, the image will be loaded from a memory address; if ``src_mode_type`` is ``IMG_SRC_FILESYS``, the image will be loaded from the file system, with ``addr`` being the path to the image. Refer to the following enumeration for loading methods:
...
If you need to update the location of image widget, use :cpp:any:`gui_simple_img_set_location` to relocate.
``x, y`` is the new coordinate.

Set Attribute
~~~~~~~~~~~~~~~~~~~~~~

You can use :cpp:any:`gui_simple_img_set_attribute` to set the attribute of an image widget, replace it with a new image and set a new coordinate.
