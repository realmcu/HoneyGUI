...
字体大小
^^^^^^^^
开发者可以使用函数 :cpp:any:`gui_text_size_set` 设置字体的大小。对于使用 FreeType 的矢量字体，可以分别设置宽度和高度；对于点阵字体，仅高度参数作为字号生效。
...
Emoji
^^^^^
开发者可以使用函数 :cpp:any:`gui_text_emoji_set` 设置 Emoji 图像文件的路径和大小。需要配合文件系统使用。
...
裁剪区域
^^^^^^^^
开发者可以使用函数 :cpp:any:`gui_text_set_scope` 为文本控件设置裁剪区域。只有裁剪矩形内的文本部分会被绘制，坐标相对于文本控件自身的左上角。

此外，可以使用 :cpp:any:`gui_text_set_scope_absolute` 以屏幕绝对坐标设置裁剪区域。
