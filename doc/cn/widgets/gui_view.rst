...
视图控件 (View) 是一种切换更为便利的容器控件，通过事件响应可以实时创建任一视图控件并可选择多种切换效果，不切换控件时内存只存在当前显示的视图控件，可以有效降低内存消耗。
...
设置视图切换事件
~~~~~~~~~~~~~~~~~~
开发者可以使用 :cpp:any:`gui_view_add_change_event` 来设置视图切换事件。具体的切换风格请参考下列枚举：
