...
作为一套综合显示框架，HoneyGUI 不仅包含 Realtek 自主研发的显示引擎，还支持直接调用如 LVGL、ARM2D 等外部 API 进行应用程序的开发。此外，HoneyGUI 提供基于 PC 的仿真环境，使开发者能够快速进行应用开发和调试，而无需依赖嵌入式硬件平台。同时，HoneyGUI 还可以与 Realtek 自研的前端设计工具 `RVD <GUI-APP/SaaA/index.rst#RVD>`_ 配合使用，实现可视化编程。
...
- 前端开发方式，包含 JavaScript 和 XML。推荐使用 ``RVisualDesigner`` 作为 `PC <Glossary.rst#term-PC>`_ 设计器进行低代码开发。

`GUI <Glossary.rst#term-GUI>`_ 框架具有很强的可移植性，可以在多种芯片和 `OS <Glossary.rst#term-OS>`_ 上运行。此次提供了 PC Windows 版本。
