...
#include "app_cluster_launcher.h"
...
gui_app_t app_cluster_launcher =
{
    .screen =
    {
        .name = "app_cluster_launcher",
        .x    = 0,
        .y    = 0,
    },
    .thread_entry = app_cluster_launcher_update_thread,
    .ui_design = app_cluster_launcher_ui_design,
};

gui_app_t *get_app_cluster_launcher(void)
{
    return &app_cluster_launcher;
}
...
void app_cluster_launcher_update_thread(void *this)
{
    while (1)
    {
        app_cluster_auto_refresh_data_demo();
...
void app_cluster_launcher_ui_design(gui_app_t *app)
{
    ...
    app_cluster_create_main_display(win_main_display);
    app_cluster_create_connected_display(win_connected_display);
    win_main_display->base.not_show = false;
    win_connected_display->base.not_show = true;
    app_cluster_initialize_data();
}
