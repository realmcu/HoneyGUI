...
void design_win_menu_phone_pairing(void *parent)
{
    ...
    text_phone_pairing_title = gui_text_create(parent, "text_phone_pairing_title", 172, 24, 160,
                                               font_size);
    gui_text_set(text_phone_pairing_title, "连接新手机", GUI_FONT_SOURCE_BMP, APP_COLOR_WHITE, 15,
                 font_size);
    ...
    text_notification = gui_text_create(parent, "text_btext_notificationuds_device", 104, 120, 256,
                                        font_size);
    gui_text_set(text_notification, "已可以连接新手机", GUI_FONT_SOURCE_BMP, APP_COLOR_WHITE,
                 24,
                 font_size);
    ...
    text_watch_device_name = gui_text_create(parent, "text_watch_device_name", 131, 206, 192,
                                             font_size);
    gui_text_set(text_watch_device_name, "手表设备名称", GUI_FONT_SOURCE_BMP, APP_COLOR_WHITE, 18,
                 font_size);
    ...
    text_watch_device_addr = gui_text_create(parent, "text_watch_device_addr", 160, 302, 134,
                                             font_size);
    gui_text_set(text_watch_device_addr, "MAC地址", GUI_FONT_SOURCE_BMP, APP_COLOR_WHITE, 9,
                 font_size);
