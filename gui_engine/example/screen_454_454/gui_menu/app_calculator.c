...
static void calculator_draw_button(void *parent)
{
    ...
    int16_t offset_x = 50; int16_t offset_y = 110;
    ...
    text = "0";
    int font_size = CAL_TEXT_FONT_SIZE_DISPLAY;
    text_display_up = gui_text_create(parent, "txt", 0, offset_y / 2 - 33,
                                      offset_x + del_x * 3 + CAL_TEXT_FONT_SIZE_DISPLAY, //20
                                      font_size);
    text_display_dn = gui_text_create(parent, "txt", 0, offset_y / 2 - 9,
                                      offset_x + del_x * 3 + CAL_TEXT_FONT_SIZE_DISPLAY, //20
                                      font_size);
    ...
    for (i = 0; i < 21; i++)
    {
        ...
        // btn text config
        GUI_TYPE(gui_button_t, button[i])->text->font_height = CAL_TEXT_FONT_SIZE_DISPLAY; //24
        ...
        // setup btn callback
        gui_button_click(button[i], (gui_event_cb_t)cb_btn[i], NULL); // revised
    }
