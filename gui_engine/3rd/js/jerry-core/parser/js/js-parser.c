...
parser_copy_identifiers(parser_context_t *context_p)  /**< context */
{
    ...
    while ((literal_p = (lexer_literal_t *) parser_list_iterator_next(&literal_iterator)))
    {
        ...
        while ((parent_literal_p = (lexer_literal_t *) parser_list_iterator_next(
                                       &parent_literal_iterator)) != NULL)
        {
                        ...
                        uint16_t *const_literal_end) /**< end of the const literal group */
{
    ...
    while ((literal_p = (lexer_literal_t *) parser_list_iterator_next(&literal_iterator)))
    {
        ...
        if (literal_p->type != LEXER_IDENT_LITERAL)
        {
            ...
            JERRY_ASSERT((literal_p->status_flags & ~(LEXER_FLAG_FUNCTION_ARGUMENT | LEXER_FLAG_SOURCE_PTR)) ==
                         0);
                                                                ...
                                                                *   encoded in one byte */
{
    ...
    {
        const uint8_t expected_status_flags = LEXER_FLAG_VAR | LEXER_FLAG_NO_REG_STORE |
...
parser_post_processing(parser_context_t *context_p)  /**< context */
{
    ...
    if ((size_t) context_p->stack_limit + (size_t) context_p->register_count >
    ...
    literal_length = (size_t)(context_p->literal_count - context_p->register_count) * sizeof(
                         ecma_value_t);
                    ...
                    parser_error_location_t *error_location_p) /**< error location */
{
    ...
    if (arg_list_p == NULL)
    {
        context.status_flags = PARSER_NO_REG_STORE | PARSER_LEXICAL_ENV_NEEDED |
                    ...
                    ecma_compiled_code_t **bytecode_data_p) /**< [out] JS bytecode */
{
    ...
    *bytecode_data_p = parser_parse_source(arg_list_p,
    ...
    if (!*bytecode_data_p)
    {
        ...
        const lit_utf8_byte_t *err_bytes_p = (const lit_utf8_byte_t *) parser_error_to_string(
                                                 parser_error.error);
