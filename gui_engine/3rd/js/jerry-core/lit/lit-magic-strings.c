...
{
    JERRY_ASSERT(JERRY_CONTEXT(lit_magic_string_ex_array) &&
 ...
 */
void
lit_magic_strings_ex_set(const lit_utf8_byte_t *const
                         *ex_str_items, /**< character arrays, representing
                         ...
                         lit_utf8_size_t string_size) /**< string size in bytes */
{
    ...
    while (first < last)
    {
        ...
        int compare = memcmp(lit_get_magic_string_utf8((lit_magic_string_id_t) middle), string_p,
                             string_size);
