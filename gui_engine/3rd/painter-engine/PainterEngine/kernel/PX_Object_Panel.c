...
PX_Object *PX_Object_PanelCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                 px_int width, px_int height)
{
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0, (px_float)width,
                                (px_float)height, 0, PX_OBJECT_TYPE_PANEL, PX_NULL, PX_Object_PanelRender, PX_Object_PanelFree, 0,
                                sizeof(PX_Object_Panel));
    if (pObject == PX_NULL)
    {
        return PX_NULL;
    }
