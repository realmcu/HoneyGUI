...
static px_void PX_LiveFramework_RenderListPixelShaderFaster(px_surface *psurface, px_int x,
                                                            px_int y, px_float z, px_float u, px_float v, px_point normal, px_texture *pTexture,
                                                            PX_TEXTURERENDER_BLEND *blend)
{
    ...
    if (pTexture)
    {
        ...
        PX_SurfaceDrawPixel(psurface, x, y, PX_SURFACECOLOR(pTexture, (px_int)(u * resWidth),
                                                            (px_int)(v * resHeight)));
    }
}
static px_void PX_LiveFramework_RenderListPixelShader(px_surface *psurface, px_int x, px_int y,
                                                      px_float z, px_float u, px_float v, px_point normal, px_texture *pTexture,
                                                      PX_TEXTURERENDER_BLEND *blend)
{
    //texture mapping
    ...
    if (pTexture)
    {
        ...
        PX_SurfaceDrawPixel(psurface, x, y, PX_COLOR((px_uchar)mixa, (px_uchar)mixr, (px_uchar)mixg,
                                                     (px_uchar)mixb));

    }
}
static px_void PX_LiveFramework_RenderListRasterization(px_surface *psurface,
                                                        PX_LiveFramework *pLiveFramework, PX_LiveRenderVertex p0, PX_LiveRenderVertex p1,
                                                        PX_LiveRenderVertex p2, px_texture *ptexture, PX_TEXTURERENDER_BLEND *blend)
{
    px_int   ix, iy;
    ...
    for (y = (px_int)(y0 + 0.5f) + 0.5f; y <= midy; y++)
    {
        ...
        for (ix = (px_int)(xleft + 0.5); ix < (px_int)(xright + 0.5f); ++ix)
        {
            ...
            {
                PX_LiveFramework_RenderListPixelShader(psurface, ix, iy, originalZ, s, t, p0.normal, ptexture,
                                                       blend);
            }
...
px_bool PX_LiveFrameworkInitialize(px_memorypool *mp, PX_LiveFramework *plive, px_int width,
                                   px_int height)
{
...
static px_void PX_LiveFrameworkUpdateLayerInterpolation(PX_LiveFramework *plive,
                                                        PX_LiveLayer *pLayer)
{
    px_float schedule;
    px_int i;
    ...
    {
        //stretch
        pLayer->rel_currentStretch = pLayer->rel_beginStretch + (pLayer->rel_endStretch -
                                                                 pLayer->rel_beginStretch) * schedule;
    }
    else
    {
        //keypoint translation
        pLayer->rel_currentTranslation.x = pLayer->rel_beginTranslation.x + (pLayer->rel_endTranslation.x -
                                                                             pLayer->rel_beginTranslation.x) * schedule;
        pLayer->rel_currentTranslation.y = pLayer->rel_beginTranslation.y + (pLayer->rel_endTranslation.y -
                                                                             pLayer->rel_beginTranslation.y) * schedule;
        pLayer->rel_currentTranslation.z = 0;
    }

    //rotation
    pLayer->rel_currentRotationAngle = pLayer->rel_beginRotationAngle + (pLayer->rel_endRotationAngle -
                                                                         pLayer->rel_beginRotationAngle) * schedule;
...
static px_void PX_LiveFramework_UpdateLayerKeyPoint(PX_LiveFramework *pLive, PX_LiveLayer *pLayer)
{
    ...
    if (pLayer->parent_index != -1)
    {
        ...
        pLayer->currentKeyPoint = PX_PointAdd(v, PX_LiveFrameworkGetLayerParent(pLive,
                                                                                pLayer)->currentKeyPoint);
    }
    else
    {
    ...
    for (i = 0; i < PX_COUNTOF(pLayer->child_index); i++)
    {
        ...
        {
            PX_LiveFramework_UpdateLayerKeyPoint(pLive, PX_LiveFrameworkGetLayerChild(pLive, pLayer,
                                                                                      pLayer->child_index[i]));
        }
        else
        {
            break;
...
static px_void PX_LiveFramework_UpdateLayerVertices(PX_LiveFramework *pLive, PX_LiveLayer *pLayer,
                                                    px_dword elapsed)
{
    px_int i;
    ...
    else
    {
        ...
        for (i = 0; i < PX_COUNTOF(pLayer->child_index); i++)
        {
            ...
            v = PX_PointNormalization(PX_PointSub(pLayer->currentKeyPoint, PX_LiveFrameworkGetLayerChild(pLive,
                                                  pLayer, pLayer->child_index[i])->currentKeyPoint));
    ...
    for (i = 0; i < pLayer->vertices.size; i++)
    {
        ...
        {
            if (plv->sourcePosition.x > pLayer->panc_x &&
                plv->sourcePosition.x < pLayer->panc_x + pLayer->panc_width)
            {
                if (plv->sourcePosition.x < pLayer->panc_sx)
                {
                    px_float disx = plv->sourcePosition.x - pLayer->panc_x;
                    resultPosition.x += disx * (pLayer->panc_currentx - pLayer->panc_x) /
                                        (pLayer->panc_sx - pLayer->panc_x) - disx;
                }
                else
                {
                    px_float disx = (pLayer->panc_x + pLayer->panc_width) - plv->sourcePosition.x;
                    resultPosition.x -= disx * (pLayer->panc_x + pLayer->panc_width - pLayer->panc_currentx) /
                                        (pLayer->panc_x + pLayer->panc_width - pLayer->panc_sx) - disx;
                }
            }

            if (plv->sourcePosition.y > pLayer->panc_y &&
                plv->sourcePosition.y < pLayer->panc_y + pLayer->panc_height)
            {
                if (plv->sourcePosition.y < pLayer->panc_sy)
                {
                    px_float disy = plv->sourcePosition.y - pLayer->panc_y;
                    resultPosition.y += disy * (pLayer->panc_currenty - pLayer->panc_y) /
                                        (pLayer->panc_sy - pLayer->panc_y) - disy;
                }
                else
                {
                    px_float disy = (pLayer->panc_y + pLayer->panc_height) - plv->sourcePosition.y;
                    resultPosition.y -= disy * (pLayer->panc_y + pLayer->panc_height - pLayer->panc_currenty) /
                                        (pLayer->panc_y + pLayer->panc_height - pLayer->panc_sy) - disy;
                }
            }
            ...
            for (j = 0; j < PX_COUNTOF(pLayer->child_index); j++)
            {
                ...
                if (pChild->rel_currentStretch != 1)
                {
                    ...
                    if (cos_v12 > 0)
                    {
                        ...
                        resultPosition = PX_PointAdd(resultPosition, PX_PointMul(u1,
                                                                                 distance * (pChild->rel_currentStretch - 1)));
                    }
                }
            }
        }
        ...
        else
        {
            ...
            while (updateelapsed)
            {
                ...
                do
                {
                    ...
                    {
                        //resistance
                        _cos = PX_Point2DDot(velocity,
                                             keyDirection) / PX_Point2DMod(velocity) / PX_Point2DMod(keyDirection);
                        ...
                        if (velocity_vx.x > 10000 || velocity_vx.y > 10000 || velocity_vy.x > 10000 ||
                            velocity_vy.y > 10000)
                        {
                            PX_ASSERT();
                        }
                    ...
                    plv->currentPosition = PX_PointAdd(plv->currentPosition, PX_PointMul(plv->velocity,
                                                                                         atomelapsed / 1000.f));
...
static px_bool PX_LiveFrameworkExecuteInstr(PX_LiveFramework *plive, px_int animation_index,
                                            px_int frameindex)
{
    //execute instr
    ...
    {
        PX_LiveLayer *pLayer;
        PX_LiveAnimationFramePayload *pPayload = (PX_LiveAnimationFramePayload *)(
                                                     pFrameInstrData + frame_offset);
        ...
        {
            px_int j;
            px_point *pVertexTranslation = (px_point *)(pFrameInstrData + frame_offset + sizeof(
                                                            PX_LiveAnimationFramePayload));

            for (j = 0; j < (px_int)pPayload->translationVerticesCount; j++)
            {
                PX_VECTORAT(PX_LiveVertex, &pLayer->vertices, j)->beginTranslation = PX_VECTORAT(PX_LiveVertex,
                                                                                     &pLayer->vertices, j)->endTranslation;
                PX_VECTORAT(PX_LiveVertex, &pLayer->vertices, j)->currentTranslation = PX_VECTORAT(PX_LiveVertex,
                                                                                       &pLayer->vertices, j)->beginTranslation;
...
static px_void PX_LiveFrameworkRenderLayer(px_surface *psurface, PX_LiveFramework *plive,
                                           PX_LiveLayer *pLayer, px_int x, px_int y, px_dword elapsed)
{
    PX_TEXTURERENDER_BLEND blend;
        ...
        if (pLayer->triangles.size && pLayer->vertices.size)
        {
            ...
            if (pLayer->showMesh)
            {
                for (t = 0; t < pLayer->triangles.size; t++)
                {
                    ...
                    PX_GeoDrawLine(psurface, (px_int)v0.position.x, (px_int)v0.position.y, (px_int)v1.position.x,
                                   (px_int)v1.position.y, 1, PX_COLOR(128, 255, 0, 0));
                    PX_GeoDrawLine(psurface, (px_int)v0.position.x, (px_int)v0.position.y, (px_int)v2.position.x,
                                   (px_int)v2.position.y, 1, PX_COLOR(128, 255, 0, 0));
                    PX_GeoDrawLine(psurface, (px_int)v1.position.x, (px_int)v1.position.y, (px_int)v2.position.x,
                                   (px_int)v2.position.y, 1, PX_COLOR(128, 255, 0, 0));

                    PX_GeoDrawSolidCircle(psurface, (px_int)v0.position.x, (px_int)v0.position.y, 3, PX_COLOR(128, 255,
                                          0, 0));
                    PX_GeoDrawSolidCircle(psurface, (px_int)v1.position.x, (px_int)v1.position.y, 3, PX_COLOR(128, 255,
                                          0, 0));
                    PX_GeoDrawSolidCircle(psurface, (px_int)v2.position.x, (px_int)v2.position.y, 3, PX_COLOR(128, 255,
                                          0, 0));
                }
                ...
                if (plive->currentEditLayerIndex >= 0 && plive->currentEditLayerIndex < plive->layers.size)
                {
                    if (pLayer == PX_VECTORAT(PX_LiveLayer, &plive->layers, plive->currentEditLayerIndex))
                    {
                        ...
                        {
                            PX_LiveVertex *pLiveVertex = PX_VECTORAT(PX_LiveVertex, &pLayer->vertices,
                                                                     plive->currentEditVertexIndex);
                            PX_GeoDrawCircle(psurface, (px_int)(pLiveVertex->currentPosition.x + x),
                                             (px_int)(pLiveVertex->currentPosition.y + y), 5, 1, PX_COLOR(255, 255, 128, 0));
                        }
                    }
                }
...
px_void PX_LiveFrameworkRender(px_surface *psurface, PX_LiveFramework *plive, px_int x, px_int y,
                               PX_ALIGN refPoint, px_dword elapsed)
{
    ...
    if (plive->showKeypoint && !plive->showlinker)
    {
        for (i = 0; i < plive->layers.size; i++)
        {
            ...
            if (pLayer->parent_index != -1)
            {
                PX_GeoDrawSolidCircle(psurface, (px_int)(pLayer->currentKeyPoint.x + plive->refer_x),
                                      (px_int)(pLayer->currentKeyPoint.y + plive->refer_y), 3, PX_COLOR(255, 0, 64, 192));
                PX_GeoDrawCircle(psurface, (px_int)(pLayer->currentKeyPoint.x + plive->refer_x),
                                 (px_int)(pLayer->currentKeyPoint.y + plive->refer_y), 5, 1, PX_COLOR(255, 0, 64, 192));
            }
            else
            {
                PX_GeoDrawSolidCircle(psurface, (px_int)(pLayer->currentKeyPoint.x + plive->refer_x),
                                      (px_int)(pLayer->currentKeyPoint.y + plive->refer_y), 3, PX_COLOR(255, 255, 0, 0));
                PX_GeoDrawCircle(psurface, (px_int)(pLayer->currentKeyPoint.x + plive->refer_x),
                                 (px_int)(pLayer->currentKeyPoint.y + plive->refer_y), 5, 1, PX_COLOR(255, 255, 0, 0));
            }
        ...
        for (i = 0; i < plive->layers.size; i++)
        {
            ...
            for (j = 0; j < PX_LIVE_LAYER_MAX_LINK_NODE; j++)
            {
                ...
                PX_GeoDrawArrow(psurface,
                                ...
                                PX_POINT2D(pLinkLayer->currentKeyPoint.x + plive->refer_x,
                                           pLinkLayer->currentKeyPoint.y + plive->refer_y),
                                1,
    ...
    if (plive->showRootHelperLine)
    {
        ...
        {
            PX_GeoDrawLine(psurface, (px_int)(x + pLayer->keyPoint.x), (px_int)(y + pLayer->keyPoint.y),
                           (px_int)(x + pLayer->currentKeyPoint.x), (px_int)(y + pLayer->currentKeyPoint.y), 1, PX_COLOR(255,
                                   255, 0, 255));
        }
    }
...
px_void PX_LiveFrameworkRenderRefer(px_surface *psurface, PX_LiveFramework *plive,
                                    PX_ALIGN refPoint, px_dword elapsed)
{
    PX_LiveFrameworkRender(psurface, plive, (px_int)plive->refer_x, (px_int)plive->refer_y, refPoint,
                           elapsed);
}
...
PX_LiveLayer *PX_LiveFrameworkCreateLayer(PX_LiveFramework *plive,
                                          const px_char id[PX_LIVEFRAMEWORK_MAX_SUPPORT_LAYER])
{
    PX_LiveLayer layer;
    px_int i;
...
PX_LiveLayer *PX_LiveFrameworkGetLayerChild(PX_LiveFramework *plive, PX_LiveLayer *pLayer,
                                            px_int childIndex)
{
...
px_bool PX_LiveFrameworkLinkLayerTexture(PX_LiveFramework *plive, const px_char layer_id[],
                                         const px_char texture_id[])
{
...
px_void PX_LiveFrameworkUpdateLayerSourceVerticesUV(PX_LiveFramework *plive, PX_LiveLayer *pLayer)
{
    ...
    for (i = 0; i < pLayer->vertices.size; i++)
    {
        ...
        {
            PX_LiveTexture *pLiveTexture = PX_VECTORAT(PX_LiveTexture, &plive->livetextures,
                                                       pLayer->LinkTextureIndex);
            pVertex->u = (pVertex->sourcePosition.x - pLiveTexture->textureOffsetX) * 1.0f /
                         pLiveTexture->Texture.width;
            pVertex->v = (pVertex->sourcePosition.y - pLiveTexture->textureOffsetY) * 1.0f /
                         pLiveTexture->Texture.height;
        }
    }
}
...
px_bool PX_LiveFrameworkLinkLayerSearchSubLayer(PX_LiveFramework *plive, PX_LiveLayer *pLayer,
                                                PX_LiveLayer *pSearchLayer)
{
    px_int i;
...
px_void PX_LiveFrameworkLinkLayer(PX_LiveFramework *plive, PX_LiveLayer *pLayer,
                                  PX_LiveLayer *linkLayer)
{
    px_int i;
    if (pLayer == linkLayer)
    {
...
px_void PX_LiveFrameworkDeleteLiveAnimationFrame(PX_LiveFramework *plive,
                                                 PX_LiveAnimation *pliveAnimation, px_int index)
{
...
px_void PX_LiveFrameworkDeleteLiveAnimationFrameByIndex(PX_LiveFramework *plive,
                                                        px_int AnimationIndex, px_int frameIndex)
{
    if (AnimationIndex >= 0 && AnimationIndex < plive->liveAnimations.size)
    {
        PX_LiveAnimation *pAnimation = PX_VECTORAT(PX_LiveAnimation, &plive->liveAnimations,
                                                   AnimationIndex);
...
{
    if (plive->currentEditAnimationIndex >= 0 &&
        plive->currentEditAnimationIndex < plive->liveAnimations.size)
    {
        PX_LiveAnimation *pAnimation = PX_VECTORAT(PX_LiveAnimation, &plive->liveAnimations,
                                                   plive->currentEditAnimationIndex);
        if (plive->currentEditFrameIndex >= 0 &&
            plive->currentEditFrameIndex < pAnimation->framesMemPtr.size)
        {
...
PX_LiveAnimationFramePayload *PX_LiveFrameworkGetCurrentEditAnimationFramePayloadIndex(
    PX_LiveFramework *plive, px_int payloadIndex)
{
    px_int boffset = 0;
...
PX_LiveAnimationFramePayload *PX_LiveFrameworkGetCurrentEditAnimationFramePayload(
    PX_LiveFramework *plive)
{
    return PX_LiveFrameworkGetCurrentEditAnimationFramePayloadIndex(plive,
                                                                    plive->currentEditLayerIndex);
}
...
PX_LiveAnimationFrameHeader *PX_LiveFrameworkGetCurrentEditAnimationFrame(PX_LiveFramework *plive)
{
    ...
    if (pAnimation)
    {
        if (plive->currentEditFrameIndex >= 0 &&
            plive->currentEditFrameIndex < pAnimation->framesMemPtr.size)
        {
            return (PX_LiveAnimationFrameHeader *)(*PX_VECTORAT(px_void *, &pAnimation->framesMemPtr,
                                                                plive->currentEditFrameIndex));
        }
    }
    return PX_NULL;
}
...
PX_LiveVertex *PX_LiveFrameworkGetCurrentEditLiveVertex(PX_LiveFramework *plive)
{
    ...
    if (currentEditLayer)
    {
        if (plive->currentEditVertexIndex >= 0 &&
            plive->currentEditVertexIndex < currentEditLayer->vertices.size)
        {
...
px_point *PX_LiveFrameworkGetCurrentEditAnimationFramePayloadVertex(PX_LiveFramework *plive)
{
    ...
    {
        if (plive->currentEditVertexIndex >= 0 &&
            plive->currentEditVertexIndex < (px_int)ppayload->translationVerticesCount)
        {
            return (px_point *)(pPayloadByte + sizeof(PX_LiveAnimationFramePayload) +
                                plive->currentEditVertexIndex * sizeof(px_point));
        }
    }
    return PX_NULL;
}
...
px_bool PX_LiveFrameworkExport(PX_LiveFramework *plive, px_memory *exportbuffer)
{
    ...
    do
    {
        ...
        for (i = 0; i < plive->layers.size; i++)
        {
            ...
            do
            {
                if (!PX_MemoryCat(exportbuffer, pLayer->triangles.data,
                                  pLayer->triangles.nodesize * pLayer->triangles.size)) { return PX_FALSE; }
            }
            while (0);
            ...
            do
            {
                if (!PX_MemoryCat(exportbuffer, pLayer->vertices.data,
                                  pLayer->vertices.nodesize * pLayer->vertices.size)) { return PX_FALSE; }
            }
            while (0);
        }
    }
...
px_bool PX_LiveFrameworkImport(px_memorypool *mp, PX_LiveFramework *plive, px_void *buffer,
                               px_int size)
{
    ...
    do
    {
        ...
        rOffset += 24;
        if (rOffset > size) { return PX_FALSE; }
    ...
    do
    {
        ...
        PX_LiveFrameworkBaseAttributes *pReadLiveFrameworkAttributes = (PX_LiveFrameworkBaseAttributes *)(
                                                                           bBuffer + rOffset);
        rOffset += sizeof(PX_LiveFrameworkBaseAttributes);
        if (rOffset > size) { return PX_FALSE; }
        ...
        //////////////////////////////////////////////////////////////////////////
        plive->mp = mp;
        if (!PX_VectorInitialize(mp, &plive->layers, sizeof(PX_LiveLayer),
                                 pReadLiveFrameworkAttributes->layerCount)) { return PX_FALSE; }
        plive->layers.size = pReadLiveFrameworkAttributes->layerCount;

        if (!PX_VectorInitialize(mp, &plive->livetextures, sizeof(PX_LiveTexture),
                                 pReadLiveFrameworkAttributes->textureCount)) { return PX_FALSE; }
        plive->livetextures.size = pReadLiveFrameworkAttributes->textureCount;

        if (!PX_VectorInitialize(mp, &plive->liveAnimations, sizeof(PX_LiveAnimation),
                                 pReadLiveFrameworkAttributes->animationCount)) { return PX_FALSE; }
    ...
    do
    {
        ...
        for (i = 0; i < plive->livetextures.size; i++)
        {
            ...
            do
            {
                ...
                if (!PX_TextureCreate(mp, &pTexture->Texture, pLiveTextureImportInfo->width,
                                      pLiveTextureImportInfo->height))
                {
                    goto _ERROR;
                }
                ...
                rOffset += sizeof(PX_LiveTextureImportInfo);
                if (rOffset > size)
                {
                    goto _ERROR;
                }
                ...
                rOffset += pTexture->Texture.width * pTexture->Texture.height * sizeof(px_color);
                if (rOffset > size)
                {
                    goto _ERROR;
                }
    ...
    do
    {
        ...
        {
            PX_LiveLayer *pLayer = PX_VECTORAT(PX_LiveLayer, &plive->layers, i);
            PX_LiveFramework_LayerExportInfo *pReadLayer = (PX_LiveFramework_LayerExportInfo *)(
                                                               bBuffer + rOffset);
            rOffset += sizeof(PX_LiveFramework_LayerExportInfo);
            if (rOffset > size) { goto _ERROR; }
            ...
            if (!PX_VectorInitialize(mp, &pLayer->triangles, sizeof(PX_Delaunay_Triangle),
                                     pReadLayer->triangleCount))
            {
                goto _ERROR;
            }
            ...
            do
            {
                PX_memcpy(pLayer->triangles.data, bBuffer + rOffset,
                          pLayer->triangles.nodesize * pLayer->triangles.size);
                rOffset += pLayer->triangles.nodesize * pLayer->triangles.size;
                if (rOffset > size) { goto _ERROR; }
            }
            while (0);
            ...
            do
            {
                PX_memcpy(pLayer->vertices.data, bBuffer + rOffset,
                          pLayer->vertices.nodesize * pLayer->vertices.size);
                rOffset += pLayer->vertices.nodesize * pLayer->vertices.size;
                if (rOffset > size) { goto _ERROR; }
            }
            while (0);
        }
    ...
    do
    {
        ...
        for (i = 0; i < plive->liveAnimations.size; i++)
        {
            ...
            do
            {
                PX_LiveAnimationImportInfo *pLiveAnimationImport = (PX_LiveAnimationImportInfo *)(
                                                                       bBuffer + rOffset);
                rOffset += sizeof(PX_LiveAnimationImportInfo);
                if (rOffset > size) { goto _ERROR; }

                PX_memcpy(pAnimation->id, pLiveAnimationImport->id, sizeof(pAnimation->id));

                if (!PX_VectorInitialize(mp, &pAnimation->framesMemPtr, sizeof(px_void *),
                                         pLiveAnimationImport->size)) { goto _ERROR; }
...
px_void PX_LiveRender(px_surface *psurface, PX_Live *plive, px_int x, px_int y, PX_ALIGN refPoint,
                      px_dword elapsed)
{
