...
px_bool PX_ExecuterVM_CreateThread(PX_VM *Ins, px_void *userptr)
{
    ...
    if (!PX_VMBeginThreadFunction(&pExecute->vm, PX_VMGetFreeThreadId(Ins), PX_VM_STACK(Ins,
...
px_bool PX_Object_ExecuterRunPayload(PX_Object *pObject, const px_byte bin[], px_int binsize)
{
    ...
    PX_VMRegistHostFunction(&pExecute->vm, "createthread", PX_ExecuterVM_CreateThread,
                            pObject);//createthread
...
px_bool PX_ExecuterRegistryHostFunction(PX_Object *pObject, const px_char Name[],
                                        PX_VM_Function_Modules function, px_void *userptr)
{
    if (pObject->Type != PX_OBJECT_TYPE_EXECUTER)
    {
        PX_ASSERT();
...
PX_Object *PX_Object_ExecuterCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                    px_int width, px_int height, PX_FontModule *fm, const px_char shellcode[])
{
    PX_Object *pObject;
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, 0, 0, 0, (px_float)width, (px_float)height, 0,
                                PX_OBJECT_TYPE_EXECUTER, PX_Object_ExecuterUpdate, 0, PX_Object_ExecuterFree, &desc, sizeof(desc));
...
PX_Object *PX_Object_ExecuterCreatePayload(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                           px_int width, px_int height, PX_FontModule *fm, const px_byte bin[], px_int binsize)
{
    PX_Object *pObject;
    PX_Object_Executer desc, * pdesc;
    //console initialize
