...
px_bool PX_JsonInterpret_Value(PX_Json *pjson, px_lexer *lexer, PX_Json_Value *_value,
                               px_bool arrayElement);
                               ...
                               px_bool arrayElement)
{
    ...
    else
    {
        if (type == PX_LEXER_LEXEME_TYPE_TOKEN)
        {
            ...
            {
                if (lexer->CurLexeme.buffer[0] == '0' && (lexer->CurLexeme.buffer[1] == 'x' ||
                                                          lexer->CurLexeme.buffer[1] == 'X'))
                {
                    _value->type = PX_JSON_VALUE_TYPE_NUMBER;
...
px_bool PX_JsonCreateStringValue(px_memorypool *mp, PX_Json_Value *pValue, const px_char name[],
                                 const px_char text[])
{
    PX_memset(pValue, 0, sizeof(PX_Json_Value));
...
px_bool PX_JsonCreateNumberValue(px_memorypool *mp, PX_Json_Value *pValue, const px_char name[],
                                 px_double value)
{
    PX_memset(pValue, 0, sizeof(PX_Json_Value));
...
px_bool PX_JsonCreateBooleanValue(px_memorypool *mp, PX_Json_Value *pValue, const px_char name[],
                                  px_bool b)
{
    PX_memset(pValue, 0, sizeof(PX_Json_Value));
...
px_bool PX_JsonAddString(PX_Json *pjson, const px_char parent_payload[], const px_char name[],
                         const px_char text[])
{
...
px_bool PX_JsonAddNumber(PX_Json *pjson, const px_char parent_payload[], const px_char name[],
                         const px_double number)
{
    PX_Json_Value *pValue = PX_JsonGetValue(pjson, parent_payload);
    PX_Json_Value newValue;
    if (!pValue)
...
px_bool PX_JsonAddBoolean(PX_Json *pjson, const px_char parent_payload[], const px_char name[],
                          const px_bool b)
{
    PX_Json_Value *pValue = PX_JsonGetValue(pjson, parent_payload);
    PX_Json_Value newValue;
