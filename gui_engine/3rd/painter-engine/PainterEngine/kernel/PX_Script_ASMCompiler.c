...
static px_void PX_ScriptAsmRegistInstr(PX_SCRIPT_ASM_COMPILER *compiler,
                                       const px_char mnemonic[__PX_SCRIPT_ASM_INSTR_MNEMONIC_NAME_LEN], px_char opcode, px_int accType[],
                                       px_char paramcout)
{
    px_int i = 0;
...
px_int PX_ScriptAsmTokenType(PX_SCRIPT_ASM_COMPILER *compiler, px_char *token)
{
    ...
    while (--matchIteration)
    {
        ...
        {
            if (PX_strequ(((PX_SCRIPT_ASM_ASSUME_NODE *)(PX_ListNodeAt(&compiler->assumeTable,
                                                                       i)->pdata))->name.buffer, token))
            {
                token = ((PX_SCRIPT_ASM_ASSUME_NODE *)(PX_ListNodeAt(&compiler->assumeTable,
                                                                     i)->pdata))->assume.buffer;
                bfound = PX_TRUE;
                break;
            }
        }
    ...
    {
        if (PX_strequ(((PX_SCRIPT_ASM_LABEL_NODE *)(PX_ListNodeAt(&compiler->LabelTable,
                                                                  i)->pdata))->mnemonic, token))
        {
...
static px_bool PX_ScriptASMMapAssume(PX_SCRIPT_ASM_COMPILER *compiler, px_string *mnenonic)
{
    ...
    while (--matchIteration)
    {
        ...
        {
            if (PX_strequ(((PX_SCRIPT_ASM_ASSUME_NODE *)(PX_ListNodeAt(&compiler->assumeTable,
                                                                       i)->pdata))->name.buffer, mnenonic->buffer))
            {

                PX_StringCopy(mnenonic, &(((PX_SCRIPT_ASM_ASSUME_NODE *)PX_ListNodeAt(&compiler->assumeTable,
                                                                                      i)->pdata)->assume));
...
static px_bool PX_ScriptAsmLocalType(PX_SCRIPT_ASM_COMPILER *compiler, px_char *Token,
                                     px_char *type, px_dword *index)
{
    ...
    if (PX_memequ(TempTokenString.buffer, "GLOBAL[", sizeof("GLOBAL[") - 1) &&
        TempTokenString.buffer[PX_strlen(TempTokenString.buffer) - 1] == ']')
    {
    ...
    if (PX_memequ(TempTokenString.buffer, "LOCAL[", sizeof("LOCAL[") - 1) &&
        TempTokenString.buffer[PX_strlen(TempTokenString.buffer) - 1] == ']')
    {
        PX_StringTrimLeft(&TempTokenString, PX_strlen("LOCAL["));
        PX_StringTrimRight(&TempTokenString, 1);
...
static px_bool PX_ScriptAsmGlobalType(PX_SCRIPT_ASM_COMPILER *compiler, px_char *Token,
                                      px_char *type, px_dword *index)
{
    px_string TempTokenString;
    PX_StringInitialize(compiler->mp, &TempTokenString);
    ...
    if (PX_memequ(TempTokenString.buffer, "BP+", sizeof("BP+") - 1) &&
        PX_ScriptAsm_isInt(TempTokenString.buffer + sizeof("BP+") - 1))
    {
    ...
    if (PX_memequ(TempTokenString.buffer, "SP+", sizeof("SP+") - 1) &&
        PX_ScriptAsm_isInt(TempTokenString.buffer + sizeof("SP+") - 1))
    {
...
px_void PX_ScriptAsmUpdateAssumeTable(PX_SCRIPT_ASM_COMPILER *compiler, px_char *name,
                                      px_char *assume)
{
    px_int i;
...
px_bool PX_ScriptAsmAddSourceLabel(PX_SCRIPT_ASM_COMPILER *compiler, px_char *Name,
                                   px_bool bFunction)
{
    px_int i;
...
px_int PX_ScriptAsmStringSize(PX_SCRIPT_ASM_COMPILER *compiler)
{
    ...
    {
        size += (PX_strlen(((PX_SCRIPT_ASM_STRING_NODE *)(PX_ListNodeAt(&compiler->StringTable,
                                                                        i)->pdata))->str.buffer)) + 1;
    }
    return size;
}
...
px_void PX_ScriptAsmAddStringConst(PX_SCRIPT_ASM_COMPILER *compiler, px_char *Str)
{
    ...
    for (i = 0; i < compiler->StringTable.size; i++)
    {
        if (PX_strequ(((PX_SCRIPT_ASM_STRING_NODE *)(PX_ListNodeAt(&compiler->StringTable,
                                                                   i)->pdata))->str.buffer, Str))
        {
            return;
        }
    }
    ...
    {
        pLastNode = (PX_SCRIPT_ASM_STRING_NODE *)(PX_ListNodeAt(&compiler->StringTable,
                                                                compiler->StringTable.size - 1)->pdata);
...
px_bool PX_ScriptAsmAddMemoryConst(PX_SCRIPT_ASM_COMPILER *compiler, px_char *StringMap)
{
    ...
    {
        pLastNode = PX_LISTAT(PX_SCRIPT_ASM_MEMORY_NODE, &compiler->memoriesTable,
                              compiler->memoriesTable.size - 1);
...
px_dword PX_ScriptAsmGetMemoryAddr(PX_SCRIPT_ASM_COMPILER *compiler, px_char *StringMap)
{
    ...
    for (i = 0; i < compiler->memoriesTable.size; i++)
    {
        ...
        if (PX_memequ(PX_LISTAT(PX_SCRIPT_ASM_MEMORY_NODE, &compiler->memoriesTable, i)->mem.buffer,
                      memoryNode.mem.buffer, memoryNode.mem.usedsize))
        {
        ...
        case PX_LEXER_LEXEME_TYPE_TOKEN:
            {
                ...
                if (PX_strequ("ASSUME", pLexeme))
                {
                    ...
                    if ((type = PX_ScriptAsmNexLexeme(&compiler->lexer)) != PX_LEXER_LEXEME_TYPE_NEWLINE &&
                        type != PX_LEXER_LEXEME_TYPE_END)
                    {
                    ...
                    if (PX_strequ(pinstr->mnemonic, pLexeme))
                    {
                        ...
                        for (j = 0; j < pinstr->paramCount; j++)
                        {
                            ...
                            else
                            {
                                ...
                                if (PX_ScriptAsmTokenType(compiler,
                                                          compiler->lexer.CurLexeme.buffer) == PX_SCRIPT_ASM_OPERAND_ACC_TYPE_UNKNOW)
                                {
                                ...
                                if (PX_ScriptAsmTokenType(compiler,
                                                          compiler->lexer.CurLexeme.buffer) == PX_SCRIPT_ASM_OPERAND_ACC_TYPE_STRING)
                                {
                                ...
                                if (PX_ScriptAsmTokenType(compiler,
                                                          compiler->lexer.CurLexeme.buffer) == PX_SCRIPT_ASM_OPERAND_ACC_TYPE_MEMORY)
                                {
                                    PX_StringInitialize(compiler->mp, &includeStr);
                                    PX_LexerGetIncludedString(&compiler->lexer, &includeStr);
                                ...
                                if (PX_ScriptAsmTokenType(compiler,
                                                          compiler->lexer.CurLexeme.buffer) == PX_SCRIPT_ASM_OPERAND_ACC_TYPE_HOST)
                                {
        ...
        case PX_LEXER_LEXEME_TYPE_TOKEN:
            {
                ...
                if (PX_strequ("ASSUME", pLexeme))
                {
                    ...
                    if ((type = PX_ScriptAsmNexLexeme(&pCompiler->lexer)) != PX_LEXER_LEXEME_TYPE_NEWLINE &&
                        type != PX_LEXER_LEXEME_TYPE_END)
                    {
                ...
                for (i = 0; i < pCompiler->GrammarInstrTable.size; i++)
                {
                    ...
                    if (PX_strequ(pinstr->mnemonic, pLexeme))
                    {
                        ...
                        for (j = 0; j < pinstr->paramCount; j++)
                        {
                            ...
                            {
                                if (((TokenType = PX_ScriptAsmTokenType(pCompiler,
                                                                        ParamMne.buffer))&pinstr->operandAccTypes[j]) == 0)
                                {
                                    PX_LexerSetState(state);
                                    continue;
                                }
...
px_void PX_ScriptAsmLink(PX_SCRIPT_ASM_COMPILER *compiler)
{
    ...
    for (i = 0; i < compiler->StreamTable.size; i++)
    {
        for (j = 0; j < 3; j++)
        {
            ...
            {
                pbin[i].param[j] = PX_LISTAT(PX_SCRIPT_ASM_LABEL_NODE, &compiler->LabelTable,
                                             pbin[i].param[j])->binaddr;
...
static px_void PX_ScriptAsmRegistInstr_0(PX_SCRIPT_ASM_COMPILER *compiler,
                                         const px_char mnemonic[__PX_SCRIPT_ASM_INSTR_MNEMONIC_NAME_LEN], px_char opcode)
{
    PX_ScriptAsmRegistInstr(compiler, mnemonic, opcode, PX_NULL, 0);
}

static px_void PX_ScriptAsmRegistInstr_1(PX_SCRIPT_ASM_COMPILER *compiler,
                                         const px_char mnemonic[__PX_SCRIPT_ASM_INSTR_MNEMONIC_NAME_LEN], px_char opcode, px_int accType1)
{
    PX_ScriptAsmRegistInstr(compiler, mnemonic, opcode, &accType1, 1);
}

static px_void PX_ScriptAsmRegistInstr_2(PX_SCRIPT_ASM_COMPILER *compiler,
                                         const px_char mnemonic[__PX_SCRIPT_ASM_INSTR_MNEMONIC_NAME_LEN], px_char opcode, px_int accType1,
                                         px_int accType2)
{
    px_int accType[2];
...
static px_void PX_ScriptAsmRegistInstr_3(PX_SCRIPT_ASM_COMPILER *compiler,
                                         const px_char mnemonic[__PX_SCRIPT_ASM_INSTR_MNEMONIC_NAME_LEN], px_char opcode, px_int accType1,
                                         px_int accType2, px_int accType3)
{
    px_int accType[3];
    accType[0] = accType1;
    accType[1] = accType2;
...
px_bool PX_ScriptAsmCompile(px_memorypool *mp, px_char *asmcode, px_memory *binmemory,
                            PX_VM_DebuggerMap *pdebuggermap)
{
    ...
    PX_ScriptAsmRegistInstr_2(&compiler, "MOV", PX_SCRIPT_ASM_INSTR_OPCODE_MOV,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_BP |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_SP | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_IP,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_NUM | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_BP | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_SP |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_IP | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_MEMORY |
    ...
    PX_ScriptAsmRegistInstr_2(&compiler, "ADD", PX_SCRIPT_ASM_INSTR_OPCODE_ADD,
                              ...
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_BP | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_SP |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_IP
                             );
    ...
    PX_ScriptAsmRegistInstr_2(&compiler, "DIV", PX_SCRIPT_ASM_INSTR_OPCODE_DIV,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_NUM | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL
                             );
    ...
    PX_ScriptAsmRegistInstr_2(&compiler, "MOD", PX_SCRIPT_ASM_INSTR_OPCODE_MOD,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_INT | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL
                             );
    ...
    PX_ScriptAsmRegistInstr_2(&compiler, "STRLEN", PX_SCRIPT_ASM_INSTR_OPCODE_STRLEN,
                              ...
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_STRING
                             );
    ...
    PX_ScriptAsmRegistInstr_3(&compiler, "STRCHR", PX_SCRIPT_ASM_INSTR_OPCODE_STRCHR,
                              ...
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_STRING,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_INT
                             );
    ...
    PX_ScriptAsmRegistInstr_2(&compiler, "STRFRF", PX_SCRIPT_ASM_INSTR_OPCODE_STRFRF,
                              ...
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_FLOAT
                             );
    ...
    PX_ScriptAsmRegistInstr_3(&compiler, "MEMBYTE", PX_SCRIPT_ASM_INSTR_OPCODE_MEMBYTE,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_INT,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_MEMORY
                             );
    ...
    PX_ScriptAsmRegistInstr_3(&compiler, "JE", PX_SCRIPT_ASM_INSTR_OPCODE_JE,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_NUM |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_STRING,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_NUM |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_STRING,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LABEL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_INT
                             );
    ...
    PX_ScriptAsmRegistInstr_1(&compiler, "CALL", PX_SCRIPT_ASM_INSTR_OPCODE_CALL,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LABEL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_HOST
                             );

    PX_ScriptAsmRegistInstr_1(&compiler, "PUSH", PX_SCRIPT_ASM_INSTR_OPCODE_PUSH,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_NUM |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LABEL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_STRING |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_BP | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_SP
                             );
    ...
    PX_ScriptAsmRegistInstr_1(&compiler, "POP", PX_SCRIPT_ASM_INSTR_OPCODE_POP,
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_REG | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_LOCAL |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_GLOBAL | PX_SCRIPT_ASM_OPERAND_ACC_TYPE_BP |
                              PX_SCRIPT_ASM_OPERAND_ACC_TYPE_SP
                             );
    ...
    for (i = 0; i < compiler.LabelTable.size; i++)
    {
        if (PX_LISTAT(PX_SCRIPT_ASM_LABEL_NODE, &compiler.LabelTable, i)->bfunction)
        {
            ...
            PX_strcpy(expFunc.name, PX_LISTAT(PX_SCRIPT_ASM_LABEL_NODE, &compiler.LabelTable, i)->mnemonic,
                      sizeof(expFunc.name));
