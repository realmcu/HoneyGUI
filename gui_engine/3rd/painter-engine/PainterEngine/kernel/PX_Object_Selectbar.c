...
static px_void PX_SelectbarOnCursorMove(PX_Object *pObject, px_float x, px_float y)
{
    ...
    if (pSelectbar->activating)
    {
        ...
        if (y < objy + objHeight || index < 0 || index >= pSelectbar->maxDisplayCount ||
            index >= pSelectbar->Items.size)
        {
            return;
        }
        else
        {
            PX_VECTORAT(PX_Object_SelectBar_Item, &pSelectbar->Items,
                        index + pSelectbar->currentDisplayOffsetIndex)->onCursor = PX_TRUE;
        }
    }
    else
    {
...
static px_void PX_SelectbarOnCursorWheel(PX_Object *pObject, px_float z)
{
    ...
    if (pSelectbar->activating)
    {
        if (pSelectbar->Items.size > pSelectbar->maxDisplayCount)
        {
            if (z < 0)
            {
                PX_Object_SliderBarSetValue(pSelectbar->sliderBar,
                                            PX_Object_SliderBarGetValue(pSelectbar->sliderBar) + 1);
            }
            else
            {
                PX_Object_SliderBarSetValue(pSelectbar->sliderBar,
...
static px_void PX_SelectbarOnCursorEvent(PX_Object *pSelectBarObject, PX_Object_Event e,
                                         px_void *ptr)
{
    switch (e.Event)
    {
    ...
    case PX_OBJECT_EVENT_CURSORDRAG:
        {
            PX_SelectbarOnCursorMove(pSelectBarObject, PX_Object_Event_GetCursorX(e),
                                     PX_Object_Event_GetCursorY(e));
        }
        break;
    case PX_OBJECT_EVENT_CURSORDOWN:
        {
            PX_SelectbarOnCursorDown(pSelectBarObject, PX_Object_Event_GetCursorX(e),
                                     PX_Object_Event_GetCursorY(e));
        }
        break;
    case PX_OBJECT_EVENT_CURSORWHEEL:
        {
            ...
            PX_Object_SelectBarSliderOnChanged(pDesc->sliderBar,
                                               PX_OBJECT_BUILD_EVENT(PX_OBJECT_EVENT_VALUECHANGED), pDesc);
        }
        break;
    default:
        break;
...
static px_void PX_SelectbarRender(px_surface *pRenderSurface, PX_Object *pObject, px_dword elpase)
{
    ...
    if (pSelectBar->style == PX_OBJECT_SELECTBAR_STYLE_RECT)
    {
        ...
        {
            PX_GeoDrawRect(pRenderSurface, (px_int)objx, (px_int)objy, (px_int)(objx + objWidth - 1),
                           (px_int)(objy + objHeight - 1), pSelectBar->cursorColor);
        }
        else
        {
            PX_GeoDrawRect(pRenderSurface, (px_int)objx, (px_int)objy, (px_int)(objx + objWidth - 1),
                           (px_int)(objy + objHeight - 1), pSelectBar->backgroundColor);
        }

        PX_GeoDrawBorder(pRenderSurface, (px_int)objx, (px_int)objy, (px_int)(objx + objWidth - 1),
                         (px_int)(objy + objHeight - 1), 1, pSelectBar->borderColor);
    ...
    else if (pSelectBar->style == PX_OBJECT_SELECTBAR_STYLE_ROUNDRECT)
    {
        if (pSelectBar->onCursor)
        {
            PX_GeoDrawSolidRoundRect(pRenderSurface, (px_int)objx, (px_int)objy, (px_int)(objx + objWidth - 1),
                                     (px_int)(objy + objHeight - 1), objHeight / 2, pSelectBar->cursorColor);
        }
        else
        {
            PX_GeoDrawSolidRoundRect(pRenderSurface, (px_int)objx, (px_int)objy, (px_int)(objx + objWidth - 1),
                                     (px_int)(objy + objHeight - 1), objHeight / 2, pSelectBar->backgroundColor);
        }

        PX_GeoDrawRoundRect(pRenderSurface, (px_int)objx, (px_int)objy, (px_int)(objx + objWidth - 1),
                            (px_int)(objy + objHeight - 1), objHeight / 2, 1, pSelectBar->borderColor);
    }
    ...
    do
    {
        ...
        {
            PX_Object_SelectBar_Item *pItem = PX_VECTORAT(PX_Object_SelectBar_Item, &pSelectBar->Items,
                                                          pSelectBar->selectIndex);
            PX_FontModuleDrawText(pRenderSurface, pSelectBar->fontmodule, (px_int)(objx + objHeight / 2 + 1),
                                  (px_int)(objy + objHeight / 2), PX_ALIGN_LEFTMID, pItem->Text, pSelectBar->fontColor);
        }
    }
    while (0);
    ...
    {
        px_int i;
        px_int count = pSelectBar->maxDisplayCount > pSelectBar->Items.size ? pSelectBar->Items.size :
                       pSelectBar->maxDisplayCount;

        for (i = 0; i < count; i++)
        {
            PX_Object_SelectBar_Item *pItem = PX_VECTORAT(PX_Object_SelectBar_Item, &pSelectBar->Items,
                                                          i + pSelectBar->currentDisplayOffsetIndex);
        ...
        if (pSelectBar->Items.size > pSelectBar->maxDisplayCount)
        {
            ...
            PX_Object_SliderBarSetRange(pSelectBar->sliderBar, 0,
                                        pSelectBar->Items.size - pSelectBar->maxDisplayCount);
            PX_Object_SliderBarSetSliderButtonLength(pSelectBar->sliderBar,
                                                     (px_int)(pSelectBar->sliderBar->Height * pSelectBar->maxDisplayCount / pSelectBar->Items.size));
        }
        else
        {
...
PX_Object *PX_Object_SelectBarCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                     px_int width, px_int height, PX_FontModule *fontmodule)
{
    ...
    pObject = PX_ObjectCreate(mp, Parent, (px_float)x, (px_float)y, 0, (px_float)width,
                              (px_float)height, 0);
    if (pObject == PX_NULL)
    {
    ...
    pSelectbar->sliderBar = PX_Object_SliderBarCreate(mp, pObject, 0, 0, 0, 0,
    ...
    PX_ObjectRegisterEvent(pSelectbar->sliderBar, PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_SelectBarSliderOnChanged, pSelectbar);
    return pObject;
}
...
PX_Object *PX_Designer_SelectBarCreate(px_memorypool *mp, PX_Object *pparent, px_float x,
