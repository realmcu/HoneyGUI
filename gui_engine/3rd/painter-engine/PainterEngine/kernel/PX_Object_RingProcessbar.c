...
px_void PX_Object_RingProcessBarRender(px_surface *psurface, PX_Object *pObject, px_uint elapsed)
{
    ...
    PX_GeoDrawRing(psurface, x, y, (px_float)r, (px_float)pDesc->linewidth, pDesc->Color, -90,
                   -90 + (px_int)(360 * process));
    PX_FontModuleDrawText(psurface, pDesc->fm, x, y, PX_ALIGN_CENTER, content, pDesc->Color);

}

PX_Object *PX_Object_RingProcessBarCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                          px_int r, PX_FontModule *fm)
{
    PX_Object *pObject;
    ...
    Desc.Value = 32;
    Desc.fm = fm;
    pObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0, (px_float)0, (px_float)0, 0,
                                PX_OBJECT_TYPE_RINGPROGRESS, PX_NULL, PX_Object_RingProcessBarRender, PX_NULL, &Desc,
