...
px_bool PX_SyncFrameServerInitialize(PX_SyncFrameServer *sync, px_memorypool *mp,
                                     px_dword updateDuration, PX_Linker *linker)
{
    sync->mp = mp;
    sync->linker = linker;
    sync->time = 0;
    ...
    PX_VectorInitialize(mp, &sync->stampsIndexTable, sizeof(PX_SyncFrame_InstrStream_StampIndex),
                        PX_SYNC_INSTRS_SIZE);
...
static px_bool PX_SyncFrameServer_Write(PX_SyncFrameServer *sync_server,
                                        PX_SyncFrame_Server_Clients *pClient, px_void *data, px_uint size)
{
...
static px_bool PX_SyncFrameServer_Read(PX_SyncFrameServer *server,
                                       PX_SyncFrame_Server_Clients **ppclient)
{
...
__RE_RECV:
    ...
    while ((server->recv_cache_size = PX_LinkerRead(server->linker, server->recv_cache_buffer,
                                                    sizeof(server->recv_cache_buffer))) != 0)
    {
        px_int i;
...
static px_void PX_SyncFrameServerHandleQueryDate(PX_SyncFrameServer *sync_server,
                                                 PX_SyncFrame_Server_Clients *pClient)
{
    ...
    PX_memcpy(sync_server->send_cache_buffer + sizeof(PX_Sync_IO_Packet),
              sync_server->data.buffer + offset, size);
    PX_SyncFrameServer_Write(sync_server, pClient, send_packet, sizeof(PX_Sync_IO_Packet) + size);
}

static px_void PX_SyncFrameServerHandle_StatusConnect(PX_SyncFrameServer *sync_server,
                                                      px_dword elapsed)
{
    PX_SyncFrame_Server_Clients *pClient = PX_NULL;
    ...
    while (PX_SyncFrameServer_Read(sync_server, &pClient))
    {
        ...
        else if (recv_packet->type == PX_SYNC_IO_TYPE_QUERYSTATE)
        {
            ...
            for (i = 0; i < sync_server->clients.size; i++)
            {
                PX_SyncFrame_Server_Clients *pEnumClient = PX_VECTORAT(PX_SyncFrame_Server_Clients,
                                                                       &sync_server->clients, i);
...
static px_void PX_SyncFrameServerHandle_StatusProcess(PX_SyncFrameServer *sync_server,
                                                      px_dword updateelapsed)
{
    ...
    do
    {
        ...
        //nop timestamp
        if ((sync_server->time + elapsed) / sync_server->updateDuration >= (px_dword)
            sync_server->stampsIndexTable.size)
        {
        ...
        while (PX_SyncFrameServer_Read(sync_server, &pClient))
        {
            ...
            case PX_SYNC_IO_TYPE_OPCODE:
                {
                    px_int max_size = 0;
                    PX_SyncFrame_InstrStream_StampIndex *plastStamp = PX_VECTORLAST(PX_SyncFrame_InstrStream_StampIndex,
                                                                                    &sync_server->stampsIndexTable);

                    //check size
                    if (sync_server->recv_cache_size < (px_int)((PX_STRUCT_OFFSET(PX_Sync_IO_Packet,
                                                                                  data) + sizeof(px_dword) * 2)))
                    {
                        break;
                    }
        ...
        if (sync_server->stampsIndexTable.size)
        {
            for (i = 0; i < sync_server->clients.size; i++)
            {
                ...
                if (((px_int)pClient->timeIndexOffset >= sync_server->stampsIndexTable.size) &&
                    ((px_int)pClient->timeStreamOffset >= sync_server->stampsInstrStream.usedsize))
                {
                    continue;
                }
                ...
                if (pClient->sendDurationTick >= PX_SYNC_SERVER_SEND_DURATION)
                {
                    ...
                    if (pClient->sendTimes <= 0)
                    {
                        ...
                        reserve_size = sizeof(pClient->send_cache_instr_buffer) - (px_int)PX_STRUCT_OFFSET(
                                           PX_Sync_IO_Packet, data);

                        wstream_size = sync_server->stampsInstrStream.usedsize - pClient->timeStreamOffset;

                        if (wstream_size > reserve_size - sizeof(pClient->timeStreamOffset) - sizeof(
                                wstream_size)) //<--- start offset dword and size dword
                        {
                        ...
                        PX_memcpy(pClient->send_cache_instr_buffer + (sizeof(sync_server->recv_cache_buffer) -
                                                                      reserve_size), &pClient->timeStreamOffset, sizeof(pClient->timeStreamOffset));
                        reserve_size -= sizeof(pClient->timeStreamOffset);
                        //stream size 4bytes
                        PX_memcpy(pClient->send_cache_instr_buffer + (sizeof(sync_server->recv_cache_buffer) -
                                                                      reserve_size), &wstream_size, sizeof(wstream_size));
                        reserve_size -= sizeof(wstream_size);

                        //Instr stream
                        if (wstream_size > reserve_size) {wstream_size = reserve_size;}
                        PX_memcpy(pClient->send_cache_instr_buffer + (sizeof(sync_server->recv_cache_buffer) -
                                                                      reserve_size), sync_server->stampsInstrStream.buffer + pClient->timeStreamOffset, wstream_size);
                        ...
                        if (reserve_size >= sizeof(PX_SyncFrame_InstrStream_StampIndex) + sizeof(px_dword) * 2)
                        {
                            if (sync_server->stampsIndexTable.size - pClient->timeIndexOffset > 1) //last stamp is not ready
                            {
                                ...
                                if (windexTableSize > (reserve_size - sizeof(px_dword) * 2) / sizeof(
                                        PX_SyncFrame_InstrStream_StampIndex))
                                {
                                    windexTableSize = (reserve_size - sizeof(px_dword) * 2) / sizeof(
                                                          PX_SyncFrame_InstrStream_StampIndex);
                                }
                                if (windexTableSize)
                                {
                                    //Index start-offset
                                    PX_memcpy(pClient->send_cache_instr_buffer + (sizeof(sync_server->recv_cache_buffer) -
                                                                                  reserve_size), &pClient->timeIndexOffset, sizeof(pClient->timeIndexOffset));
                                    reserve_size -= sizeof(pClient->timeIndexOffset);
                                    //Index Size
                                    PX_memcpy(pClient->send_cache_instr_buffer + (sizeof(sync_server->recv_cache_buffer) -
                                                                                  reserve_size), &windexTableSize, sizeof(windexTableSize));
                                    reserve_size -= sizeof(windexTableSize);
                                    //data
                                    PX_memcpy(pClient->send_cache_instr_buffer + (sizeof(sync_server->recv_cache_buffer) -
                                                                                  reserve_size), (px_byte *)sync_server->stampsIndexTable.data + pClient->timeIndexOffset * sizeof(
                                                  PX_SyncFrame_InstrStream_StampIndex),
                                              windexTableSize * sizeof(PX_SyncFrame_InstrStream_StampIndex));
                        ...
                        if (datasize)
                        {
                            ...
                            PX_SyncFrameServer_Write(sync_server, pClient, pClient->send_cache_instr_buffer,
                                                     pClient->send_cache_instr_size);
                            //////////////////////////////////////////////////////////////////////////
...
px_bool PX_SyncFrameServerAddClient(PX_SyncFrameServer *sync, px_dword server_verify_id,
                                    px_dword client_id, px_dword c_id)
{
...
px_bool PX_SyncFrameClientInitialize(PX_SyncFrameClient *client, px_memorypool *mp,
                                     px_dword updateDuration, px_dword server_verify_id, px_dword client_id, PX_Linker *linker)
{
    px_int i;
    ...
    PX_VectorInitialize(mp, &client->stampsIndexTable, sizeof(PX_SyncFrame_InstrStream_StampIndex),
                        PX_SYNC_INSTRS_SIZE);
...
static px_bool PX_SyncFrameClient_Read(PX_SyncFrameClient *client)
{
    ...
    while ((client->recv_cache_buffer_size = PX_LinkerRead(client->linker, client->recv_cache_buffer,
                                                           sizeof(client->recv_cache_buffer))) != 0)
    {
...
static px_void PX_SyncFrame_ClientHandle_StatusQueryData(PX_SyncFrameClient *client,
                                                         px_dword elapsed)
{
    //read
...
static px_void PX_SyncFrame_ClientHandle_StatusConneting(PX_SyncFrameClient *client,
                                                         px_dword elapsed)
{
    //read
    PX_Sync_IO_Packet *packet;
...
static px_void PX_SyncFrame_ClientHandle_StatusProcessing(PX_SyncFrameClient *client,
                                                          px_dword elapsed)
{
    PX_Sync_IO_Packet *recv_packet, *send_packet;
    px_int i;
    ...
    if (client->send_repeat_times == 0)
    {
        ...
        if (client->Input_InstrStream.usedsize)
        {
            ...
            if ((px_uint)client->Input_InstrStream.usedsize < sizeof(client->send_cache_Instr_buffer) -
                (px_int)PX_STRUCT_OFFSET(PX_Sync_IO_Packet, data))
            {
                client->send_cache_Instr_size = client->Input_InstrStream.usedsize + (px_int)PX_STRUCT_OFFSET(
                                                    PX_Sync_IO_Packet, data);
    ...
    while (PX_SyncFrameClient_Read(client))
    {
        ...
        if (recv_packet->type == PX_SYNC_IO_TYPE_OPCODE)
        {
            ...
            reserve_size = client->recv_cache_buffer_size - streamSize - sizeof(px_dword) * 2 -
                           (px_int)PX_STRUCT_OFFSET(PX_Sync_IO_Packet, data);
            if (reserve_size >= sizeof(PX_SyncFrame_InstrStream_StampIndex) + sizeof(px_dword) * 2)
            {
                ...
                pStamps = (PX_SyncFrame_InstrStream_StampIndex *)(recv_packet->data + sizeof(
                                                                      px_dword) * 2 + streamSize + sizeof(px_dword) * 2);
            }
            else
            {
                continue;
...
{
    px_uint32 sum = 0;
    sum += PX_sum32(sync->stampsIndexTable.data,
                    sync->stampsIndexTable.nodesize * sync->stampsIndexTable.size);
...
{
    return size % PX_SYNCDATA_BLOCK_SIZE ? size / PX_SYNCDATA_BLOCK_SIZE + 1 : size /
           PX_SYNCDATA_BLOCK_SIZE;
}

px_bool PX_SyncDataServerInitialize(PX_SyncDataServer *syncdata_server, px_memorypool *mp,
                                    px_dword serverID, PX_Linker *linker)
{
...
px_bool PX_SyncDataServer_ReadBlock(PX_SyncDataServer *s, px_byte *data, px_int bufferSize,
                                    px_int *readsize)
{
...
px_bool PX_SyncDataClientInitialize(PX_SyncDataClient *syncdata_client, px_memorypool *mp,
                                    px_dword serverID, px_dword clientID, PX_Linker *linker)
{
...
px_bool PX_SyncDataClient_ReadBlock(PX_SyncDataClient *s, px_byte *data, px_int bufferSize,
                                    px_int *readsize)
{
    ...
    case PX_SYNCDATA_CLIENT_STATUS_QUERY:
        {
            ...
            if (PX_SyncDataClient_ReadBlock(syncdata_client, (px_byte *)&R_datagram, sizeof(R_datagram),
                                            &readSize))
            {
    ...
    case PX_SYNCDATA_CLIENT_STATUS_SYNCHRONIZING:
        {
            ...
            if (PX_SyncDataClient_ReadBlock(syncdata_client, (px_byte *)&R_datagram, sizeof(R_datagram),
                                            &readSize))
            {
                if (R_datagram.requestAck.serverID == syncdata_client->serverID &&
                    R_datagram.header.opcode == PX_SYNCDATA_OPCODE_REQUESTACK)
                {
                    px_int size;
