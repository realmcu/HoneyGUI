...
px_void PX_Object_ProtractorRender(px_surface *rendersurface, PX_Object *pObject, px_dword elapsed)
{
    ...
    PX_GeoDrawCircle(rendersurface, (px_int)objx, (px_int)objy, (px_int)pProtractor->radius, 1,
                     pProtractor->color);
    ...
    case PX_OBJECT_PROTRACTOR_STATE_STANDBY:
        {
            PX_GeoDrawLine(rendersurface, (px_int)objx, (px_int)objy,
                           (px_int)(objx + PX_cos_angle(pProtractor->startAngle)*pProtractor->radius),
                           (px_int)(objy + PX_sin_angle(pProtractor->startAngle)*pProtractor->radius), 1, pProtractor->color);

            PX_GeoDrawSolidCircle(rendersurface,
                                  (px_int)(objx + PX_cos_angle(pProtractor->startAngle)*pProtractor->radius),
    ...
    case PX_OBJECT_PROTRACTOR_STATE_DONE:
        {
            ...
            PX_GeoDrawSector(rendersurface, (px_int)objx, (px_int)objy, pProtractor->radius, 1, acolor,
                             (px_int)pProtractor->startAngle, (px_int)pProtractor->endAngle);
            ...
            PX_GeoDrawLine(rendersurface, (px_int)objx, (px_int)objy,
                           (px_int)(objx + PX_cos_angle(pProtractor->endAngle)*pProtractor->radius),
            ...
            PX_FontModuleDrawText(rendersurface, PX_NULL, (px_int)objx, (px_int)objy - 2, PX_ALIGN_MIDBOTTOM,
                                  degree, pProtractor->fontColor);
        }
        break;
    }
...
PX_Object *PX_Object_ProtractorCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                      px_int radius)
{
    PX_Object *pObject;
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0, 0, 0, 0,
                                PX_OBJECT_TYPE_PROTRACTOR, 0, PX_Object_ProtractorRender, 0, &protractor, sizeof(protractor));
    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORDOWN, PX_Object_ProtractorOnCursorDown,
                           PX_NULL);
    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORMOVE, PX_Object_ProtractorOnCursorMove,
                           PX_NULL);
    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORRDOWN, PX_Object_ProtractorOnCursorRDown,
                           PX_NULL);
    return pObject;
}
