...
px_void PX_Object_CursorSliderRender(px_surface *psurface, PX_Object *pObject, px_dword elapsed)
{
    ...
    while (update_elapsed)
    {
        ...
        for (i = 1; i < PX_COUNTOF(pDesc->sliderpoints); i++)
        {
            ...
            {
                pDesc->sliderpoints[i].x = pDesc->sliderpoints[i].x + (pDesc->sliderpoints[i - 1].x -
                                                                       pDesc->sliderpoints[i].x) / 2;
                pDesc->sliderpoints[i].y = pDesc->sliderpoints[i].y + (pDesc->sliderpoints[i - 1].y -
                                                                       pDesc->sliderpoints[i].y) / 2;
            }
        }
    }
    ...
    PX_GeoDrawPenCircle(psurface, pDesc->sliderpoints[0].x, pDesc->sliderpoints[0].y,
                        pObject->diameter / 2, pDesc->color);
}
...
PX_Object *PX_Object_CursorSliderCreate(px_memorypool *mp, PX_Object *Parent, px_float x,
                                        px_float y, px_float radius, px_color Color)
{
    PX_Object_CursorSlider desc = { 0 }, * pDesc;
    PX_Object *pObject;
    px_int i;
    pObject = PX_ObjectCreateEx(mp, Parent, x, y, 0, radius * 2, radius * 2, 0,
                                PX_OBJECT_TYPE_CURSORSLIDER, PX_Object_CursorSliderUpdate, PX_Object_CursorSliderRender, 0, &desc,
                                sizeof(desc));
