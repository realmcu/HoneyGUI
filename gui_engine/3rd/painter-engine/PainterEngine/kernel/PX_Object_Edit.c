...
PX_Object *PX_Object_EditCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                px_int Width, px_int Height, PX_FontModule *fontModule)
{
    ...
    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORDOWN, PX_Object_EditOnMouseLButtonDown,
                           PX_NULL);
...
px_void PX_Object_EditUpdateCursorOnDown(PX_Object *pObject, px_int cx, px_int cy)
{
    ...
    while (PX_TRUE)
    {
        ...
        if (pEdit->fontModule)
        {
            ...
            {
                if (cx <= x_draw_oft + width / 2 + pEdit->xFontSpacing / 2 &&
                    cy <= y_draw_oft + pEdit->fontModule->max_Height + pEdit->yFontSpacing / 2)
                {
        ...
        else
        {
            ...
            {
                if (cx <= x_draw_oft + __PX_FONT_ASCSIZE / 2 + pEdit->xFontSpacing / 2 &&
                    cy <= y_draw_oft + __PX_FONT_HEIGHT + pEdit->yFontSpacing / 2)
                {
                    pEdit->cursor_index = cursor;
                    return;
                }
...
px_void PX_Object_EditRender(px_surface *psurface, PX_Object *pObject, px_uint elapsed)
{
    ...
    {
        //clear
        PX_SurfaceClear(&pEdit->EditSurface, 0, 0, (px_int)objWidth - 1, (px_int)objHeight - 1,
                        pEdit->BackgroundColor);
        if (pEdit->Border)
        {
            if (pEdit->state == PX_OBJECT_EDIT_STATE_NORMAL)
            {
                PX_GeoDrawBorder(&pEdit->EditSurface, 0, 0, (px_int)objWidth - 1, (px_int)objHeight - 1, 1,
                                 pEdit->BorderColor);
            }
            else
            {
    ...
    {
        PX_SurfaceClear(&pEdit->EditSurface, 0, 0, (px_int)objWidth - 1, (px_int)objHeight - 1, PX_COLOR(0,
                        0, 0, 0));
        PX_GeoDrawSolidRoundRect(&pEdit->EditSurface, 0, 0, (px_int)objWidth - 1, (px_int)objHeight - 1,
                                 objHeight / 2, pEdit->BackgroundColor);
        if (pEdit->Border)
        {
            if (pEdit->state == PX_OBJECT_EDIT_STATE_NORMAL)
            {
                PX_GeoDrawRoundRect(&pEdit->EditSurface, 0, 0, (px_int)objWidth - 1, (px_int)objHeight - 1,
                                    objHeight / 2, 1, pEdit->BorderColor);
            }
            else
            {
                PX_GeoDrawRoundRect(&pEdit->EditSurface, 0, 0, (px_int)objWidth - 1, (px_int)objHeight - 1,
                                    objHeight / 2, 1, pEdit->CursorColor);
            }
        }
    }
    ...
    while (PX_TRUE)
    {
        ...
        if (pEdit->onFocus && pEdit->cursor_index == cursor)
        {
            ...
            if ((pEdit->elapsed / 200) & 1)
            {
                if (pEdit->fontModule)
                {
                    PX_GeoDrawRect(&pEdit->EditSurface, x_draw_oft + 1, y_draw_oft, x_draw_oft,
                                   y_draw_oft + pEdit->fontModule->max_Height - 1, pEdit->TextColor);
                }
                else
                {
                    PX_GeoDrawRect(&pEdit->EditSurface, x_draw_oft + 1, y_draw_oft, x_draw_oft,
        ...
        if (pEdit->fontModule)
        {
            ...
            {
                PX_FontModuleDrawCharacter(&pEdit->EditSurface, pEdit->fontModule, x_draw_oft, y_draw_oft, code,
                                           pEdit->TextColor);
                x += width + pEdit->xFontSpacing;
            }
    ...
    PX_SurfaceRender(psurface, &pEdit->EditSurface, (px_int)objx, (px_int)objy, PX_ALIGN_LEFTTOP,
                     PX_NULL);
}
...
//edit
//////////////////////////////////////////////////////////////////////////
PX_Object *PX_Designer_EditCreate(px_memorypool *mp, PX_Object *pparent, px_float x, px_float y,
