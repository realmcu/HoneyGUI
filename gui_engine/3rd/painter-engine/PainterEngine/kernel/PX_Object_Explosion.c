...
PX_Object *PX_Object_Explosion01Create(px_memorypool *mp, PX_Object *parent, px_float x, px_float y,
                                       px_int count, px_float size, px_float power, px_int alive, px_texture *ptexture)
{
    PX_Object *pObject;
    ...
    pObject = PX_ObjectCreateEx(mp, parent, x, y, 0, 0, 0, 0, PX_OBJECT_TYPE_EXPLOSIONX, 0,
                                PX_Object_ExplosionRender, PX_Object_ExplosionFree, &desc, sizeof(desc));
    return pObject;
}
...
px_void PX_Object_Explosion02Render(px_surface *psurface, PX_Object *pObject, px_dword elapsed)
{
    ...
    PX_GeoDrawRing(psurface, (px_int)pObject->x, (px_int)pObject->y, 50, 5, PX_COLOR((px_byte)(a * 255),
                   pDesc->color._argb.r, pDesc->color._argb.g, pDesc->color._argb.b), -90 + (px_int)angle,
                   (px_int)angle);
    PX_GeoDrawRing(psurface, (px_int)pObject->x, (px_int)pObject->y, 50, 5, PX_COLOR((px_byte)(a * 255),
                   pDesc->color._argb.r, pDesc->color._argb.g, pDesc->color._argb.b), 90 + (px_int)angle,
                   (px_int)angle + 180);
...
PX_Object *PX_Object_Explosion02Create(px_memorypool *mp, PX_Object *parent, px_float x, px_float y,
                                       px_color color)
{
    ...
    PX_ParticalLauncher_InitializeInfo info;
    PX_memset(&Desc, 0, sizeof(Desc));
    pObject = PX_ObjectCreateEx(mp, parent, x, y, 0, 0, 0, 0, 0, 0, PX_Object_Explosion02Render,
                                PX_Object_Explosion02Free, &Desc, sizeof(Desc));
...
PX_Object *PX_Object_Explosion03Create(px_memorypool *mp, PX_Object *parent, px_float x, px_float y,
                                       px_int dirx, px_int diry,  px_float size, px_dword switch_duration, px_dword alive, px_color color)
{
    ...
    Desc.alive = alive;
    pObject = PX_ObjectCreateEx(mp, parent, x, y, 0, 0, 0, 0, 0, 0, PX_Object_Explosion03Render, 0,
                                &Desc, sizeof(Desc));
...
px_void PX_Object_Explosion04Render(px_surface *psurface, PX_Object *pObject, px_dword elpased)
{
    ...
    PX_GeoDrawPenRing(psurface, objx, objy, (PX_sqrt(fr) * pDesc->range * 4),
                      ((1 - fr) * pDesc->range * 0.3f + 1), pDesc->color);
}

PX_Object *PX_Object_Explosion04Create(px_memorypool *mp, PX_Object *parent, px_float x, px_float y,
                                       px_color color, px_float range, px_dword alive)
{
    PX_Object_Explosion04 *pDesc;
    PX_Object *pObject;
    pObject = PX_ObjectCreateEx(mp, parent, x, y, 0, 0, 0, 0, 0, PX_NULL, PX_Object_Explosion04Render,
                                0, PX_NULL, sizeof(PX_Object_Explosion04));
