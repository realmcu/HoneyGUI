...
px_void PX_Object_MemoryViewRender(px_surface *prendersurface, PX_Object *pObject, px_dword elapsed)
{
    ...
    for (j = 0; j < column; j++)
    {
        ...
        for (i = 0; i < row; i++)
        {
            ...
            if ((rel_address_offset * 2 >= mst && rel_address_offset * 2 <= med) ||
                (rel_address_offset * 2 + 1 >= mst && rel_address_offset * 2 + 1 <= med))
            {
                PX_GeoDrawRect(prendersurface, x + 96 + row * 24 + 8 * i, drawy, x + 96 + row * 24 + 8 * i + 7,
                               drawy + 15, pdesc->selectcolor);
            }
...
px_void PX_Object_MemoryViewOnCursorDown(PX_Object *pObject, PX_Object_Event e, px_void *ptr)
{
    ...
    pdesc->mark_start_cursor = pdesc->mark_end_cursor = PX_Object_MemoryViewGetCursorMapIndex(pObject,
                                                        x, y);
}
...
PX_Object *PX_Object_MemoryViewCreate(px_memorypool *mp, PX_Object *pParent, px_int x, px_int y,
                                      px_int Width, px_int Height)
{
    ...
    pObject = PX_ObjectCreateEx(mp, pParent, (px_float)x, (px_float)y, 0, (px_float)Width,
                                (px_float)Height, 0, PX_OBJECT_TYPE_MEMORYVIEW, PX_NULL, PX_Object_MemoryViewRender, PX_NULL, &desc,
                                sizeof(desc));
    pdesc = PX_ObjectGetDesc(PX_Object_MemoryView, pObject);
    pdesc->scrollbar = PX_Object_SliderBarCreate(mp, pObject, 0, 0, 10, 10,
                                                 PX_OBJECT_SLIDERBAR_TYPE_VERTICAL, PX_OBJECT_SLIDERBAR_STYLE_BOX);
    pdesc->scrollbar->Visible = PX_FALSE;

    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORCLICK, PX_Object_MemoryViewOnCursorClick,
                           PX_NULL);
    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORDOWN, PX_Object_MemoryViewOnCursorDown,
                           PX_NULL);
    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORDRAG, PX_Object_MemoryViewOnCursorDrag,
                           PX_NULL);
...
px_bool     PX_Object_MemoryViewReadData(PX_Object *pObject, px_dword offset, px_void *poutdata,
                                         px_dword size)
{
    PX_Object_MemoryView *pdesc = PX_Object_GetMemoryView(pObject);
