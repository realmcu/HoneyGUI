...
px_void PX_Object_PancRender(px_surface *pSurface, PX_Object *pObject, px_dword elapsed)
{
    ...
    for (i = 0; i < 5; i++)
    {
        PX_GeoDrawLine(pSurface, (px_int)(objx + step * i), (px_int)objy, (px_int)(objx + step * i),
                       (px_int)(objy + objHeight), 1, renderColor);
    }
    ...
    for (i = 0; i < 5; i++)
    {
        PX_GeoDrawLine(pSurface, (px_int)(pDesc->sourceX + step * i), (px_int)objy,
                       (px_int)(pDesc->sourceX + step * i), (px_int)(objy + objHeight), 1, renderColor);
    }
    ...
    for (i = 0; i < 5; i++)
    {
        PX_GeoDrawLine(pSurface, (px_int)objx, (px_int)(objy + step * i), (px_int)(objx + objWidth),
                       (px_int)(objy + step * i), 1, renderColor);
    }
    ...
    for (i = 0; i < 5; i++)
    {
        PX_GeoDrawLine(pSurface, (px_int)objx, (px_int)(pDesc->sourceY + step * i),
                       (px_int)(objx + objWidth), (px_int)(pDesc->sourceY + step * i), 1, renderColor);
    }
    ...
    for (i = 0; i < 5; i++)
    {
        PX_GeoDrawLine(pSurface, (px_int)(pDesc->currentX + step * i), (px_int)objy,
                       (px_int)(pDesc->currentX + step * i), (px_int)(objy + objHeight), 1, renderColor);
    }
    ...
    for (i = 0; i < 5; i++)
    {
        PX_GeoDrawLine(pSurface, (px_int)objx, (px_int)(pDesc->currentY + step * i),
                       (px_int)(objx + objWidth), (px_int)(pDesc->currentY + step * i), 1, renderColor);
    }
    ...
    PX_GeoDrawSolidCircle(pSurface, (px_int)(objx + objWidth), (px_int)(objy + objHeight), 5,
                          renderColor);

    PX_GeoDrawBorder(pSurface, (px_int)objx, (px_int)objy, (px_int)(objx + objWidth),
                     (px_int)(objy + objHeight), 3, pDesc->borderColor);
...
px_void PX_Object_PancOnCursorDown(PX_Object *pObject, PX_Object_Event e, px_void *ptr)
{
    ...
    if ((x - pDesc->currentX) * (x - pDesc->currentX) + (y - pDesc->currentY) *
        (y - pDesc->currentY) < 64)
    {
    ...
    if ((x - objx - objWidth) * (x - objx - objWidth) + (y - objy - objHeight) *
        (y - objy - objHeight) < 25)
    {
...
PX_Object *PX_Object_PancCreate(px_memorypool *mp, PX_Object *Parent, px_float x, px_float y,
                                px_float widht, px_float height)
{
    PX_Object_Panc panc;
    PX_Object *pObject;
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, x, y, 0, widht, height, 0, PX_OBJECT_TYPE_PANC, PX_NULL,
                                PX_Object_PancRender, PX_NULL, &panc, sizeof(panc));
...
px_void PX_Object_PancReset(PX_Object *pObject, px_float x, px_float y, px_float width,
                            px_float height)
{
    PX_Object_Panc *pDesc = PX_ObjectGetDesc(PX_Object_Panc, pObject);
    pObject->x = x;
