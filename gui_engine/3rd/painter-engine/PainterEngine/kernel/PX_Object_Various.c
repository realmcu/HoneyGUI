...
PX_Object *PX_Object_VariousCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                   px_int Width, px_int Height, const px_char *Text, PX_OBJECT_VARIOUS_TYPE type,  PX_FontModule *fm)
{
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0, (px_float)Width,
                                (px_float)Height, 0, PX_OBJECT_TYPE_VARIOUS, PX_NULL, PX_Object_VariousRender, PX_NULL, &var,
                                sizeof(var));
    pdesc = PX_ObjectGetDesc(PX_Object_Various, pObject);
    pdesc->label = PX_Object_LabelCreate(mp, pObject, 0, 0, 1, 1, Text, fm,
    ...
    case PX_OBJECT_VARIOUS_TYPE_EDIT:
        {
            ...
            PX_ObjectRegisterEvent(pdesc->various, PX_OBJECT_EVENT_VALUECHANGED, PX_Object_VariousOnEditChange,
                                   pObject);
        }
        break;
    case PX_OBJECT_VARIOUS_TYPE_LABEL:
        {
            pdesc->various = PX_Object_LabelCreate(mp, pObject, 0, 0, 1, 1, "", fm,
                                                   PX_OBJECT_UI_DEFAULT_FONTCOLOR);
        }
        break;
