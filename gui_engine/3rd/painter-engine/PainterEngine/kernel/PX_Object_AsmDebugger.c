...
px_void PX_Object_AsmDebuggerMonitorOnRender(px_surface *psurface, PX_Object *pObject,
                                             px_dword elapsed)
{
    PX_Object_AsmDebugger_VarMonitor *pvar = (PX_Object_AsmDebugger_VarMonitor *)
                                             PX_Object_ListItemGetData(pObject);
    ...
    PX_FontModuleDrawText(psurface, pDesc->fm, (px_int)pObject->x + 8,
                          (px_int)pObject->y + (px_int)(pObject->Height / 2), PX_ALIGN_LEFTMID, pvar->name, PX_COLOR(255, 255,
                                  255, 0));
}

px_bool PX_Object_AsmDebuggerMonitorOnCreate(px_memorypool *mp, PX_Object *ItemObject,
                                             px_void *userptr)
{
...
px_void PX_Object_AsmDebuggerPrintVar(PX_Object_AsmDebugger *pAsm, PX_VM_VARIABLE *pvar,
                                      px_char name[])
{
    px_char content[64];
    switch (pvar->type)
...
px_void PX_Object_AsmDebuggerSourceOnRender(px_surface *psurface, PX_Object *pObject,
                                            px_dword elapsed)
{
    PX_Object_AsmDebugger_Line *pline = (PX_Object_AsmDebugger_Line *)PX_Object_ListItemGetData(
                                            pObject);
    PX_Object *pDescObject = (PX_Object *)pObject->User_ptr;
    PX_Object_AsmDebugger *pDesc = PX_ObjectGetDesc(PX_Object_AsmDebugger, pDescObject);
    ...
    if (pline->bbreak)
    {
        PX_GeoDrawRect(psurface, (px_int)pObject->x, (px_int)pObject->y,
                       (px_int)(pObject->x + pObject->Width), (px_int)(pObject->y + pObject->Height), PX_COLOR(64, 255,
                               255, 0));
        PX_GeoDrawSolidCircle(psurface, (px_int)(pObject->x + pObject->Height / 2),
                              (px_int)(pObject->y + pObject->Height / 2), 6, PX_COLOR(192, 255, 0, 0));
    }

    if (pline->brun)
    {
        PX_GeoDrawRect(psurface, (px_int)pObject->x, (px_int)pObject->y,
                       (px_int)(pObject->x + pObject->Width), (px_int)(pObject->y + pObject->Height), PX_COLOR(64, 0, 255,
                               0));
        PX_GeoDrawTriangle(psurface, PX_POINT2D(pObject->x, pObject->y), PX_POINT2D(pObject->x,
                                                                                    pObject->y + pObject->Height), PX_POINT2D(pObject->x + pObject->Height / 2,
                                                                                            pObject->y + pObject->Height / 2), PX_COLOR(255, 0, 255, 0));
    }
    ...
    else if (PX_memequ(pline->source.buffer, "EXPORT FUNCTION", sizeof("EXPORT FUNCTION") - 1) ||
             PX_memequ(pline->source.buffer, "FUNCTION", sizeof("FUNCTION") - 1))
    {
    ...
    PX_FontModuleDrawText(psurface, pDesc->fm, (px_int)pObject->x + 50,
                          (px_int)pObject->y + (px_int)(pObject->Height / 2), PX_ALIGN_RIGHTMID, num, PX_COLOR(255, 192, 128,
                                  255));
    PX_FontModuleDrawText(psurface, pDesc->fm, (px_int)pObject->x + 64,
                          (px_int)pObject->y + (px_int)(pObject->Height / 2), PX_ALIGN_LEFTMID, pline->source.buffer, clr);
}

px_bool PX_Object_AsmDebuggerSourceOnCreate(px_memorypool *mp, PX_Object *ItemObject,
                                            px_void *userptr)
{
...
px_void PX_Object_AsmDebuggerRender(px_surface *prendersurface, PX_Object *pObject,
                                    px_dword elapsed)
{
    ...
    if (pDesc->vm && pDesc->map)
    {
        ...
        px_char content[64];
        PX_sprintf4(content, sizeof(content), " Thread:%1 IP:%2 SP:%3 BP:%4", PX_STRINGFORMAT_INT(T),
                    PX_STRINGFORMAT_INT(ip), PX_STRINGFORMAT_INT(sp), PX_STRINGFORMAT_INT(bp));
        ...
        if (ip != pDesc->lastip && ip >= 0)
        {
            ...
            if (/*pDesc->vm->Suspend &&*/PX_VMDebuggerInstruction(pDesc->vm, ip, &opcode, opcode_name,
                                                                  &paramcount, optype, param))
            {
                ...
                for (i = 0; i < paramcount; i++)
                {
                    ...
                    if (pvar)
                    {
                        ...
                        {
                            PX_sprintf2(pvarMonitor.name, sizeof(pvarMonitor.name), "%1:%2", PX_STRINGFORMAT_INT(line + 1),
                                        PX_STRINGFORMAT_STRING(name));
                ...
                {
                    PX_Object_ListAdd(pDesc->list_monitor, PX_VECTORAT(PX_Object_AsmDebugger_VarMonitor,
                                                                       &pDesc->monitor, i));
                }
...
px_void PX_Object_AsmDebuggerOnMonitorChanged(PX_Object *pObject, PX_Object_Event e, px_void *ptr)
{
    ...
    if (index != -1)
    {
        PX_Object_AsmDebugger_VarMonitor *pm = PX_VECTORAT(PX_Object_AsmDebugger_VarMonitor, &pAsm->monitor,
                                                           index);
        ...
        if (PX_VMDebuggerInstruction(pAsm->vm, ip, &opcode, opcode_name, &paramcount, optype, param))
        {
            ...
            for (i = 0; i < paramcount; i++)
            {
                ...
                if (pvar)
                {
                    ...
                    {
                        PX_sprintf2(pvarMonitor.name, sizeof(pvarMonitor.name), "%1:%2", PX_STRINGFORMAT_INT(index + 1),
                                    PX_STRINGFORMAT_STRING(name));
            ...
            {
                PX_Object_ListAdd(pAsm->list_monitor, PX_VECTORAT(PX_Object_AsmDebugger_VarMonitor, &pAsm->monitor,
                                                                  i));
            }
...
PX_Object *PX_Object_AsmDebuggerCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                       px_int width, px_int height, PX_FontModule *fm)
{
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, (px_float) x, (px_float)y, 0, (px_float)width,
                                (px_float)height, 0, PX_OBJECT_TYPE_ASMDEBUGGER, 0, PX_Object_AsmDebuggerRender,
                                PX_Object_AsmDebuggerFree, &Desc, sizeof(Desc));
    ...
    pDesc->label_register = PX_Object_LabelCreate(mp, pObject, 0, 0, 256, 24, "", PX_NULL, PX_COLOR(255,
                                                  0, 255, 0));
    PX_ObjectRegisterEvent(pDesc->button_break, PX_OBJECT_EVENT_EXECUTE,
                           PX_Object_AsmDebuggerOnButtonBreak, pObject);
    PX_ObjectRegisterEvent(pDesc->button_step, PX_OBJECT_EVENT_EXECUTE,
                           PX_Object_AsmDebuggerOnButtonStep, pObject);
    PX_ObjectRegisterEvent(pDesc->button_run, PX_OBJECT_EVENT_EXECUTE, PX_Object_AsmDebuggerOnButtonRun,
                           pObject);
    PX_ObjectRegisterEvent(pDesc->button_pause, PX_OBJECT_EVENT_EXECUTE,
                           PX_Object_AsmDebuggerOnButtonPause, pObject);
    PX_ObjectRegisterEvent(pDesc->button_reset, PX_OBJECT_EVENT_EXECUTE,
                           PX_Object_AsmDebuggerOnButtonReset, pObject);
    pDesc->list_source = PX_Object_ListCreate(mp, pObject, 0, 0, 10, 10, 18,
                                              PX_Object_AsmDebuggerSourceOnCreate, pObject);
    pDesc->autotext_data = PX_Object_AutoTextCreate(mp, pObject, 0, 0, 10, fm);
    pDesc->list_monitor = PX_Object_ListCreate(mp, pObject, 0, 0, 10, 10, 18,
                                               PX_Object_AsmDebuggerMonitorOnCreate, pObject);
    PX_ObjectRegisterEvent(pDesc->list_monitor, PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_AsmDebuggerOnMonitorChanged, pObject);
    PX_ObjectRegisterEvent(pDesc->list_source, PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_AsmDebuggerOnSourceChanged, pObject);
    pDesc->label_name = PX_Object_LabelCreate(mp, pObject, 0, 0, 128, 24, "", PX_NULL, PX_COLOR(255, 64,
                                              255, 64));
    ...
    PX_VectorInitialize(mp, &pDesc->monitor, sizeof(PX_Object_AsmDebugger_VarMonitor),
                        PX_OBJECT_ASMDEBUGGER_VARMONITOR_MAX_COUNT);
