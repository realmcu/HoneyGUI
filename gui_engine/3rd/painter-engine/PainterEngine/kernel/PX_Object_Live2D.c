...
PX_Object *PX_Object_Live2DCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                  PX_LiveFramework *pLiveFramework)
{
    PX_Object *pObject;
    PX_Object_Live2D *pLive2D;
    pObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0, 0, 0, 0, PX_OBJECT_TYPE_LIVE2D,
                                0, PX_Object_PX_Live2DRender, 0, 0, sizeof(PX_Object_Live2D));
    if (pObject == PX_NULL)
    {
        return PX_NULL;
    }
