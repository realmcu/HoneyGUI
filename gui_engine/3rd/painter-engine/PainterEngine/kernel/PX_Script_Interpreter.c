...
PX_LEXER_LEXEME_TYPE PX_ScriptTranslatorNextTokenSN(px_lexer *lexer)
{
    PX_LEXER_LEXEME_TYPE type;
    while ((type = PX_LexerGetNextLexeme(lexer)) == PX_LEXER_LEXEME_TYPE_SPACER ||
           type == PX_LEXER_LEXEME_TYPE_NEWLINE);
...
static px_bool PX_ScriptParseInclude(PX_ScriptInterpreter *analysis, px_string *codes,
                                     PX_SCRIPT_LIBRARY *lib, const px_char *name)
{
    px_lexer lexer;
        ...
        if (type == PX_LEXER_LEXEME_TYPE_TOKEN)
        {
            if (PX_strequ(lexer.CurLexeme.buffer, "#INCLUDE"))
            {
                if (PX_ScriptTranslatorNextToken(&lexer) == PX_LEXER_LEXEME_TYPE_CONATINER)
                {
                    ...
                    for (i = 0; i < lib->codeLibraries.size; i++)
                    {
                        if (PX_strequ(lexer.CurLexeme.buffer, PX_VECTORAT(PX_SCRIPT_CODE, &lib->codeLibraries,
                                                                          i)->name.buffer))
                        {
                            ...
                            allocsize = lexerState.offset + PX_strlen(PX_VECTORAT(PX_SCRIPT_CODE, &lib->codeLibraries,
                                                                                  i)->code.buffer) + PX_strlen(lexer.Sources + PX_LexerGetState(&lexer).offset - 1) + 1;
...
static px_bool PX_ScriptParseDefine(PX_ScriptInterpreter *analysis, px_string *codes,
                                    PX_SCRIPT_LIBRARY *lib, const px_char *name)
{
    px_lexer lexer;
    px_int startIndex;
    ...
    while (PX_TRUE)
    {
        ...
        if (type == PX_LEXER_LEXEME_TYPE_TOKEN)
        {
            if (PX_strequ(lexer.CurLexeme.buffer, "#DEFINE"))
            {
                ...
                {
                    if (lexer.Sources[lexer.SourceOffset] != '\n' && lexer.Sources[lexer.SourceOffset] != '\0' &&
                        lexer.Sources[lexer.SourceOffset] != '\r')
                    {
    ...
    while (PX_TRUE)
    {
        ...
        if (type == PX_LEXER_LEXEME_TYPE_TOKEN)
        {
            ...
            {
                if (PX_strequ(lexer.CurLexeme.buffer, PX_VECTORAT(PX_SCRIPT_TRANSLATOR_DEFINE_ST, &defines,
                                                                  i)->name.buffer))
                {
                    startIndex = lexer.SourceOffset - PX_strlen(lexer.CurLexeme.buffer);
                    PX_StringReplaceRange(codes, startIndex, lexer.SourceOffset - 1,
                                          PX_VECTORAT(PX_SCRIPT_TRANSLATOR_DEFINE_ST, &defines, i)->token.buffer);
                    PX_LexerSetSourcePointer(&lexer, codes->buffer);
                    lexer.SourceOffset = startIndex + PX_strlen(PX_VECTORAT(PX_SCRIPT_TRANSLATOR_DEFINE_ST, &defines,
                                                                            i)->token.buffer);
                    break;
                }
            }
...
static px_bool PX_ScriptParsePretreatment(PX_ScriptInterpreter *analysis, px_string *codes,
                                          PX_SCRIPT_LIBRARY *lib, const px_char *name)
{
...
static PX_SCRIPT_STRUCT *PX_ScriptParseGetStructByIndex(PX_ScriptInterpreter *analysis,
                                                        px_int index)
{
...
static px_bool PX_ScriptParse_AST_PushToken(PX_ScriptInterpreter *analysis, px_vector *op,
                                            px_vector *tk, PX_SCRIPT_EXPR_STREAM atom, PX_SCRIPT_STRUCT *_inset, PX_SCRIPT_STRUCT **_outset)
{
    px_int i;
    PX_SCRIPT_VARIABLES *pvar;
...
static px_bool PX_ScriptParseAST_PushOpcode(PX_ScriptInterpreter *analysis, px_vector *op,
                                            PX_SCRIPT_EXPR_STREAM atom)
{
...
{
    if (ope.operandType == PX_SCRIPT_AST_OPERAND_TYPE_INT ||
        ope.operandType == PX_SCRIPT_AST_OPERAND_TYPE_INT_CONST)
    {
        return PX_TRUE;
    }
    if (ope.operandType == PX_SCRIPT_AST_OPERAND_TYPE_FLOAT ||
        ope.operandType == PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_CONST)
    {
        return PX_TRUE;
    }
    if (ope.operandType == PX_SCRIPT_AST_OPERAND_TYPE_STRING_IDX ||
        ope.operandType == PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_IDX)
    {
        return PX_TRUE;
    }
    return PX_FALSE;
...
static px_bool PX_ScriptParseIsCompareAbleOperand(PX_SCRIPT_AST_OPERAND operand1,
                                                  PX_SCRIPT_AST_OPERAND operand2)
{
...
static px_bool PX_ScriptParseAST_MapTokenToR2(PX_ScriptInterpreter *analysis,
                                              PX_SCRIPT_AST_OPERAND operand, px_string *out)
{
    px_string fmrString;
            ...
            case PX_SCRIPT_VARIABLE_REGION_LOCAL:
                {
                    ...
                    else
                    {
                        PX_StringFormat2(&fmrString, "STRCHR R2,LOCAL[%1],%2\n", PX_STRINGFORMAT_INT(operand._oft),
                                         PX_STRINGFORMAT_INT(operand.bAtomPopIndex));
                        PX_StringCat(out, fmrString.buffer);
                    }
                    PX_StringFree(&fmrString);
                }
...
static px_bool PX_ScriptParseAST_MapTokenToR1(PX_ScriptInterpreter *analysis,
                                              PX_SCRIPT_AST_OPERAND operand, px_string *out)
{
    px_string fmrString;
...
static px_bool PX_ScriptParseLastInstr_DOT(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
...
static px_bool PX_ScriptParseLastInstr_EQUAL(PX_ScriptInterpreter *analysis, px_vector *op,
                                             px_vector *tk, px_string *out)
{
                    ...
                    case PX_SCRIPT_VARIABLE_REGION_LOCAL:
                        {
                            ...
                            else
                            {
                                PX_StringFormat2(&fmrString, "STRSET LOCAL[%1],%2,R2\n", PX_STRINGFORMAT_INT(operandLeft._oft),
                                                 PX_STRINGFORMAT_INT(operandLeft.AtomIndex));
                                PX_StringCat(out, fmrString.buffer);
                                PX_StringFormat1(&fmrString, "PUSH LOCAL[%1]\n", PX_STRINGFORMAT_INT(operandLeft._oft));
                                PX_StringCat(out, fmrString.buffer);
                            }
    ...
    case PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR_CONST:
        {
            if (operandRight.operandType == PX_SCRIPT_AST_OPERAND_TYPE_INT_PTR_CONST &&
                operandLeft.operandType != PX_SCRIPT_AST_OPERAND_TYPE_INT_PTR)
            {
                return PX_FALSE;
            }
            if (operandRight.operandType == PX_SCRIPT_AST_OPERAND_TYPE_HANDLE_PTR_CONST &&
                operandLeft.operandType != PX_SCRIPT_AST_OPERAND_TYPE_HANDLE_PTR)
            {
                return PX_FALSE;
            }
            if (operandRight.operandType == PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_PTR_CONST &&
                operandLeft.operandType != PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_PTR)
            {
                return PX_FALSE;
            }
            if (operandRight.operandType == PX_SCRIPT_AST_OPERAND_TYPE_STRING_PTR_CONST &&
                operandLeft.operandType != PX_SCRIPT_AST_OPERAND_TYPE_STRING_PTR)
            {
                return PX_FALSE;
            }
            if (operandRight.operandType == PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_PTR_CONST &&
                operandLeft.operandType != PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_PTR)
            {
                return PX_FALSE;
            }
            if (operandRight.operandType == PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR_CONST &&
                operandLeft.operandType != PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR)
            {
                return PX_FALSE;
            }
    ...
    case PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_CONST:
        {
            ...
            pTop->_oft = 0;
            //////////////////////////////////////////////////////////////////////////
            if (operandRight.operandType == PX_SCRIPT_AST_OPERAND_TYPE_STRING_CONST &&
                operandLeft.operandType != PX_SCRIPT_AST_OPERAND_TYPE_STRING)
            {
                return PX_FALSE;
            }

            if (operandRight.operandType == PX_SCRIPT_AST_OPERAND_TYPE_MEMORY &&
                operandLeft.operandType != PX_SCRIPT_AST_OPERAND_TYPE_MEMORY)
            {
                return PX_FALSE;
            }
    ...
    case PX_SCRIPT_AST_OPERAND_TYPE_STRING_IDX:
        {
            ...
            pTop->_oft = 0;
            if (operandLeft.operandType != PX_SCRIPT_AST_OPERAND_TYPE_INT &&
                operandLeft.operandType != PX_SCRIPT_AST_OPERAND_TYPE_STRING_IDX &&
                operandLeft.operandType != PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_IDX)
            {
                return PX_FALSE;
            }
                ...
                case PX_SCRIPT_VARIABLE_REGION_LOCAL:
                    {
                        ...
                        {
                            PX_StringFormat2(&fmrString, "STRSET LOCAL[%1],%2,R2\n", PX_STRINGFORMAT_INT(operandLeft._oft),
                                             PX_STRINGFORMAT_INT(operandLeft.bAtomPopIndex));
                            PX_StringCat(out, fmrString.buffer);
                            PX_StringFormat1(&fmrString, "PUSH LOCAL[%1]\n", PX_STRINGFORMAT_INT(operandLeft._oft));
                            PX_StringCat(out, fmrString.buffer);
                        }
...
static px_bool PX_ScriptParseLastInstr_IDX(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
...
static px_bool PX_ScriptParseLastInstr_OFT(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
    px_string fmrString;
...
static px_bool PX_ScriptParseLastInstr_PTR(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
...
static px_bool PX_ScriptParseLastInstr_ADR(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
...
static px_bool PX_ScriptParseLastInstr_ADD(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
    ...
    PX_VectorPop(op);
    PX_VectorPop(tk);
    if (operand1.operandType == PX_SCRIPT_AST_OPERAND_TYPE_INT_CONST &&
        operand1.region == PX_SCRIPT_VARIABLE_REGION_GLOBAL)
    {
        if (operand2.operandType == PX_SCRIPT_AST_OPERAND_TYPE_INT_CONST &&
            operand2.region == PX_SCRIPT_VARIABLE_REGION_GLOBAL)
        {
            pTop = PX_VECTORLAST(PX_SCRIPT_AST_OPERAND, tk);
        ...
        if (operand2.operandType == PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_CONST &&
            operand2.region == PX_SCRIPT_VARIABLE_REGION_GLOBAL)
        {
            pTop = PX_VECTORLAST(PX_SCRIPT_AST_OPERAND, tk);
            pTop->operandType = PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_CONST;
    ...
    if (operand1.operandType == PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_CONST &&
        operand1.region == PX_SCRIPT_VARIABLE_REGION_GLOBAL)
    {
        if (operand2.operandType == PX_SCRIPT_AST_OPERAND_TYPE_INT_CONST &&
            operand2.region == PX_SCRIPT_VARIABLE_REGION_GLOBAL)
            ...
            case PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_IDX:
                {
                    ...
                    pTop = PX_VECTORLAST(PX_SCRIPT_AST_OPERAND, tk);
                    if (operand2.operandType == PX_SCRIPT_AST_OPERAND_TYPE_FLOAT ||
...
static px_bool PX_ScriptParseLastInstr_INC(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    ...
    PX_VectorPop(op);
    if (operand1.operandType != PX_SCRIPT_AST_OPERAND_TYPE_INT &&
        operand1.operandType != PX_SCRIPT_AST_OPERAND_TYPE_INT_PTR &&
        operand1.operandType != PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_PTR &&
        operand1.operandType != PX_SCRIPT_AST_OPERAND_TYPE_STRING_PTR &&
        operand1.operandType != PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_PTR &&
        operand1.operandType != PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR)
    {
        return PX_FALSE;
    }
...
static px_bool PX_ScriptParseLastInstr_POSITIVE(PX_ScriptInterpreter *analysis, px_vector *op,
                                                px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1;
...
static px_bool PX_ScriptParseLastInstr_SUB(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_DEC(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1;
    px_string fmrString;
...
static px_bool PX_ScriptParseLastInstr_NEGATIVE(PX_ScriptInterpreter *analysis, px_vector *op,
                                                px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_MUL(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_DIV(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_AND(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_OR(PX_ScriptInterpreter *analysis, px_vector *op,
                                          px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_NOT(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_XOR(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_INV(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_MOD(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_SHL(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_SHR(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_INT(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_FLOAT(PX_ScriptInterpreter *analysis, px_vector *op,
                                             px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_STRING(PX_ScriptInterpreter *analysis, px_vector *op,
                                              px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_MEMORY(PX_ScriptInterpreter *analysis, px_vector *op,
                                              px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_STRLEN(PX_ScriptInterpreter *analysis, px_vector *op,
                                              px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_MEMLEN(PX_ScriptInterpreter *analysis, px_vector *op,
                                              px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_LARGE(PX_ScriptInterpreter *analysis, px_vector *op,
                                             px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_LARGEEQU(PX_ScriptInterpreter *analysis, px_vector *op,
                                                px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_LESS(PX_ScriptInterpreter *analysis, px_vector *op,
                                            px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_LAND(PX_ScriptInterpreter *analysis, px_vector *op,
                                            px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_LOR(PX_ScriptInterpreter *analysis, px_vector *op,
                                           px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_LESSEQU(PX_ScriptInterpreter *analysis, px_vector *op,
                                               px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_LGEQU(PX_ScriptInterpreter *analysis, px_vector *op,
                                             px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_UNEQU(PX_ScriptInterpreter *analysis, px_vector *op,
                                             px_vector *tk, px_string *out)
{
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr(PX_ScriptInterpreter *analysis, px_vector *op, px_vector *tk,
                                       px_string *out)
{
...
static px_bool PX_ScriptParseExpressionStream(PX_ScriptInterpreter *analysis, px_vector *stream,
                                              px_int *offset, PX_SCRIPT_AST_OPERAND *pretOperand,
                                              PX_SCRIPT_TRANSLATOR_EXPRESSION_TYPE expectedEndl, px_string *out)
{
    px_vector vOp;
    px_vector vTk;
    px_vector *op = &vOp;
...
_EXPR_OUT:
                    ...
                    //for (i=0;i<paramcount;i++)
                {
                    if (i == paramcount - 1)
                    {
                        if (!PX_ScriptParseExpressionStream(analysis, stream, &paramBeginIndex[paramcount - 1], &retOperand,
                                                            PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_LBRACKETEND, out))
                        {
                    ...
                    {
                        if (!PX_ScriptParseExpressionStream(analysis, stream, &paramBeginIndex[i], &retOperand,
                                                            PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_COMMA, out))
                        {
                    ...
                    if (retOperand.operandType == PX_SCRIPT_AST_OPERAND_TYPE_STRUCT ||
                        retOperand.operandType == PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR ||
...
//                  break;
                    ...
                    case PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_CONST:
                        if (pfunc->parameters[i].type != PX_SCRIPT_PARSER_VAR_TYPE_FLOAT &&
                            pfunc->parameters[i].type != PX_SCRIPT_PARSER_VAR_TYPE_INT)
                        {
                            PX_ScriptTranslatorError(analysis, "Parameter not matched.");
                            goto _ERROR;
                        }
                    ...
                    case PX_SCRIPT_AST_OPERAND_TYPE_INT_PTR:
                    case PX_SCRIPT_AST_OPERAND_TYPE_INT_PTR_CONST:
                        if (pfunc->parameters[i].type != PX_SCRIPT_PARSER_VAR_TYPE_INT_ARRAY &&
                            pfunc->parameters[i].type != PX_SCRIPT_PARSER_VAR_TYPE_INT_PTR)
                        {
                            PX_ScriptTranslatorError(analysis, "Parameter not matched.");
                            goto _ERROR;
                        }
                    ...
                    case PX_SCRIPT_AST_OPERAND_TYPE_HANDLE_PTR:
                    case PX_SCRIPT_AST_OPERAND_TYPE_HANDLE_PTR_CONST:
                        if (pfunc->parameters[i].type != PX_SCRIPT_PARSER_VAR_TYPE_HANDLE_ARRAY &&
                            pfunc->parameters[i].type != PX_SCRIPT_PARSER_VAR_TYPE_HANDLE_PTR)
                        {
                            PX_ScriptTranslatorError(analysis, "Parameter not matched.");
                            goto _ERROR;
                        }
                    ...
                    case PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_PTR:
                    case PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_PTR_CONST:
                        if (pfunc->parameters[i].type != PX_SCRIPT_PARSER_VAR_TYPE_FLOAT_ARRAY &&
                            pfunc->parameters[i].type != PX_SCRIPT_PARSER_VAR_TYPE_FLOAT_PTR)
                        {
                            PX_ScriptTranslatorError(analysis, "Parameter not matched.");
                            goto _ERROR;
                        }
                    ...
                    case PX_SCRIPT_AST_OPERAND_TYPE_STRING_PTR:
                    case PX_SCRIPT_AST_OPERAND_TYPE_STRING_PTR_CONST:
                        if (pfunc->parameters[i].type != PX_SCRIPT_PARSER_VAR_TYPE_STRING_ARRAY &&
                            pfunc->parameters[i].type != PX_SCRIPT_PARSER_VAR_TYPE_STRING_PTR)
                        {
                            PX_ScriptTranslatorError(analysis, "Parameter not matched.");
                            goto _ERROR;
                        }
                    ...
                    case PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_PTR:
                    case PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_PTR_CONST:
                        if (pfunc->parameters[i].type != PX_SCRIPT_PARSER_VAR_TYPE_MEMORY_ARRAY &&
                            pfunc->parameters[i].type != PX_SCRIPT_PARSER_VAR_TYPE_MEMORY_PTR)
                        {
                            PX_ScriptTranslatorError(analysis, "Parameter not matched.");
                            goto _ERROR;
                        }
                    ...
                    case PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR:
                    case PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR_CONST:
                        if (pfunc->parameters[i].type != PX_SCRIPT_PARSER_VAR_TYPE_STRUCT_PTR ||
                            PX_ScriptParseGetStructByIndex(analysis, pfunc->parameters[i].setIndex) != retOperand.pStruct)
                        {
                            PX_ScriptTranslatorError(analysis, "Parameter not matched.");
                            goto _ERROR;
                        }
                    ...
                    if (retOperand.operandType == PX_SCRIPT_AST_OPERAND_TYPE_FLOAT ||
                    ...
                    if (retOperand.operandType == PX_SCRIPT_AST_OPERAND_TYPE_INT ||
                        retOperand.operandType == PX_SCRIPT_AST_OPERAND_TYPE_INT_CONST\
                        || retOperand.operandType == PX_SCRIPT_AST_OPERAND_TYPE_STRING_IDX ||
                        retOperand.operandType == PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_IDX)
                    {
                        if (pfunc->parameters[i].type == PX_SCRIPT_PARSER_VAR_TYPE_INT)
                        {
                            PX_StringCat(out, "INT R1\n");
        ...
        case PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_LBRACKETBEGIN:
            {
                ...
                if (!PX_ScriptParseExpressionStream(analysis, stream, offset, &retOperand,
                                                    PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_LBRACKETEND, out))
                {
                    goto _ERROR;
                }
        ...
        case PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_MBRACKETBEGIN:
            {
                ...
                (*offset)++;
                if (!PX_ScriptParseExpressionStream(analysis, stream, offset, &retOperand,
                                                    PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_MBRACKETEND, out))
                {
                    goto _ERROR;
                }
        ...
        case PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_STRLEN:
            {
                if (pVec[*offset].type == PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_DOT ||
                    pVec[*offset].type == PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_OFT)
                {
...
static px_bool PX_ScriptParseExpression(PX_ScriptInterpreter *analysis, px_char *expr,
                                        px_string *out, PX_SCRIPT_AST_OPERAND *retOperand)
{
    px_int i;
    px_uint quotes;
    ...
    //
    accept_type = PX_SCRIPT_EXPRESSION_ACCEPT_SINGLE | PX_SCRIPT_EXPRESSION_ACCEPT_TOKEN |
                  PX_SCRIPT_EXPRESSION_ACCEPT_BRACKET_START;
        ...
        if (type == PX_LEXER_LEXEME_TYPE_TOKEN)
        {
            ...
            else if (PX_ScriptParseIsValidToken(lexer.CurLexeme.buffer))
            {
                ...
                else
                {
                    ...
                    Op.oplevel = 0;
                    accept_type = PX_SCRIPT_EXPRESSION_ACCEPT_BINARY | PX_SCRIPT_EXPRESSION_ACCEPT_MBRACKET_START |
                                  PX_SCRIPT_EXPRESSION_ACCEPT_BRACKET_END;
                }
                PX_VectorPushback(&stream, &Op);
            ...
            case '+':
                {
                    if ((accept_type & PX_SCRIPT_EXPRESSION_ACCEPT_BINARY) == 0 &&
                        (accept_type & PX_SCRIPT_EXPRESSION_ACCEPT_SINGLE) == 0)
                    {
                        ret = PX_FALSE;
                        goto _CLEAR;
                    }
                    ...
                    else
                    {
                        ...
                        if (accept_type & PX_SCRIPT_EXPRESSION_ACCEPT_BINARY)
                        {
                            ...
                            accept_type = PX_SCRIPT_EXPRESSION_ACCEPT_TOKEN | PX_SCRIPT_EXPRESSION_ACCEPT_LBRACKET_START |
                                          PX_SCRIPT_EXPRESSION_ACCEPT_SINGLE;
                        }
                        else
                        {
            ...
            case  '[':
                {
                    ...
                    PX_StringCat(&Op.code, "[");
                    PX_VectorPushback(&stream, &Op);
                    accept_type = PX_SCRIPT_EXPRESSION_ACCEPT_TOKEN | PX_SCRIPT_EXPRESSION_ACCEPT_BRACKET_START |
                                  PX_SCRIPT_EXPRESSION_ACCEPT_SINGLE;
                }
                break;
            ...
            case  ']':
                {
                    ...
                    PX_VectorPushback(&stream, &Op);
                    accept_type = PX_SCRIPT_EXPRESSION_ACCEPT_BINARY | PX_SCRIPT_EXPRESSION_ACCEPT_BRACKET_END |
                                  PX_SCRIPT_EXPRESSION_ACCEPT_MBRACKET_START;
                }
                break;
            ...
            case  '(':
                {
                    ...
                    PX_VectorPushback(&stream, &Op);
                    accept_type = PX_SCRIPT_EXPRESSION_ACCEPT_TOKEN | PX_SCRIPT_EXPRESSION_ACCEPT_LBRACKET_START |
                                  PX_SCRIPT_EXPRESSION_ACCEPT_LBRACKET_END | PX_SCRIPT_EXPRESSION_ACCEPT_SINGLE;
                }
                break;
    ...
    PX_StringCat(out, "\n");
    if (!PX_ScriptParseExpressionStream(analysis, &stream, &streamOft, &operand,
                                        PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_LBRACKETEND, out))
    {
        ret = PX_FALSE;
    }
...
static px_bool PX_ScriptParseVar(PX_ScriptInterpreter *analysis)
{
    ...
    while (PX_TRUE)
    {
        ...
        if (analysis->functionInside)
        {
            ...
            for (i = 0; i < analysis->v_variablesStackTable.size; i++)
            {
                if (PX_strequ(variable.Mnemonic.buffer, PX_VECTORAT(PX_SCRIPT_VARIABLES,
                                                                    &analysis->v_variablesStackTable, i)->Mnemonic.buffer))
                {
            ...
            pvar = PX_VECTORAT(PX_SCRIPT_VARIABLES, &analysis->v_variablesStackTable,
                               analysis->v_variablesStackTable.size - 1);
            ...
            if (variable.bInitialized)
            {
                ...
                PX_StringFormat2(&code, "%1=%2", PX_STRINGFORMAT_STRING(variable.Mnemonic.buffer),
                                 PX_STRINGFORMAT_STRING(variable.GlobalInitializeValue.buffer));
        ...
        else
        {
            ...
            {
                if (PX_strequ(variable.Mnemonic.buffer, PX_VECTORAT(PX_SCRIPT_VARIABLES,
                                                                    &analysis->v_variablesGlobalTable, i)->Mnemonic.buffer))
                {
                    PX_ScriptTranslatorError(analysis, "variable redefined.");
                    PX_StringFree(&variable.Mnemonic);
                    if (variable.bInitialized)
            ...
            {
                pvar = PX_VECTORAT(PX_SCRIPT_VARIABLES, &analysis->v_variablesGlobalTable,
                                   analysis->v_variablesGlobalTable.size - 1);
...
px_bool PX_ScriptParseFunctionDefined(PX_ScriptInterpreter *analysis,
                                      PX_SCRIPT_TRANSLATOR_FUNCTION_TYPE functype)
{
    ...
    else if ((func.retSetIndex = PX_ScriptParseGetSetIndex(analysis,
                                                           analysis->lexer.CurLexeme.buffer)) != -1)
    {
    ...
    if ((PX_ScriptTranslatorNextToken(&analysis->lexer)) != PX_LEXER_LEXEME_TYPE_DELIMITER ||
        analysis->lexer.Symbol != '(')
    {
        PX_ScriptTranslatorError(analysis, "Unexpected Lexeme.");
        goto _ERROR;
    }
    ...
    while (PX_TRUE)
    {
        ...
        {
            PX_ScriptTranslatorError(analysis,
                                     "Could not use a struct for parameter.consider use a struct pointer.");
            goto _ERROR;
        }
        ...
        {
            PX_SCRIPT_VARIABLES *plastvar = PX_VECTORLAST(PX_SCRIPT_VARIABLES,
                                                          &analysis->v_variablesStackTable);
...
px_bool PX_ScriptParseBootCode(PX_ScriptInterpreter *analysis)
{
    ...
    for (i = 0; i < analysis->v_variablesGlobalTable.size; i++)
    {
        ...
        if (pvar->bInitialized)
        {
            PX_StringInitialize(analysis->mp, &code);
            PX_StringInitialize(analysis->mp, &exprgen);
            PX_StringFormat2(&code, "%1=%2\n", PX_STRINGFORMAT_STRING(pvar->Mnemonic.buffer),
                             PX_STRINGFORMAT_STRING(pvar->GlobalInitializeValue.buffer));
            if (!PX_ScriptParseExpression(analysis, code.buffer, &exprgen, &retOperand))
            {
                PX_StringFree(&code);
                PX_StringFree(&exprgen);
    ...
    case PX_SCRIPT_AST_STRUCTURE_TYPE_IF:
        {
            ...
            if (type == PX_LEXER_LEXEME_TYPE_TOKEN &&
                PX_strequ(analysis->lexer.CurLexeme.buffer, PX_SCRIPT_TRANSLATOR_KEYWORD_ELSE))
            {
                //generate code
                PX_StringInitialize(analysis->mp, &fmrString);
...
px_bool PX_ScriptParseGetExpression(PX_ScriptInterpreter *analysis, px_string *expr, px_char end)
{
    ...
    while (PX_TRUE)
    {
        ...
        if (type == PX_LEXER_LEXEME_TYPE_DELIMITER && analysis->lexer.Symbol == end && !mBracket &&
            !lBracket)
        {
            break;
        }
...
px_bool PX_ScriptCompilerCompile(PX_SCRIPT_LIBRARY *lib, const px_char *name, px_string *ASM,
                                 px_int LocalStackSize, px_char error[], px_int err_size)
{
    PX_SCRIPT_SETMEMBER *psetmem;
    PX_SCRIPT_STRUCT *pset;
    px_string codes;
...
//                  }
        ...
        if (PX_strequ(analysis.lexer.CurLexeme.buffer, PX_SCRIPT_TRANSLATOR_KEYWORD_VAR_STRUCT))
        {
            ...
            if ((type = PX_ScriptTranslatorNextTokenSN(&analysis.lexer)) == PX_LEXER_LEXEME_TYPE_DELIMITER &&
                analysis.lexer.Symbol == '{')
            {
                PX_LexerSetState(state);
        ...
        if (PX_strequ(analysis.lexer.CurLexeme.buffer, PX_SCRIPT_TRANSLATOR_KEYWORD_EXPORT) ||
            PX_strequ(analysis.lexer.CurLexeme.buffer, PX_SCRIPT_TRANSLATOR_KEYWORD_INLINE))
        {
        ...
        if (PX_strequ(analysis.lexer.CurLexeme.buffer, PX_SCRIPT_TRANSLATOR_KEYWORD_ASM))
        {
            ...
            while (PX_TRUE)
            {
                ...
                {
                    pvar = PX_VECTORAT(PX_SCRIPT_VARIABLES, &analysis.v_variablesStackTable, i);
                    if (pvar->type == PX_SCRIPT_PARSER_VAR_TYPE_INT || pvar->type == PX_SCRIPT_PARSER_VAR_TYPE_FLOAT ||
                        pvar->type == PX_SCRIPT_PARSER_VAR_TYPE_STRING || pvar->type == PX_SCRIPT_PARSER_VAR_TYPE_MEMORY)
...
//              }
                ...
                while (PX_TRUE)
                {
                    ...
                    if (type == PX_LEXER_LEXEME_TYPE_DELIMITER && analysis.lexer.Symbol == ';' && !mBracket &&
                        !lBracket)
                    {
                        break;
                    }
...
_CONTINUEOUT:
        ...
        if (PX_strequ(analysis.lexer.CurLexeme.buffer, PX_SCRIPT_TRANSLATOR_KEYWORD_RETURN))
        {
            ...
            case PX_SCRIPT_PARSER_VAR_TYPE_INT:
                if (retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_INT &&
                    retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_INT_CONST && \
                    retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_FLOAT &&
                    retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_CONST && \
                    retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_STRING_IDX &&
                    retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_IDX)
                {
                ...
                break;
            case PX_SCRIPT_PARSER_VAR_TYPE_HANDLE:
                if (retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_HANDLE &&
                    retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_HANDLE_CONST)
                {
                    PX_ScriptTranslatorError(&analysis, "Return-type not matched.");
                    goto _ERROR;
            ...
            case PX_SCRIPT_PARSER_VAR_TYPE_FLOAT:
                ...
                break;
            case PX_SCRIPT_PARSER_VAR_TYPE_STRING:
                if (retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_STRING &&
                    retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_STRING_CONST)
                {
                    PX_ScriptTranslatorError(&analysis, "Return-type not matched.");
                    goto _ERROR;
                ...
                break;
            case PX_SCRIPT_PARSER_VAR_TYPE_MEMORY:
                if (retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_MEMORY &&
                    retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_CONST)
                {
                    PX_ScriptTranslatorError(&analysis, "Return-type not matched.");
                    goto _ERROR;
                ...
                break;
            case PX_SCRIPT_PARSER_VAR_TYPE_INT_PTR:
                if (retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_INT_PTR &&
                    retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_INT_PTR_CONST)
                {
                    PX_ScriptTranslatorError(&analysis, "Return-type not matched.");
                    goto _ERROR;
                ...
                break;
            case PX_SCRIPT_PARSER_VAR_TYPE_FLOAT_PTR:
                if (retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_PTR &&
                    retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_PTR_CONST)
                {
                    PX_ScriptTranslatorError(&analysis, "Return-type not matched.");
                    goto _ERROR;
                ...
                break;
            case PX_SCRIPT_PARSER_VAR_TYPE_STRING_PTR:
                if (retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_STRING_PTR &&
                    retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_STRING_PTR_CONST)
                {
                    PX_ScriptTranslatorError(&analysis, "Return-type not matched.");
                    goto _ERROR;
                ...
                break;
            case PX_SCRIPT_PARSER_VAR_TYPE_MEMORY_PTR:
                if (retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_PTR &&
                    retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_PTR_CONST)
                {
                    PX_ScriptTranslatorError(&analysis, "Return-type not matched.");
                    goto _ERROR;
                ...
                break;
            case PX_SCRIPT_PARSER_VAR_TYPE_STRUCT_PTR:
                if (retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR &&
                    retOperand.operandType != PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR_CONST)
                {
                    PX_ScriptTranslatorError(&analysis, "Return-type not matched.");
                    goto _ERROR;
                }
                if (retOperand.pStruct != PX_ScriptParseGetStructByIndex(&analysis,
                                                                         analysis.currentFunc.retSetIndex))
                {
                    PX_ScriptTranslatorError(&analysis, "Return-type not matched.");
                    goto _ERROR;
                }
    ...
    PX_StringInitialize(analysis.mp, &fmrString);
    PX_StringFormat3(&fmrString, ".GLOBAL %1\n.STACK %2\n.THREAD %3\n", PX_STRINGFORMAT_INT(globalSize),
                     PX_STRINGFORMAT_INT(LocalStackSize), PX_STRINGFORMAT_INT(thread));
    ...
    for (i = 0; i < analysis.v_variablesGlobalTable.size; i++)
    {
        ...
        {
            PX_StringFree(&PX_VECTORAT(PX_SCRIPT_VARIABLES, &analysis.v_variablesGlobalTable,
                                       i)->GlobalInitializeValue);
        }
    }
...
_ERROR:
    ...
    for (i = 0; i < analysis.v_variablesStackTable.size; i++)
    {
        ...
        {
            PX_StringFree(&PX_VECTORAT(PX_SCRIPT_VARIABLES, &analysis.v_variablesStackTable,
                                       i)->GlobalInitializeValue);
        }
    }
...
px_bool PX_ScriptInterpreterExpression(PX_ScriptInterpreter *analysis, px_char *expr,
                                       px_string *out, PX_SCRIPT_AST_OPERAND *retOperand)
{
