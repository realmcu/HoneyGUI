...
PX_Object *PX_Object_COMBoxCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                  px_int width, px_int height, const px_char title[], PX_FontModule *fontmodule)
{
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0, (px_float)width,
                                (px_float)height, 0, PX_OBJECT_TYPE_COMBOX, PX_NULL, PX_NULL, PX_NULL, &desc, sizeof(desc));
    pdesc = PX_ObjectGetDesc(PX_Object_COMBox, pObject);
    ...
    pdesc->various_com = PX_Object_VariousCreate(mp, pdesc->widget, width / 2 - 128, 32, 256, 32,
                                                 "COM:", PX_OBJECT_VARIOUS_TYPE_SELECTBAR, fontmodule);
    pdesc->various_baudrate = PX_Object_VariousCreate(mp, pdesc->widget, width / 2 - 128, 72, 256, 32,
                                                      "BaudRate:", PX_OBJECT_VARIOUS_TYPE_SELECTBAR, fontmodule);
    ...
    pdesc->various_databits = PX_Object_VariousCreate(mp, pdesc->widget, width / 2 - 128, 112, 256, 32,
                                                      "DataBits:", PX_OBJECT_VARIOUS_TYPE_SELECTBAR, fontmodule);
    ...
    pdesc->various_stopbits = PX_Object_VariousCreate(mp, pdesc->widget, width / 2 - 128, 152, 256, 32,
                                                      "StopBits:", PX_OBJECT_VARIOUS_TYPE_SELECTBAR, fontmodule);
    ...
    pdesc->various_parity = PX_Object_VariousCreate(mp, pdesc->widget, width / 2 - 128, 192, 256, 32,
                                                    "Parity:", PX_OBJECT_VARIOUS_TYPE_SELECTBAR, fontmodule);
    ...
    PX_ObjectRegisterEvent(pdesc->btn_close, PX_OBJECT_EVENT_EXECUTE, PX_Object_COMBoxOnCancel,
                           pObject);
