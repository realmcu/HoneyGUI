...
px_void PX_Object_VirtualKeyBoardRender(px_surface *psurface, PX_Object *pObject, px_uint elapsed)
{
    ...
    //clear background
    PX_SurfaceClear(psurface, (px_int)objx, (px_int)objy, (px_int)(objx + objWidth),
                    (px_int)(objy + objHeight), keyBoard->backgroundColor);
    ...
    for (i = 0; i < PX_COUNTOF(keyBoard->Keys); i++)
    {
        ...
        if (keyBoard->bShift || keyBoard->bUpper)
        {
            PX_FontModuleDrawText(psurface, PX_NULL, (px_int)(oftx + keyBoard->Keys[i].x + 2),
                                  (px_int)(ofty + keyBoard->Keys[i].y + 2), PX_ALIGN_LEFTTOP, keyBoard->Keys[i].u_key,
                                  keyBoard->borderColor);
        }
        else
        {
            PX_FontModuleDrawText(psurface, PX_NULL, (px_int)(oftx + keyBoard->Keys[i].x + 2),
                                  (px_int)(ofty + keyBoard->Keys[i].y + 2), PX_ALIGN_LEFTTOP, keyBoard->Keys[i].d_key,
                                  keyBoard->borderColor);
        }
    }
...
px_void PX_Object_VirtualKeyBoardOnCursorMove(PX_Object *pObject, PX_Object_Event e, px_void *ptr)
{
    ...
    for (i = 0; i < PX_COUNTOF(pkb->Keys); i++)
    {
        ...
        {
            if (!PX_isPointInRect(PX_POINT((px_float)x, (px_float)y, 0), PX_RECT((px_float)pkb->Keys[i].x,
                                                                                 (px_float)pkb->Keys[i].y, (px_float)pkb->Keys[i].width, (px_float)pkb->Keys[i].height)))
            {
                pkb->Keys[i].bDown = PX_FALSE;
            }
        }
...
PX_Object *PX_Object_VirtualKeyBoardCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                           px_int width, px_int height)
{
    ...
    keyboardObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0, (px_float)width,
                                       (px_float)height, 0, PX_OBJECT_TYPE_VKEYBOARD, PX_NULL, PX_Object_VirtualKeyBoardRender, PX_NULL,
                                       &keyboard, sizeof(PX_Object_VirtualKeyBoard));
    PX_Object_VirtualKeyBoardUpdateSize(keyboardObject);
    PX_ObjectRegisterEvent(keyboardObject, PX_OBJECT_EVENT_CURSORMOVE,
                           PX_Object_VirtualKeyBoardOnCursorMove, PX_NULL);
    PX_ObjectRegisterEvent(keyboardObject, PX_OBJECT_EVENT_CURSORDRAG,
                           PX_Object_VirtualKeyBoardOnCursorMove, PX_NULL);
    PX_ObjectRegisterEvent(keyboardObject, PX_OBJECT_EVENT_CURSORDOWN,
                           PX_Object_VirtualKeyBoardOnCursorDown, PX_NULL);
    PX_ObjectRegisterEvent(keyboardObject, PX_OBJECT_EVENT_CURSORUP,
                           PX_Object_VirtualKeyBoardOnCursorUp, PX_NULL);
...
px_void PX_Object_VirtualNumberKeyBoardRender(px_surface *psurface, PX_Object *pObject,
                                              px_uint elapsed)
{
    px_int i;
    px_int oftx;
    ...
    //clear background
    PX_SurfaceClear(psurface, (px_int)objx, (px_int)objy, (px_int)(objx + objWidth - 1),
                    (px_int)(objy + objHeight - 1), keyBoard->backgroundColor);
...
px_void PX_Object_VirtualNumberKeyBoardOnCursorUp(PX_Object *pObject, PX_Object_Event e,
                                                  px_void *ptr)
{
    px_float x = PX_Object_Event_GetCursorX(e);
    px_float y = PX_Object_Event_GetCursorY(e);
...
px_void PX_Object_VirtualNumberKeyBoardOnCursorMove(PX_Object *pObject, PX_Object_Event e,
                                                    px_void *ptr)
{
    px_float x = PX_Object_Event_GetCursorX(e);
    px_float y = PX_Object_Event_GetCursorY(e);
...
px_void PX_Object_VirtualNumberKeyBoardOnCursorDown(PX_Object *pObject, PX_Object_Event e,
                                                    px_void *ptr)
{
    px_float x = PX_Object_Event_GetCursorX(e);
    px_float y = PX_Object_Event_GetCursorY(e);
...
PX_Object *PX_Object_VirtualNumberKeyBoardCreate(px_memorypool *mp, PX_Object *Parent, px_int x,
                                                 px_int y, px_int width, px_int height)
{
    px_int i;
    px_float kw, kh;
    ...
    keyboard.LinkerObject = PX_NULL;
    keyboardObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0, (px_float)width,
                                       (px_float)height, 0, PX_OBJECT_TYPE_VNKEYBOARD, PX_NULL, PX_Object_VirtualNumberKeyBoardRender,
                                       PX_NULL, &keyboard, sizeof(PX_Object_VirtualNumberKeyBoard));
    PX_Object_VirtualNumberKeyBoardUpdateSize(keyboardObject);

    PX_ObjectRegisterEvent(keyboardObject, PX_OBJECT_EVENT_CURSORMOVE,
                           PX_Object_VirtualNumberKeyBoardOnCursorMove, PX_NULL);
    PX_ObjectRegisterEvent(keyboardObject, PX_OBJECT_EVENT_CURSORDRAG,
                           PX_Object_VirtualNumberKeyBoardOnCursorMove, PX_NULL);
    PX_ObjectRegisterEvent(keyboardObject, PX_OBJECT_EVENT_CURSORDOWN,
                           PX_Object_VirtualNumberKeyBoardOnCursorDown, PX_NULL);
    PX_ObjectRegisterEvent(keyboardObject, PX_OBJECT_EVENT_CURSORUP,
                           PX_Object_VirtualNumberKeyBoardOnCursorUp, PX_NULL);
    return keyboardObject;
}
