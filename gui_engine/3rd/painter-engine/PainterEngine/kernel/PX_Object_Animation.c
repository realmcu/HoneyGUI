...
PX_Object *PX_Object_AnimationCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                     PX_AnimationLibrary *lib)
{
    PX_Object *pObject;
    PX_Object_Animation *pDesc;
    pObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0, 0, 0, 0,
                                PX_OBJECT_TYPE_ANIMATION, 0, PX_Object_AnimationRender, PX_Object_AnimationFree, 0,
                                sizeof(PX_Object_Animation));
    if (pObject == PX_NULL)
    {
        return PX_NULL;
    }
