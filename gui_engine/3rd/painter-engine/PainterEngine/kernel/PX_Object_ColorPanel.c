...
px_bool PX_Object_ColorPanelUpdatePanelTexture(PX_Object *pObject)
{
    ...
    PX_GeoDrawBorder(&pdesc->panel, (px_int)ed, (px_int)ed, (px_int)(d - ed), (px_int)(d - ed), 1,
                     PX_COLOR_BLACK);
    return PX_TRUE;
}
...
px_void PX_Object_ColorPanelRender(px_surface *psurface, PX_Object *pObject, px_uint elapsed)
{
    ...
    //render
    PX_TextureRender(psurface, &pdesc->panel, (px_int)(x + w / 2), (px_int)(y + h / 2), PX_ALIGN_CENTER,
                     0);
    ...
    do
    {
        ...
        PX_GeoDrawCircle(psurface, (px_int)(x + w / 2 + hx), (px_int)(y + h / 2 + hy), 6, 3,
                         PX_COLOR_BLACK);
        PX_GeoDrawCircle(psurface, (px_int)(x + w / 2 + hx), (px_int)(y + h / 2 + hy), 6, 1,
                         PX_COLOR_WHITE);
...
PX_Object *PX_Object_ColorPanelCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                      px_int width, px_int height)
{
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, x * 1.f, y * 1.f, 0, width * 1.f, height * 1.f, 0,
                                PX_OBJECT_TYPE_COLORPANEL, 0, PX_Object_ColorPanelRender, PX_Object_ColorPanelFree, &desc,
                                sizeof(desc));
    pdesc = PX_ObjectGetDesc(PX_Object_ColorPanel, pObject);
    pdesc->mp = mp;
    pdesc->color_hsv.a = 1;
    ...
    PX_Object_ColorPanelUpdatePanelTexture(pObject);
    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORDOWN, PX_Object_ColorPanelOnCursorDown,
                           pObject);
    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORDRAG, PX_Object_ColorPanelOnCursorDrag,
                           pObject);
    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORMOVE, PX_Object_ColorPanelOnCursorCancel,
                           pObject);
    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORUP, PX_Object_ColorPanelOnCursorCancel,
                           pObject);
    return pObject;
}


PX_Object *PX_Designer_ColorPanelCreate(px_memorypool *mp, PX_Object *pparent, px_float x,
