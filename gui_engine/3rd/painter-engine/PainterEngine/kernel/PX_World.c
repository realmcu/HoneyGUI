...
px_bool PX_World_VM_GetObjectPosition(PX_VM *Ins, px_void *userptr)
{
    ...
    if (PX_VM_GLOBAL(Ins, x).type != PX_VM_VARIABLE_TYPE_FLOAT ||
        PX_VM_GLOBAL(Ins, y).type != PX_VM_VARIABLE_TYPE_FLOAT)
    {
        PX_VM_RET(Ins, PX_VM_Variable_int(0));
        return PX_TRUE;
    }
...
px_bool PX_World_VM_Abi_int(PX_VM *Ins, px_void *userptr)
{
    ...
    PX_AbiMemoryWrite_int(&(PX_VM_GLOBAL(Ins, ptr)._memory), PX_VM_STACK(Ins, 1)._string.buffer,
                          PX_VM_STACK(Ins, 2)._int);
    return PX_TRUE;
}
px_bool PX_World_VM_Abi_float(PX_VM *Ins, px_void *userptr)
{
    ...
    PX_AbiMemoryWrite_float(&(PX_VM_GLOBAL(Ins, ptr)._memory), PX_VM_STACK(Ins, 1)._string.buffer,
                            PX_VM_STACK(Ins, 2)._float);
    return PX_TRUE;
}
px_bool PX_World_VM_Abi_string(PX_VM *Ins, px_void *userptr)
{
    ...
    PX_AbiMemoryWrite_string(&(PX_VM_GLOBAL(Ins, ptr)._memory), PX_VM_STACK(Ins, 1)._string.buffer,
                             PX_VM_STACK(Ins, 2)._string.buffer);
    return PX_TRUE;
}
px_bool PX_World_VM_Abi_point(PX_VM *Ins, px_void *userptr)
{
    ...
    PX_AbiMemoryWrite_point(&(PX_VM_GLOBAL(Ins, ptr)._memory), PX_VM_STACK(Ins, 1)._string.buffer,
                            PX_POINT(x, y, z));
    return PX_TRUE;
}
px_bool PX_World_VM_Abi_color(PX_VM *Ins, px_void *userptr)
{
    ...
    PX_AbiMemoryWrite_color(&(PX_VM_GLOBAL(Ins, ptr)._memory), PX_VM_STACK(Ins, 1)._string.buffer,
                            PX_COLORF(x, y, z, w));
    return PX_TRUE;
}
...
px_bool PX_World_VM_CreateThread(PX_VM *Ins, px_void *userptr)
{
    ...
    if (!PX_VMBeginThreadFunction(&pWorld->vm, PX_VMGetFreeThreadId(Ins), PX_VM_STACK(Ins,
                                  0)._string.buffer, PX_NULL, 0))
    {
        PX_VM_RET(Ins, PX_VM_Variable_int(0));
    }
    return PX_TRUE;
...
px_bool PX_WorldInitialize(px_memorypool *mp, PX_World *pWorld, px_int world_width,
                           px_int world_height, px_int surface_width, px_int surface_height, const px_char script[])
{
    px_void *ptr;
...
host int createthread(string callname);\n\
 ";
    ...
    if (script && script[0])
    {
        ...
        PX_VMRegistHostFunction(&pWorld->vm, "getworldsize", PX_World_VM_GetWorldSize,
                                pWorld);//getworldsize
        PX_VMRegistHostFunction(&pWorld->vm, "getobjectname", PX_World_VM_GetObjectName,
                                pWorld);//getobjectname
        PX_VMRegistHostFunction(&pWorld->vm, "setobjectname", PX_World_VM_SetObjectName,
                                pWorld);//setobjectname
        PX_VMRegistHostFunction(&pWorld->vm, "setobjectposition", PX_World_VM_SetObjectPosition,
                                pWorld);//setobjectvelocity
        PX_VMRegistHostFunction(&pWorld->vm, "getobjectposition", PX_World_VM_GetObjectPosition,
                                pWorld);//setobjectvelocity
        PX_VMRegistHostFunction(&pWorld->vm, "setobjectvelocity", PX_World_VM_SetObjectVelocity,
                                pWorld);//setobjectvelocity
        PX_VMRegistHostFunction(&pWorld->vm, "setobjectforce", PX_World_VM_SetObjectForce,
                                pWorld);//setobjectforce
        PX_VMRegistHostFunction(&pWorld->vm, "setobjectresistance", PX_World_VM_SetObjectResistance,
                                pWorld);//setobjectresistance
        PX_VMRegistHostFunction(&pWorld->vm, "setobjecttype", PX_World_VM_SetObjectType,
                                pWorld);//setobjecttype
        PX_VMRegistHostFunction(&pWorld->vm, "setobjectimpacttargettype",
                                PX_World_VM_SetObjectImpactTargetType, pWorld);//setobjectimpacttargettype
        ...
        PX_VMRegistHostFunction(&pWorld->vm, "createthread", PX_World_VM_CreateThread,
                                pWorld);//createthread
...
px_void PX_WorldSoundPlay(PX_World *pWorld, const px_char key[], px_float x, px_float y)
{
    ...
    {
        if (PX_isXYInRegion(x, y, pWorld->offsetx - 100, pWorld->offsety - 100,
                            pWorld->surface_width + 200.f, pWorld->surface_height + 200.f))
        {
...
px_void PX_WorldUpdate(PX_World *pworld, px_uint elapsed)
{
    ...
    for (i = 0; i < updateCount; i++)
    {
        ...
        PX_MapPut(&pworld->idSearchmap, (const px_byte *)pwo->pObject->id, PX_strlen(pwo->pObject->id),
                  pwo->pObject);
    ...
    if (pworld->surface_height && pworld->surface_width && pworld->mp_WorldCalc.Size)
    {
        ...
        for (i = 0; i < sizeof(pwo->pObject->impact_object_type) * 8; i++)
        {
            if (impact_count[i])
            {
                PX_QuadtreeCreate(calcmp, &pworld->Impact_Test_array[i], 0, 0, (px_float)pworld->world_width,
                                  (px_float)pworld->world_height, impact_count[i], 2);
            }
            else
            {
                PX_QuadtreeCreate(calcmp, &pworld->Impact_Test_array[i], 0, 0, (px_float)pworld->world_width,
                                  (px_float)pworld->world_height, impact_count[i], 0);
            }
        }
            ...
            if (pwo->pObject->impact_object_type)
            {
                for (b = 0; b < sizeof(pwo->pObject->impact_object_type) * 8; b++)
                {
                    if ((pwo->pObject->impact_object_type & (1 << b)))
                    {
                        ...
                        {
                            PX_QuadtreeAddNode(&pworld->Impact_Test_array[b], (px_float)pwo->pObject->x,
                                               (px_float)pwo->pObject->y, (px_float)pwo->pObject->diameter, (px_float)pwo->pObject->diameter,
                                               userData);
                        }
                        else
                        {
                            PX_QuadtreeAddNode(&pworld->Impact_Test_array[b], (px_float)pwo->pObject->x,
                                               (px_float)pwo->pObject->y, (px_float)pwo->pObject->Width, (px_float)pwo->pObject->Height, userData);
                        }
            ...
            if (pwo->pObject->impact_target_type)
            {
                for (b = 0; b < sizeof(pwo->pObject->impact_target_type) * 8; b++)
                {
                    if ((pwo->pObject->impact_target_type & (1 << b)))
                    {
                        ...
                        {
                            PX_QuadtreeTestNode(&pworld->Impact_Test_array[b], (px_float)pwo->pObject->x,
                                                (px_float)pwo->pObject->y, (px_float)pwo->pObject->diameter, (px_float)pwo->pObject->diameter,
                                                userData);
                        }
                        else
                        ...
                        {
                            PX_Quadtree_UserData *puData = PX_VECTORAT(PX_Quadtree_UserData,
                                                                       &pworld->Impact_Test_array[b].Impacts, im_i);
                            ...
                            {
                                if (!PX_isCircleCrossCircle(PX_POINT(pObj1->x, pObj1->y, 0), pObj1->diameter / 2, PX_POINT(pObj2->x,
                                                            pObj2->y, 0), pObj2->diameter / 2))
                                {
                                    continue;
                                }
                            }
                            ...
                            {
                                if (!PX_isRectCrossCircle(PX_RECT(pObj1->x - pObj1->Width / 2, pObj1->y - pObj1->Height / 2,
                                                                  pObj1->Width, pObj1->Height), PX_POINT(pObj2->x, pObj2->y, 0), pObj2->diameter / 2))
                                {
                                    continue;
                                }
                            }
                            ...
                            {
                                if (!PX_isRectCrossCircle(PX_RECT(pObj2->x - pObj2->Width / 2, pObj2->y - pObj2->Height / 2,
                                                                  pObj2->Width, pObj2->Height), PX_POINT(pObj1->x, pObj1->y, 0), pObj1->diameter / 2))
                                {
                                    continue;
                                }
                            }
...
static px_void PX_WorldClildRender(PX_World *pworld, px_surface *pSurface, PX_Object *pObject,
                                   px_uint elapsed, px_float oftX, px_float oftY)
{
    if (pObject == PX_NULL)
    {
        return;
        ...
        if (pworld->showImpactRegion)
        {
            if (pObject->diameter)
            {
                PX_GeoDrawSolidCircle(pSurface, (px_int)pObject->x, (px_int)pObject->y,
                                      (px_int)pObject->diameter / 2, PX_COLOR(32, 255, 0, 0));
            }
            else
            {
                PX_GeoDrawRect(pSurface, (px_int)(pObject->x - pObject->Width / 2),
                               (px_int)(pObject->y - pObject->Height / 2), (px_int)(pObject->x + pObject->Width / 2),
                               (px_int)(pObject->y + pObject->Height / 2), PX_COLOR(32, 255, 0, 0));
            }
        }
...
px_void PX_WorldRender(px_surface *psurface, PX_World *pw, px_dword elapsed)
{
    ...
    {
        for (sy = pw->auxiliaryYSpacer - ((px_int)pw->offsety % pw->auxiliaryYSpacer); sy < surface_height;
             sy += pw->auxiliaryYSpacer)
        {
            PX_GeoDrawLine(psurface, 0, sy, surface_width - 1, sy, 1, pw->auxiliaryline_color);
        }

        for (sx = pw->auxiliaryXSpacer - ((px_int)pw->offsetx % pw->auxiliaryXSpacer); sx < surface_width;
    ...
    if (ArrayIndex)
    {
        ...
        {
            PX_WorldClildRender(pw, psurface, (PX_Object *)ArrayIndex[i].pData, elapsed, pw->offsetx,
                                pw->offsety);
        }
    }
    MP_Reset(calcmp);
...
px_bool PX_WorldRegisterClass(PX_World *pw, PX_WorldClass cls)
{
    ...
    *pNewClass = cls;
    if (PX_MapPut(&pw->classes, (const px_byte *)cls.name, PX_strlen(cls.name),
                  pNewClass) != PX_HASHMAP_RETURN_OK)
    {
...
PX_Object *PX_WorldCreateClassObject(PX_World *pWorld, px_char name[], px_float x, px_float y,
                                     px_float z, px_float width, px_float height, px_float length, px_abi *abi)
{
    PX_WorldClass *pClass = (PX_WorldClass *)PX_MapGet(&pWorld->classes, (const px_byte *)name,
                                                       PX_strlen(name));
    if (pClass)
    {
...
_LIMIT px_int PX_WorldSearchRegion(PX_World *world, px_float x, px_float y, px_float raduis,
                                   PX_Object *pObject[], px_int MaxSearchCount, px_dword impact_test_type)
{
    px_int b;
    px_int m = 0;
    for (b = 0; b < sizeof(impact_test_type) * 8; b++)
    {
        if ((impact_test_type & (1 << b)))
        {
            ...
            {
                PX_Quadtree_UserData *puData = PX_VECTORAT(PX_Quadtree_UserData,
                                                           &world->Impact_Test_array[b].Impacts, im_i);
                PX_WorldObject *pimpactWo = (PX_WorldObject *)puData->ptr;
                PX_Object *pObj2 = ((PX_WorldObject *)(puData->ptr))->pObject;

                if (pObj2->diameter)
                {
                    if (!PX_isCircleCrossCircle(PX_POINT(x, y, 0), raduis, PX_POINT(pObj2->x, pObj2->y, 0),
                                                pObj2->diameter / 2))
                    {
                        continue;
                    }
                ...
                {
                    if (!PX_isRectCrossCircle(PX_RECT(pObj2->x - pObj2->Width / 2, pObj2->y - pObj2->Height / 2,
                                                      pObj2->Width, pObj2->Height), PX_POINT(x, y, 0), raduis))
                    {
                        continue;
                    }
                }
...
_LIMIT px_int PX_WorldSearch(PX_World *pw, PX_Object *pObject[], px_int MaxSearchCount,
                             px_dword impact_object_type)
{
    ...
    for (i = 0; i < pw->pObjects.size; i++)
    {
        ...
        if (pwo && pwo->pObject && !pwo->DeleteMark &&
            pwo->pObject->impact_object_type == impact_object_type)
        {
            if (MaxSearchCount)
            {
