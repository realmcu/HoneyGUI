...
px_void PX_Object_ScaleBoxRender(px_surface *psurface, PX_Object *pObject, px_uint elapsed)
{
    ...
    default:
        return;
    case 1:
        PX_GeoDrawLine(psurface, (px_int)(objx + pScaleBox->edit_pt[0].x),
                       (px_int)(objx + pScaleBox->edit_pt[0].y), \
                       (px_int)(objx + pScaleBox->cursor_pt.x), (px_int)(objx + pScaleBox->cursor_pt.y),
                       1,
                       (pScaleBox->linecolor)
                      );
        PX_GeoDrawSolidCircle(psurface, (px_int)(objx + pScaleBox->edit_pt[0].x),
                              (px_int)(objx + pScaleBox->edit_pt[0].y), 3, pScaleBox->linecolor);
        PX_GeoDrawSolidCircle(psurface, (px_int)(objx + pScaleBox->cursor_pt.x),
                              (px_int)(objx + pScaleBox->cursor_pt.y), 3, pScaleBox->linecolor);
        break;
    case 2:
        {
            px_point2D pt[4];
            ...
            PX_GeoDrawSolidCircle(psurface, (px_int)(objx + pt[0].x), (px_int)(objx + pt[0].y), 3,
                                  pScaleBox->linecolor);
            PX_GeoDrawSolidCircle(psurface, (px_int)(objx + pt[1].x), (px_int)(objx + pt[1].y), 3,
                                  pScaleBox->linecolor);
            PX_GeoDrawSolidCircle(psurface, (px_int)(objx + pt[2].x), (px_int)(objx + pt[2].y), 3,
                                  pScaleBox->linecolor);
            PX_GeoDrawSolidCircle(psurface, (px_int)(objx + pt[3].x), (px_int)(objx + pt[3].y), 3,
                                  pScaleBox->linecolor);
        }
        break;
    case 3:
        {
            ...
            PX_GeoDrawTriangle(psurface, pScaleBox->edit_pt[0], pScaleBox->edit_pt[1], pScaleBox->edit_pt[2],
                               pScaleBox->regioncolor);
            PX_GeoDrawTriangle(psurface, pScaleBox->edit_pt[0], pScaleBox->edit_pt[2], pScaleBox->edit_pt[3],
                               pScaleBox->regioncolor);

            PX_GeoDrawTriangle(psurface, pScaleBox->scale_pt[0], pScaleBox->scale_pt[1], pScaleBox->scale_pt[2],
                               pScaleBox->regioncolor);
            PX_GeoDrawTriangle(psurface, pScaleBox->scale_pt[0], pScaleBox->scale_pt[2], pScaleBox->scale_pt[3],
                               pScaleBox->regioncolor);
            ...
            PX_GeoDrawLine(psurface, (px_int)(objx + pScaleBox->scale_pt[0].x),
                           (px_int)(objx + pScaleBox->scale_pt[0].y), \
            ...
            PX_GeoDrawLine(psurface, (px_int)(objx + pScaleBox->edit_pt[1].x),
                           (px_int)(objx + pScaleBox->edit_pt[1].y), \
                           (px_int)(objx + pScaleBox->edit_pt[2].x), (px_int)(objx + pScaleBox->edit_pt[2].y),
                           1,
                           (pScaleBox->linecolor)
                          );

            PX_GeoDrawLine(psurface, (px_int)(objx + pScaleBox->scale_pt[1].x),
                           (px_int)(objx + pScaleBox->scale_pt[1].y), \
                           (px_int)(objx + pScaleBox->scale_pt[2].x), (px_int)(objx + pScaleBox->scale_pt[2].y),
                           1,
                           (pScaleBox->linecolor)
                          );

            PX_GeoDrawLine(psurface, (px_int)(objx + pScaleBox->edit_pt[2].x),
                           (px_int)(objx + pScaleBox->edit_pt[2].y), \
                           (px_int)(objx + pScaleBox->edit_pt[3].x), (px_int)(objx + pScaleBox->edit_pt[3].y),
                           1,
                           (pScaleBox->linecolor)
                          );
            PX_GeoDrawLine(psurface, (px_int)(objx + pScaleBox->scale_pt[2].x),
                           (px_int)(objx + pScaleBox->scale_pt[2].y), \
            ...
            PX_GeoDrawSolidCircle(psurface, (px_int)(objx + mpt.x), (px_int)(objx + mpt.y), 3,
                                  pScaleBox->linecolor);
            if (pScaleBox->current_cursorpt == 0 || pScaleBox->current_selectpt == 0)
            {
                PX_GeoDrawCircle(psurface, (px_int)(objx + mpt.x), (px_int)(objx + mpt.y), 5, 1,
                                 pScaleBox->linecolor);
            }
...
px_void PX_Object_ScaleBoxOnCursorDown(PX_Object *pObject, PX_Object_Event e, px_void *ptr)
{
    ...
    {
        pscalebox->edit_pt[pscalebox->editpt_count] = PX_POINT2D(PX_Object_Event_GetCursorX(e) - inheritX,
                                                                 PX_Object_Event_GetCursorY(e) - inheritY);
        pscalebox->editpt_count++;
        if (pscalebox->editpt_count == 3)
        {
            PX_memcpy(pscalebox->scale_pt, pscalebox->edit_pt, 3 * sizeof(px_point2D));
            pscalebox->edit_pt[3] = PX_Point2DAdd(pscalebox->edit_pt[0], PX_Point2DSub(pscalebox->edit_pt[2],
                                                                                       pscalebox->edit_pt[1]));
...
px_void PX_Object_ScaleBoxOnCursorMove(PX_Object *pObject, PX_Object_Event e, px_void *ptr)
{
    ...
    {
        pscalebox->cursor_pt = PX_POINT2D(PX_Object_Event_GetCursorX(e) - inheritX,
                                          PX_Object_Event_GetCursorY(e) - inheritY);
    }
}
PX_Object *PX_Object_ScaleBoxCreate(px_memorypool *mp, PX_Object *Parent)
{
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, 0, 0, 0, 0, 0, 0, PX_OBJECT_TYPE_SCALEBOX, PX_NULL,
                                PX_Object_ScaleBoxRender, PX_NULL, &scaleBox, sizeof(scaleBox));
    if (!pObject)
    {
        return pObject;
    }
    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORDOWN, PX_Object_ScaleBoxOnCursorDown,
                           PX_NULL);
    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORMOVE, PX_Object_ScaleBoxOnCursorMove,
                           PX_NULL);
    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORDRAG, PX_Object_ScaleBoxOnCursorDrag,
                           PX_NULL);
    return pObject;
}
