...
px_void PX_Object_ScrollArea_EventDispatcher(PX_Object *pObject, PX_Object_Event e,
                                             px_void *user_ptr)
{
    ...
    if (e.Event == PX_OBJECT_EVENT_EXECUTE || e.Event == PX_OBJECT_EVENT_CURSORDRAG ||
        e.Event == PX_OBJECT_EVENT_CURSORDOWN || e.Event == PX_OBJECT_EVENT_CURSORUP ||
...
PX_Object *PX_Object_ScrollAreaCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                      px_int Width, px_int Height)
{
    ...
    pSA->hscroll = PX_Object_SliderBarCreate(mp, pObject, 0, 0, 16, 16,
                                             PX_OBJECT_SLIDERBAR_TYPE_HORIZONTAL, PX_OBJECT_SLIDERBAR_STYLE_BOX);
    pSA->vscroll = PX_Object_SliderBarCreate(mp, pObject, 0, 0, 16, 16,
    ...
    PX_ObjectRegisterEvent(pSA->hscroll, PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_ScrollAreaHSliderChanged, pSA);
    PX_ObjectRegisterEvent(pSA->vscroll, PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_ScrollAreaVSliderChanged, pSA);
...
px_void PX_Object_ScrollAreaGetRegion(PX_Object *pObject, px_float *left, px_float *top,
                                      px_float *right, px_float *bottom)
{
    px_float objx, objy, objWidth, objHeight;
    px_float inheritX, inheritY;
...
px_void PX_Object_ScrollAreaRender(px_surface *psurface, PX_Object *pObject, px_uint elapsed)
{
    ...
    PX_SurfaceClear(&pSA->surface, 0, 0, (px_int)objWidth - 1, (px_int)objHeight - 1,
                    pSA->BackgroundColor);
    ...
    if (pSA->bBorder)
    {
        PX_GeoDrawBorder(&pSA->surface, 0, 0, (px_int)objWidth - 1, (px_int)objHeight - 1, 1,
                         pSA->borderColor);
    }
    PX_SurfaceRender(psurface, &pSA->surface, (px_int)objx, (px_int)objy, PX_ALIGN_LEFTTOP, PX_NULL);
}
