...
px_void PX_Object_3DModel_PixelShader(px_surface *psurface, px_int x, px_int y, px_point position,
                                      px_float u, px_float v, px_point4D normal, px_texture *pTexture, px_color color)
{
    ...
    {
        alpha = (1 - cosv) * 128;
        PX_SurfaceDrawPixel(psurface, x, y, PX_COLOR(255, (px_uchar)(128 + alpha), (px_uchar)(128 + alpha),
                                                     (px_uchar)(128 + alpha)));
    }
}

PX_Object *PX_Object_3DModelCreate(px_memorypool *mp, PX_Object *parent, px_float x, px_float y,
                                   px_float width, px_float height, PX_3D_ObjectData *pdata)
{
    PX_Object_3DModel *pdesc;
    PX_Object *pObject;
    pObject = PX_ObjectCreateEx(mp, parent, x, y, 0, 0, 0, 0, PX_OBJECT_TYPE_3DMODEL, 0,
                                PX_Object_3DModelRender, PX_Object_3DModelFree, 0, sizeof(PX_Object_3DModel));
    ...
    if (!PX_3D_RenderListInitialize(mp, &pdesc->renderlist,
                                    PX_3D_PRESENTMODE_TEXTURE | PX_3D_PRESENTMODE_PURE, PX_3D_CULLMODE_CCW, PX_NULL))
    {
        PX_ObjectDelete(pObject);
        PX_TextureFree(&pdesc->renderSurface);
        return PX_NULL;
    ...
    if (!PX_3D_CameraUVNInitialize(mp, &pdesc->camera, PX_POINT4D(0, 0, 0), PX_POINT4D(0, 0, 1), 1,
                                   32767, 90, width, height))
    {
        PX_ObjectDelete(pObject);
        PX_TextureFree(&pdesc->renderSurface);
        PX_3D_RenderListFree(&pdesc->renderlist);
...
px_void PX_Object_3DModelSetWorld(PX_Object *pObject, px_float x, px_float y, px_float z,
                                  px_float rotX, px_float rotY, px_float rotZ, px_float scale)
{
    PX_Object_3DModel *pdesc;
    pdesc = PX_ObjectGetDesc(PX_Object_3DModel, pObject);
    if (pObject->Type == PX_OBJECT_TYPE_3DMODEL)
