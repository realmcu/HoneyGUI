...
{
    PX_RequestData("open", painterengine_loadbuffer, sizeof(painterengine_loadbuffer), 0,
                   PainterEngine_OnDesignerLoad);
}
...
px_bool PainterEngine_Initialize(px_int _screen_width, px_int _screen_height)
{
    ...
    if (!PX_RuntimeInitialize(runtime, _screen_width, _screen_height, _screen_width, _screen_height,
                              App.cache, sizeof(App.cache), PX_APPLICATION_MEMORYPOOL_STATIC_SIZE,
                              PX_APPLICATION_MEMORYPOOL_DYNAMIC_SIZE))
    {
        return PX_FALSE;
    }
#ifdef PAINTERENGIN_FILE_SUPPORT
    if (PX_LoadFontModuleFromTTF(&runtime->mp_static, &App.fontmodule, "assets/font.ttf",
                                 PX_FONTMODULE_CODEPAGE_UTF8, 18))
    {
    ...
    App.object_printer = PX_Object_PrinterCreate(&runtime->mp, App.object_root, 0, 0, _screen_width,
                                                 _screen_height, App.pfontmodule);
    App.object_printer->Visible = PX_FALSE;
    App.object_designer = PX_Object_DesignerCreate(&runtime->mp, App.object_root, App.object_root,
                                                   PX_NULL, PX_NULL, PX_NULL);
    PX_ObjectRegisterEvent(App.object_designer, PX_OBJECT_EVENT_SAVE, PainterEngine_DesignerOnSave,
                           PX_NULL);
    PX_ObjectRegisterEvent(App.object_designer, PX_OBJECT_EVENT_OPEN, PainterEngine_DesignerOnOpen,
                           PX_NULL);
...
px_bool  PainterEngine_InitializeWorld(px_int view_width, px_int view_height, px_int world_width,
                                       px_int world_height)
{
    ...
    if (PX_FileExist("assets/game.c"))
    {
        ...
        if (!PX_WorldInitialize(&App.runtime.mp_dynamic, &App.world, world_width, world_height, view_width,
                                view_height, (const px_char *)io.buffer))
        {
            return PX_FALSE;
        }
        PX_FreeIOData(&io);
...
{
#ifdef PAINTERENGIN_FILE_SUPPORT
    PX_LoadUIFormFile(&App.runtime.mp_dynamic, App.object_designer, App.object_root, PX_NULL,
                      "assets/objects.json");
#endif
}
...
px_bool PX_ApplicationInitialize(PX_Application *pApp, px_int screen_width, px_int screen_height)
{
    ...
    if (pApp->runtime.window_height && pApp->runtime.window_width && pApp->runtime.surface_height &&
