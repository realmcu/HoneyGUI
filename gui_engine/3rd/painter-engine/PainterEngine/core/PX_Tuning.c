...
px_void PX_TuningInitialize(px_memorypool *mp, PX_Tuning *tuning, px_int SampleRate,
                            px_double pitchShift, px_double timeShift, px_double window[], px_double filter[], px_double fix[],
                            PX_TUNING_WINDOW_SIZE windowsize)
{
    px_int i;
    px_int N;
...
px_void PX_TuningSortSpectrum(PX_Tuning *tuning, px_complex AnalysisFrame[])
{
    ...
    {
        amplitude = PX_sqrtd(AnalysisFrame[i].re * AnalysisFrame[i].re + AnalysisFrame[i].im *
                             AnalysisFrame[i].im);
...
px_int PX_TuningFilter(PX_Tuning *tuning, _IN px_double frame_unit[], px_int Size,
                       _OUT px_double out_unit[])
{
    ...
    resampleAllocSize += (px_int)((1 - PX_FRAC(tuning->previousInterpolationOffset)) /
                                  tuning->interpolationFactor);
    resampleAllocSize += 1;
    ...
    while (PX_TRUE)
    {
        ...
        {
            PX_memcpy(tuning->previous_frame, resampledFrame + (blockCount - 3)*blocksize,
                      blocksize * 3 * sizeof(px_double));
        ...
        {
            out_unit[outSize] = AnalysisFrame[i].re * tuning->window[i] + tuning->half_previous_out[i] *
                                tuning->window[i + blocksize];
            outSize++;
