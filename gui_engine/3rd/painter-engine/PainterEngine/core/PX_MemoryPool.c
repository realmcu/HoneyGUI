...
px_void PX_UpdateMaxFreqSize(px_memorypool *MP)
{
    ...
    for (i = 0; i < MP->FreeTableCount; i++)
    {
        ...
        if ((Size = (px_uint)(((px_char *)itNode->EndAddr - (px_char *)itNode->StartAddr) + 1)) >
...
px_void *MP_Malloc(px_memorypool *MP, px_uint Size)
{
    ...
    for (DEBUG_i = 0; DEBUG_i < sizeof(MP->DEBUG_allocdata) / sizeof(MP->DEBUG_allocdata[0]); DEBUG_i++)
    {
        ...
        {
            MP_Append_data *pAppend;
            pAppend = (MP_Append_data *)((px_uchar *)MP->DEBUG_allocdata[DEBUG_i].endAddr - sizeof(
                                             MP_Append_data) + 1);
    ...
    if (MemNode != 0)
    {
        ...
        for (DEBUG_i = 0; DEBUG_i < sizeof(MP->DEBUG_allocdata) / sizeof(MP->DEBUG_allocdata[0]); DEBUG_i++)
        {
            ...
            {
                MP->DEBUG_allocdata[DEBUG_i].offset = (px_dword)((px_byte *)MemNode->StartAddr -
                                                                 (px_byte *)MP->StartAddr);
    ...
    if ((px_char *)FreeNode.EndAddr + 1 == (px_char *)MP->AllocAddr)
    {
        for (i = 0; i < MP->FreeTableCount; i++)
        {
            ...
            if ((px_char *)itNode->EndAddr + 1 == (px_char *)FreeNode.StartAddr)
            {
                ...
                MP->FreeSize += (px_uint32)((px_char *)FreeNode.EndAddr - (px_char *)FreeNode.StartAddr + sizeof(
                                                MemoryNode) + 1);
                MP->FreeSize += (px_uint32)((px_char *)itNode->EndAddr - (px_char *)itNode->StartAddr + 1 + sizeof(
                                                MemoryNode));
                PX_MemoryPoolRemoveFreeNode(MP, i);
                goto _END;
    ...
    for (DEBUG_i = 0; DEBUG_i < sizeof(MP->DEBUG_allocdata) / sizeof(MP->DEBUG_allocdata[0]); DEBUG_i++)
    {
        if (MP->DEBUG_allocdata[DEBUG_i].startAddr != PX_NULL)
        {
            for (i = 0; i < MP->FreeTableCount; i++)
            {
                itNode = PX_MemoryPool_GetFreeTable(MP, i);
                if (MP->DEBUG_allocdata[DEBUG_i].startAddr >= itNode->StartAddr &&
                    MP->DEBUG_allocdata[DEBUG_i].startAddr <= itNode->EndAddr)
                {
                    //error free
                    PX_ASSERT();
                }
                if (MP->DEBUG_allocdata[DEBUG_i].endAddr >= itNode->StartAddr &&
                    MP->DEBUG_allocdata[DEBUG_i].endAddr <= itNode->EndAddr)
                {
                    PX_ASSERT();
                }
            }
...
px_uint MP_Size(px_memorypool *Pool, px_void *pAddress)
{
    ...
    TempNode = (MemoryNode *)TempPointer;
#if defined(PX_DEBUG_MODE) && defined(PX_MEMORYPOOL_DEBUG_CHECK)
    return (px_uint)(((px_char *)(TempNode->EndAddr) - (px_char *)(TempNode->StartAddr)) + 1 - sizeof(
                         MP_Append_data));
#else
