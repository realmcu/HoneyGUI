    ...
    case PX_WEBSOCKET_STATE_OPEN:
        {
            ...
            if (PX_HttpCheckContent((const px_char *)pInstance->cache.buffer))
            {
                ...
                if (packetSize)
                {
                    ...
                    if (PX_HttpGetContent((const px_char *)pInstance->cache.buffer, "Sec-WebSocket-Key", payload,
                                          sizeof(payload)))
                    {
            ...
            while (PX_TRUE)
            {
                if (pInstance->cache.usedsize >= headersize)
                {
                    ...
                    if (pInstance->cache.usedsize >= payloadLen + headersize + maskkey_size)
                    {
                        ...
                        PX_MemoryRemove(&pInstance->cache, 0,
                                        payloadLen + (px_int)sizeof(PX_WebSocketMessageHeader) + maskkey_size - 1);
                    }
                }
                else
                {
