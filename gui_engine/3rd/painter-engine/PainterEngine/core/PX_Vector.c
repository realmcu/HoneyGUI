...
#endif
    PX_memcpy((px_byte *)vec->data + (index + 1)*vec->nodesize,
              (px_byte *)vec->data + (index)*vec->nodesize, (vec->size - index - 1)*vec->nodesize);
    PX_memcpy((px_byte *)vec->data + (index)*vec->nodesize, data, vec->nodesize);
    return PX_TRUE;
}
...
px_bool PX_VectorErase(px_vector *vec, px_int index)
{
    ...
    {
        PX_memcpy((px_byte *)vec->data + i * vec->nodesize, (px_byte *)vec->data + (i + 1)*vec->nodesize,
                  vec->nodesize);
    }
    vec->size--;
    return PX_TRUE;
