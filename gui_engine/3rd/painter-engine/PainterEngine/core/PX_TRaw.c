...
px_bool PX_TRawVerify(px_void *data, px_int size)
{
    ...
    if (pHeader->Magic[0] != 'T' || pHeader->Magic[1] != 'R' || pHeader->Magic[2] != 'A' ||
        pHeader->Magic[3] != 'W')
    {
        return PX_FALSE;
    }
...
px_bool PX_TRawBuild(px_surface *psurface, px_byte *TRawBuffer, px_int *size)
{
    ...
    *size = _outsize;
    ...
    if (TRawBuffer)
    {
        ...
        PX_memcpy(TRawBuffer + sizeof(header), psurface->surfaceBuffer,
                  psurface->height * psurface->width * sizeof(px_color));
        return PX_TRUE;
    }
    else
    {
