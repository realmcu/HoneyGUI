...
px_void PX_TextureRenderClipMirror(px_surface *psurface, px_texture *tex, px_int x, px_int y,
                                   px_int clipx, px_int clipy, px_int clipw, px_int cliph, PX_ALIGN refPoint,
                                   PX_TEXTURERENDER_BLEND *blend, PX_TEXTURERENDER_MIRRROR_MODE mirrorMode)
{
    px_int i, j;
    px_int bR, bG, bB, bA;
    ...
    if (blend)
    {
        ...
        for (j = 0; j < cliph; j++)
        {
            for (i = 0; i < clipw; i++)
            {
                ...
                if (x + i < psurface->limit_left || x + i > psurface->limit_right || y + j < psurface->limit_top ||
                    y + j > psurface->limit_bottom)
                {
                    PX_ASSERT();
                }
...
px_void PX_TextureRenderClip(px_surface *psurface, px_texture *tex, px_int x, px_int y,
                             px_int clipx, px_int clipy, px_int clipw, px_int cliph, PX_ALIGN refPoint,
                             PX_TEXTURERENDER_BLEND *blend)
{
    PX_TextureRenderClipMirror(psurface, tex, x, y, clipx, clipy, clipw, cliph, refPoint, blend,
                               PX_TEXTURERENDER_MIRRROR_MODE_NONE);
}

px_void PX_TextureRenderMirror(px_surface *psurface, px_texture *tex, px_int x, px_int y,
                               PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend, PX_TEXTURERENDER_MIRRROR_MODE mirrorMode)
{
    PX_TextureRenderClipMirror(psurface, tex, x, y, 0, 0, tex->width, tex->height, refPoint, blend,
                               mirrorMode);
}
...
px_void PX_TextureCover(px_surface *psurface, px_texture *tex, px_int x, px_int y,
                        PX_ALIGN refPoint)
{
    ...
    {
        PX_memcpy(&psurface->surfaceBuffer[x + left + psurface->width * (y + j)],
                  &tex->surfaceBuffer[tex->width * (j) + left], sizeof(px_color) * (right - left + 1));
    }


}

px_void PX_TextureGetVisibleRange(px_texture *ptexture, px_int *pLeft, px_int *pRight, px_int *pTop,
                                  px_int *pBottom)
{
    px_int x = 0, y = 0;
...
px_void PX_TextureRenderPixelShader(px_surface *psurface, px_texture *tex, px_int x, px_int y,
                                    PX_ALIGN refPoint, PX_TexturePixelShader shader, px_void *ptr)
{
...
px_void PX_TextureRenderRotation(px_surface *psurface, px_texture *tex, px_int x, px_int y,
                                 PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend, px_int Angle)
{
    PX_TextureRenderRotation_sincos(psurface, tex, x, y, refPoint, blend, PX_sin_angle((px_float)Angle),
                                    PX_cos_angle((px_float)Angle));
}

px_void PX_TextureRenderRotation_vector(px_surface *psurface, px_texture *tex, px_int x, px_int y,
                                        PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend, px_point2D p_vector)
{
    if (p_vector.x || p_vector.y)
    {
        PX_TextureRenderRotation_sincos(psurface, tex, x, y, refPoint, blend, PX_Point2D_sin(p_vector),
                                        PX_Point2D_cos(p_vector));
    }
    else
    {
...
px_void PX_TextureRenderRotation_sincos(px_surface *psurface, px_texture *tex, px_int x, px_int y,
                                        PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend, px_float sinx, px_float cosx)
{
    ...
    rotMat._11 = cosx;        rotMat._12 = sinx;            rotMat._13 = 0.0f;
    rotMat._14 = 0.0f;
    rotMat._21 = -sinx;       rotMat._22 = cosx;            rotMat._23 = 0.0f;
    rotMat._24 = 0.0f;
    rotMat._31 = 0.0f;        rotMat._32 = 0.0f;            rotMat._33 = 1.0f;
    rotMat._34 = 0.0f;
    rotMat._41 = 0.0f;        rotMat._42 = 0.0f;            rotMat._43 = 0.0f;
    rotMat._44 = 1.0f;
    ...
    for (j = top; j < bottom; j++)
    {
        for (i = left; i < right; i++)
        {
            ...
            {
                PX_SurfaceDrawPixel(psurface, i + x, j + y, PX_COLOR((px_uchar)mixa, (px_uchar)mixr, (px_uchar)mixg,
                                                                     (px_uchar)mixb));
            }
...
px_void PX_TextureRenderMask(px_surface *psurface, px_texture *mask_tex, px_texture *map_tex,
                             px_int x, px_int y, PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend)
{
    px_int left, right, top, bottom, i, j;
    px_int bR, bG, bB, bA;
...
px_bool PX_TextureCreateScale(px_memorypool *mp, px_texture *resTexture, px_int newWidth,
                              px_int newHeight, px_texture *out)
{
...
px_bool PX_TextureCreateRotationRadian(px_memorypool *mp, px_texture *resTexture, px_float radian,
                                       px_texture *out)
{
    return PX_TextureCreateRotationAngle(mp, resTexture, (px_float)(radian * 180 / PX_PI), out);
}

px_bool PX_TextureCreateRotationAngle(px_memorypool *mp, px_texture *resTexture, px_float Angle,
                                      px_texture *out)
{
    ...
    for (j = 0; j < height; j++)
    {
        for (i = 0; i < width; i++)
        {
            ...
            PX_SurfaceSetPixel(out, i, j, PX_COLOR((px_uchar)mixa, (px_uchar)mixr, (px_uchar)mixg,
                                                   (px_uchar)mixb));
...
px_int PX_TextureGetRenderRange(px_texture *tex, px_int x, px_int y, px_int clipw, px_int cliph,
                                px_recti *render_range)
{
    px_int i, j;
...
px_void PX_SurfaceRender(px_surface *psurface, px_surface *surface, px_int x, px_int y,
                         PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend)
{
    PX_TextureRender(psurface, surface, x, y, refPoint, blend);
}

px_void PX_SurfaceCover(px_surface *pdestSurface, px_surface *pResSurface, px_int x, px_int y,
                        PX_ALIGN refPoint)
{
    PX_TextureCover(pdestSurface, pResSurface, x, y, refPoint);
}

px_void PX_SurfaceSetRect(px_surface *psurface, px_int left, px_int top, px_int right,
                          px_int bottom, px_color color)
{
    px_int i, mid;
    ...
    {
        PX_memdwordset(psurface->surfaceBuffer + i * psurface->width + left, color._argb.ucolor,
                       right - left + 1);
    }
}
...
px_void PX_TextureRenderEx(px_surface *psurface, px_texture *resTexture, px_int x, px_int y,
                           PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend, px_float scale, px_float Angle)
{
...
px_void PX_TextureRenderMaskEx(px_surface *psurface, px_texture *mask_tex, px_texture *map_tex,
                               px_int x, px_int y, PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend, px_float scale,
                               px_float Angle)
{
    px_int newWidth, newHeight;
...
px_void PX_TextureFill(px_memorypool *mp, px_texture *ptexture, px_int x, px_int y,
                       px_color test_color, px_float testRange0_1, px_color fill_color)
{
    typedef struct
    {
        px_int x, y;
    ...
    while (pstack.size)
    {
        ...
        //xo mark
        while (x > 0 &&
               PX_TextureFillTestColor(PX_SurfaceGetPixel(ptexture, x - 1, y), test_color, testRange0_1) &&
        ...
        for (; x < ptexture->width; x++)
        {
            ...
            if (y > 0)
            {
                ...
                if (PX_TextureFillTestColor(targettestcolor, test_color, testRange0_1) &&
                    targettestcolor._argb.ucolor != fill_color._argb.ucolor)
                {
                    if (up_newnode)
                    {
                        __PX_POINT pt;
            ...
            if (y < ptexture->height - 1)
            {
                ...
                if (PX_TextureFillTestColor(PX_SurfaceGetPixel(ptexture, x, y + 1), test_color, testRange0_1) &&
                    targettestcolor._argb.ucolor != fill_color._argb.ucolor)
                {
                    if (down_newnode)
                    {
...
px_void PX_TextureRegionRender(px_surface *psurface, px_texture *resTexture, px_int x, px_int y,
                               px_int oft_left, px_int oft_top, px_int oft_right, px_int oft_bottom, PX_ALIGN refPoint,
                               PX_TEXTURERENDER_BLEND *blend)
{
    px_int left, right, top, bottom, i, j;
    px_int bR, bG, bB, bA;
...
px_void PX_TextureRegionCopy(px_surface *psurface, px_texture *resTexture, px_int x, px_int y,
                             px_int oft_left, px_int oft_top, px_int oft_right, px_int oft_bottom, PX_ALIGN refPoint,
                             PX_TEXTURERENDER_BLEND *blend)
{
    px_int left, right, top, bottom, i, j;
...
px_bool PX_ShapeCreateFromTexture(px_memorypool *mp, px_shape *shape, px_texture *texture)
{
    ...
    if (p != PX_NULL)
    {
        ...
        for (y = 0; y < texture->height; y++)
        {
            ...
            {
                shape->alpha[y * shape->width + x] = ((px_color *)texture->surfaceBuffer)[y * texture->width +
                                                                                          x]._argb.a;
            }
        }
        return PX_TRUE;
...
px_void PX_ShapeRender(px_surface *psurface, px_shape *shape, px_int x, px_int y, PX_ALIGN refPoint,
                       px_color blendColor)
{
    px_int left, right, top, bottom, i, j;
    px_uchar *pdata;
...
px_void PX_ShapeRenderEx(px_surface *psurface, px_shape *shape, px_int x, px_int y,
                         PX_ALIGN refPoint, px_color blendColor, px_float scale, px_float Angle)
{
    px_int newWidth, newHeight;
...
px_void PX_ShapeRenderEx_sincos(px_surface *psurface, px_shape *shape, px_int x, px_int y,
                                PX_ALIGN refPoint, px_color blendColor, px_float scale, px_float sinx, px_float cosx)
{
    px_int newWidth, newHeight;
...
px_void PX_ShapeRenderEx_vector(px_surface *psurface, px_shape *shape, px_int x, px_int y,
                                PX_ALIGN refPoint, px_color blendColor, px_float scale, px_point2D p_vector)
{
    PX_ShapeRenderEx_sincos(psurface, shape, x, y, refPoint, blendColor, scale,
                            PX_Point2D_sin(p_vector), PX_Point2D_cos(p_vector));
}

px_void PX_TextureRenderEx_sincos(px_surface *psurface, px_texture *resTexture, px_int x, px_int y,
                                  PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend, px_float scale, px_float sinx, px_float cosx)
{
    px_int newWidth, newHeight;
...
px_void PX_TextureRenderEx_vector(px_surface *psurface, px_texture *resTexture, px_int x, px_int y,
                                  PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend, px_float scale, px_point2D p_vector)
{
    PX_TextureRenderEx_sincos(psurface, resTexture, x, y, refPoint, blend, scale,
                              PX_Point2D_sin(p_vector), PX_Point2D_cos(p_vector));
}
...
px_bool PX_TextureCopy(px_memorypool *mp, px_texture *restexture, px_texture *dest)
{
    ...
    PX_memcpy(dest->surfaceBuffer, restexture->surfaceBuffer,
              restexture->width * restexture->height * sizeof(px_color));
    return PX_TRUE;
}
...
px_bool PX_TextureScaleToTexture(px_texture *resTexture,  px_texture *out)
{
    ...
    for (yoft = 0; yoft < newHeight; yoft++)
    {
        for (xoft = 0; xoft < newWidth; xoft++)
        {
            ...
            mixg > 255 ? mixg = 255 : 0;
            mixb > 255 ? mixb = 255 : 0;
            Dst[xoft + yoft * newWidth] = PX_COLOR((px_uchar)PX_APO(mixa), (px_uchar)mixr, (px_uchar)mixg,
                                                   (px_uchar)mixb);;
        }
    }
    return PX_TRUE;
