...
px_bool PX_PngVerify(px_byte *ppngbuffer, px_int size, px_int *width, px_int *height,
                     PX_PNG_FORMAT *format)
{
    ...
    do //check header
    {
        ...
        if (width)
        {
            *width = (ppngbuffer[16] << 24) + (ppngbuffer[17] << 16) + (ppngbuffer[18] << 8) +
                     (ppngbuffer[19]);
        }
        if (height)
        {
            *height = (ppngbuffer[20] << 24) + (ppngbuffer[21] << 16) + (ppngbuffer[22] << 8) +
                      (ppngbuffer[23]);
        }
...
static px_bool PX_png_unfilter_scanline(px_byte *recon, const px_byte *scanline,
                                        const px_byte *precon, px_uint bytewidth, px_byte filterType, px_uint length)
{
    px_uint i;
    switch (filterType)
    {
    ...
    case 4:
        if (precon)
        {
            ...
            for (i = bytewidth; i < length; i++)
            {
                recon[i] = (px_byte)(scanline[i] + PX_png_paeth_predictor(recon[i - bytewidth], precon[i],
                                                                          precon[i - bytewidth]));
            }
        }
        else
...
static px_bool PX_png_unfilter(px_byte *out, const px_byte *in, px_uint w, px_uint h, px_uint bpp)
{
    ...
    for (y = 0; y < h; y++)
    {
        ...
        if (!PX_png_unfilter_scanline(&out[outindex], &in[inindex + 1], prevline, bytewidth, filterType,
                                      linebytes))
        {
            return PX_FALSE;
        }
...
static px_void PX_png_remove_padding_bits(px_byte *out, const px_byte *in, px_uint olinebits,
                                          px_uint ilinebits, px_uint h)
{
    px_uint y;
...
static px_bool PX_png_process_scanlines(px_byte *out, const px_byte *in, px_uint width,
                                        px_uint height, px_uint bpp)
{
...
px_bool PX_PngToXBuffer(px_memorypool *mp, px_byte *ppngbuffer, px_int size,
                        px_surface *prendersurface, px_shape *pshape)
{
    px_int width, height;
    PX_PNG_FORMAT format;
    px_byte *pchunk;
    ...
    do
    {
        ...
        if (!PX_png_process_scanlines(imagedata, inflate_data.buffer, width, height,
                                      PX_PngGetFormatBits(format)))
        {
            PX_MemoryFree(&inflate_data);
...
px_bool PX_PngToRenderBuffer(px_memorypool *calcBuffer, px_byte *ppngbuffer, px_int size,
                             px_surface *prenderbuffer)
{
    return PX_PngToXBuffer(calcBuffer, ppngbuffer, size, prenderbuffer, PX_NULL);
}

px_bool PX_PngToShapeBuffer(px_memorypool *calcBuffer, px_byte *ppngbuffer, px_int size,
                            px_shape *pshapebuffer)
{
