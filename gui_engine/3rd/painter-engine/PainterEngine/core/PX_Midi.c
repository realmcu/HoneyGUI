#include "PX_Midi.h"
px_bool PX_MidiInitialize(PX_Midi *midi, px_memorypool *mp, PX_MidiCallback callback,
                          px_void *userptr)
{
...
px_void PX_MidiNoteToString(px_byte note, px_char out[8])
{
    ...
    PX_sprintf2(out, 8, "%1%2", PX_STRINGFORMAT_STRING(note_content[note % 12]),
                PX_STRINGFORMAT_INT(note / 12 - 1));
}
