...
px_void PX_ArleCompress(px_byte *_in, px_uint input_size, px_byte *_out, px_uint *_outsize)
{
    ...
    *_outsize = 0;
    ...
    while (r_cursor < input_size)
    {
        ...
        {
            wsize = PX_ArleCompressData(_in + p_cursor, r_cursor - p_cursor, type,
                                        _out ? _out + w_cursor : PX_NULL);
            *_outsize += wsize;
            w_cursor += wsize;
            p_cursor = r_cursor;
    ...
    //encode last token
    *_outsize += PX_ArleCompressData(_in + p_cursor, r_cursor - p_cursor, type,
                                     _out ? _out + w_cursor : PX_NULL);
    return;
}
