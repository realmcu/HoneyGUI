    ...
    case PX_UDP_IP_TYPE_IPV4:
        {
            ...
            while (size > 0)
            {
                if ((length = sendto(udp->socket, (const char *)buffer, size, 0, (LPSOCKADDR)&to,
    ...
    case PX_UDP_IP_TYPE_IPV4:
        {
            ...
            if ((ReturnSize = recvfrom(udp->socket, (char *)buffer, buffersize, 0, (LPSOCKADDR)&in,
...
unsigned int PX_UDPGetHostByName(const char *host, unsigned int dns_addr)
{
    ...
    if (PX_UDPInitialize(&udp, PX_UDP_IP_TYPE_IPV4))
    {
        ...
        while (_tryt--)
        {
            ...
            {
                return (content[size - 1] << 24) + (content[size - 2] << 16) + (content[size - 3] << 8) +
                       (content[size - 4]);
            }
        }
