...
void nsvgDeleteRasterizer(NSVGrasterizer *r)
{
    ...
    if (r->edges) { gui_free(r->edges); }
    if (r->points) { gui_free(r->points); }
    if (r->points2) { gui_free(r->points2); }
    if (r->scanline) { gui_free(r->scanline); }
...
static void nsvg__addPathPoint(NSVGrasterizer *r, float x, float y, int flags)
{
    ...
    if (r->npoints + 1 > r->cpoints)
    {
        ...
        r->points = (NSVGpoint *)gui_realloc(r->points, sizeof(NSVGpoint) * r->cpoints);
...
static void nsvg__duplicatePoints(NSVGrasterizer *r)
{
    if (r->npoints > r->cpoints2)
    {
        ...
        r->points2 = (NSVGpoint *)gui_realloc(r->points2, sizeof(NSVGpoint) * r->cpoints2);
...
static void nsvg__addEdge(NSVGrasterizer *r, float x0, float y0, float x1, float y1)
{
    ...
    if (r->nedges + 1 > r->cedges)
    {
        ...
        r->edges = (NSVGedge *)gui_realloc(r->edges, sizeof(NSVGedge) * r->cedges);
                               ...
                               int *xmin, int *xmax)
{
    ...
    if (i < len && j >= 0)
    {
        ...
        else
        {
            ...
            {
                scanline[i] = (unsigned char)(scanline[i] + (((NSVG__FIX - (x0 & NSVG__FIXMASK)) * maxWeight) >>
                                                             NSVG__FIXSHIFT));
            }
            else
            {
            ...
            {
                scanline[j] = (unsigned char)(scanline[j] + (((x1 & NSVG__FIXMASK) * maxWeight) >>
                                                             NSVG__FIXSHIFT));
            }
            else
            {
...
//                  NSVG__assert(z->valid);
        ...
        if (xmin <= xmax)
        {
            nsvg__scanlineSolid(&r->bitmap[y * r->stride] + xmin * 4, xmax - xmin + 1, &r->scanline[xmin], xmin,
                                y, tx, ty, scale, cache);
        }
    }
...
static void nsvg__initPaint(NSVGcachedPaint *cache, NSVGpaint *paint, float opacity)
{
    ...
    {
        cache->colors[0] = nsvg__applyOpacity(paint->d.color, opacity);
        return;
    }

    grad = paint->d.gradient;
                   ...
                   unsigned char *dst, int w, int h, int stride)
{
    ...
    {
        r->cscanline = w;
        r->scanline = (unsigned char *)gui_realloc(r->scanline, w);
