                                      ...
                                      uint8_t *qrcode)
{
    ...
    for (size_t i = 0; i < len; i++)
    {
        ...
        appendBitsToBuffer((unsigned int)seg->numChars, numCharCountBits(seg->mode, version), qrcode,
                           &bitLen);
...
uint8_t *qrcode_get_data(unsigned char *data, uint16_t data_length, uint8_t flag)
{
    ...
    uint8_t *tempBuffer = gui_malloc(qrcodegen_BUFFER_LEN_MAX);
    app_qrcode.gen_version_min = qrcodegen_VERSION_MIN;
    app_qrcode.gen_version_max = qrcodegen_VERSION_MAX;

    if (flag == 0)//text
    {
        bool gen_qbcode_ret = qrcodegen_encodeText((char *)data, tempBuffer, app_qrcode.qrcode, errCorLvl,
                                                   app_qrcode.gen_version_min, app_qrcode.gen_version_max,
                                                   qrcodegen_Mask_AUTO, true);
        //gui_log("qrcode size %d", qrcodegen_getSize(app_qrcode.qrcode));
        if (gen_qbcode_ret)
        {
    ...
    {
        memcpy(tempBuffer, data, qrcodegen_BUFFER_LEN_MAX);
        bool gen_qbcode_ret = qrcodegen_encodeBinary(tempBuffer, data_length, app_qrcode.qrcode, errCorLvl,
