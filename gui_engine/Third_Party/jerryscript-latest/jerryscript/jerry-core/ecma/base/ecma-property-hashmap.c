 ...
 */
static const uint8_t ecma_property_hashmap_steps[ECMA_PROPERTY_HASHMAP_NUMBER_OF_STEPS]
JERRY_CONST_DATA =
{
...
ecma_property_hashmap_create(ecma_object_t *object_p)  /**< object */
{
    ...
    ecma_property_hashmap_t *hashmap_p = (ecma_property_hashmap_t *)
                                         jmem_heap_alloc_block_null_on_error(total_size);
    ...
    while (prop_iter_p != NULL)
    {
        ...
        for (int i = 0; i < ECMA_PROPERTY_PAIR_ITEM_COUNT; i++)
        {
            ...
            uint32_t step = ecma_property_hashmap_steps[entry_index & (ECMA_PROPERTY_HASHMAP_NUMBER_OF_STEPS -
                                                                       1)];
