...
{
    JERRY_UNUSED(this_arg);
    return ecma_builtin_global_object_decode_uri_helper(encoded_uri_component,
                                  ...
                                  ecma_value_t arg) /**< routine's first argument */
{
    ...
    while (input_curr_p < input_end_p)
    {
        ...
        else if (chr > ECMA_ESCAPE_MAXIMUM_BYTE_VALUE)
        {
            ...
            ecma_builtin_global_object_byte_to_hex(output_char_p + 1,
                                                   (lit_utf8_byte_t)(chr >> JERRY_BITSINBYTE));
