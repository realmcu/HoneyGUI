            ...
            else
            {
                ...
                if (length >= ECMA_FUNCTION_APPLY_ARGUMENT_COUNT_LIMIT)
                {
                    ret_value = ecma_raise_range_error(
                                    ECMA_ERR_MSG("Too many arguments declared for Function.apply()."));
                }
                else
                {
                    /* 6. */
                                            ...
                                            ecma_length_t arguments_number) /**< number of arguments */
{
    ...
    else
    {
            ...
            || (arguments_number == 1 && !ecma_is_value_integer_number(arguments_list_p[0])))
        {
            ...
            if (arguments_number != 0)
            {
                ext_function_p->u.bound_function.args_len_or_this = ecma_copy_value_if_not_object(
                                                                        arguments_list_p[0]);
            }
        }
        else
 ...
 */
ecma_value_t
ecma_builtin_function_prototype_dispatch_call(const ecma_value_t
 ...
 */
ecma_value_t
ecma_builtin_function_prototype_dispatch_construct(const ecma_value_t
                                                   *arguments_list_p,  /**< arguments list */
                                                   ecma_length_t arguments_list_len) /**< number of arguments */
{
    JERRY_ASSERT(arguments_list_len == 0 || arguments_list_p != NULL);
