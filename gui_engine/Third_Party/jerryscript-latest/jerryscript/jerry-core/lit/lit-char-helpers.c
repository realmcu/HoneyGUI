...
lit_char_is_unicode_letter(ecma_char_t c)  /**< code unit */
{
    return (search_char_in_interval_array(c,
            ...
            || search_char_in_char_array(c, lit_unicode_letter_chars,
                                         NUM_OF_ELEMENTS(lit_unicode_letter_chars)));
                                     ...
                                     *    one of HexDigit characters */
{
    ...
    else if (c >= LIT_CHAR_ASCII_LOWERCASE_LETTERS_HEX_BEGIN &&
             c <= LIT_CHAR_ASCII_LOWERCASE_LETTERS_HEX_END)
    {
                                          ...
                                          bool is_lowercase)            /**< is lowercase conversion */
{
    ...
    for (int i = 0; i < number_of_case_ranges; i++)
    {
        ...
        if (i % 2 == 0)
        {
            output_buffer_p[0] = is_lowercase ? (ecma_char_t)(lit_character_case_ranges[i + 1] + char_dist) :
                                 character;
        }
        else
        {
            output_buffer_p[0] = is_lowercase ? character : (ecma_char_t)(lit_character_case_ranges[i - 1] +
                                                                          char_dist);
        }
                       ...
                       ecma_length_t buffer_size) /**< buffer size */
{
    ...
    ecma_length_t lowercase_sequence = search_in_bidirectional_conversion_tables(character,
                                                                                 output_buffer_p, true);
                       ...
                       ecma_length_t buffer_size) /**< buffer size */
{
    ...
    ecma_length_t uppercase_sequence = search_in_bidirectional_conversion_tables(character,
                                                                                 output_buffer_p, false);
    ...
    for (int i = 0, j = 0; i < num_of_upper_case_special_ranges; i += 3, j++)
    {
        ...
        if (start_point <= character && character <= start_point + range_length)
        {
            output_buffer_p[0] = (ecma_char_t)(lit_upper_case_special_ranges[i + 1] +
                                               (character - start_point));
