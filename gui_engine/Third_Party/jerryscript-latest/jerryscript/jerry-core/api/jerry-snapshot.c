                           ...
                           snapshot_globals_t *globals_p) /**< snapshot globals */
{
    ...
    if (globals_p->snapshot_buffer_write_offset > JERRY_SNAPSHOT_MAXIMUM_WRITE_OFFSET)
    {
        ...
        globals_p->snapshot_error = jerry_create_error(JERRY_ERROR_RANGE,
                                                       (const jerry_char_t *) error_message_p);
        return 0;
    }

    /* The snapshot generator always parses a single file,
     * so the base always starts right after the snapshot header. */
    uint32_t start_offset = (uint32_t)(globals_p->snapshot_buffer_write_offset - sizeof(
                                          ...
                                          ecma_value_t literal) /**< literal form the literal pool */
{
    ...
    ecma_string_t *error_message_p = ecma_new_ecma_string_from_utf8(error_prefix,
                                                                    sizeof(error_prefix) - 1);
 ...
 */
size_t
jerry_merge_snapshots(const uint32_t
                      ...
                      const char **error_p) /**< error description */
{
    ...
    for (uint32_t i = 0; i < number_of_snapshots; i++)
    {
        const jerry_snapshot_header_t *current_header_p = (const jerry_snapshot_header_t *)
                                                          inp_buffers_p[i];
        ...
        const uint8_t *literal_base_p = ((const uint8_t *) inp_buffers_p[i]) +
                                        current_header_p->lit_table_offset;
 ...
 */
jerry_value_t
jerry_load_function_snapshot(const uint32_t
                             *function_snapshot_p,  /**< snapshot of the function(s) */
                             const size_t function_snapshot_size, /**< size of the snapshot */
                             size_t func_index, /**< index of the function to load */
                             uint32_t exec_snapshot_opts) /**< jerry_exec_snapshot_opts_t option bits */
{
#ifdef JERRY_ENABLE_SNAPSHOT_EXEC
    return jerry_snapshot_result(function_snapshot_p, function_snapshot_size, func_index,
                                 exec_snapshot_opts, true);
#else /* !JERRY_ENABLE_SNAPSHOT_EXEC */
