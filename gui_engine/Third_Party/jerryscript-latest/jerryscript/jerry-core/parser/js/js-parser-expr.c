...
parser_push_result(parser_context_t *context_p)  /**< context */
{
    ...
    {
        context_p->last_cbc_opcode = (uint16_t) PARSER_TO_BINARY_OPERATION_WITH_RESULT(
    ...
    while (true)
    {
            ...
            context_p->token.type == LEXER_PROPERTY_SETTER)
        {
            ...
            lexer_expect_object_literal_id(context_p,
                                           LEXER_OBJ_IDENT_CLASS_METHOD | LEXER_OBJ_IDENT_ONLY_IDENTIFIERS);
        ...
        if (is_static)
        {
            context_p->last_cbc_opcode = PARSER_TO_EXT_OPCODE(is_computed ?
                                                              CBC_EXT_SET_STATIC_COMPUTED_PROPERTY_LITERAL
                                                              : CBC_EXT_SET_STATIC_PROPERTY_LITERAL);
            is_static = false;
        }
        else
        {
            context_p->last_cbc_opcode = (is_computed ? PARSER_TO_EXT_OPCODE(
                                              CBC_EXT_SET_COMPUTED_PROPERTY_LITERAL)
                                        ...
                                        *   false - otherwise (as an expression) */
{
    ...
    lexer_literal_t *constructor_literal_p = (lexer_literal_t *) parser_list_append(context_p,
                                                                                    &context_p->literal_pool);
