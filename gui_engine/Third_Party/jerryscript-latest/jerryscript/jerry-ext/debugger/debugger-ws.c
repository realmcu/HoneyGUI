                           ...
                           jerry_debugger_transport_receive_context_t *receive_context_p) /**< receive context */
{
    ...
    if ((message_p[0] & ~JERRYX_DEBUGGER_WEBSOCKET_OPCODE_MASK) != JERRYX_DEBUGGER_WEBSOCKET_FIN_BIT
        || (message_p[1] & JERRYX_DEBUGGER_WEBSOCKET_LENGTH_MASK) >
    ...
    if ((message_p[0] & JERRYX_DEBUGGER_WEBSOCKET_OPCODE_MASK) !=
        JERRYX_DEBUGGER_WEBSOCKET_BINARY_FRAME)
    {
    ...
    {
        /* Datagram packet. */
        JERRYX_ASSERT(receive_context_p->message_length == (message_length + sizeof(
                                                                jerryx_websocket_receive_header_t)));
    }
