...
inline renderer::adapter<BitmapT, BlenderT>::adapter(BitmapT &bitmap_, const rect_i *window,
    ...
    : _blender(blender),
      ...
      _limit_x((!window ||
                static_cast<int>(bitmap_.width()) < width(*window) ? bitmap_.width() : width(*window)) + _offset_x),
      _bitmap(bitmap_),
      _offset_y(window ? window->y1 : 0),
      _limit_y((!window ||
                static_cast<int>(bitmap_.height()) < height(*window) ? bitmap_.height() : height(
                    *window)) + _offset_y)
{   }
...
inline void fill(BitmapT &bitmap_, const rect_i &area, const BlenderT &blender)
{
    ...
    if (width > 0)
    {
        for (int y = agge_max(0, area.y1), limit_y = agge_min<int>(bitmap_.height(), area.y2); y < limit_y;
             ++y)
        {
