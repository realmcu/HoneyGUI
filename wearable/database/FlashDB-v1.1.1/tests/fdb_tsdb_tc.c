...
{
    uassert_true(fdb_tsdb_init(&test_tsdb, "test_ts", TEST_TS_PART_NAME, get_time, 128,
                               NULL) == FDB_NO_ERR);
}
...
static void test_fdb_tsl_append(void)
{
    ...
    for (i = 0; i < TEST_TS_COUNT; ++i)
    {
        ...
        uassert_true(fdb_tsl_append(&test_tsdb, fdb_blob_make(&blob, log, rt_strnlen(log,
                                                                                     sizeof(log)))) == FDB_NO_ERR);
    }
}
...
static void test_fdb_tsl_set_status(void)
{
    ...
    uassert_true(fdb_tsl_query_count(&test_tsdb, from, to,
                                     FDB_TSL_USER_STATUS1) == TEST_TS_USER_STATUS1_COUNT);
