...
static void test_fdb_create_kv_blob(void)
{
    ...
    read_len = fdb_kv_get_blob(&test_kvdb, TEST_KV_BLOB_NAME, fdb_blob_make(&blob, &read_tick,
                                                                            sizeof(read_tick)));
...
static void test_fdb_change_kv_blob(void)
{
    ...
    read_len = fdb_kv_get_blob(&test_kvdb, TEST_KV_BLOB_NAME, fdb_blob_make(&blob_obj, &read_tick,
                                                                            sizeof(read_tick)));
    uassert_int_equal(blob->saved.len, sizeof(read_tick));
    uassert_int_equal(blob->saved.len, read_len);
    uassert_int_not_equal(tick, read_tick);

    result = fdb_kv_set_blob(&test_kvdb, TEST_KV_BLOB_NAME, fdb_blob_make(&blob_obj, &tick,
                                                                          sizeof(tick)));
    uassert_true(result == FDB_NO_ERR);
