...
static fdb_err_t read_sector_info(fdb_tsdb_t db, uint32_t addr, tsdb_sec_info_t sector,
                                  bool traversal)
{
    ...
    sector->status = (fdb_sector_store_status_t) _fdb_get_status(sec_hdr.status,
                                                                 FDB_SECTOR_STORE_STATUS_NUM);
    sector->start_time = sec_hdr.start_time;
    sector->end_info_stat[0] = (fdb_tsl_status_t) _fdb_get_status(sec_hdr.end_info[0].status,
                                                                  FDB_TSL_STATUS_NUM);
    sector->end_info_stat[1] = (fdb_tsl_status_t) _fdb_get_status(sec_hdr.end_info[1].status,
                                                                  FDB_TSL_STATUS_NUM);
...
static void sector_iterator(fdb_tsdb_t db, tsdb_sec_info_t sector, fdb_sector_store_status_t status,
                            void *arg1,
...
static fdb_err_t update_sec_status(fdb_tsdb_t db, tsdb_sec_info_t sector, fdb_blob_t blob,
                                   fdb_time_t cur_time)
{
    fdb_err_t result = FDB_NO_ERR;
        ...
        sector->remain < LOG_IDX_DATA_SIZE + FDB_WG_ALIGN(blob->size))
    {
        ...
        uint32_t end_index = sector->empty_idx - LOG_IDX_DATA_SIZE, new_sec_addr,
                 cur_sec_addr = sector->addr;
        ...
        if (sector->end_info_stat[0] == FDB_TSL_UNUSED)
        {
            ...
            FLASH_WRITE(db, cur_sec_addr + SECTOR_END0_TIME_OFFSET, (uint32_t *)&db->last_time,
                        sizeof(fdb_time_t), false);
        ...
        else if (sector->end_info_stat[1] == FDB_TSL_UNUSED)
        {
            _FDB_WRITE_STATUS(db, cur_sec_addr + SECTOR_END1_STATUS_OFFSET, end_status, FDB_TSL_STATUS_NUM,
                              FDB_TSL_PRE_WRITE, false);
            FLASH_WRITE(db, cur_sec_addr + SECTOR_END1_TIME_OFFSET, (uint32_t *)&db->last_time,
                        sizeof(fdb_time_t), false);
 ...
 * @param arg callback argument
 */
void fdb_tsl_iter_by_time(fdb_tsdb_t db, fdb_time_t from, fdb_time_t to, fdb_tsl_cb cb,
                          void *cb_arg)
{
    struct tsdb_sec_info sector;
...
//    FDB_INFO("to %s", ctime((const time_t * )&to));
    ...
    do
    {
        ...
        if ((sector.status == FDB_SECTOR_STORE_USING || sector.status == FDB_SECTOR_STORE_FULL))
        {
            ...
            if ((found_start_tsl) || (!found_start_tsl && ((from >= sector.start_time &&
 ...
 * @return result
 */
fdb_err_t fdb_tsdb_init(fdb_tsdb_t db, const char *name, const char *path, fdb_get_time get_time,
                        size_t max_len, void *user_data)
{
    fdb_err_t result = FDB_NO_ERR;
    struct tsdb_sec_info sector;
