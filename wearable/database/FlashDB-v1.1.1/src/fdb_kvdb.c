...
static bool get_kv_from_cache(fdb_kvdb_t db, const char *name, size_t name_len, uint32_t *addr)
{
    ...
    for (i = 0; i < FDB_KV_CACHE_TABLE_SIZE; i++)
    {
            ...
            (db->kv_cache_table[i].name_crc == name_crc))
        {
            ...
            _fdb_flash_read((fdb_db_t)db, db->kv_cache_table[i].addr + KV_HDR_DATA_SIZE,
...
static uint32_t find_next_kv_addr(fdb_kvdb_t db, uint32_t start, uint32_t end)
{
    ...
    if (get_sector_from_cache(db, FDB_ALIGN_DOWN(start, db_sec_size(db)), &empty_kv) &&
        start == empty_kv)
    {
        return FAILED_ADDR;
    }
...
static fdb_err_t read_kv(fdb_kvdb_t db, fdb_kv_t kv)
{
    ...
    if (kv->len == ~0UL || kv->len > db_max_size(db) || kv->len < KV_NAME_LEN_OFFSET)
    {
        ...
        if (kv->status != FDB_KV_ERR_HDR)
        {
            ...
            _fdb_write_status((fdb_db_t)db, kv->addr.start, kv_hdr.status_table, FDB_KV_STATUS_NUM,
                              FDB_KV_ERR_HDR, true);
        }
        kv->crc_is_ok = false;
        return FDB_READ_ERR;
    ...
    for (len = 0, size = 0; len < crc_data_len; len += size)
    {
        ...
        _fdb_flash_read((fdb_db_t)db, kv->addr.start + KV_NAME_LEN_OFFSET + len, (uint32_t *) buf,
                        FDB_WG_ALIGN(size));
...
static fdb_err_t read_sector_info(fdb_kvdb_t db, uint32_t addr, kv_sec_info_t sector,
                                  bool traversal)
{
    fdb_err_t result = FDB_NO_ERR;
    ...
    sector->status.store = (fdb_sector_store_status_t) _fdb_get_status(sec_hdr.status_table.store,
                                                                       FDB_SECTOR_STORE_STATUS_NUM);
    sector->status.dirty = (fdb_sector_dirty_status_t) _fdb_get_status(sec_hdr.status_table.dirty,
...
static size_t get_kv(fdb_kvdb_t db, const char *key, void *value_buf, size_t buf_len,
                     size_t *value_len)
{
    struct fdb_kv kv;
    size_t read_len = 0;
...
char *fdb_kv_get(fdb_kvdb_t db, const char *key)
{
    ...
    if ((get_size = fdb_kv_get_blob(db, key, fdb_blob_make(&blob, value,
                                                           FDB_STR_KV_VALUE_MAX_SIZE))) > 0)
    {
...
static fdb_err_t write_kv_hdr(fdb_kvdb_t db, uint32_t addr, kv_hdr_data_t kv_hdr)
{
    ...
    result = _fdb_write_status((fdb_db_t)db, addr, kv_hdr->status_table, FDB_KV_STATUS_NUM,
                               FDB_KV_PRE_WRITE, false);
    if (result != FDB_NO_ERR)
    {
        return result;
    }
    /* write other header data */
    result = _fdb_flash_write((fdb_db_t)db, addr + KV_MAGIC_OFFSET, &kv_hdr->magic,
...
static fdb_err_t update_sec_status(fdb_kvdb_t db, kv_sec_info_t sector, size_t new_kv_len,
                                   bool *is_full)
{
    ...
    if (sector->status.store == FDB_SECTOR_STORE_EMPTY)
    {
        ...
        result = _fdb_write_status((fdb_db_t)db, sector->addr, status_table, FDB_SECTOR_STORE_STATUS_NUM,
...
static uint32_t alloc_kv(fdb_kvdb_t db, kv_sec_info_t sector, size_t kv_size)
{
    ...
    sector_iterator(db, sector, FDB_SECTOR_STORE_UNUSED, &empty_sector, &using_sector,
...
static fdb_err_t del_kv(fdb_kvdb_t db, const char *key, fdb_kv_t old_kv, bool complete_del)
{
    ...
    if (!complete_del)
    {
        result = _fdb_write_status((fdb_db_t)db, old_kv->addr.start, status_table, FDB_KV_STATUS_NUM,
                                   FDB_KV_PRE_DELETE, false);
    ...
    {
        result = _fdb_write_status((fdb_db_t)db, dirty_status_addr, status_table,
                                   FDB_SECTOR_DIRTY_STATUS_NUM, FDB_SECTOR_DIRTY_TRUE, true);
    }
...
static fdb_err_t move_kv(fdb_kvdb_t db, fdb_kv_t kv)
{
    ...
    /* start move the KV */
    {
        ...
        for (len = 0, size = 0; len < kv_len; len += size)
        {
            ...
            _fdb_flash_read((fdb_db_t)db, kv->addr.start + KV_MAGIC_OFFSET + len, (uint32_t *) buf,
                            FDB_WG_ALIGN(size));
...
static bool do_gc(kv_sec_info_t sector, void *arg1, void *arg2)
{
                             ...
                             sector->status.dirty == FDB_SECTOR_DIRTY_GC))
    {
        ...
        _fdb_write_status((fdb_db_t)db, sector->addr + SECTOR_DIRTY_OFFSET, status_table,
                          FDB_SECTOR_DIRTY_STATUS_NUM, FDB_SECTOR_DIRTY_GC, true);
        /* search all KV */
        kv.addr.start = sector->addr + SECTOR_HDR_DATA_SIZE;
        do
        {
...
static fdb_err_t create_kv_blob(fdb_kvdb_t db, kv_sec_info_t sector, const char *key,
                                const void *value, size_t len)
{
    ...
    if (kv_addr != FAILED_ADDR || (kv_addr = new_kv(db, sector, kv_hdr.len)) != FAILED_ADDR)
    {
        ...
        if (result == FDB_NO_ERR)
        {
            result = _fdb_write_status((fdb_db_t)db, kv_addr, kv_hdr.status_table, FDB_KV_STATUS_NUM,
                                       FDB_KV_WRITE, true);
        }
...
static void kv_auto_update(fdb_kvdb_t db)
{
    ...
    if (get_kv(db, VER_NUM_KV_NAME, &saved_ver_num, sizeof(size_t), NULL) > 0)
    {
        ...
        {
            size_t i, value_len;
            FDB_DEBUG("Update the KV from version %u to %u.\n", (uint32_t)saved_ver_num,
                      (uint32_t)setting_ver_num);
            for (i = 0; i < db->default_kvs.num; i++)
            {
                ...
                if (!find_kv(db, db->default_kvs.kvs[i].key, &db->cur_kv))
                {
                    ...
                    create_kv_blob(db, &db->cur_sector, db->default_kvs.kvs[i].key, db->default_kvs.kvs[i].value,
                                   value_len);
                }
            }
        }
...
fdb_err_t _fdb_kv_load(fdb_kvdb_t db)
{
    ...
    sector_iterator(db, &sector, FDB_SECTOR_STORE_UNUSED, &check_failed_count, db, check_sec_hdr_cb,
                    false);
