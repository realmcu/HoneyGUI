...
int fal_partition_init(void)
{
    ...
    {
        log_e("Setting partition table end offset address(%ld) out of flash bound(<%d).", part_table_offset,
              flash_dev->len);
        goto _exit;
    }
    ...
    /* find partition table location */
    {
        ...
        while (part_table_offset >= 0)
        {
            if (flash_dev->ops.read(part_table_offset, read_buf, sizeof(read_buf)) > 0)
            {
                ...
                {
                    read_magic_word = read_buf[0 + i] + (read_buf[1 + i] << 8) + (read_buf[2 + i] << 16) +
                                      (read_buf[3 + i] << 24);
...
int fal_partition_read(const struct fal_partition *part, uint32_t addr, uint8_t *buf, size_t size)
{
    ...
    if (flash_dev == NULL)
    {
        log_e("Partition read error! Don't found flash device(%s) of the partition(%s).", part->flash_name,
              part->name);
        return -1;
    }
...
int fal_partition_erase(const struct fal_partition *part, uint32_t addr, size_t size)
{
    ...
    if (flash_dev == NULL)
    {
        log_e("Partition erase error! Don't found flash device(%s) of the partition(%s).", part->flash_name,
              part->name);
        return -1;
    }
