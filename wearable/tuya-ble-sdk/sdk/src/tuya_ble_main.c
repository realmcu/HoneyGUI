...
{
    if (tuya_ble_timer_create(&tuya_ble_xtimer_connect_monitor, tuya_ble_connect_monitor_timeout_ms,
                              TUYA_BLE_TIMER_SINGLE_SHOT, tuya_ble_vtimer_conncet_monitor_callback) != TUYA_BLE_SUCCESS)
    {
...
#if TUYA_BLE_SELF_BUILT_TASK
    tuya_ble_os_task_create(&tuya_ble_task_handle, "tuya", tuya_ble_main_task, 0,
                            TUYA_BLE_TASK_STACK_SIZE, TUYA_BLE_TASK_PRIORITY);
    tuya_ble_os_msg_queue_create(&tuya_ble_queue_handle, MAX_NUMBER_OF_TUYA_MESSAGE,
                                 sizeof(tuya_ble_evt_param_t));
#endif
...
void tuya_ble_adv_change(void)
{
    ...
    if (tuya_ble_current_para.sys_settings.bound_flag == 1)
    {
        ...
        tuya_ble_encrypt_old_with_key(aes_key, tuya_ble_current_para.auth_settings.device_id, DEVICE_ID_LEN,
                                      aes_buf);
        ...
        tuya_ble_device_id_encrypt_v4(&adv_data[11], adv_data[7] - 3,
                                      tuya_ble_current_para.auth_settings.device_id, DEVICE_ID_LEN, encry_device_id);
...
||(TUYA_BLE_SECURE_CONNECTION_TYPE==TUYA_BLE_SECURE_CONNECTION_WITH_AUTH_KEY_FOR_QR_CODE_V2))
    ...
    if (tuya_ble_current_para.adv_local_name_len > 0)
    {
        ...
        memcpy(&scan_rsp_data[17], tuya_ble_current_para.adv_local_name,
               tuya_ble_current_para.adv_local_name_len);
    }
    adv_data_length = tuya_ble_current_para.pid_len + 15;
    scan_rsp_data_length = (tuya_ble_current_para.adv_local_name_len == 0 ?
                            (-2) : tuya_ble_current_para.adv_local_name_len) + 17;
    ...
    if (tuya_ble_current_para.adv_local_name_len > 0)
    {
        ...
        memcpy(&scan_rsp_data[26], tuya_ble_current_para.adv_local_name,
               tuya_ble_current_para.adv_local_name_len);
    }
    adv_data_length = tuya_ble_current_para.pid_len + 15;
    scan_rsp_data_length = (tuya_ble_current_para.adv_local_name_len == 0 ?
                            (-2) : tuya_ble_current_para.adv_local_name_len) + 26;

#endif

    TUYA_BLE_LOG_INFO("adv data changed ,current bound flag = %d",
    ...
    tuya_ble_device_info_characteristic_value_update(p_buf,
                                                     (adv_data_length + scan_rsp_data_length + 3));
...
void tuya_ble_adv_change(void)
{
    ...
    if (tuya_ble_current_para.sys_settings.bound_flag == 1)
    {
        ...
        tuya_ble_device_id_encrypt(aes_key, tuya_ble_current_para.pid_len,
                                   tuya_ble_current_para.auth_settings.device_id, DEVICE_ID_LEN, encry_device_id);
    ...
    else
    {
        ...
        tuya_ble_device_id_encrypt(tuya_ble_current_para.pid, tuya_ble_current_para.pid_len,
                                   tuya_ble_current_para.auth_settings.device_id, DEVICE_ID_LEN, encry_device_id);
...
void tuya_ble_adv_change(void)
{
    ...
    else
    {
        ...
        tuya_ble_device_id_encrypt(tuya_ble_current_para.pid, 8,
                                   tuya_ble_current_para.auth_settings.device_id, DEVICE_ID_LEN, encry_device_id);
