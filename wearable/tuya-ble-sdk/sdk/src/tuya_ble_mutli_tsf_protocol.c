...
#if MTP_DEBUG
#define MTP_PR_DEBUG(_fmt_, ...) \
    DBG_DIRECT("[mtp:dbg]%s:%d "_fmt_"\n\r", __FILE__, __LINE__, ##__VA_ARGS__)
#define MTP_PR_DEBUG_RAW(_fmt_, ...) \
    DBG_DIRECT(_fmt_, ##__VA_ARGS__)
#else
#define MTP_PR_DEBUG(...)
#define MTP_PR_DEBUG_RAW(_fmt_, ...)
#endif
...
#define MTP_PR_NOTICE(_fmt_, ...) \
    DBG_DIRECT("[mtp:notice]%s:%d "_fmt_"\n\r", __FILE__, __LINE__, ##__VA_ARGS__)
#define MTP_PR_ERR(_fmt_, ...) \
    DBG_DIRECT("[mtp:err]%s:%d "_fmt_"\n\r", __FILE__, __LINE__, ##__VA_ARGS__)
#endif
...
static frame_seq_t get_frame_seq(
    void) //Because this type of data is not used temporarily, it can be used for multi-threaded calls temporarily
{
...
***********************************************************/
mtp_ret trsmitr_send_pkg_encode(frm_trsmitr_proc_s *frm_trsmitr, uint8_t version, uint8_t *buf,
                                uint32_t len)
{
    if (((void *)0) == frm_trsmitr)
    {
...
*  Note: could get from encode data len and encode data by calling method
         get_trsmitr_subpkg_len() and get_trsmitr_subpkg()
***********************************************************/
mtp_ret trsmitr_send_pkg_encode_with_packet_length(frm_trsmitr_proc_s *frm_trsmitr,
                                                   uint32_t pkg_len_max, uint8_t version, uint8_t *buf, uint32_t len)
{
    if (((void *)0) == frm_trsmitr)
    {
        return MTP_INVALID_PARAM;
...
mtp_ret trsmitr_recv_pkg_decode(frm_trsmitr_proc_s *frm_trsmitr, uint8_t *raw_data,
