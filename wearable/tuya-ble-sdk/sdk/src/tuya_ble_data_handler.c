...
void tuya_ble_set_device_attachment_version(tuya_ble_attachment_version_data_t
                                            *p_attachment_version)
{
    memcpy(&tuya_ble_attachment_version, p_attachment_version,
           sizeof(tuya_ble_attachment_version_data_t));
}
...
static uint32_t ble_data_unpack(uint8_t *buf, uint32_t len)
{
    ...
    if ((offset + get_trsmitr_subpkg_len(&ty_trsmitr_proc)) <= air_recv_packet.recv_len_max)
    {
        if (air_recv_packet.recv_data)
        {
            memcpy(air_recv_packet.recv_data + offset, get_trsmitr_subpkg(&ty_trsmitr_proc),
                   get_trsmitr_subpkg_len(&ty_trsmitr_proc));
...
void tuya_ble_commonData_rx_proc(uint8_t *buf, uint16_t len)
{
    ...
    if (tuya_ble_current_para.sys_settings.bound_flag == 1)
    {
        if ((ENCRYPTION_MODE_NONE == air_recv_packet.recv_data[0]) ||
            (ENCRYPTION_MODE_FTM_KEY == air_recv_packet.recv_data[0]))
        {
    ...
    else
    {
        ...
        temp = tuya_ble_decryption(p_version, (uint8_t *)air_recv_packet.recv_data,
                                   air_recv_packet.recv_len, &air_recv_packet.decrypt_buf_len,
    ...
    TUYA_BLE_LOG_HEXDUMP_DEBUG("decryped data", (uint8_t *)air_recv_packet.de_encrypt_buf,
                               air_recv_packet.decrypt_buf_len);

    if (ble_cmd_data_crc_check((uint8_t *)air_recv_packet.de_encrypt_buf,
                               air_recv_packet.decrypt_buf_len) != 0)
    {
    ...
    if ((BONDING_CONN != tuya_ble_connect_status_get()) && (FRM_QRY_DEV_INFO_REQ != current_cmd) &&
        (PAIR_REQ != current_cmd)
        && (FRM_LOGIN_KEY_REQ != current_cmd) && (FRM_FACTORY_TEST_CMD != current_cmd) &&
        (FRM_NET_CONFIG_INFO_REQ != current_cmd) && (FRM_ANOMALY_UNBONDING_REQ != current_cmd)
        && (FRM_AUTHENTICATE_PHASE_1_REQ != current_cmd) && (FRM_AUTHENTICATE_PHASE_2_REQ != current_cmd) &&
    ...
    memcpy(ble_evt_buffer + 1, (uint8_t *)air_recv_packet.de_encrypt_buf,
           air_recv_packet.decrypt_buf_len);
    ...
    TUYA_BLE_LOG_DEBUG("BLE EVENT SEND-CMD:0x%02x - LEN:0x%02x", current_cmd,
                       air_recv_packet.decrypt_buf_len + 1);
...
static void tuya_ble_handle_authenticate_phase1(uint8_t *recv_data, uint16_t recv_len)
{
    ...
    if (((tuya_ble_current_para.sys_settings.bound_flag == 0) && (recv_data[13] == 0)) ||
        ((tuya_ble_current_para.sys_settings.bound_flag == 1) && (recv_data[13] == 1)))
    {
    ...
    if (recv_data[13] == 1)
    {
        if (data_len == 130)
        {
            ...
            TUYA_BLE_LOG_HEXDUMP_DEBUG("received server public key:", current_ser_cert_pub_key,
                                       sizeof(current_ser_cert_pub_key));
            TUYA_BLE_LOG_HEXDUMP_DEBUG("received server public key signature data:", sig_data,
                                       sizeof(sig_data));
    ...
    else
    {
        ...
        TUYA_BLE_LOG_HEXDUMP_DEBUG("get server public key from tuya ble current para:",
                                   current_ser_cert_pub_key, sizeof(current_ser_cert_pub_key));
    }
    ...
    if (p_buf[0] == 0)
    {
        ...
        TUYA_BLE_LOG_HEXDUMP_DEBUG("device generate random:", current_dev_random_for_sign,
...
static void tuya_ble_handle_authenticate_phase2(uint8_t *recv_data, uint16_t recv_len)
{
    ...
    if (recv_data[13] == 0)
    {
        ...
        TUYA_BLE_LOG_HEXDUMP_DEBUG("received signature data for device random:", sig_data,
                                   sizeof(sig_data));
    }
    else
    {
    ...
    TUYA_BLE_LOG_HEXDUMP_DEBUG("current device random:", current_dev_random_for_sign,
                               sizeof(current_dev_random_for_sign));
    TUYA_BLE_LOG_HEXDUMP_DEBUG("verify signature data of device random use server public key:",
                               current_ser_cert_pub_key, sizeof(current_ser_cert_pub_key));
    ...
    if (tuya_ble_current_para.sys_settings.bound_flag == 0)
    {
        ...
        {
            TUYA_BLE_LOG_DEBUG("tuya_ble_handle_authenticate_phase2 -> get dev cert length = %d .",
                               dev_cert_data_len);
            dev_cert_data[0] = 0;
            dev_cert_data[1] = 0;
            ...
            tuya_ble_commData_send(FRM_AUTHENTICATE_PHASE_2_RESP, ack_sn, dev_cert_data, dev_cert_data_len + 2,
                                   encry_mode);
        }
    }
    else
...
static void tuya_ble_handle_authenticate_phase3(uint8_t *recv_data, uint16_t recv_len)
{
    ...
    {
        TUYA_BLE_LOG_HEXDUMP_DEBUG("tuya_ble_handle_authenticate_phase3 -> server random sign succeed.",
                                   p_buf + 1, 64);
    }
...
static void tuya_ble_handle_pair_req(uint8_t *recv_data, uint16_t recv_len)
{
    ...
    if (0 == memcmp(&recv_data[13], tuya_ble_current_para.auth_settings.device_id, DEVICE_ID_LEN))
    {
        ...
        else
        {
            ...
            memcpy(tuya_ble_current_para.sys_settings.device_virtual_id, recv_data + 29 + LOGIN_KEY_LEN,
            ...
            if (TUYA_BLE_BEACON_KEY_ENABLE)
            {
                ...
                memcpy(event.device_login_key_vid_data.beacon_key, tuya_ble_current_para.sys_settings.beacon_key,
                       BEACON_KEY_LEN);
            }
#endif

            memcpy(event.device_login_key_vid_data.login_key, tuya_ble_current_para.sys_settings.login_key,
                   LOGIN_KEY_LEN);
            memcpy(event.device_login_key_vid_data.vid, tuya_ble_current_para.sys_settings.device_virtual_id,
                   DEVICE_VIRTUAL_ID_LEN);
...
void tuya_ble_device_unbond(void)
{
    ...
    TUYA_BLE_LOG_INFO("tuya_ble_device_unbond current bound flag = %d",
...
static void tuya_ble_handle_dp_data_write_req(uint8_t *recv_data, uint16_t recv_len)
{
    ...
    else
    {
        ...
        event.dp_received_data.sn = (recv_data[14] << 24) | (recv_data[15] << 16) |
                                    (recv_data[16] << 8) | recv_data[17];
...
static void tuya_ble_handle_dp_data_send_response(uint8_t *recv_data, uint16_t recv_len)
{
    ...
    event.dp_send_response_data.sn = recv_data[14] << 24 | recv_data[15] << 16 | recv_data[16] << 8 |
                                     recv_data[17];
    event.dp_send_response_data.ack = ((recv_data[18] & 0x80) ? DP_SEND_WITHOUT_RESPONSE :
...
static void tuya_ble_handle_dp_data_with_time_send_response(uint8_t *recv_data, uint16_t recv_len)
{
    ...
    event.dp_with_time_send_response_data.sn = recv_data[14] << 24 | recv_data[15] << 16 | recv_data[16]
                                               << 8 | recv_data[17];
    event.dp_with_time_send_response_data.ack = ((recv_data[18] & 0x80) ? DP_SEND_WITHOUT_RESPONSE :
                                                 DP_SEND_WITH_RESPONSE);
    if ((recv_data[18] & 0x0F) == 1)
    {
...
static void tuya_ble_handle_dp_data_with_flag_and_time_report_res(uint8_t *recv_data,
                                                                  uint16_t recv_len)
{
    tuya_ble_cb_evt_param_t event;
    uint16_t data_len = recv_data[11] << 8 | recv_data[12];
...
uint8_t tuya_ble_commData_send(uint16_t cmd, uint32_t ack_sn, uint8_t *data, uint16_t len,
                               uint8_t encryption_mode)
{
    ...
    TUYA_BLE_LOG_HEXDUMP_DEBUG("ble_commData_send plain data", (uint8_t *)air_send_packet.send_data,
                               air_send_packet.send_len);//
    ...
    p_version = (TUYA_BLE_PROTOCOL_VERSION_HIGN << 8) + TUYA_BLE_PROTOCOL_VERSION_LOW;
    if (tuya_ble_encryption(p_version, encryption_mode, iv, (uint8_t *)air_send_packet.send_data,
                            air_send_packet.send_len, &out_len,
                            (uint8_t *)(air_send_packet.encrypt_data_buf + en_len), &tuya_ble_current_para,
                            tuya_ble_pair_rand) == 0)
    {
    ...
    do
    {
        ret = trsmitr_send_pkg_encode_with_packet_length(&ty_trsmitr_proc_send, send_packet_data_len,
                                                         TUYA_BLE_PROTOCOL_VERSION_HIGN,
    ...
    TUYA_BLE_LOG_INFO("ble_commData_send len = %d , package_number = %d , protocol version : 0x%02x , error code : 0x%02x",
                      air_send_packet.encrypt_data_buf_len, package_number, TUYA_BLE_PROTOCOL_VERSION_HIGN, err);
