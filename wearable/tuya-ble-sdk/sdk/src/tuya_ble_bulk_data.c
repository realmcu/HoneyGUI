...
void tuya_ble_handle_bulk_data_req(uint16_t cmd, uint8_t *p_recv_data, uint32_t recv_data_len)
{
    ...
    case FRM_BULK_DATA_READ_DATA_REQ:
        ...
        event.bulk_req_data.bulk_type = p_recv_data[14];
        event.bulk_req_data.params.block_data_req_data.block_number = (p_recv_data[15] << 8) +
                                                                      p_recv_data[16];
        break;
...
tuya_ble_status_t tuya_ble_bulk_data_response(tuya_ble_bulk_data_response_t *p_data)
{
    ...
    if ((p_data->evt == TUYA_BLE_BULK_DATA_EVT_SEND_DATA) &&
        (p_data->params.send_res_data.current_block_length > (TUYA_BLE_SEND_MAX_DATA_LEN - 8)))
    {
    ...
    if (p_data->evt == TUYA_BLE_BULK_DATA_EVT_SEND_DATA)
    {
        ...
        evt.bulk_res_data.params.send_res_data.p_current_block_data = (uint8_t *)tuya_ble_malloc(
                                                                          p_data->params.send_res_data.current_block_length);
        if (evt.bulk_res_data.params.send_res_data.p_current_block_data == NULL)
        {
            return TUYA_BLE_ERR_NO_MEM;
        }
        memcpy(evt.bulk_res_data.params.send_res_data.p_current_block_data,
               p_data->params.send_res_data.p_current_block_data,
               p_data->params.send_res_data.current_block_length);
    }
...
void tuya_ble_handle_bulk_data_evt(tuya_ble_evt_param_t *evt)
{
    ...
    case TUYA_BLE_BULK_DATA_EVT_SEND_DATA :
        ...
        p_buf = (uint8_t *)tuya_ble_malloc(evt->bulk_res_data.params.send_res_data.current_block_length +
                                           8);
        if (p_buf)
        {
            p_buf[0] = 0;
            ...
            memcpy(&p_buf[8], evt->bulk_res_data.params.send_res_data.p_current_block_data,
                   evt->bulk_res_data.params.send_res_data.current_block_length);
    ...
    if (data_length > 0)
    {
        if (evt->bulk_res_data.evt == TUYA_BLE_BULK_DATA_EVT_READ_BLOCK)
        {
            ...
            if (evt->bulk_res_data.params.block_res_data.status == 0)
            {
                ...
                event.bulk_req_data.params.send_data_req_data.block_number =
                    evt->bulk_res_data.params.block_res_data.block_number;
