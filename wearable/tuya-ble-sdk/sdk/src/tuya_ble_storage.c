...
static uint32_t auth_settings_crc_get(tuya_ble_auth_settings_t const *p_settings)
{
    ...
    return tuya_ble_crc32_compute((uint8_t *)(p_settings) + 4, sizeof(tuya_ble_auth_settings_t) - 4,
                                  NULL);
}
...
static bool auth_settings_crc_ok(tuya_ble_auth_settings_t const *p_settings, uint8_t *flash_update)
{
    ...
    if (p_settings->crc != 0xFFFFFFFF)
    {
        ...
        {
            crc_old = tuya_ble_crc32_compute((uint8_t *)(p_settings) + 4,
                                             sizeof(tuya_ble_auth_settings_old_t) - 4, NULL);
...
{
    // The crc is calculated from the s_dfu_settings struct, except the crc itself and the init command
    return tuya_ble_crc32_compute((uint8_t *)(p_settings) + 4, sizeof(tuya_ble_sys_settings_t) - 4,
                                  NULL);
}
...
static bool sys_settings_crc_ok(tuya_ble_sys_settings_t  const *p_settings, uint8_t *flash_update)
{
    ...
    if (p_settings->crc != 0xFFFFFFFF)
    {
        ...
        {
            crc_old = tuya_ble_crc32_compute((uint8_t *)(p_settings) + 4,
                                             sizeof(tuya_ble_sys_settings_old_t) - 4, NULL);
            if (crc_old == p_settings->crc)
            {
                *flash_update = 1;
                return true;
...
uint32_t tuya_ble_storage_load_settings(void)
{
    ...
    p_storage_settings_auth = (tuya_ble_storage_auth_settings_t *)tuya_ble_malloc(sizeof(
            tuya_ble_storage_auth_settings_t));
    ...
    {
        tuya_ble_nv_read(TUYA_BLE_AUTH_FLASH_ADDR, (uint8_t *)&p_storage_settings_auth->flash_settings_auth,
                         sizeof(tuya_ble_auth_settings_t));
        tuya_ble_nv_read(TUYA_BLE_AUTH_FLASH_BACKUP_ADDR,
                         (uint8_t *)&p_storage_settings_auth->flash_settings_auth_backup, sizeof(tuya_ble_auth_settings_t));

        settings_valid = auth_settings_crc_ok(&p_storage_settings_auth->flash_settings_auth,
                                              &auth_settings_update);
        settings_backup_valid = auth_settings_crc_ok(&p_storage_settings_auth->flash_settings_auth_backup,
                                                     &auth_settings_update);

        if (settings_valid)
        {
            memcpy(&tuya_ble_current_para.auth_settings, &p_storage_settings_auth->flash_settings_auth,
                   sizeof(tuya_ble_auth_settings_t));
        }
        else if (settings_backup_valid)
        {
            memcpy(&tuya_ble_current_para.auth_settings, &p_storage_settings_auth->flash_settings_auth_backup,
                   sizeof(tuya_ble_auth_settings_t));
        }
        else
        {
    ...
    p_storage_settings_sys = (tuya_ble_storage_sys_settings_t *)tuya_ble_malloc(sizeof(
                                                                                    tuya_ble_storage_sys_settings_t));
    ...
    {
        tuya_ble_nv_read(TUYA_BLE_SYS_FLASH_ADDR, (uint8_t *)&p_storage_settings_sys->flash_settings_sys,
                         sizeof(tuya_ble_sys_settings_t));
        tuya_ble_nv_read(TUYA_BLE_SYS_FLASH_BACKUP_ADDR,
                         (uint8_t *)&p_storage_settings_sys->flash_settings_sys_backup, sizeof(tuya_ble_sys_settings_t));

        settings_valid = sys_settings_crc_ok(&p_storage_settings_sys->flash_settings_sys,
                                             &sys_settings_update);
        settings_backup_valid = sys_settings_crc_ok(&p_storage_settings_sys->flash_settings_sys_backup,
                                                    &sys_settings_update);

        if (settings_valid)
        {
            memcpy(&tuya_ble_current_para.sys_settings, &p_storage_settings_sys->flash_settings_sys,
                   sizeof(tuya_ble_sys_settings_t));
        }
        else if (settings_backup_valid)
        {
            memcpy(&tuya_ble_current_para.sys_settings, &p_storage_settings_sys->flash_settings_sys_backup,
                   sizeof(tuya_ble_sys_settings_t));
        }
        else
        {
    ...
    memcpy(tuya_ble_current_para.pid, tuya_ble_current_para.sys_settings.common_pid,
           tuya_ble_current_para.pid_len);
...
uint32_t tuya_ble_storage_save_auth_settings(void)
{
    ...
    tuya_ble_current_para.auth_settings.crc = tuya_ble_crc32_compute((uint8_t *)
                                                                     &tuya_ble_current_para.auth_settings + 4, sizeof(tuya_ble_current_para.auth_settings) - 4, NULL);

    if (tuya_ble_nv_erase(TUYA_BLE_AUTH_FLASH_ADDR, TUYA_NV_ERASE_MIN_SIZE) == TUYA_BLE_SUCCESS)
    {
        err_code = tuya_ble_nv_write(TUYA_BLE_AUTH_FLASH_ADDR,
                                     (uint8_t *)&tuya_ble_current_para.auth_settings, sizeof(tuya_ble_auth_settings_t));
        ...
        if (err_code == TUYA_BLE_SUCCESS)
        {
            ...
            {
                if (tuya_ble_nv_write(TUYA_BLE_AUTH_FLASH_BACKUP_ADDR,
                                      (uint8_t *)&tuya_ble_current_para.auth_settings,
                                      sizeof(tuya_ble_auth_settings_t)) != TUYA_BLE_SUCCESS)
                {
...
uint32_t tuya_ble_storage_save_sys_settings(void)
{
    ...
    tuya_ble_current_para.sys_settings.crc = tuya_ble_crc32_compute((uint8_t *)
                                                                    &tuya_ble_current_para.sys_settings + 4, sizeof(tuya_ble_sys_settings_t) - 4, NULL);

    if (tuya_ble_nv_erase(TUYA_BLE_SYS_FLASH_ADDR, TUYA_NV_ERASE_MIN_SIZE) == TUYA_BLE_SUCCESS)
    {
        err_code = tuya_ble_nv_write(TUYA_BLE_SYS_FLASH_ADDR,
                                     (uint8_t *)&tuya_ble_current_para.sys_settings, sizeof(tuya_ble_sys_settings_t));
        ...
        if (err_code == TUYA_BLE_SUCCESS)
        {
            ...
            {
                if (tuya_ble_nv_write(TUYA_BLE_SYS_FLASH_BACKUP_ADDR,
                                      (uint8_t *)&tuya_ble_current_para.sys_settings,
                                      sizeof(tuya_ble_sys_settings_t)) != TUYA_BLE_SUCCESS)
                {
 ...
 *
 * */
tuya_ble_status_t tuya_ble_storage_write_pid(tuya_ble_product_id_type_t pid_type, uint8_t pid_len,
                                             uint8_t *pid)
{
 ...
 *
 * */
tuya_ble_status_t tuya_ble_storage_write_auth_key_device_id_mac(uint8_t *auth_key,
                                                                uint8_t auth_key_len, uint8_t *device_id, uint8_t device_id_len,
                                                                uint8_t *mac, uint8_t mac_len, uint8_t *mac_string, uint8_t mac_string_len, uint8_t *pid,
                                                                uint8_t pid_len)
{
    tuya_ble_status_t ret = TUYA_BLE_SUCCESS;
    uint8_t is_write = 0;

    if (((auth_key_len != AUTH_KEY_LEN) && (auth_key_len != 0)) || ((device_id_len != DEVICE_ID_LEN) &&
                                                                    (device_id_len != 0)) || ((mac_len != MAC_LEN) && (mac_len != 0)))
    {
        ret = TUYA_BLE_ERR_INVALID_PARAM;
    }
    else
    {
        ...
        if (pid_len > 0)
        {
            if ((tuya_ble_current_para.auth_settings.pid_len != pid_len) ||
                (memcmp(tuya_ble_current_para.auth_settings.factory_pid, pid, pid_len) != 0))
            {
        ...
        if (is_write == 1)
        {
            ...
            if (tuya_ble_storage_private_data(PRIVATE_DATA_TUYA_AUTH_TOKEN,
                                              (uint8_t *)&tuya_ble_current_para.auth_settings, sizeof(tuya_ble_auth_settings_t)))
            {
                ret = TUYA_BLE_ERR_BUSY;
            }
            ...
            else
            {
                ...
                //if(tuya_ble_current_para.sys_settings.bound_flag==1)
                {
                    ...
                    TUYA_BLE_LOG_INFO("The state has changed, current bound flag = %d",
