...
bool tuya_ble_sleep_allowed_check(void)
{
    ...
    if ((tuya_ble_scheduler_queue_events_get() == 0) &&
        (tuya_ble_internal_production_test_with_ble_flag_get() == 0))
#endif
    {
        return true;
    }
...
static volatile tuya_ble_uart_rev_state_type_t current_uart_rev_state_type =
    TUYA_BLE_UART_REV_STATE_FOUND_NULL;
...
static bool tuya_ble_common_uart_data_unpack(uint8_t data)
{
    ...
    if (((UART_RX_Buffer_temp[0] == 0x55) || (UART_RX_Buffer_temp[0] == 0x66)) &&
        (UART_RX_Buffer_temp[1] == 0xAA) && ((UART_RX_Buffer_temp[2] == 0x00) ||
                                             (UART_RX_Buffer_temp[2] == 0x01)))
    {
 ...
 *
 * */
tuya_ble_status_t tuya_ble_device_update_product_id(tuya_ble_product_id_type_t type, uint8_t len,
                                                    uint8_t *p_buf)
{
    tuya_ble_evt_param_t event;
 ...
 *
 * */
tuya_ble_status_t tuya_ble_dp_data_send(uint32_t sn, tuya_ble_dp_data_send_type_t type,
                                        tuya_ble_dp_data_send_mode_t mode, tuya_ble_dp_data_send_ack_t ack, uint8_t *p_dp_data,
                                        uint32_t dp_data_len)
{
    ...
    {
        TUYA_BLE_LOG_ERROR("send dp data len error,data len = %d , max data len = %d", dp_data_len,
                           TUYA_BLE_SEND_MAX_DATA_LEN - 7);
 ...
 *
 * */
tuya_ble_status_t tuya_ble_dp_data_with_time_send(uint32_t sn, tuya_ble_dp_data_send_mode_t mode,
                                                  tuya_ble_dp_data_send_time_type_t time_type, uint8_t *p_time_data, uint8_t *p_dp_data,
                                                  uint32_t dp_data_len)
{
    tuya_ble_evt_param_t evt;
    uint8_t *ble_evt_buffer = NULL;
...
tuya_ble_status_t tuya_ble_dp_data_report(uint8_t *p_data, uint32_t len)
{
    ...
    {
        TUYA_BLE_LOG_ERROR("report dp data len error,data len = %d , max data len = %d", len,
 ...
 *
 * */
tuya_ble_status_t tuya_ble_dp_data_with_flag_report(uint16_t sn, tuya_ble_dp_data_send_mode_t mode,
                                                    uint8_t *p_data, uint32_t len)
{
    ...
    {
        TUYA_BLE_LOG_ERROR("report flag dp data len error,data len = %d , max data len = %d", len,
                           (TUYA_BLE_REPORT_MAX_DP_DATA_LEN - 3));
        return TUYA_BLE_ERR_INVALID_LENGTH;
    }
    ret = data_2_klvlist(p_data, len, &list, 0);
    if (MTP_OK != ret)
...
tuya_ble_status_t tuya_ble_dp_data_with_time_report(uint32_t timestamp, uint8_t *p_data,
                                                    uint32_t len)
{
    tuya_ble_evt_param_t evt;
    uint8_t *ble_evt_buffer;
 ...
 *
 * */
tuya_ble_status_t tuya_ble_dp_data_with_flag_and_time_report(uint16_t sn,
                                                             tuya_ble_dp_data_send_mode_t mode, uint32_t timestamp, uint8_t *p_data, uint32_t len)
{
    tuya_ble_evt_param_t evt;
    uint8_t *ble_evt_buffer = NULL;
    mtp_ret ret;
...
tuya_ble_status_t tuya_ble_dp_data_with_time_ms_string_report(uint8_t *time_string, uint8_t *p_data,
                                                              uint32_t len)
{
    tuya_ble_evt_param_t evt;
    uint8_t *ble_evt_buffer;
 ...
 *
 * */
tuya_ble_status_t tuya_ble_dp_data_with_flag_and_time_ms_string_report(uint16_t sn,
                                                                       tuya_ble_dp_data_send_mode_t mode, uint8_t *time_string, uint8_t *p_data, uint32_t len)
{
    tuya_ble_evt_param_t evt;
    uint8_t *ble_evt_buffer = NULL;
    mtp_ret ret;
 ...
 *.
 * */
tuya_ble_status_t tuya_ble_production_test_asynchronous_response(uint8_t channel, uint8_t *p_data,
                                                                 uint32_t len)
{
    tuya_ble_evt_param_t evt;
    uint8_t *ble_evt_buffer;
...
tuya_ble_status_t tuya_ble_br_edr_data_info_update(tuya_ble_br_edr_data_info_t *p_data)
{
    ...
    if ((p_data->name_len > sizeof(p_data->name)) || (p_data->connect_status >= BR_EDR_UNKNOW_STATUS) ||
        (p_data->is_paired > 1))
    {
        return TUYA_BLE_ERR_INVALID_PARAM;
    }
...
tuya_ble_status_t tuya_ble_sdk_init(tuya_ble_device_param_t *param_data)
{
    ...
    if ((param_data->use_ext_license_key != 1) || ((param_data->device_id_len != 16) &&
                                                   (param_data->device_id_len != 20)))
    {
    ...
    else
    {
        ...
        memcpy(tuya_ble_current_para.pid, tuya_ble_current_para.auth_settings.factory_pid,
    ...
    {
        memcpy(tuya_ble_current_para.adv_local_name, param_data->adv_local_name,
               param_data->adv_local_name_len);
    ...
    memcpy(tuya_ble_current_para.sys_settings.device_virtual_id, param_data->device_vid,
    ...
    TUYA_BLE_LOG_HEXDUMP_DEBUG("device_uuid", tuya_ble_current_para.auth_settings.device_id,
                               DEVICE_ID_LEN);
    TUYA_BLE_LOG_HEXDUMP_DEBUG("device_authkey", tuya_ble_current_para.auth_settings.auth_key,
                               AUTH_KEY_LEN);
