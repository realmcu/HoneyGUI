 ...
 *
 * */
static tuya_ble_status_t weather_key_string_to_enum_type(char *key, uint8_t key_str_len,
                                                         tuya_ble_weather_key_type_t *type, uint8_t *day)
{
...
tuya_ble_status_t tuya_ble_feature_weather_key_enum_type_to_string(tuya_ble_weather_key_type_t type,
                                                                   char *key)
{
    int bit;
 ...
 *
 * */
static tuya_ble_status_t weather_data_lktlv_parse(uint8_t *in, uint16_t in_len, uint8_t *out,
                                                  uint16_t *out_len, uint16_t *object_nums)
{
    ...
    for (;;)
    {
        ...
        if (weather_key_string_to_enum_type((char *)&in[1 + object_len], key_len, &weather_type,
                                            &n_day) != TUYA_BLE_SUCCESS)
        {
            return TUYA_BLE_ERR_INVALID_PARAM;
        }

        TUYA_BLE_LOG_DEBUG("parse weather data, n_day=[%d] type=[0x%08x] val_type=[%d] ,", n_day,
                           weather_type, value_type);
...
tuya_ble_status_t tuya_ble_feature_weather_data_request_with_location(
    tuya_ble_weather_location_type_t location_type, uint32_t combine_type, uint8_t n_days)
{
    ...
    TUYA_BLE_LOG_DEBUG("request weather location=[%d] type=[0x%x], n_days=[%d]", location_type,
                       combine_type, n_days);
...
{
    return tuya_ble_feature_weather_data_request_with_location(WLT_APP_CURRENT_LOCATION, combine_type,
                                                               n_days);
}
...
void tuya_ble_handle_weather_data_request_evt(tuya_ble_evt_param_t *evt)
{
    ...
    tuya_ble_commData_send(FRM_WEATHER_DATA_REQUEST, 0, evt->weather_req_data.p_data,
                           evt->weather_req_data.data_len, encry_mode);
...
void tuya_ble_handle_weather_data_received(uint8_t *recv_data, uint16_t recv_len)
{
    ...
    if ((data_len < 5) || (data_len > TUYA_BLE_RECEIVE_MAX_DATA_LEN))
    {
        ...
        p_buf[0] = 0x01;
        tuya_ble_commData_send(FRM_WEATHER_DATA_RECEIVED_RESP, ack_sn, p_buf, 1,
    ...
    if (weather_data_lktlv_parse(&recv_data[weather_data_start_pos], weather_data_len,
                                 ble_cb_evt_buffer, &buffer_len, &object_count) != TUYA_BLE_SUCCESS)
    {
