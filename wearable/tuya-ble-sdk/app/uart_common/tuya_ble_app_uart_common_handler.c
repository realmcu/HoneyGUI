...
void tuya_ble_uart_common_mcu_ota_data_from_ble_handler(uint16_t cmd, uint8_t *recv_data,
                                                        uint32_t recv_len)
{
...
static void tuya_ble_uart_common_mcu_ota_data_from_uart_handler(uint8_t cmd, uint8_t *data_buffer,
                                                                uint16_t data_len)
{
...
{
    if (tuya_ble_timer_create(&tuya_ble_xtimer_heartbeat, tuya_ble_heartbeat_timeout_ms,
                              TUYA_BLE_TIMER_REPEATED, tuya_ble_vtimer_heartbeat_callback) != TUYA_BLE_SUCCESS)
    {
...
void tuya_ble_uart_common_send_current_ble_state(void)
{
    ...
    if ((ble_state == UNBONDING_UNCONN) || (ble_state == UNBONDING_UNCONN) ||
        (ble_state == UNBONDING_UNCONN))
    {
        ble_work_state = 1;
    }
    ...
    tuya_ble_common_uart_protocol_send(TUYA_BLE_UART_COMMON_REPORT_WORK_STATE_TYPE,
                                       (uint8_t *)&ble_work_state, 1);
...
void tuya_ble_uart_common_process(uint8_t *p_in_data, uint16_t in_len)
{
    ...
    case TUYA_BLE_UART_COMMON_QUERY_MCU_VERSION:
        ...
        TUYA_BLE_LOG_DEBUG("reveived mcu_firmware_version : 0x%04x mcu_hardware_version : 0x%04x",
...
static void  tuya_ble_ota_data_process_test(uint8_t *p_data, uint16_t p_data_len)
{
    ...
    case TUYA_BLE_UART_COMMON_MCU_OTA_FILE_INFO:
        ...
        else
        {
            ...
            {
                mcu_ota_file_data.file_version = (ota_data_buffer[8] << 16) + (ota_data_buffer[9] << 8) +
                                                 ota_data_buffer[10];
                memcpy(mcu_ota_file_data.file_md5, ota_data_buffer + 11, 16);
                mcu_ota_file_data.file_length = (ota_data_buffer[27] << 24) + (ota_data_buffer[28] << 16) +
                                                (ota_data_buffer[29] << 8) + ota_data_buffer[30];
                mcu_ota_file_data.file_crc32 = (ota_data_buffer[31] << 24) + (ota_data_buffer[32] << 16) +
                                               (ota_data_buffer[33] << 8) + ota_data_buffer[34];
    ...
    case TUYA_BLE_UART_COMMON_MCU_OTA_DATA:
        ...
        if ((mcu_ota_flag == 3) || (mcu_ota_flag == 4))
        {
            ...
            {
                TUYA_BLE_LOG_ERROR("mcu ota received package number error.received package number : %d",
                                   current_package);
                ota_data_response_buffer[6] = 1;
            }
            else if ((len > MCU_OTA_MAX_DATA_SIZE) || ((len + 6) != (p_data_len - 7)))
            {
                TUYA_BLE_LOG_ERROR("mcu ota received package data length error : len = %d,p_data_len = %d", len,
                                   p_data_len);
                ota_data_response_buffer[6] = 2;
            }
            else
            {
                ...
                {
                    TUYA_BLE_LOG_ERROR("mcu ota received package data crc16 error,crc_16_cal = 0x%04x ; crc16_rev = 0x%04x",
                                       crc16_temp, crc16_rev);
                    ota_data_response_buffer[6] = 3;
                }
    ...
    {
        ota_data_response_buffer[ota_data_response_len] = tuya_ble_check_sum(ota_data_response_buffer,
                                                                             ota_data_response_len);
        ota_data_response_len++;
        tuya_ble_common_uart_send_full_instruction_received(ota_data_response_buffer,
                                                            ota_data_response_len);
    }
