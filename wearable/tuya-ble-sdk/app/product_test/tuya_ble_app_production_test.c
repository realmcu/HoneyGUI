...
{
    if (tuya_ble_timer_create(&tuya_ble_xTimer_prod_monitor, tuya_ble_prod_monitor_timeout_ms,
                              TUYA_BLE_TIMER_SINGLE_SHOT, tuya_ble_vtimer_prod_monitor_callback) != TUYA_BLE_SUCCESS)
    {
...
static  void tuya_ble_auc_write_auth_info(uint8_t *para, uint16_t len)
{
    ...
    uint8_t pid_len = 0, i = 0;
    uint8_t pid_pos = 0;
    if ((memcmp(&para[2], "auzkey", 6) != 0) || (memcmp(&para[46], "uuid", 4) != 0) ||
        (memcmp(&para[72], "mac", 3) != 0) || ((memcmp(&para[110], "pid\":\"", 6) != 0) &&
                                               (memcmp(&para[111], "pid\":\"", 6) != 0)))
    {
        tuya_ble_uart_prod_send(TUYA_BLE_AUC_CMD_WRITE_AUTH_INFO, (uint8_t *)false_buf, strlen(false_buf));
    ...
    if (tuya_ble_storage_write_auth_key_device_id_mac(&para[11], AUTH_KEY_LEN, &para[53], DEVICE_ID_LEN,
                                                      mac_temp, MAC_LEN, mac_char, MAC_LEN * 2, &para[pid_pos], pid_len) == TUYA_BLE_SUCCESS)
    {
    ...
    if (tuya_ble_storage_write_auth_key_device_id_mac(&para[11], AUTH_KEY_LEN, &para[53], DEVICE_ID_LEN,
                                                      mac_temp, MAC_LEN, mac_char, MAC_LEN * 2, NULL, 0) == TUYA_BLE_SUCCESS)
    {
...
static void tuya_ble_auc_write_comm_cfg(uint8_t *para, uint16_t len)
{
    ...
    else if (type == 3)
    {
        ...
        {
            TUYA_BLE_LOG_DEBUG("WRITE COMM CFG size err, recv=[%d] total=[%d]", comm_cfg_cur_rcv_len,
                               comm_cfg_total_len);
            goto EXIT_ERR;
        }
        ...
        else
        {
            ...
            if (memcmp(key, "deviceCertificate", strlen("deviceCertificate")) == 0)
            {
                if (tuya_ble_storage_private_data(PRIVATE_DATA_DEV_CERT, p_comm_cfg,
                                                  comm_cfg_total_len) != TUYA_BLE_SUCCESS)
                {
                    reply_ret = false;
                }
            }
            else if (memcmp(key, "privateKey", strlen("privateKey")) == 0)
            {
                ...
                else
                {
                    ...
                    if (tuya_ble_storage_private_data(PRIVATE_DATA_ECC_KEY, private_key,
                                                      private_key_len) != TUYA_BLE_SUCCESS)
                    {
                        reply_ret = false;
                    }
                }
...
static void tuya_ble_auc_query_info(uint8_t *para, uint16_t len)
{
    ...
    alloc_buf[i++] = ':';
    alloc_buf[i++] = '\"';
    memcpy(&alloc_buf[i], TUYA_BLE_APP_BUILD_FIRMNAME_STRING,
           strlen(TUYA_BLE_APP_BUILD_FIRMNAME_STRING));
...
static void tuya_ble_auc_query_fingerprint(uint8_t *para, uint16_t len)
{
    ...
    length = sprintf((char *)alloc_buf, "{\"ret\":true,\"firmName\":\"%s\",\"firmVer\":\"%s\"}",
                     TUYA_BLE_APP_BUILD_FIRMNAME_STRING, TUYA_BLE_APP_VERSION_STRING);
...
static void tuya_ble_auc_write_oem_info(uint8_t *para, uint16_t len)
{
    ...
    if ((write_subpkg_info.need_subpkg == false) &&
        (write_subpkg_info.total_nums == 0))  // need not subpkg
    {
...
static void tuya_ble_auc_write_subpkg_start(uint8_t *para, uint16_t len)
{
    ...
    {
        tuya_ble_uart_prod_send(TUYA_BLE_AUC_CMD_WRITE_SUBPKG_START, (uint8_t *)false_buf,
                                strlen(false_buf));
    ...
    write_subpkg_info.file_type   =
        cmd; //!< "09"-oem ocnfig data "10"-homekit atoken data. format: hexstring
...
static void tuya_ble_auc_write_subpkg_end(uint8_t *para, uint16_t len)
{
    ...
    calc_crc32 = tuya_ble_crc32_compute(write_subpkg_info.p_file_data, write_subpkg_info.rcv_file_len,
                                        NULL);
...
__TUYA_BLE_WEAK void tuya_ble_custom_app_production_test_process(uint8_t channel,
                                                                 uint8_t *p_in_data, uint16_t in_len)
{
    uint16_t sub_cmd = 0;
...
__TUYA_BLE_WEAK void tuya_ble_app_sdk_test_process(uint8_t channel, uint8_t *p_in_data,
                                                   uint16_t in_len)
{
    uint16_t sub_cmd = 0;
    uint8_t *data_buffer = NULL;
