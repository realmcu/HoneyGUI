...
def PrepareBuilding(env, root_directory, has_libcpu=False, remove_components = []):
    ...
    Env = env
    Tool_ROOT = os.path.abspath(root_directory)

    # make an absolute root directory
    TOOL_ROOT = Tool_ROOT
    Export('TOOL_ROOT')

    # set TOOL_ROOT in ENV
    Env['TOOL_ROOT'] = Tool_ROOT
    # set BSP_ROOT in ENV
    ...
    if configfn:
        from menuconfig import mk_wristband_sdk_config
        mk_wristband_sdk_config(configfn)
        exit(0)
    ...
    # include kernel
#    objs.extend(SConscript(Tool_ROOT + '/src/SConscript', variant_dir=kernel_vdir + '/src', duplicate=0))
    # include libcpu
#    if not has_libcpu:
#        objs.extend(SConscript(Tool_ROOT + '/libcpu/SConscript',
#                    variant_dir=kernel_vdir + '/libcpu', duplicate=0))

    # include components
#    objs.extend(SConscript(Tool_ROOT + '/components/SConscript',
...
def GenTargetProject(program = None):
    ...
    if GetOption('target') == 'ua':
        ...
        PrepareUA(Projects, Tool_ROOT, str(Dir('#')))
...
def EndBuilding(target, program = None):
    ...
    if GetOption('make-dist') and program != None:
        ...
        MkDist(program, BSP_ROOT, Tool_ROOT, Env)
    if GetOption('make-dist-strip') and program != None:
        from mkdist import MkDist_Strip
        MkDist_Strip(program, BSP_ROOT, Tool_ROOT, Env)
        need_exit = True
    if GetOption('make-dist-ide') and program != None:
        ...
        MkDist(program, BSP_ROOT, Tool_ROOT, Env, rtt_ide)
        need_exit = True
    if GetOption('cscope'):
...
def GetVersion():
    ...
    rtdef = os.path.join(Tool_ROOT, 'include', 'rtdef.h')
