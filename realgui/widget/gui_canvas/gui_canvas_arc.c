...
static void gui_canvas_arc_prepare_ftl(gui_canvas_arc_t *this)
{
    float degree = this->to - this->from;
    ...
    else
    {
        ...
        if (degree > 30)
        {
            for (int i = 0; i < t; i++)
            {
                if (i == t - 1)
                {
                    set_arc_img_ftl(this, this->arc_data, &this->arc_img[i], this->from + degree - 15, img_angle, img_w,
                                    img_h);
                }
                else
                {
                    set_arc_img_ftl(this, this->arc_data, &this->arc_img[i], this->from + 15 + 30 * i, img_angle, img_w,
                                    img_h);
                }
            }
        }
        ...
        {
            set_arc_img_ftl(this, this->arc_data, &this->arc_img[0], this->from + degree / 2, img_angle, img_w,
                            img_h);
        }
...
static void gui_canvas_arc_draw_ftl(gui_canvas_arc_t *this)
{
    ...
    for (int i = 0; i < num; i++)
    {
        if (this->use_external_picture)
        {
            draw_img_t *image = this->arc_img[i];
            gui_dispdev_t *dc = gui_get_dc();
...
static void gui_canvas_arc_cb_ftl(gui_obj_t *obj, T_OBJ_CB_TYPE cb_type)
{
    if (obj != NULL)
    {
        switch (cb_type)
        {
        case OBJ_PREPARE:
            gui_canvas_arc_prepare_ftl((gui_canvas_arc_t *)obj);
            gui_canvas_circle_prepare((gui_canvas_arc_t *)obj);
            break;

        case OBJ_DRAW:
            gui_canvas_arc_draw_ftl((gui_canvas_arc_t *)obj);
            break;
                                            ...
                                            gui_color_t  color)
{
    ...
    obj->obj_cb = gui_canvas_arc_cb_ftl;
    obj->has_prepare_cb = true;
    obj->has_draw_cb = true;
    obj->has_end_cb = true;
    obj->has_destroy_cb = true;
