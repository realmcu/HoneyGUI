...
*****************************************************************************************
  ...
  * @brief lite geometry round_rect widget
  * @details lite geometry round_rect widget is used to draw round_rect shapes on the screen
  * @author
  * @date 2025/11/07
  * @version 1.0
  ***************************************************************************************
 ...
 *                           Private Functions
 *============================================================================*/
static bool init_block_drawing(gui_lite_round_rect_t *this, gui_obj_t *obj);
static void draw_one_tile(gui_lite_round_rect_t *this, gui_obj_t *obj,
...
static void gui_lite_round_rect_input_prepare(gui_obj_t *obj)
{
    ...
    if (tp->type == TOUCH_SHORT)
    {
        ...
        if (is_point_in_round_rect(this, local_x, local_y))
        {
            ...
            // gui_log("gui_lite_round_rect_input_prepare: %d, %d\n", local_x, local_y);
        }
...
static void gui_lite_round_rect_prepare(gui_lite_round_rect_t *this)
{
...
static void gui_lite_round_rect_draw(gui_lite_round_rect_t *this)
{
...
static void gui_lite_round_rect_end(gui_lite_round_rect_t *this)
{
...
static void gui_lite_round_rect_destroy(gui_lite_round_rect_t *this)
{
    GUI_UNUSED(this);
}
...
static void gui_lite_round_rect_cb(gui_obj_t *obj, T_OBJ_CB_TYPE cb_type)
{
    if (obj != NULL)
    {
        ...
        case OBJ_INPUT_PREPARE:
            ...
            break;
        case OBJ_PREPARE:
            gui_lite_round_rect_prepare((gui_lite_round_rect_t *)obj);
            break;

        case OBJ_DRAW:
            gui_lite_round_rect_draw((gui_lite_round_rect_t *)obj);
            break;

        case OBJ_END:
            gui_lite_round_rect_end((gui_lite_round_rect_t *)obj);
            break;

        case OBJ_DESTROY:
            gui_lite_round_rect_destroy((gui_lite_round_rect_t *)obj);
            break;
...
/** Draw the round_rect onto the Tile (coordinates are offset from the Tile origin) */
static void draw_round_rect_to_tile(DrawContext *ctx, gui_lite_round_rect_t *round_rect, int ox,
                                    int oy)
{
    fill_rounded_rect_df_aa(ctx,
                            round_rect->x - ox,
                            round_rect->y - oy,
                            round_rect->w, round_rect->h,
                            round_rect->radius,
                            round_rect->color);
}

/** Rough determination of whether round_rect intersects with a Tile (for cropping to enhance efficiency) */
static inline bool round_rect_intersects_tile(const gui_lite_round_rect_t *round_rect,
                                              int tile_x, int tile_y, int tile_w, int tile_h)
{
    return rect_intersects(round_rect->x, round_rect->y, round_rect->w, round_rect->h,
                          ...
                          int tile_x, int tile_y, int tile_w, int tile_h)
{
    ...
    // draw round_rect if it intersects with the Tile
    if (round_rect_intersects_tile(this, tile_x, tile_y, tile_w, tile_h))
    {
        draw_round_rect_to_tile(&this->draw_ctx, this, tile_x, tile_y);
    }
...
/**
 * @brief Create round_rect geometry widget
 ...
 * @param w Width
 * @param h Height
 * @return gui_lite_round_rect_t* Created widget pointer
 */
gui_lite_round_rect_t *gui_lite_round_rect_create(void *parent, const char *name)
{
    ...
    round_rect->draw_img = NULL;
    round_rect->block_buffer = NULL;
    round_rect->use_block_drawing = false;
    round_rect->draw_ctx.format = PIXEL_FORMAT_ARGB8888;
    round_rect->draw_ctx.enable_aa = 1;
