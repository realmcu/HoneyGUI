...
{
    char filename[256];
    snprintf(filename, sizeof(filename), "realgui/test/dc_test/%s.bin", test_case_name);

    char golden_filename[256];
    snprintf(golden_filename, sizeof(golden_filename), "realgui/test/dc_test/%s_golden.bin",
             test_case_name);
...
static void save_buffer_to_file(const uint8_t *buffer, size_t size)
{
    ...
    {
        AUTO_TEST_LOG("Save Pass! Data successfully written to realgui/test/dc_test/%s", test_case_name,
                      filename);
    }
    fclose(file);
}
