...
uintptr_t app_prism_user_cb(uint16_t cmd, uintptr_t param, uintptr_t user_data)
{
    if (cmd == LX_CMD_ID_INIT)
    {
        ...
        gui_widget_litegfx_set_image(obj, 0, (void *)PRSIM_SIDE_BIN);
        gui_widget_litegfx_set_image(obj, 1, (void *)PRSIM_BACKBOARD_BIN);
...
void design_tab_app_prsim(void *parent)
{
    gui_widget_litegfx_t *widget = gui_widget_litegfx_create(parent, "lx vglite prism",
                                                             LX_VGWIDGET_INS_PRISM, app_prism_user_cb, 0, 0, 454, 454);
}