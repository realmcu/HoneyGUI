...
jmem_heap_alloc_block_internal(const size_t size)  /**< size of requested block */
{
        ...
        && JERRY_LIKELY(JERRY_HEAP_CONTEXT(first).next_offset != JMEM_HEAP_END_OF_LIST))
    {
        ...
        {
            JERRY_CONTEXT(jmem_heap_list_skip_p) = JMEM_HEAP_GET_ADDR_FROM_OFFSET(JERRY_HEAP_CONTEXT(
                                                                                      first).next_offset);
        }
    }
                     ...
                     const size_t size) /**< size of allocated region */
{
    ...
    while (JERRY_CONTEXT(jmem_heap_allocated_size) + CONFIG_MEM_HEAP_DESIRED_LIMIT <= JERRY_CONTEXT(
               jmem_heap_limit))
    {
        JERRY_CONTEXT(jmem_heap_limit) -= CONFIG_MEM_HEAP_DESIRED_LIMIT;
    }
