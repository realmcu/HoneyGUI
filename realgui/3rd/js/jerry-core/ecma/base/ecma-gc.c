...
ecma_gc_mark_promise_object(ecma_extended_object_t *ext_object_p)  /**< extended object */
{
    ...
    ecma_value_p = ecma_collection_iterator_init(((ecma_promise_object_t *)
                                                  ext_object_p)->fulfill_reactions);
    ...
    if (obj_is_not_lex_env)
    {
        ...
        if (ecma_get_object_is_builtin(object_p))
        {
                ...
                || object_type == ECMA_OBJECT_TYPE_ARRAY)
            {
                ...
                length_and_bitset_size = ((ecma_extended_built_in_object_t *)
                                          object_p)->built_in.length_and_bitset_size;
            }
            else
            {
 ...
 */
void
ecma_free_unused_memory(jmem_free_unused_memory_severity_t
