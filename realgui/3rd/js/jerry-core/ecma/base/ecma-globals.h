...
typedef struct
{
    ...
    ecma_property_t
    types[ECMA_PROPERTY_PAIR_ITEM_COUNT]; /**< two property type slot. The first represent
...
typedef struct
{
    ecma_value_t items[ECMA_MAP_OBJECT_ITEM_COUNT +
                                                  1]; /**< the last item is always a pointer to the next chunk,
...
typedef struct
{
    ecma_value_t items[ECMA_COLLECTION_CHUNK_ITEMS +
                                                   1]; /**< ecma-value list, where the last value is a special
