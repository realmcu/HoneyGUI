...
static __inline__ __attribute__((always_inline)) void minimp3_cpuid(int CPUInfo[],
                                                                    const int InfoType)
{
#if defined(__PIC__)
...
static const L12_subband_alloc_t *L12_subband_alloc_table(const uint8_t *hdr, L12_scale_info *sci)
{
    ...
    int nbands, stereo_bands = (mode == MODE_MONO) ? 0 : (mode == MODE_JOINT_STEREO) ?
...
#define BSPOS         ((bs_next_ptr - bs->buf)*8 - 24 + bs_sh)
    ...
    uint32_t bs_cache = (((bs_next_ptr[0] * 256u + bs_next_ptr[1]) * 256u + bs_next_ptr[2]) * 256u +
...
static void L3_decode(mp3dec_t *h, mp3dec_scratch_t *s, L3_gr_info_t *gr_info, int nch)
{
    ...
    {
        int aa_bands = 31;
        int n_long_bands = (gr_info->mixed_block_flag ? 2 : 0) << (int)(HDR_GET_MY_SAMPLE_RATE(
                                                                            h->header) == 2);
