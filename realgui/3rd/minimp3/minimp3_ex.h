...
typedef int (*MP3D_ITERATE_CB)(void *user_data, const uint8_t *frame, int frame_size,
                               int free_format_bytes, size_t buf_size, uint64_t offset, mp3dec_frame_info_t *info);
typedef int (*MP3D_PROGRESS_CB)(void *user_data, size_t file_size, uint64_t offset,
                                mp3dec_frame_info_t *info);
...
int mp3dec_load_cb(mp3dec_t *dec, mp3dec_io_t *io, uint8_t *buf, size_t buf_size,
                   mp3dec_file_info_t *info, MP3D_PROGRESS_CB progress_cb, void *user_data);
/* iterate through frames */
int mp3dec_iterate_buf(const uint8_t *buf, size_t buf_size, MP3D_ITERATE_CB callback,
                       void *user_data);
int mp3dec_iterate_cb(mp3dec_io_t *io, uint8_t *buf, size_t buf_size, MP3D_ITERATE_CB callback,
                      void *user_data);
...
size_t mp3dec_ex_read_frame(mp3dec_ex_t *dec, mp3d_sample_t **buf, mp3dec_frame_info_t *frame_info,
                            size_t max_samples);
...
int mp3dec_load_w(mp3dec_t *dec, const wchar_t *file_name, mp3dec_file_info_t *info,
                  MP3D_PROGRESS_CB progress_cb, void *user_data);
...
#define MINIMP3_ID3_DETECT_SIZE 10
    ...
    if (buf_size >= MINIMP3_ID3_DETECT_SIZE && !memcmp(buf, "ID3", 3) && !((buf[5] & 15) ||
                                                                           (buf[6] & 0x80) || (buf[7] & 0x80) || (buf[8] & 0x80) || (buf[9] & 0x80)))
    {
        size_t id3v2size = (((buf[6] & 0x7f) << 21) | ((buf[7] & 0x7f) << 14) | ((buf[8] & 0x7f) << 7) |
                            (buf[9] & 0x7f)) + 10;
        if ((buf[5] & 16))
        {
            id3v2size += 10;    /* footer */
...
static int mp3dec_check_vbrtag(const uint8_t *frame, int frame_size, uint32_t *frames, int *delay,
                               int *padding)
{
...
int mp3dec_detect_cb(mp3dec_io_t *io, uint8_t *buf, size_t buf_size)
{
    ...
    if (io)
    {
        size_t readed = io->read(buf + MINIMP3_ID3_DETECT_SIZE, buf_size - MINIMP3_ID3_DETECT_SIZE,
                                 io->read_data);
                   ...
                   mp3dec_file_info_t *info, MP3D_PROGRESS_CB progress_cb, void *user_data)
{
    ...
    do
    {
        if ((allocated - info->samples * sizeof(mp3d_sample_t)) < MINIMP3_MAX_SAMPLES_PER_FRAME * sizeof(
                mp3d_sample_t))
        {
            allocated *= 2;
        ...
        if (io)
        {
            ...
            samples = mp3dec_decode_frame(dec, buf + consumed, filled - consumed, info->buffer + info->samples,
                                          &frame_info);
    ...
    {
        mp3d_sample_t *alloc_buf = (mp3d_sample_t *)realloc(info->buffer,
                       ...
                       void *user_data)
{
    ...
    do
    {
        ...
        if (callback)
        {
            if ((ret = callback(user_data, hdr, frame_size, free_format_bytes, buf_size, hdr - orig_buf,
                                &frame_info)))
            {
                return ret;
            }
                      ...
                      void *user_data)
{
    ...
    do
    {
        ...
        if (callback)
        {
            if ((ret = callback(user_data, hdr, frame_size, free_format_bytes, filled - consumed, readed,
                                &frame_info)))
            {
                return ret;
            }
...
static int mp3dec_load_index(void *user_data, const uint8_t *frame, int frame_size,
                             int free_format_bytes, size_t buf_size, uint64_t offset, mp3dec_frame_info_t *info)
{
    ...
    if (dec->index.num_frames + 1 > dec->index.capacity)
    {
        ...
        mp3dec_frame_t *alloc_buf = (mp3dec_frame_t *)realloc((void *)dec->index.frames,
    ...
    if (!dec->buffer_samples && dec->index.num_frames < 256)
    {
        ...
        dec->buffer_samples = mp3dec_decode_frame(&dec->mp3d, frame, MINIMP3_MIN(buf_size, (size_t)INT_MAX),
                                                  dec->buffer, info);
...
seek_zero:
    ...
    if (!dec->indexes_built)
    {
        ...
        if (dec->io)
        {
            ...
            int ret = mp3dec_iterate_cb(dec->io, (uint8_t *)dec->file.buffer, dec->file.size, mp3dec_load_index,
                                        dec);
            if (ret && MP3D_E_USER != ret)
            {
                return ret;
        ...
        else
        {
            int ret = mp3dec_iterate_buf(dec->file.buffer + dec->start_offset,
                                         dec->file.size - dec->start_offset, mp3dec_load_index, dec);
            if (ret && MP3D_E_USER != ret)
            {
                return ret;
            }
    ...
    *buf = NULL;
    ...
    while (dec->buffer_consumed == dec->buffer_samples)
    {
        ...
        if (dec->io)
        {
            ...
            dec->buffer_samples = mp3dec_decode_frame(&dec->mp3d, dec_buf,
                                                      dec->input_filled - dec->input_consumed, dec->buffer, frame_info);
        ...
        else
        {
            ...
            dec->buffer_samples = mp3dec_decode_frame(&dec->mp3d, dec_buf, MINIMP3_MIN(buf_size,
                                                                                       (uint64_t)INT_MAX), dec->buffer, frame_info);
        }
        dec->buffer_consumed = 0;
...
int mp3dec_ex_open_cb(mp3dec_ex_t *dec, mp3dec_io_t *io, int flags)
{
    ...
    int ret = mp3dec_iterate_cb(io, (uint8_t *)dec->file.buffer, dec->file.size, mp3dec_load_index,
                                dec);
    if (ret && MP3D_E_USER != ret)
    {
        return ret;
...
retry_mmap:
    map_info->buffer = (const uint8_t *)mmap(NULL, st.st_size, PROT_READ, MAP_PRIVATE | MAP_POPULATE,
                                             file, 0);
...
retry_mmap2:
    buffer = mmap((void *)map_info->buffer, map_info->size, PROT_READ | PROT_WRITE,
...
static int mp3dec_load_mapinfo(mp3dec_t *dec, mp3dec_map_info_t *map_info, mp3dec_file_info_t *info,
                               MP3D_PROGRESS_CB progress_cb, void *user_data)
{
...
static int mp3dec_iterate_mapinfo(mp3dec_map_info_t *map_info, MP3D_ITERATE_CB callback,
                                  void *user_data)
{
