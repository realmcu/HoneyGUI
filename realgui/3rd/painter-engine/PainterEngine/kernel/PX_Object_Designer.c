...
static px_void PX_Designer_RefreshProperties(PX_Object *pObject)
{
    ...
    {
        PX_Designer_Object *pdobject = PX_VECTORAT(PX_Designer_Object, &pDesignerDesc->Objects,
                                                   pDesignerDesc->selectObjectIndex);
        if (PX_VectorCheckIndex(&pDesignerDesc->ObjectDesc, pdobject->descIndex))
        {
            px_int j;
            PX_Designer_ObjectDesc *pObjectDesc = PX_VECTORAT(PX_Designer_ObjectDesc,
                                                              &pDesignerDesc->ObjectDesc, pdobject->descIndex);
...
static px_void PX_Designer_UpdateObjectsList(PX_Object *pObject)
{
    ...
    {
        PX_Object_ListAdd(pDesignerDesc->list_Objects, PX_VECTORAT(PX_Designer_Object,
                                                                   &pDesignerDesc->Objects, i));
    }
...
px_void PX_DesignerOnDesignerBoxValueChanged(PX_Object *pObject, PX_Object_Event e, px_void *ptr)
{
    ...
    {
        PX_Designer_Object *pdobject = PX_VECTORAT(PX_Designer_Object, &pDesc->Objects,
                                                   pDesc->selectObjectIndex);
...
px_void PX_DesignerUpdate(PX_Object *pObject, px_dword elapsed)
{
        ...
        case PX_DESIGNER_OBJECT_TYPE_GAME:
            {
                ...
                pDesc->designerbox->x = pdobject->pObject->x - pDesc->pLinkWorld->offsetx - pdobject->pObject->Width
                                        / 2;
                pDesc->designerbox->y = pdobject->pObject->y - pDesc->pLinkWorld->offsety -
                                        pdobject->pObject->Height / 2;
                pDesc->designerbox->Width = pdobject->pObject->Width;
                pDesc->designerbox->Height = pdobject->pObject->Height;
    ...
    if (pDesc->selectObjectIndex != -1)
    {
        pDesc->button_delete->x = pDesc->designerbox->x + pDesc->designerbox->Width -
                                  pDesc->button_delete->Width;
...
px_void PX_Designer_ControllersListItemOnRender(px_surface *psurface, PX_Object *pObject,
                                                px_dword elapsed)
{
    PX_Designer_ObjectDesc *pDesc = (PX_Designer_ObjectDesc *)PX_Object_ListItemGetData(pObject);
    PX_Object_Designer *pDesignerDesc = PX_ObjectGetDesc(PX_Object_Designer,
                                                         (PX_Object *)pObject->User_ptr);
    ...
    {
    case PX_DESIGNER_OBJECT_TYPE_UI:
        PX_GeoDrawBorder(psurface, (px_int)objx + 1, (px_int)objy + 1, (px_int)objx + 64,
                         (px_int)objy + PX_DESIGNER_CONTROLLER_ITEM_HEIGHT - 2, 1, PX_COLOR(255, 192, 64, 255));
        PX_FontModuleDrawText(psurface, pDesignerDesc->fm, (px_int)objx + 32,
                              (px_int)objy + PX_DESIGNER_CONTROLLER_ITEM_HEIGHT / 2, PX_ALIGN_CENTER, "UI", PX_COLOR(255, 192, 64,
                                      255));
        PX_FontModuleDrawText(psurface, pDesignerDesc->fm, (px_int)objx + 68,
                              (px_int)objy + PX_DESIGNER_CONTROLLER_ITEM_HEIGHT / 2, PX_ALIGN_LEFTMID, pDesc->Name, PX_COLOR(255,
                                      192, 64, 255));
        break;
    case PX_DESIGNER_OBJECT_TYPE_GAME:
        PX_GeoDrawBorder(psurface, (px_int)objx + 1, (px_int)objy + 1, (px_int)objx + 64,
                         (px_int)objy + PX_DESIGNER_CONTROLLER_ITEM_HEIGHT - 2, 1, PX_COLOR(255, 128, 255, 64));
        PX_FontModuleDrawText(psurface, pDesignerDesc->fm, (px_int)objx + 32,
                              (px_int)objy + PX_DESIGNER_CONTROLLER_ITEM_HEIGHT / 2, PX_ALIGN_CENTER, "Game", PX_COLOR(255, 128,
                                      255, 64));
        PX_FontModuleDrawText(psurface, pDesignerDesc->fm, (px_int)objx + 68,
                              (px_int)objy + PX_DESIGNER_CONTROLLER_ITEM_HEIGHT / 2, PX_ALIGN_LEFTMID, pDesc->Name, PX_COLOR(255,
                                      128, 255, 64));
    ...
    case PX_DESIGNER_OBJECT_TYPE_FUNCTION:
        PX_GeoDrawBorder(psurface, (px_int)objx + 1, (px_int)objy + 1, (px_int)objx + 64,
                         (px_int)objy + PX_DESIGNER_CONTROLLER_ITEM_HEIGHT - 2, 1, PX_COLOR(255, 255, 128, 64));
        PX_FontModuleDrawText(psurface, pDesignerDesc->fm, (px_int)objx + 32,
                              (px_int)objy + PX_DESIGNER_CONTROLLER_ITEM_HEIGHT / 2, PX_ALIGN_CENTER, "Function", PX_COLOR(255,
                                      255, 128, 64));
        PX_FontModuleDrawText(psurface, pDesignerDesc->fm, (px_int)objx + 68,
                              (px_int)objy + PX_DESIGNER_CONTROLLER_ITEM_HEIGHT / 2, PX_ALIGN_LEFTMID, pDesc->Name, PX_COLOR(255,
                                      255, 128, 64));
...
px_bool PX_Designer_ControllersListItemOnCreate(px_memorypool *mp, PX_Object *ItemObject,
                                                px_void *userptr)
{
...
px_void PX_Designer_ObjectsListItemOnRender(px_surface *psurface, PX_Object *pObject,
                                            px_dword elapsed)
{
    ...
    case PX_DESIGNER_OBJECT_TYPE_UI:
        ...
        PX_FontModuleDrawText(psurface, pDesignerDesc->fm, (px_int)objx + 32,
                              (px_int)objy + PX_DESIGNER_CONTROLLER_ITEM_HEIGHT / 2, PX_ALIGN_CENTER, "UI", PX_COLOR(255, 192, 64,
                                      255));
        PX_FontModuleDrawText(psurface, pDesignerDesc->fm, (px_int)objx + 68,
                              (px_int)objy + PX_DESIGNER_CONTROLLER_ITEM_HEIGHT / 2, PX_ALIGN_LEFTMID, pDesc->pObject->id,
                              PX_COLOR(255, 192, 64, 255));
        break;
    case PX_DESIGNER_OBJECT_TYPE_GAME:
        PX_GeoDrawBorder(psurface, (px_int)objx + 1, (px_int)objy + 1, (px_int)objx + 64,
                         (px_int)objy + PX_DESIGNER_CONTROLLER_ITEM_HEIGHT - 2, 1, PX_COLOR(255, 128, 255, 64));
...
px_bool PX_Designer_ObjectsListItemOnCreate(px_memorypool *mp, PX_Object *ItemObject,
                                            px_void *userptr)
{
...
px_void PX_Designer_OnCursorDown(PX_Object *pObject, PX_Object_Event e, px_void *ptr)
{
    ...
    if (e.Event == PX_OBJECT_EVENT_CURSORDOWN)
    {
        ...
        {
            if (PX_isXYInRegion(x, y, pDesignerDesc->designerbox->x - 6, pDesignerDesc->designerbox->y - 6,
                                pDesignerDesc->designerbox->Width + 12, pDesignerDesc->designerbox->Height + 12))
            {
                return;
            }
        ...
        if (selectObjectIndex == -1 && pDesignerDesc->selectObjectIndex == -1)
        {
            ...
            if (selectDescIndex != -1)
            {
                //create new object
                PX_Designer_ObjectDesc *pObjectDesc = PX_VECTORAT(PX_Designer_ObjectDesc,
                                                                  &pDesignerDesc->ObjectDesc, selectDescIndex);
                ...
                case PX_DESIGNER_OBJECT_TYPE_UI:
                    {
                        if (pDesignerDesc->pLinkObject)
                        {
                            PX_Object *pNewObject = pObjectDesc->createfunc(pDesignerDesc->mp, pDesignerDesc->pLinkObject, x, y,
                                                                            100, 100, pDesignerDesc->userptr);
                            PX_sprintf2(pNewObject->id, sizeof(pNewObject->id), "%1%2",
                                        PX_STRINGFORMAT_STRING(pObjectDesc->Name), PX_STRINGFORMAT_INT(pDesignerDesc->allocID++));
                ...
                case PX_DESIGNER_OBJECT_TYPE_GAME:
                    {
                        if (pDesignerDesc->pLinkWorld)
                        {
                            PX_Object *pNewObject = pObjectDesc->createfunc(pDesignerDesc->mp, PX_NULL,
                                                                            x + pDesignerDesc->pLinkWorld->offsetx, y + pDesignerDesc->pLinkWorld->offsety, 100, 100,
                                                                            pDesignerDesc->userptr);
                            PX_sprintf2(pNewObject->id, sizeof(pNewObject->id), "%1%2",
                                        PX_STRINGFORMAT_STRING(pObjectDesc->Name), PX_STRINGFORMAT_INT(pDesignerDesc->allocID++));
                ...
                case PX_DESIGNER_OBJECT_TYPE_FUNCTION:
                    {
                        PX_Object *pNewObject = pObjectDesc->createfunc(pDesignerDesc->mp, PX_NULL, x, y, 100, 100,
                                                                        pDesignerDesc->userptr);
        ...
        if (selectObjectIndex != -1)
        {
            ...
            if (pDesignerDesc->selectObjectIndex != selectObjectIndex)
            {
                PX_Designer_Object *pdobject = PX_VECTORAT(PX_Designer_Object, &pDesignerDesc->Objects,
                                                           selectObjectIndex);
                PX_Object_DesignerBox *pDesignerBox = PX_ObjectGetDesc(PX_Object_DesignerBox,
                                                                       pDesignerDesc->designerbox);
                ...
                case PX_DESIGNER_OBJECT_TYPE_GAME:
                case PX_DESIGNER_OBJECT_TYPE_FUNCTION:
                    pDesignerDesc->designerbox->x = pdobject->pObject->x - pDesignerDesc->pLinkWorld->offsetx -
                                                    pdobject->pObject->Width / 2;
                    pDesignerDesc->designerbox->y = pdobject->pObject->y - pDesignerDesc->pLinkWorld->offsety -
                                                    pdobject->pObject->Height / 2;
                    pDesignerDesc->designerbox->Width = pdobject->pObject->Width;
                    pDesignerDesc->designerbox->Height = pdobject->pObject->Height;
                    break;
...
PX_Object *PX_Object_DesignerCreate(px_memorypool *mp, PX_Object *pparent, PX_Object *pLinkObject,
                                    PX_World *pLinkWorld, PX_FontModule *fm, px_void *userptr)
{
    px_int i;
    PX_Object *pObject;
    ...
    pObject = PX_ObjectCreateEx(mp, pparent, 0, 0, 0, 0, 0, 0, 0x21080210, PX_DesignerUpdate,
                                PX_DesignerRender, PX_Object_DesignerFree, &desc, sizeof(desc));
    ...
    pdesc->widget_property = PX_Object_WidgetCreate(mp, pObject, 5, 36 + 256, 192, 192, "properties",
                                                    PX_NULL);
    ...
    pdesc->widget_propertyscrollarea = PX_Object_ScrollAreaCreate(mp, pdesc->widget_property, 0, 0, 192,
                                                                  162);
    ...
    do
    {
        ...
        pItem = PX_Object_MenuAddItem(pdesc->menu, PX_Object_MenuGetRootItem(pdesc->menu), "designer",
                                      PX_NULL, PX_NULL);
        ...
        pItem = PX_Object_MenuAddItem(pdesc->menu, PX_Object_MenuGetRootItem(pdesc->menu), "window",
                                      PX_NULL, PX_NULL);
        PX_Object_MenuAddItem(pdesc->menu, pItem, "controllers", PX_DesignerOnMenuWindowControllers,
                              pObject);
        ...
        pItem = PX_Object_MenuAddItem(pdesc->menu, PX_Object_MenuGetRootItem(pdesc->menu), "view", PX_NULL,
                                      PX_NULL);
    ...
    for (i = 0; i < PX_DESIGNER_MAX_PROPERTYS; i++)
    {
        pdesc->label_propertys[i] = PX_Object_LabelCreate(mp, pdesc->widget_propertyscrollarea, 5,
                                                          3 + i * 24, 64, 24, "property", fm, PX_OBJECT_UI_DEFAULT_FONTCOLOR);
        PX_Object_LabelSetAlign(pdesc->label_propertys[i], PX_ALIGN_LEFTMID);
        pdesc->label_propertys[i]->Visible = PX_FALSE;
        pdesc->edit_propertys[i] = PX_Object_EditCreate(mp, pdesc->widget_propertyscrollarea, 74,
                                                        3 + i * 24, 96, 24, fm);
        PX_Object_GetEdit(pdesc->edit_propertys[i])->multiLines = PX_TRUE;
        pdesc->edit_propertys[i]->Visible = PX_FALSE;
        PX_ObjectRegisterEvent(pdesc->edit_propertys[i], PX_OBJECT_EVENT_VALUECHANGED,
                               PX_Designer_OnPropertyChanged, pObject);
        //PX_ObjectRegisterEvent(pdesc->edit_propertys[i],PX_OBJECT_EVENT_LOSTFOCUS,PX_Designer_OnPropertyRefresh,pObject);
    }


    pdesc->widget_controllers = PX_Object_WidgetCreate(mp, pObject, 5, 25, 192, 256, "controllers",
                                                       PX_NULL);
    if (!pdesc->widget_controllers) { return PX_FALSE; }
    if (pdesc->pLinkWorld)
    {
        pdesc->widget_controllers->x = pdesc->pLinkWorld->surface_width - pdesc->widget_controllers->Width -
                                       25;
    }
    ...
    pdesc->list_ObjectDesc = PX_Object_ListCreate(mp,
                                                  PX_Object_WidgetGetRoot(pdesc->widget_controllers), 0, 1,
    ...
    PX_ObjectRegisterEvent(pdesc->list_Objects, PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Designer_OnObjectsListChanged, pObject);
    ...
    PX_ObjectRegisterEvent(pdesc->button_delete, PX_OBJECT_EVENT_EXECUTE,
                           PX_Designer_OnDeleteCurrentObject, pObject);

    pdesc->world_hscroll = PX_Object_SliderBarCreate(mp, pObject, 0, 0, 10, 10,
    ...
    {
        PX_Object_SliderBarSetRange(pdesc->world_hscroll,
                                    (px_int)(pdesc->pLinkWorld->surface_width / 2) - 1,
                                    (px_int)(pdesc->pLinkWorld->world_width - pdesc->pLinkWorld->surface_width / 2) + 1);
    }
    else
    {
        pdesc->world_hscroll->Visible = PX_FALSE;
    }
    PX_ObjectRegisterEvent(pdesc->world_hscroll, PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Designer_OnScrollHChanged, pObject);


    pdesc->world_vscroll = PX_Object_SliderBarCreate(mp, pObject, 0, 0, 10, 10,
                                                     PX_OBJECT_SLIDERBAR_TYPE_VERTICAL, PX_OBJECT_SLIDERBAR_STYLE_BOX);
    if (pdesc->pLinkWorld && pdesc->pLinkWorld->world_height > pdesc->pLinkWorld->surface_height)
    {
        PX_Object_SliderBarSetRange(pdesc->world_vscroll,
                                    (px_int)(pdesc->pLinkWorld->surface_height / 2) - 1,
                                    (px_int)(pdesc->pLinkWorld->world_height - pdesc->pLinkWorld->surface_height / 2) + 1);
    }
    else
    {
        pdesc->world_vscroll->Visible = PX_FALSE;
    }
    PX_ObjectRegisterEvent(pdesc->world_vscroll, PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Designer_OnScrollVChanged, pObject);


    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORDOWN, PX_Designer_OnCursorDown, pObject);
    PX_ObjectRegisterEvent(pdesc->designerbox, PX_OBJECT_EVENT_VALUECHANGED,
                           PX_DesignerOnDesignerBoxValueChanged, pObject);
...
px_bool PX_Object_DesignerExport(PX_Object *pObject, px_string *pText)
{
    ...
    for (i = 0; i < designer->Objects.size; i++)
    {
        ...
        PX_Designer_ObjectDesc *pobjectdesc = PX_VECTORAT(PX_Designer_ObjectDesc, &designer->ObjectDesc,
                                                          pdobject->descIndex);
        px_int j;
        if (!pobjectdesc)
        {
...
px_bool PX_Object_DesignerImportToUIObject(px_memorypool *mp, PX_Object *DesignerObject,
                                           PX_Object *pRootObject, const px_char *pText, PX_FontModule *fm)
{
    PX_Json Json;
    px_int i;
    ...
    for (i = 0; i < Json.rootValue._object.values.size; i++)
    {
        ...
        if (pObjectDesc)
        {
            ...
            if (TypeValue && TypeValue->type == PX_JSON_VALUE_TYPE_STRING)
            {
                ...
                case PX_DESIGNER_OBJECT_TYPE_UI:
                    {
                        if (pDesignerDesc->pLinkObject)
                        {
                            pNewObject = pObjectDesc->createfunc(pDesignerDesc->mp, pDesignerDesc->pLinkObject, 0, 0, 100, 100,
                                                                 pDesignerDesc->userptr);
                            dobject.pObject = pNewObject;
                            dobject.type = PX_DESIGNER_OBJECT_TYPE_UI;
                ...
                case PX_DESIGNER_OBJECT_TYPE_GAME:
                    {
                        if (pDesignerDesc->pLinkWorld)
                        {
                            pNewObject = pObjectDesc->createfunc(pDesignerDesc->mp, PX_NULL,
                                                                 0 + pDesignerDesc->pLinkWorld->offsetx, 0 + pDesignerDesc->pLinkWorld->offsety, 100, 100,
                                                                 pDesignerDesc->userptr);
                            PX_WorldAddObject(pDesignerDesc->pLinkWorld, pNewObject);
                            dobject.pObject = pNewObject;
                            dobject.type = PX_DESIGNER_OBJECT_TYPE_GAME;
                ...
                case PX_DESIGNER_OBJECT_TYPE_FUNCTION:
                    {
                        pNewObject = pObjectDesc->createfunc(pDesignerDesc->mp, PX_NULL, 0, 0, 100, 100,
                                                             pDesignerDesc->userptr);
                        if (pNewObject != PX_NULL)
                        {
                            PX_ASSERT();
...
px_bool PX_Object_DesignerImport(PX_Object *pObject, const px_char *pText)
{
    ...
    for (i = 0; i < Json.rootValue._object.values.size; i++)
    {
        ...
        {
            PX_Designer_ObjectDesc *pEnumObjectDesc = PX_VECTORAT(PX_Designer_ObjectDesc,
                                                                  &pdesigner->ObjectDesc, j);
            if (PX_strequ(pJsonValue->name.buffer, pEnumObjectDesc->Name))
            {
                pObjectDesc = pEnumObjectDesc;
                descindex = j;
        ...
        if (pObjectDesc)
        {
            ...
            if (TypeValue && TypeValue->type == PX_JSON_VALUE_TYPE_STRING)
            {
                ...
                case PX_DESIGNER_OBJECT_TYPE_UI:
                    {
                        PX_Designer_Object dobject;
                        pNewObject = pObjectDesc->createfunc(pdesigner->mp, pdesigner->pLinkObject, 0, 0, 100, 100,
                                                             pdesigner->userptr);
                        dobject.pObject = pNewObject;
                        dobject.type = PX_DESIGNER_OBJECT_TYPE_UI;
                        dobject.descIndex = descindex;
