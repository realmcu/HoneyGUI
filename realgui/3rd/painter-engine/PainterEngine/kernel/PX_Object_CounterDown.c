...
px_void PX_Object_CounterDownRender(px_surface *psurface, PX_Object *pObject, px_dword elapsed)
{
    ...
    do
    {
        ...
        PX_GeoDrawCircle(psurface, (px_int)objx, (px_int)objy,
                         (px_int)(pObject->diameter / 2 + (pObject->diameter * (a))), (px_int)(16 * a), clr);
        ...
        PX_GeoDrawSolidCircle(psurface, (px_int)objx, (px_int)objy, (px_int)(a * pObject->diameter / 2),
                              pdesc->backgroundcolor);
        PX_GeoDrawSector(psurface, (px_int)objx, (px_int)objy, (a * (pObject->diameter / 2 - 5)),
                         (a * (pObject->diameter / 2 - 1)), pdesc->ringcolor, -90,
                         (px_int)(-90 + pdesc->ms_down % 1000 / 1000.f * 360));
        PX_itoa(pdesc->ms_down / 1000 + 1, n, 16, 10);
        PX_FontModuleDrawText(psurface, pdesc->numericFM, (px_int)objx, (px_int)objy, PX_ALIGN_CENTER, n,
                              pdesc->fontcolor);
...
PX_Object *PX_Object_CounterDownCreate(px_memorypool *mp, PX_Object *Parent, px_float x, px_float y,
                                       px_float radius, PX_FontModule *fm)
{
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, x, y, 0, radius * 2, radius * 2, 0,
                                PX_OBJECT_TYPE_COUNTERDOWN, PX_Object_CounterDownUpdate, PX_Object_CounterDownRender, 0, &Desc,
                                sizeof(Desc));
