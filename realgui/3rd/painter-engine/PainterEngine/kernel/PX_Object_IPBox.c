...
PX_Object *PX_Object_IPBoxCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                 px_int width, px_int height, const px_char title[], PX_FontModule *fontmodule)
{
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0, (px_float)width,
                                (px_float)height, 0, PX_OBJECT_TYPE_IPBOX, PX_NULL, PX_NULL, PX_NULL, &desc, sizeof(desc));
    ...
    pdesc->various_ip = PX_Object_VariousCreate(mp, pdesc->widget, width / 2 - 128, height / 2 - 64,
                                                256, 32, "IP:", PX_OBJECT_VARIOUS_TYPE_EDIT, fontmodule);
    PX_Object_VariousSetEditStyle(pdesc->various_ip, PX_OBJECT_VARIOUS_EDIT_TYPE_STRING);
    pdesc->various_port = PX_Object_VariousCreate(mp, pdesc->widget, width / 2 - 128, height / 2 - 24,
                                                  256, 32, "Port:", PX_OBJECT_VARIOUS_TYPE_EDIT, fontmodule);
    ...
    PX_ObjectRegisterEvent(pdesc->various_ip, PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_IPBoxOnEditChanged, pObject);
