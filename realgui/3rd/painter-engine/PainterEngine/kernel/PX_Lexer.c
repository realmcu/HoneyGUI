...
px_char *PX_LexerIsCommentEnd(px_lexer *lexer, const px_char startch[], const px_char ch[])
{
    ...
    for (i = 0; i < lexer->CommentCount; i++)
    {
        if (PX_memequ(startch, lexer->Comment[i].CommentBegin, PX_strlen(lexer->Comment[i].CommentBegin)) &&
            PX_memequ(ch, lexer->Comment[i].CommentEnd, PX_strlen(lexer->Comment[i].CommentEnd)))
        {
...
px_int PX_LexerRegisterContainer(px_lexer *lexer, const px_char Begin[], const px_char End[])
{
    ...
    if (lexer->ContainerCount < PX_LEXER_CA_DELIMITER_MAX_COUNT)
    {
        ...
        PX_strcpy(lexer->Container[lexer->ContainerCount].ContainerBegin, Begin,
                  sizeof(lexer->Container[lexer->ContainerCount].ContainerBegin));
        PX_strcpy(lexer->Container[lexer->ContainerCount].ContainerEnd, End,
                  sizeof(lexer->Container[lexer->ContainerCount].ContainerEnd));
...
px_char *PX_LexerIsContainerStart(px_lexer *lexer, const px_char chr[])
{
    ...
    for (i = 0; i < lexer->ContainerCount; i++)
    {
        if (PX_memequ(chr, lexer->Container[i].ContainerBegin,
                      PX_strlen(lexer->Container[i].ContainerBegin)))
        {
...
px_char *PX_LexerIsContainerEnd(px_lexer *lexer, const px_char startch[], const px_char ch[])
{
    ...
    for (i = 0; i < lexer->ContainerCount; i++)
    {
        if (PX_memequ(startch, lexer->Container[i].ContainerBegin, PX_strlen(startch)) &&
            PX_memequ(ch, lexer->Container[i].ContainerEnd, PX_strlen(lexer->Container[i].ContainerEnd)))
        {
    ...
    while (SourceText[r_Offset])
    {
        ...
        if ((chrst = PX_LexerIsContainerStart(lexer, SourceText + r_Offset)) != 0)
        {
            ...
            while (!(chred = PX_LexerIsContainerEnd(lexer, chrst, SourceText + r_Offset)))
            {
                ...
                if (PX_LexerIsContainerTransfer(lexer, chrst, SourceText[r_Offset]) &&
                    PX_memequ(SourceText + r_Offset + 1, chrst, PX_strlen(chrst)))
                {
        ...
        if (PX_LexerFilterChar(lexer, SourceText[r_Offset]))
        {
            ...
            {
                if (PX_LexerIsNewLine(SourceText[r_Offset]) &&
                    PX_LexerIsSpacer(lexer, lexer->Sources[w_Offset - 1]))
                {
                    lexer->Sources[w_Offset - 1] = (SourceText[r_Offset]) == '\r' ? '\n' : (SourceText[r_Offset]);
                }
                else if (PX_LexerIsDelimiter(lexer, SourceText[r_Offset]) &&
                         PX_LexerIsSpacer(lexer, lexer->Sources[w_Offset - 1]))
                {
...
PX_LEXER_LEXEME_TYPE PX_LexerGetNextLexeme(px_lexer *lexer)
{
    ...
    if ((chrst = PX_LexerIsContainerStart(lexer, (&lexer->Sources[lexer->SourceOffset]))) != 0)
    {
        ...
        while (!(chred = PX_LexerIsContainerEnd(lexer, chrst, &lexer->Sources[lexer->SourceOffset])))
        {
            ...
            if (PX_LexerIsContainerTransfer(lexer, chrst, lexer->Sources[lexer->SourceOffset]) &&
                PX_memequ(&lexer->Sources[lexer->SourceOffset + 1], chrst, PX_strlen(chrst)))
            {
    ...
    PX_StringClear(&lexer->CurLexeme);
    lexer->Symbol = '\0';
    while (!PX_LexerIsSourcsEnd(lexer) &&
           !PX_LexerIsDelimiter(lexer, lexer->Sources[lexer->SourceOffset]) &&
           !PX_LexerIsSpacer(lexer, lexer->Sources[lexer->SourceOffset]) &&
           !PX_LexerIsNewLine(lexer->Sources[lexer->SourceOffset]) &&
           !PX_LexerIsContainerStart(lexer, &lexer->Sources[lexer->SourceOffset]))
    {
        PX_StringCatChar(&lexer->CurLexeme, lexer->Sources[lexer->SourceOffset++]);
        oft = lexer->SourceOffset;

        if (lexer->NumericMath && lexer->Sources[lexer->SourceOffset] == '.' &&
            PX_LexerIsDelimiter(lexer, '.'))
        {
            match = PX_TRUE;
...
px_void PX_LexerRegisterComment(px_lexer *lexer, const px_char Begin[], const px_char End[])
{
    ...
    if (lexer->CommentCount < PX_LEXER_CA_COMMENT_MAX_COUNT)
    {
        ...
        PX_strcpy(lexer->Comment[lexer->CommentCount].CommentBegin, Begin,
                  sizeof(lexer->Comment[lexer->CommentCount].CommentBegin));
        PX_strcpy(lexer->Comment[lexer->CommentCount].CommentEnd, End,
                  sizeof(lexer->Comment[lexer->CommentCount].CommentEnd));
        lexer->CommentCount++;
    }
}
...
px_bool PX_LexerIsContainerTransfer(px_lexer *lexer, const px_char startch[], px_char ch)
{
    ...
    {
        if (PX_memequ(startch, lexer->Container[i].ContainerBegin, PX_strlen(startch)) &&
            lexer->Container[i].transfer && lexer->Container[i].transfer == ch)
        {
            return PX_TRUE;
        }
    }
