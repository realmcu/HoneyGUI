...
px_bool PX_CompilerCompile(PX_Compiler *compiler, px_memory *bin, PX_VM_DebuggerMap *pdebugmap,
                           const px_char entryScript[])
{
    px_string asmcodeString;

    PX_StringInitialize(compiler->mp, &asmcodeString);

    if (PX_ScriptCompilerCompile(&compiler->lib, entryScript, &asmcodeString,
                                 PX_COMPILER_DEFAULT_STACK_SIZE, compiler->error, sizeof(compiler->error)))
    {
