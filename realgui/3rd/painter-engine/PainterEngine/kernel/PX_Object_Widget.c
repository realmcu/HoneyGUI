...
px_void PX_Object_WidgetRender(px_surface *psurface, PX_Object *pObject, px_uint elapsed)
{
    ...
    if (pObject->Width != (px_int)(pwidget->renderTarget.width) ||
        pObject->Height != (px_int)(pwidget->renderTarget.height + PX_OBJECT_WIDGET_BAR_SIZE))
    {
        px_texture *old = &pwidget->renderTarget;
        if (!PX_TextureCreate(pObject->mp, &pwidget->renderTarget, (px_int)pObject->Width,
                              (px_int)pObject->Height + PX_OBJECT_WIDGET_BAR_SIZE)) { return; }
        PX_TextureFree(old);
    }
    ...
    do
    {
        ...
        PX_SurfaceClear(&pwidget->renderTarget, 0, 0, pwidget->renderTarget.width - 1,
                        pwidget->renderTarget.height - 1, pwidget->backgroundcolor);

        PX_ObjectRender(&pwidget->renderTarget, pwidget->root, elapsed);
        PX_SurfaceRender(psurface, &pwidget->renderTarget, (px_int)objx,
                         (px_int)objy + PX_OBJECT_WIDGET_BAR_SIZE, PX_ALIGN_LEFTTOP, PX_NULL);
        if (pwidget->showShader)
        {
            px_color sd = pwidget->backgroundcolor;
            sd._argb.a = sd._argb.a / 3 * 2;
            PX_GeoDrawRect(psurface, (px_int)(objx + objWidth), (px_int)objy + 6, (px_int)(objx + objWidth + 6),
                           (px_int)(objy + objHeight + 6), sd);
            PX_GeoDrawRect(psurface, (px_int)(objx + 6), (px_int)(objy + objHeight),
                           (px_int)(objx + objWidth - 1), (px_int)(objy + objHeight + 6), sd);
        }
    ...
    PX_GeoDrawRect(psurface, (px_int)objx, (px_int)objy, (px_int)(objx + pObject->Width - 1),
                   (px_int)objy + PX_OBJECT_WIDGET_BAR_SIZE - 1, pwidget->barColor);

    if (pObject->OnFocus)
    {
        PX_GeoDrawBorder(psurface, (px_int)objx, (px_int)objy, (px_int)(objx + pObject->Width - 1),
                         (px_int)(objy + pObject->Height - 1), 1, pwidget->focusColor);
    }
    else
    {
        PX_GeoDrawBorder(psurface, (px_int)objx, (px_int)objy, (px_int)(objx + pObject->Width - 1),
                         (px_int)(objy + pObject->Height - 1), 1, pwidget->borderColor);
    }

    PX_FontModuleDrawText(psurface, pwidget->fontmodule, (px_int)objx + 3, (px_int)objy + 3,
                          PX_ALIGN_LEFTTOP, pwidget->title, pwidget->fontcolor);

}


PX_Object *PX_Object_WidgetCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                  px_int width, px_int height, const px_char title[], PX_FontModule *fontmodule)
{
    PX_Object *pObject;
    ...
    pWidget->btn_close = PX_Object_PushButtonCreate(mp, pObject, width - PX_OBJECT_WIDGET_BAR_SIZE, 3,
                                                    PX_OBJECT_WIDGET_BAR_SIZE - 6, PX_OBJECT_WIDGET_BAR_SIZE - 6, "-", PX_NULL);
    ...
    PX_ObjectRegisterEvent(pWidget->btn_close, PX_OBJECT_EVENT_EXECUTE, PX_Object_WidgetOnButtonClose,
                           pObject);
