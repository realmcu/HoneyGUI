...
static px_void PX_Object_MessageBox_BtnYesClick(PX_Object *pObject, PX_Object_Event e,
                                                px_void *user)
{
    ...
    if (pm->function_yes)
    {
        pm->function_yes((PX_Object *)user, PX_OBJECT_BUILD_EVENT(PX_OBJECT_EVENT_EXECUTE),
                         pm->function_yes_ptr);
    }
...
static px_void PX_Object_MessageBox_BtnNoClick(PX_Object *pObject, PX_Object_Event e, px_void *user)
{
    ...
    if (pm->function_no)
    {
        pm->function_no((PX_Object *)user, PX_OBJECT_BUILD_EVENT(PX_OBJECT_EVENT_EXECUTE),
                        pm->function_yes_ptr);
    }
}

static px_void PX_Object_MessageBox_EditOnEnter(PX_Object *pObject, PX_Object_Event e,
                                                px_void *user)
{
...
static px_void PX_Object_MessageBoxRender(px_surface *pSurface, PX_Object *pObject,
                                          px_dword elapsed)
{
    px_float x, y, w, h;
    ...
    {
        px_int width = pSurface->width * pm->schedule * pm->schedule /
                       (PX_OBJECT_MESSAGEBOX_DEFAULT_STAGE_1_TIME * PX_OBJECT_MESSAGEBOX_DEFAULT_STAGE_1_TIME);
        PX_GeoDrawRect(pSurface, (pSurface->width - width) / 2,
                       (pSurface->height - pm->PX_MESSAGEBOX_STAGE_1_HEIGHT) / 2, (pSurface->width + width) / 2,
                       (pSurface->height + pm->PX_MESSAGEBOX_STAGE_1_HEIGHT) / 2, backGroundColor);
    }
    else if (pm->schedule < PX_OBJECT_MESSAGEBOX_DEFAULT_STAGE_2_TIME)
    {
        px_int height = PX_OBJECT_MESSAGEBOX_DEFAULT_STAGE_2_HEIGHT * pm->schedule * pm->schedule /
                        (PX_OBJECT_MESSAGEBOX_DEFAULT_STAGE_2_TIME * PX_OBJECT_MESSAGEBOX_DEFAULT_STAGE_2_TIME);
        PX_GeoDrawRect(pSurface, 0, (pSurface->height - height) / 2, pSurface->width - 1,
                       (pSurface->height + height) / 2, backGroundColor);
    }
    else
    {
        PX_GeoDrawRect(pSurface, 0, (pSurface->height - pm->PX_MESSAGEBOX_STAGE_2_HEIGHT) / 2,
                       pSurface->width - 1, (pSurface->height + pm->PX_MESSAGEBOX_STAGE_2_HEIGHT) / 2, backGroundColor);
        PX_GeoDrawRect(pSurface, 0, (pSurface->height - pm->PX_MESSAGEBOX_STAGE_2_HEIGHT) / 2 - 30,
                       pSurface->width - 1, (pSurface->height - pm->PX_MESSAGEBOX_STAGE_2_HEIGHT) / 2 - 10,
                       backGroundColor);
        PX_GeoDrawRect(pSurface, 0, (pSurface->height + pm->PX_MESSAGEBOX_STAGE_2_HEIGHT) / 2 + 10,
                       pSurface->width - 1, (pSurface->height + pm->PX_MESSAGEBOX_STAGE_2_HEIGHT) / 2 + 30,
                       backGroundColor);

        if (pm->edit_inputbox->Visible)
        {
            //inputbox mode
            PX_FontModuleDrawText(pSurface, pm->fontmodule, pSurface->width / 2 - 108, pSurface->height / 2,
                                  PX_ALIGN_RIGHTMID, pm->Message, frontColor);
        }
        else
        {
            PX_FontModuleDrawText(pSurface, pm->fontmodule, pSurface->width / 2, pSurface->height / 2,
                                  PX_ALIGN_CENTER, pm->Message, frontColor);
        }
    }
                                      ...
                                      PX_FontModule *fontmodule)
{
    ...
    pObject = PX_ObjectCreateEx(mp, parent, 0, 0, 0, 0, 0, 0, PX_OBJECT_TYPE_MESSAGEBOX, PX_NULL,
                                PX_Object_MessageBoxRender, PX_Object_MessageBoxFree, pm, sizeof(PX_Object_MessageBox));
    pObject->Visible = PX_FALSE;
    ...
    PX_ObjectRegisterEvent(pm->edit_inputbox, PX_OBJECT_EVENT_KEYDOWN, PX_Object_MessageBox_EditOnEnter,
                           pObject);
    PX_ObjectRegisterEvent(pm->btn_Ok, PX_OBJECT_EVENT_EXECUTE, PX_Object_MessageBox_BtnYesClick,
                           pObject);
    PX_ObjectRegisterEvent(pm->btn_Cancel, PX_OBJECT_EVENT_EXECUTE, PX_Object_MessageBox_BtnNoClick,
                           pObject);
