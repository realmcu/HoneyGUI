...
px_void PX_Object_ProcessBarRender(px_surface *psurface, PX_Object *pObject, px_uint elapsed)
{
    ...
    {
        px_char text[32];
        PX_sprintf1(text, sizeof(text), "%1%",
                    PX_STRINGFORMAT_INT((px_int)((pProcessBar->Value * 1.0 / pProcessBar->MAX) * 100)));
        PX_FontModuleDrawText(psurface, 0, x + w / 2, y + h / 2, PX_ALIGN_CENTER, text,
                              pProcessBar->ProgressTextColor);
    }

}

PX_Object *PX_Object_ProcessBarCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                      px_int Width, px_int Height)
{
    PX_Object *pObject;
    PX_Object_ProcessBar *ProcessBar = (PX_Object_ProcessBar *)MP_Malloc(mp,
                                                                         sizeof(PX_Object_ProcessBar));
...
//process bar
//////////////////////////////////////////////////////////////////////////
PX_Object *PX_Designer_ProcessBarCreate(px_memorypool *mp, PX_Object *pparent, px_float x,
