...
px_bool PX_AnimationLibraryCreateFromMemory(px_memorypool *mp, PX_AnimationLibrary *panimation,
                                            px_byte *_2dxBuffer, px_uint size)
{
    ...
    PX_VectorInitialize(mp, &panimation->animation, sizeof(PX_Animationlibrary_tagInfo),
                        _header.animationCount);
...
px_void PX_AnimationLibraryFree(PX_AnimationLibrary *panimation)
{
    ...
    for (i = 0; i < panimation->animation.size; i++)
    {
        PX_Animationlibrary_tagInfo *tagInfo = PX_VECTORAT(PX_Animationlibrary_tagInfo,
                                                           &panimation->animation, i);
    ...
    while (elapsed)
    {
        ...
        case PX_2DX_OPCODE_FRAME:
            if (pInstr->param < panimation->linker->frames.size)
            {
                ...
                panimation->reg_clipw = PX_AnimationLibraryGetFrameWidth(panimation->linker,
                                                                         panimation->reg_currentFrameIndex);
                panimation->reg_cliph = PX_AnimationLibraryGetFrameHeight(panimation->linker,
                                                                          panimation->reg_currentFrameIndex);
            }
            else
            {
...
px_void PX_AnimationRenderMirror(px_surface *psurface, PX_Animation *animation, px_int x, px_int y,
                                 PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend, PX_TEXTURERENDER_MIRRROR_MODE mirror)
{
    px_texture *pTexture;
    if (!animation->linker)
    {
    ...
    if (animation->reg_clipw && animation->reg_cliph)
        if (animation->reg_currentFrameIndex >= 0 &&
            animation->reg_currentFrameIndex < animation->linker->frames.size)
        {
            px_int cxc;
            px_int clx, cly;
            ...
            PX_TextureRenderClipMirror(psurface, pTexture, x, y, clx, cly, animation->reg_clipw,
                                       animation->reg_cliph, refPoint, blend, mirror);
        }
}

px_void PX_AnimationRender(px_surface *psurface, PX_Animation *animation, px_int x, px_int y,
                           PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend)
{
    PX_AnimationRenderMirror(psurface, animation, x, y, refPoint, blend,
                             PX_TEXTURERENDER_MIRRROR_MODE_NONE);
}

px_void PX_AnimationRenderEx(px_surface *psurface, PX_Animation *animation, px_int x, px_int y,
                             px_float scale, px_point2D direction, PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend)
{
    px_texture *pTexture;
    if (!animation->linker)
    {
...
px_void PX_AnimationRender_scale(px_surface *psurface, PX_Animation *animation, px_int x, px_int y,
                                 px_float scale, PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend)
{
    px_texture *pTexture;
    if (!animation->linker)
    {
...
px_void PX_AnimationRender_vector(px_surface *psurface, PX_Animation *animation, px_int x, px_int y,
                                  px_point2D direction, PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend)
{
    px_texture *pTexture;
    if (!animation->linker)
    {
...
px_bool PX_AnimationLibrary_CreateEffect_JumpVertical(px_memorypool *mp,
                                                      PX_AnimationLibrary *panimation, px_texture *effectTexture)
{
    px_int i;
...
px_bool PX_AnimationShellCompile(px_memorypool *mp, const px_char script[], px_vector *taginfos,
                                 px_vector *texinfos, px_memory *out)
{
    px_int i;
    px_lexer     lexer;
    PX_LEXER_LEXEME_TYPE type;
...
px_void PX_AnimationLibraryAddInstr(PX_AnimationLibrary *panimationLib, PX_2DX_OPCODE opcode,
                                    px_word param)
{
    PX_2DX_INSTR instr;
    instr.opcode = opcode;
...
px_bool PX_AnimationIsEnd(PX_Animation *panimation)
{
    ...
    return (((PX_2DX_INSTR *)(panimation->linker->code.buffer + panimation->ip))->opcode ==
            PX_2DX_OPCODE_END);
}
...
px_void PX_AnimationRenderRotation(px_surface *psurface, PX_Animation *animation, px_point position,
                                   px_int angle, PX_ALIGN refPoint, PX_TEXTURERENDER_BLEND *blend)
{
    px_texture *pTexture;
    if (!animation->linker)
    {
    ...
    {
        pTexture = PX_VECTORAT(px_texture, &animation->linker->frames, animation->reg_currentFrameIndex);
        PX_TextureRenderRotation(psurface, pTexture, (px_int)position.x, (px_int)position.y, refPoint,
                                 blend, angle);
    }
}
...
px_bool PX_AnimationGetRenderRange(PX_Animation *animation, px_recti *range)
{
    ...
    if (pTexture)
    {
        ...
        return PX_TextureGetRenderRange(pTexture, clx, cly, animation->reg_clipw, animation->reg_cliph,
                                        range);
    }
    return PX_FALSE;
}
...
{
    if (animation->linker && animation->reg_currentAnimation >= 0 &&
        animation->reg_currentAnimation < (px_dword)animation->linker->animation.size)
    {
        PX_Animationlibrary_tagInfo *tag = PX_VECTORAT(PX_Animationlibrary_tagInfo,
                                                       &animation->linker->animation, animation->reg_currentAnimation);
...
px_int PX_AnimationLibraryGetPlayAnimationIndexByName(PX_AnimationLibrary *pLib,
                                                      const px_char *name)
{
    px_int i;
    if (pLib)
