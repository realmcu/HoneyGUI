...
PX_Object *PX_Object_ModbusVarBoxCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                        px_int width, px_int height, const px_char title[], PX_FontModule *fontmodule)
{
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0, (px_float)width,
                                (px_float)height, 0, PX_OBJECT_TYPE_MODBUSVARBOX, PX_NULL, PX_NULL, PX_NULL, &desc, sizeof(desc));
    ...
    pdesc->various_name = PX_Object_VariousCreate(mp, pdesc->widget, width / 2 - 128, 32, 256, 32,
                                                  "Name:", PX_OBJECT_VARIOUS_TYPE_EDIT, fontmodule);
    PX_Object_VariousSetEditStyle(pdesc->various_name, PX_OBJECT_VARIOUS_EDIT_TYPE_STRING);
    pdesc->various_address = PX_Object_VariousCreate(mp, pdesc->widget, width / 2 - 128, 72, 256, 32,
                                                     "Address:", PX_OBJECT_VARIOUS_TYPE_EDIT, fontmodule);
    PX_Object_VariousSetEditStyle(pdesc->various_address, PX_OBJECT_VARIOUS_EDIT_TYPE_HEX);

    pdesc->various_value = PX_Object_VariousCreate(mp, pdesc->widget, width / 2 - 128, 112, 256, 32,
                                                   "Value:", PX_OBJECT_VARIOUS_TYPE_EDIT, fontmodule);
    PX_Object_VariousSetEditStyle(pdesc->various_value, PX_OBJECT_VARIOUS_EDIT_TYPE_HEX);

    pdesc->selectbar_type = PX_Object_SelectBarCreate(mp, pdesc->widget, width / 2 - 128, 152, 256, 32,
                                                      fontmodule);
    ...
    PX_ObjectRegisterEvent(pdesc->selectbar_type, PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_ModbusVarBoxOnTypeChanged, pdesc);

    pdesc->checkbox_readonly = PX_Object_CheckBoxCreate(mp, pdesc->widget, width / 2 - 128, 192, 256,
                                                        32, "Read Only", fontmodule);
    pdesc->btn_ok = PX_Object_PushButtonCreate(mp, pdesc->widget, width - 256, height - 68, 96, 32,
                                               "OK", fontmodule);
    pdesc->btn_close = PX_Object_PushButtonCreate(mp, pdesc->widget, width - 128, height - 68, 96, 32,
                                                  "Cancel", fontmodule);

    PX_ObjectRegisterEvent(pdesc->btn_ok, PX_OBJECT_EVENT_EXECUTE, PX_Object_ModbusVarBoxOnConfirm,
                           pObject);
    PX_ObjectRegisterEvent(pdesc->btn_close, PX_OBJECT_EVENT_EXECUTE, PX_Object_ModbusVarBoxOnCancel,
                           pObject);
    PX_ObjectRegisterEvent(pdesc->widget, PX_OBJECT_EVENT_KEYDOWN, PX_Object_ModbusVarBoxOnKeyDown,
                           pObject);
...
px_void PX_Object_ModbusVarBoxShowEdit(PX_Object *pObject, const px_char name[], px_dword address,
                                       px_int type, px_word value)
{
    PX_Object_ModbusVarBox *pDesc = PX_ObjectGetDesc(PX_Object_ModbusVarBox, pObject);
    if (pObject->Type != PX_OBJECT_TYPE_MODBUSVARBOX)
    {
