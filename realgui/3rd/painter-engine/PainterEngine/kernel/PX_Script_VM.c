...
px_bool PX_VMInitialize(PX_VM *Ins, px_memorypool *mp, const px_byte *code, px_int size)
{
    ...
    if ((Ins->_mem = (PX_VM_VARIABLE *)MP_Malloc(mp,
                                                 sizeof(PX_VM_VARIABLE) * Ins->VM_memsize)) == PX_NULL)
    {
        goto _ERROR;
    }


    if ((Ins->pThread = (PX_VM_Thread *)MP_Malloc(mp,
                                                  sizeof(PX_VM_Thread) * Ins->maxThreadCount)) == PX_NULL)
    {
        goto _ERROR;
    }
    ...
    if (Ins->funcCount != 0)
    {
        if ((Ins->_func = (PX_VM_EXPORT_FUNCTION *)MP_Malloc(mp,
                                                             sizeof(PX_VM_EXPORT_FUNCTION) * Ins->funcCount)) == PX_NULL)
        {
            goto _ERROR;
        }
    ...
    if (Ins->hostCount != 0)
    {
        if ((Ins->_host = (PX_ASM_HOST_NODE *)MP_Malloc(mp,
                                                        sizeof(PX_ASM_HOST_NODE) * Ins->hostCount)) == PX_NULL)
        {
            goto _ERROR;
        }
...
static PX_VM_VARIABLE  PX_VM_GetParamConst(PX_VM *Ins, px_char optype, px_int param,
...
PX_VM_RUNRETURN PX_VMRunThread(PX_VM *Ins, px_int tick)
{
    ...
    while (PX_TRUE)
    {
        ...
        if (opCode != PX_SCRIPT_ASM_INSTR_OPCODE_MOV && opCode != PX_SCRIPT_ASM_INSTR_OPCODE_ADD &&
            opCode != PX_SCRIPT_ASM_INSTR_OPCODE_SUB && opCode != PX_SCRIPT_ASM_INSTR_OPCODE_PUSH &&
            opCode != PX_SCRIPT_ASM_INSTR_OPCODE_POP)
        {
            if (opType[0] == PX_SCRIPT_ASM_OPTYPE_IP || opType[0] == PX_SCRIPT_ASM_OPTYPE_SP ||
                opType[0] == PX_SCRIPT_ASM_OPTYPE_BP)
            {
                goto _ERROR;
            }
        }
        ...
        case PX_SCRIPT_ASM_INSTR_OPCODE_STRCUT:
            {
                ...
                if (pVar->type != PX_VM_VARIABLE_TYPE_STRING || sVar.type != PX_VM_VARIABLE_TYPE_INT ||
                    tVar.type != PX_VM_VARIABLE_TYPE_INT)
                {
        ...
        case PX_SCRIPT_ASM_INSTR_OPCODE_DATACPY:
            {
                ...
                if (cVar._int + tVar._int > Ins->VM_memsize || cVar._int < 0 ||
                    sVar._int + tVar._int > Ins->VM_memsize || sVar._int < 0)
                {
                ...
                if (sVar.type == tVar.type)
                {
                    ...
                    else
                    {
                        ...
                        if (sVar.type == PX_VM_VARIABLE_TYPE_MEMORY)
                            if ((sVar._memory.usedsize == tVar._memory.usedsize) &&
                                PX_memequ(sVar._memory.buffer, tVar._memory.buffer, sVar._memory.usedsize))
                            {
                                pVar->_int = 1;
                                pT->IP += (4 + 3 * 4);
                                break;
...
px_bool PX_VMCallFunctionEx(PX_VM *Ins, px_int threadID, const px_char *functionName,
                            PX_VM_VARIABLE args[], px_int paramcount, px_int returnIP)
{
    px_int i, j, ip = -1;
    px_int old_T;
...
px_bool PX_VMRunFunction(PX_VM *Ins, px_int threadID, const px_char *functionName,
                         PX_VM_VARIABLE args[], px_int paramcount)
{
...
px_bool PX_VMCallFunction(PX_VM *Ins, px_int threadID, const px_char *functionName,
                          PX_VM_VARIABLE args[], px_int paramcount)
{
    return PX_VMCallFunctionEx(Ins, threadID, functionName, args, paramcount,
                               Ins->pThread[threadID].IP);
}

px_bool PX_VMRunFunctionIndex(PX_VM *Ins, px_int threadID, px_int funcIndex, PX_VM_VARIABLE args[],
                              px_int paramcount)
{
    px_int i, j, ip;
    px_int old_T;
...
px_bool PX_VMBeginThreadFunction(PX_VM *Ins, px_int threadID, const px_char *func,
                                 PX_VM_VARIABLE args[], px_int paramcount)
{
    px_int i, j, ip = -1;
    px_int old_T;
...
px_bool PX_VMBeginThreadFunctionIndex(PX_VM *Ins, px_int threadID, px_int funcIndex,
                                      PX_VM_VARIABLE args[], px_int paramcount)
{
    px_int i, j, ip;
    px_int old_T;
...
px_bool PX_VMRegistHostFunction(PX_VM *Ins, const px_char *name, PX_VM_Function_Modules funcModules,
                                px_void *userptr)
{
    px_int i;
    px_char uprname[__PX_SCRIPT_ASM_MNEMONIC_NAME_LEN];
...
px_bool  PX_VMDebuggerGetVarParamName(PX_VM *Ins, px_char optype, px_int param, px_char content[],
                                      px_int size)
{
    PX_VM_Thread *pT = Ins->pThread + Ins->T;
    switch (optype)
...
px_void  PX_VMDebuggerGetParamName(PX_VM *Ins, px_char optype, px_int param, px_char content[],
                                   px_int size)
{
    PX_VM_Thread *pT = Ins->pThread + Ins->T;
    switch (optype)
...
px_int PX_VMDebuggerInstruction(PX_VM *Ins, px_int IP, px_char *opcode, px_char opcode_name[16],
                                px_int *paramCount, px_char opType[3], px_int param[3])
{
    px_char opCode;
    px_int i;
    PX_VM_Thread *pT = &Ins->pThread[Ins->T];
