...
px_void PX_Object_CanvasOnRender(px_surface *psurface, PX_Object *pObject, px_dword elapsed)
{
    ...
    PX_SurfaceRender(psurface, &pdesc->pCanvasVM->view_surface, (px_int)x, (px_int)y, PX_ALIGN_LEFTTOP,
                     0);
    PX_GeoDrawBorder(psurface, (px_int)x, (px_int)y, (px_int)(x + w), (px_int)(y + h), 1,
                     PX_COLOR_BLACK);
    PX_SurfaceSetLimit(psurface, (px_int)x, (px_int)y, (px_int)(x + w), (px_int)(h + y));
    PX_GeoDrawCircle(psurface, (px_int)(x + pdesc->reg_x), (px_int)(y + pdesc->reg_y),
                     (px_int)(pdesc->pCanvasVM->reg_size * pdesc->pCanvasVM->scale), 1, PX_COLOR(255, 0, 0, 0));
...
px_void PX_Object_CanvasOnHSliderBarChanged(PX_Object *pObject, PX_Object_Event  e, px_void *ptr)
{
    ...
    PX_CanvasVMSetViewPosition(pdesc->pCanvasVM, PX_Object_SliderBarGetValue(pObject) * 1.f,
                               pdesc->pCanvasVM->view_y);
...
{
    PX_Object_Canvas *pdesc = PX_ObjectGetDesc(PX_Object_Canvas, (PX_Object *)ptr);
    PX_CanvasVMSetViewPosition(pdesc->pCanvasVM, pdesc->pCanvasVM->view_x,
                               PX_Object_SliderBarGetValue(pObject) * 1.f);
}
...
px_void PX_Object_CanvasEnterEdit(PX_Object *pObject, px_float x, px_float y, px_float z)
{
    ...
    {
        canvaspos = PX_CanvasVMViewPositionToCanvasPostion(pdesc->pCanvasVM, PX_POINT2D(pdesc->reg_x,
                                                                                        pdesc->reg_y));
...
PX_Object *PX_Object_CanvasCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                  PX_CanvasVM *pCanvasVM)
{
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0,
                                (px_float)pCanvasVM->view_surface.width + 24, (px_float)pCanvasVM->view_surface.height + 24, 0,
                                PX_OBJECT_TYPE_CANVAS, 0, PX_Object_CanvasOnRender, PX_NULL, &desc, sizeof(desc));
    pdesc = PX_ObjectGetDesc(PX_Object_Canvas, pObject);
    pdesc->mp = mp;
    pdesc->pCanvasVM = pCanvasVM;

    pdesc->pHSliderBar = PX_Object_SliderBarCreate(mp, pObject, 0, pCanvasVM->view_surface.height,
                                                   pCanvasVM->view_surface.width, 24, PX_OBJECT_SLIDERBAR_TYPE_HORIZONTAL,
                                                   PX_OBJECT_SLIDERBAR_STYLE_BOX);
    pdesc->pVSliderBar = PX_Object_SliderBarCreate(mp, pObject, pCanvasVM->view_surface.width, 0, 24,
                                                   pCanvasVM->view_surface.height, PX_OBJECT_SLIDERBAR_TYPE_VERTICAL, PX_OBJECT_SLIDERBAR_STYLE_BOX);
    ...
    PX_ObjectRegisterEvent(pdesc->pHSliderBar, PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_CanvasOnHSliderBarChanged, pObject);
    PX_ObjectRegisterEvent(pdesc->pVSliderBar, PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_CanvasOnVSliderBarChanged, pObject);
