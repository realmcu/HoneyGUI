...
px_void PX_Object_ExplorerOnCursorMove(PX_Object *pObject, PX_Object_Event e, px_void *ptr)
{
    ...
    if (PX_isXYInRegion(x, y, objx, objy + PX_OBJECT_EXPOLRER_MENU_HEIGHT,
                        objWidth - PX_OBJECT_EXPOLRER_SLIDERBAR_WIDTH, objHeight - PX_OBJECT_EXPOLRER_MENU_HEIGHT * 2))
    {
        //get cursor index
        px_int i;
        px_int ofty;
...
static px_void PX_Object_ExplorerOnCursorWheel(PX_Object *pObject, PX_Object_Event e, px_void *ptr)
{
                        ...
                        objWidth - PX_OBJECT_EXPOLRER_SLIDERBAR_WIDTH, objHeight - PX_OBJECT_EXPOLRER_MENU_HEIGHT * 2))
    {
        if (z < 0)
        {
            PX_Object_SliderBarSetValue(pExp->SliderBar,
                                        PX_Object_SliderBarGetValue(pExp->SliderBar) + PX_OBJECT_EXPOLRER_ITEM_HEIGHT / 2);
        }
        else
        {
            PX_Object_SliderBarSetValue(pExp->SliderBar,
...
px_void PX_Object_ExplorerRender(px_surface *psurface, PX_Object *pObject, px_uint elapsed)
{
    ...
    PX_GeoDrawRect(psurface, (px_int)objx, (px_int)objy, (px_int)(objx + objWidth),
                   (px_int)(objy + objHeight), pExp->backgroundcolor);
    ...
    pExp->edit_Path->Width = objWidth - PX_OBJECT_EXPOLRER_BUTTON_WIDTH -
                             PX_OBJECT_EXPOLRER_BUTTON_WIDTH - 8 * 4;
    if (pExp->fontmodule)
    {
    ...
    pExp->SliderBar->Height = objHeight - PX_OBJECT_EXPOLRER_MENU_HEIGHT -
                              PX_OBJECT_EXPOLRER_MENU_HEIGHT;
    ...
    if (pExp->ItemCount * PX_OBJECT_EXPOLRER_ITEM_HEIGHT > objHeight - PX_OBJECT_EXPOLRER_MENU_HEIGHT -
        PX_OBJECT_EXPOLRER_MENU_HEIGHT)
    {
        pExp->SliderBar->Visible = PX_TRUE;
        PX_Object_SliderBarSetRange(pExp->SliderBar, 0,
                                    pExp->ItemCount * PX_OBJECT_EXPOLRER_ITEM_HEIGHT - ((px_int)objHeight -
                                                                                        PX_OBJECT_EXPOLRER_MENU_HEIGHT - PX_OBJECT_EXPOLRER_MENU_HEIGHT));
        PX_Object_SliderBarSetSliderButtonLength(pExp->SliderBar,
                                                 (px_int)(pExp->SliderBar->Height * pExp->SliderBar->Height / pExp->ItemCount /
                                                          PX_OBJECT_EXPOLRER_ITEM_HEIGHT));
    }
    else
    {
    ...
    do
    {
        ...
        PX_SurfaceSetLimit(psurface, (px_int)objx, (px_int)objy + PX_OBJECT_EXPOLRER_MENU_HEIGHT,
                           (px_int)(objx + objWidth), (px_int)(objy + objHeight - PX_OBJECT_EXPOLRER_MENU_HEIGHT));
        while (startIndex < pExp->ItemCount && drawy < objHeight - PX_OBJECT_EXPOLRER_MENU_HEIGHT)
        {
            ...
            if (pItem->bcursor)
            {
                PX_GeoDrawRect(psurface, (px_int)objx, (px_int)objy + drawy, (px_int)(objx + objWidth),
                               (px_int)objy + drawy + PX_OBJECT_EXPOLRER_ITEM_HEIGHT - 1, pExp->cursorcolor);
            }

            if (pItem->bFolder)
            {
                PX_ShapeRender(psurface, &pExp->folder, (px_int)objx + 16 + 3,
                               (px_int)objy + drawy + PX_OBJECT_EXPOLRER_ITEM_HEIGHT / 2, PX_ALIGN_CENTER, pExp->fontcolor);
            }
            else
            {
                PX_ShapeRender(psurface, &pExp->file, (px_int)objx + 16 + 3,
                               (px_int)objy + drawy + PX_OBJECT_EXPOLRER_ITEM_HEIGHT / 2, PX_ALIGN_CENTER, pExp->fontcolor);
            }

            PX_FontModuleDrawText(psurface, pExp->fontmodule, (px_int)objx + 32 + 8,
                                  (px_int)objy + drawy + PX_OBJECT_EXPOLRER_ITEM_HEIGHT / 2, PX_ALIGN_LEFTMID, pItem->name,
                                  pExp->fontcolor);

            if (pItem->bselect)
            {
                PX_GeoDrawSolidCircle(psurface, (px_int)(objx + objWidth) - 24 - 16,
                                      (px_int)objy + drawy + PX_OBJECT_EXPOLRER_ITEM_HEIGHT / 2, 8, pExp->fontcolor);
            }
    ...
    PX_GeoDrawLine(psurface, (px_int)objx, (px_int)(objy + PX_OBJECT_EXPOLRER_MENU_HEIGHT),
                   (px_int)(objx + objWidth), (px_int)(objy + PX_OBJECT_EXPOLRER_MENU_HEIGHT), 1, pExp->fontcolor);
    PX_GeoDrawLine(psurface, (px_int)objx, (px_int)(objy + objHeight - PX_OBJECT_EXPOLRER_MENU_HEIGHT),
                   (px_int)(objx + objWidth), (px_int)(objy + objHeight - PX_OBJECT_EXPOLRER_MENU_HEIGHT), 1,
                   pExp->fontcolor);

    pExp->btn_Ok->x = objWidth - 8 - PX_OBJECT_EXPOLRER_BUTTON_WIDTH - 8 -
                      PX_OBJECT_EXPOLRER_BUTTON_WIDTH;
...
PX_Object *PX_Object_ExplorerCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                    px_int Width, px_int Height, PX_FontModule *fm, PX_ExplorerGetPathFolderCount _func_gpfdc,
                                    PX_ExplorerGetPathFileCount _func_gpfec, PX_ExplorerGetPathFolderName _func_gpfdn,
                                    PX_ExplorerGetPathFileName _func_gpfcn, const px_char path[])
{
    ...
    if (!PX_ShapeCreateFromMemory(mp, PX_Object_Explorer_fileLogoData,
                                  sizeof(PX_Object_Explorer_fileLogoData), &explorer.file)) { return PX_NULL; }
    if (!PX_ShapeCreateFromMemory(mp, PX_Object_Explorer_folderLogoData,
                                  sizeof(PX_Object_Explorer_folderLogoData), &explorer.folder)) { return PX_NULL; }

    pObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0, (px_float)Width,
                                (px_float)Height, 0, PX_OBJECT_TYPE_EXPLORER, PX_NULL, PX_Object_ExplorerRender,
                                PX_Object_ExplorerFree, &explorer, sizeof(PX_Object_Explorer));
    ...
    do
    {
        ...
        PX_Object_EditSetText(pExp->edit_Path, path);
        pExp->SliderBar = PX_Object_SliderBarCreate(mp, pObject, 0, 0, 24, 24,
                                                    PX_OBJECT_SLIDERBAR_TYPE_VERTICAL, PX_OBJECT_SLIDERBAR_STYLE_BOX);
        pExp->btn_Ok->Visible = PX_FALSE;
        PX_ObjectRegisterEvent(pExp->btn_Back, PX_OBJECT_EVENT_EXECUTE, PX_Object_ExplorerOnButtonBack,
                               pObject);
        PX_ObjectRegisterEvent(pExp->btn_go, PX_OBJECT_EVENT_EXECUTE, PX_Object_ExplorerOnButtonGo,
                               pObject);
        PX_ObjectRegisterEvent(pExp->btn_Ok, PX_OBJECT_EVENT_EXECUTE, PX_Object_ExplorerOnButtonOk,
                               pObject);
        PX_ObjectRegisterEvent(pExp->btn_Cancel, PX_OBJECT_EVENT_EXECUTE, PX_Object_ExplorerOnButtonCancel,
                               pObject);
        PX_ObjectRegisterEvent(pExp->edit_FileName, PX_OBJECT_EVENT_VALUECHANGED,
                               PX_Object_ExplorerOnEditChanged, pObject);

        PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORMOVE, PX_Object_ExplorerOnCursorMove,
                               pObject);
        PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORDOWN, PX_Object_ExplorerOnCursorDown,
                               pObject);
        PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORWHEEL, PX_Object_ExplorerOnCursorWheel,
                               pObject);
...
px_void PX_Object_ExplorerRefresh(PX_Object *pObject)
{
    ...
    if (pExp)
    {
        ...
        if (!pExp->func_getpathfilecount || !pExp->func_getpathfilename || !pExp->func_getpathfoldercount ||
            !pExp->func_getpathfoldername)
        {
            return;
        }
