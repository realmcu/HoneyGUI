    ...
    while (time > pDesc->reg_delay)
    {
        ...
        if (pDesc->fontModule)
        {
            ...
            else
            {
                if (psurface)
                {
                    PX_FontModuleDrawCharacter(psurface, pDesc->fontModule, x_draw_oft, y_draw_oft, code,
                                               pDesc->reg_color);
                }
                if (pDesc->x_spacer == 0)
                {
...
PX_Object *PX_Object_TyperCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                 px_int limit_width, PX_FontModule *fm)
{
    ...
    pObject = PX_ObjectCreateEx(mp, Parent,  x * 1.0f, y * 1.0f, 0, limit_width * 1.0f, 0, 0,
                                PX_OBJECT_TYPE_TYPER, PX_NULL, PX_Object_TyperRender, PX_Object_TyperFree, 0,
                                sizeof(PX_Object_Typer));
    if (pObject == PX_NULL)
    {
        return PX_NULL;
    }
                    ...
                    if (PX_strequ(content, "color"))
                    {
                        ...
                        if (untillen)
                        {
                            ...
                            if (PX_strlen(content) == 8)
                            {
                                ...
                                if (!PX_Object_TyperSetColor(pObject, (px_byte)(color), (px_byte)(color >> 8),
                                                             (px_byte)(color >> 16), (px_byte)(color >> 24)))
                                {
                                    return PX_FALSE;
                                }
                                cursor = offset;
