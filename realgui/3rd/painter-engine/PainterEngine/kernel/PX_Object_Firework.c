...
void PX_Object_Firework01ParticalRender(PX_Firework01_paritcal *pdesc, px_surface *psurface,
                                        px_bool bb, px_dword elapsed)
{
    px_int i;
    px_float scale;
    px_color color;
    ...
    for (i = PX_COUNTOF(pdesc->array) - 1; i >= 0; i--)
    {
        ...
        PX_GeoDrawPenCircleDecay(psurface, pdesc->array[i].x, pdesc->array[i].y, (pdesc->size * scale),
                                 color, 1);
    }
}
...
PX_Object *PX_Object_Firework01Create(px_memorypool *mp, PX_Object *Parent, px_float x, px_float y)
{
    ...
    desc.particals[0].clr = PX_COLOR(255, 128 + (PX_rand() % 128), 128 + (PX_rand() % 128),
                                     128 + (PX_rand() % 128));
    return PX_ObjectCreateEx(mp, Parent, x, y, 0, 0, 0, 0, PX_OBJECT_TYPE_FIREWORK, 0,
                             PX_Object_Firework01Render, 0, &desc, sizeof(desc));
}
...
void PX_Firework02ParticleRender(px_surface *psurface, PX_Object_Firework02 *pdesc)
{
    ...
    if (pdesc->state == RISE)
    {
        ...
        {
            riseClr._argb.a = (px_uchar)(255.0f * (i - PX_FIREWORK02_PointNum / 2) /
                                         (PX_FIREWORK02_PointNum / 2));
            PX_GeoDrawPenCircleDecay(psurface, pdesc->particleList[0].x * 1.0f,
                                     pdesc->particleList[0].pointList[i].y * 1.0f, riseSize, riseClr, 1);
        }
    }
    else
    {
        ...
        for (i = 0; i < PX_FIREWORK02_ParticleNum; i++)
        {
            for (j = 0; j < PX_FIREWORK02_PointNum - 1; j++)
            {
                ...
                {
                    boomClr._argb.a = (px_uchar)(255.0f * (PX_FIREWORK02_DeleteTime - pdesc->livingTime) /
                                                 (PX_FIREWORK02_DeleteTime - PX_FIREWORK02_DisappearTime));
                    boomSize = (px_float)PX_FIREWORK02_BoomSize * (PX_FIREWORK02_DeleteTime - pdesc->livingTime) /
                               (PX_FIREWORK02_DeleteTime - PX_FIREWORK02_DisappearTime);
                }
                PX_GeoDrawPenCircleDecay(psurface, pdesc->particleList[i].pointList[j].x * 1.0f,
                                         pdesc->particleList[i].pointList[j].y * 1.0f, boomSize * j / PX_FIREWORK02_PointNum, boomClr, 1);
            }
        }
    }
}
...
PX_Object *PX_Object_Firework02Create(px_memorypool *mp, PX_Object *Parent, px_float x, px_float y)
{
    ...
    return PX_ObjectCreateEx(mp, Parent, x, y, 0, 0, 0, 0, 0, 0, PX_Object_Firework02Render, 0, &desc,
                             sizeof(desc));
}
...
void ParticleUpdate(PX_Object_Firework03 *pdesc, px_uint elapsed)
{
    ...
    if (pdesc->particleTime > 20)
    {
        ...
        for (i = 0; i < PX_FIREWORK02_ParticleNum; i++)
        {
            for (j = 0; j < PX_FIREWORK02_PointNum - 1; j++)
            {
                pdesc->particleList[i].pointList[j].x = pdesc->particleList[i].pointList[j + 1].x +
                                                        (PX_rand() % 2) - 1;
                pdesc->particleList[i].pointList[j].y = pdesc->particleList[i].pointList[j + 1].y +
                                                        (PX_rand() % 2) - 1;
            }
        }
    }
        ...
        for (i = 0; i < PX_FIREWORK02_ParticleNum; i++)
        {
            for (j = 0; j < PX_FIREWORK02_PointNum - 1; j++)
            {
                ...
                if (pdesc->state == DISAPPEAR)
                {
                    boomClr._argb.a = (px_uchar)(255.0f * (PX_FIREWORK02_DeleteTime - pdesc->livingTime) /
                                                 (PX_FIREWORK02_DeleteTime - PX_FIREWORK02_DisappearTime));
                    boomSize = 4.0f * (PX_FIREWORK02_DeleteTime - pdesc->livingTime) / (PX_FIREWORK02_DeleteTime -
                                                                                        PX_FIREWORK02_DisappearTime);
                }
                PX_GeoDrawPenCircleDecay(psurface, pdesc->particleList[i].pointList[j].x * 1.f,
                                         pdesc->particleList[i].pointList[j].y * 1.f, boomSize * j / PX_FIREWORK02_PointNum, boomClr, 1);
            }
        }
    }
}
...
PX_Object *PX_Object_Firework03Create(px_memorypool *mp, PX_Object *Parent, px_float x, px_float y)
{
    ...
    return PX_ObjectCreateEx(mp, Parent, x, y, 0, 0, 0, 0, 0, 0, PX_Object_Firework03Render, 0, &desc,
                             sizeof(desc));
}
