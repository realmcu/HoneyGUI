...
PX_Object *PX_Object_PrinterPrintText(PX_Object *pObject, const px_char *text)
{
    if (pObject->Type == PX_OBJECT_TYPE_PRINTER)
    {
        ...
        PX_Object_Printer *pDesc = PX_ObjectGetDesc(PX_Object_Printer, pObject);
        PX_Object *pAutoObject = PX_Object_AutoTextCreate(pObject->mp, pDesc->Area, 0, 0,
                                                          (px_int)pDesc->Area->Width, pDesc->fm);
...
PX_Object *PX_Object_PrinterPrintProcessBar(PX_Object *pObject)
{
    if (pObject->Type == PX_OBJECT_TYPE_PRINTER)
    {
        PX_Object_PrinterLine obj;
        PX_Object_Printer *pDesc = PX_ObjectGetDesc(PX_Object_Printer, pObject);
        PX_Object *pNewObject = PX_Object_ProcessBarCreate(pObject->mp, pDesc->Area, 3, 0,
                                                           (px_int)pDesc->Area->Width * 2 / 3, 24);
...
PX_Object *PX_Object_PrinterPrintButton(PX_Object *pObject, px_int width, px_int height,
                                        const px_char *text)
{
    if (pObject->Type == PX_OBJECT_TYPE_PRINTER)
    {
        PX_Object_PrinterLine obj;
        PX_Object_Printer *pDesc = PX_ObjectGetDesc(PX_Object_Printer, pObject);
        PX_Object *pNewObject = PX_Object_PushButtonCreate(pObject->mp, pDesc->Area, 0, 0, width, height,
                                                           text, pDesc->fm);
...
PX_Object *PX_Object_PrinterPrintImage(PX_Object *pObject, px_texture *pTexture)
{
    if (pObject->Type == PX_OBJECT_TYPE_PRINTER)
    {
        ...
        if (pTexture)
        {
            pImageObject = PX_Object_ImageCreate(pObject->mp, pDesc->Area, 0, 0, pTexture->width,
                                                 pTexture->height, pTexture);
        }
        else
        {
...
px_void PX_Object_PrinterOnEnter(PX_Object *pObject, PX_Object_Event e, px_void *user_ptr)
{
    ...
    if (e.Event == PX_OBJECT_EVENT_KEYDOWN)
    {
        ...
        {
            PX_ObjectExecuteEvent((PX_Object *)user_ptr, PX_OBJECT_BUILD_EVENT_STRING(PX_OBJECT_EVENT_EXECUTE,
                                                                                      PX_Object_EditGetText(pDesc->Input)));
...
PX_Object *PX_Object_PrinterCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                   px_int width, px_int height, PX_FontModule *fm)
{
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0, (px_float)width,
                                (px_float)height, 0, PX_OBJECT_TYPE_PRINTER, 0, PX_Object_PrinterRender, PX_Object_PrinterFree,
                                &Desc, sizeof(Desc));
    ...
    PX_ObjectRegisterEvent(pDesc->Area, PX_OBJECT_EVENT_CURSORDOWN, PX_Object_PrinterOnMouseDown,
                           pObject);
    ...
    PX_VectorInitialize(mp, &pDesc->pObjects, sizeof(PX_Object_PrinterLine),
                        PX_OBJECT_PRINTER_DEFAULT_MAX_COLUMN);
...
//Printer
/////////////////////////////////////////////////////////
PX_Object *PX_Designer_PrinterCreate(px_memorypool *mp, PX_Object *pparent, px_float x, px_float y,
                                     px_float width, px_float height, px_void *ptr)
{
    PX_FontModule *fm = (PX_FontModule *)ptr;
    return PX_Object_PrinterCreate(mp, pparent, (px_int)x, (px_int)y, (px_int)width, (px_int)height,
                                   fm);
}
