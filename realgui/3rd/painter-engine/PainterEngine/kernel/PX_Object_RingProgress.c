...
px_void PX_Object_RingProgressRender(px_surface *psurface, PX_Object *pObject, px_uint elapsed)
{
    ...
    PX_GeoDrawRing(psurface, (px_int)objx, (px_int)objy, pObject->diameter / 2, 6,
                   pDesc->backgroundColor, 0, 360);
    PX_GeoDrawRing(psurface, (px_int)objx, (px_int)objy, pObject->diameter / 2, 7, pDesc->borderColor,
                   0, 360);


    angle = (px_int)((pDesc->current_value) / pDesc->max * 360);

    PX_GeoDrawRing(psurface, (px_int)objx, (px_int)objy, pObject->diameter / 2, 5, pDesc->processColor,
                   -90, -90 + angle);
...
PX_Object *PX_Object_RingProgressCreate(px_memorypool *mp, PX_Object *Parent, px_float x,
                                        px_float y, px_float radius, PX_FontModule *fm)
{
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, x, y, 0, radius + radius, radius + radius, 0,
                                PX_OBJECT_TYPE_RINGPROGRESS, PX_NULL, PX_Object_RingProgressRender, PX_NULL, &Desc, sizeof(Desc));
