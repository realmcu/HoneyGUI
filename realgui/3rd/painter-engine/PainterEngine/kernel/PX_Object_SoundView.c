    ...
    if (PX_SoundPlayReadCurrentPlayingData(pDesc->pSoundPlay, 0, 0, uframe, SOUNDVIEW_FRAME_SIZE))
    {
        ...
        for (i = 0; i < SOUNDVIEW_FRAME_SIZE / 2; i++)
        {
            ...
            for (j = 0; j < lheight; j++)
            {
                ...
                r = (r > 255 ? 255 : r);
                PX_SurfaceDrawPixel(psurface, (px_int)(objx + objWidth / 2 + i), (px_int)(objy + objHeight - 1 - j),
                                    PX_COLOR(255 - r, r, 255 - r, 255 - r));
                PX_SurfaceDrawPixel(psurface, (px_int)(objx + objWidth / 2 - i), (px_int)(objy + objHeight - 1 - j),
                                    PX_COLOR(255 - r, r, 255 - r, 255 - r));
...
PX_Object *PX_Object_SoundViewCreate(px_memorypool *mp, PX_Object *Parent, px_float x, px_float y,
                                     px_float width, px_float height, PX_SoundPlay *pSoundPlay)
{
    PX_Object *pObject;
    PX_Object_SoundView *pDesc;
    pObject = PX_ObjectCreateEx(mp, Parent, x, y, 0, width, height, 0, PX_OBJECT_TYPE_SOUNDVIEW, 0,
                                PX_Object_SoundViewRender, PX_NULL, 0, sizeof(PX_Object_SoundView));
