...
px_void PX_Object_RadioBoxOnMouseLButtonDown(PX_Object *pObject, PX_Object_Event e,
                                             px_void *user_ptr)
{
    PX_Object_RadioBox *pcb = PX_Object_GetRadioBox(pObject);
    px_float x = (PX_Object_Event_GetCursorX(e));
...
px_void PX_Object_RadioBoxRender(px_surface *psurface, PX_Object *pObject, px_uint elapsed)
{
    ...
    PX_FontModuleDrawText(psurface, pcb->fm, (px_int)objx + 20, (px_int)(objy + objHeight / 2),
                          PX_ALIGN_LEFTMID, pcb->Text, pcb->TextColor);

    //draw RadioState
    PX_GeoDrawCircle(psurface, (px_int)objx + 8, (px_int)(objy + objHeight / 2), 7, 1,
                     pcb->BorderColor);

    if (pcb->bCheck)
    {
        PX_GeoDrawSolidCircle(psurface, (px_int)objx + 8, (px_int)(objy + objHeight / 2), 5,
                              pcb->BorderColor);
    }

}

PX_Object *PX_Object_RadioBoxCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                    px_int Width, px_int Height, const char text[], PX_FontModule *fm)
{
    ...
    pObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0, (px_float)Width,
                                (px_float)Height, 0, PX_OBJECT_TYPE_RADIOBOX, PX_NULL, PX_Object_RadioBoxRender, PX_NULL, &cbx,
                                sizeof(cbx));
    if (!pObject)
    {
        return PX_NULL;
    ...
    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORDOWN, PX_Object_RadioBoxOnMouseLButtonDown,
                           PX_NULL);
    PX_ObjectRegisterEvent(pObject, PX_OBJECT_EVENT_CURSORUP, PX_Object_RadioBoxOnMouseLButtonUp,
                           PX_NULL);
    return pObject;
}
...
//radiobox
//////////////////////////////////////////////////////////////////////////
PX_Object *PX_Designer_RadioBoxCreate(px_memorypool *mp, PX_Object *pparent, px_float x, px_float y,
