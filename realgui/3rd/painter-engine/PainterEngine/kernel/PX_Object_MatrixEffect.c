...
px_void PX_Object_MatrixEffectCharactersInitialize(PX_Object_MatrixEffect *effect,
                                                   px_surface *TargetSurface)
{
...
px_void PX_Object_MatrixEffectCharactersDisplayMode1(PX_Object_MatrixEffect *effect,
                                                     px_surface *TargetSurface)
{
    px_int oft = -48, i;
...
px_void PX_Object_MatrixEffectCharactersDisplayMode2(PX_Object_MatrixEffect *effect,
                                                     px_surface *TargetSurface)
{
    px_int oft = -24, i;
    PX_Object_MatrixEffectCharactersInitialize(effect, TargetSurface);
...
px_bool PX_Object_MatrixEffectCharactorsRender(PX_Object_MatrixEffect *effect)
{
    ...
    PX_SurfaceClear(&effect->SwitchTarget, 0, 0, effect->SwitchTarget.width,
                    effect->SwitchTarget.height, PX_COLOR(255, 0, 0, 0));
...
px_bool PX_Object_MatrixEffectSwitch(PX_Object_MatrixEffect *effect, px_surface *snapshot,
                                     px_int mode)
{
    switch (mode)
    {
...
px_bool PX_Object_MatrixEffectCreate(px_memorypool *mp, PX_Object *parent, px_surface *pMapsurface,
                                     px_int Mode_0or1)
{
    px_int i, j;
    ...
    pObject = PX_ObjectCreateEx(mp, parent, 0, 0, 0, pMapsurface->width * 1.0f,
                                pMapsurface->height * 1.0f, 0, PX_OBJECT_TYPE_MATRIXEFFECT, 0, PX_Object_MatrixEffectRender,
                                PX_NULL, PX_NULL, sizeof(PX_Object_MatrixEffect));
    ...
    if (!PC_Ui_SurfacebInitialized)
    {
        ...
        for (i = 0; i < 16; i++)
        {
            for (j = 0; j < 4; j++)
            {
                if (!PX_SurfaceCreate(pDesc->mp, PC_UI_CHAR_WIDTH * 2, PC_UI_CHAR_HEIGHT * 2,
                                      &PC_Ui_SurfaceChar[i][j]))
                {
                    goto _ERROR;
                }
        ...
        for (i = 0; i < 16; i++)
        {
            for (j = 0; j < 4; j++)
            {
                PX_FontDrawChar(&PC_Ui_SurfaceChar[i][j], PC_UI_CHAR_WIDTH / 2, PC_UI_CHAR_HEIGHT / 2,
                                PC_Ui_RandChar[i], PX_COLOR(255, 0, 240, 64));
    ...
    pDesc->CharMap = (PX_UI_MATRIXEFFECT_MAP *)MP_Malloc(mp,
                                                         pDesc->CharYCount * pDesc->CharXCount * sizeof(PX_UI_MATRIXEFFECT_MAP));
