...
px_void PX_Object_ListOnCursorMove(PX_Object *pObject, PX_Object_Event e, px_void *ptr)
{
    ...
    pList->currentCursorIndex = (px_int)(PX_Object_Event_GetCursorY(e) + pList->offsety - objy) /
                                pList->ItemHeight;
...
px_void PX_Object_ListOnCursorDown(PX_Object *pObject, PX_Object_Event e, px_void *ptr)
{
    ...
    currentSelectedIndex = (px_int)(PX_Object_Event_GetCursorY(e) + pList->offsety - objy) /
                           pList->ItemHeight;
...
px_void PX_Object_ListMoveToBottom(PX_Object *pObject)
{
    ...
    if (pList->ItemHeight * pList->pData.size > pObject->Height)
    {
        ...
        PX_Object_SliderBarSetRange(pList->SliderBar, 0,
                                    (px_int)(pList->ItemHeight * (pList->pData.size) - pObject->Height));
        PX_Object_SliderBarSetValue(pList->SliderBar,
                                    (px_int)(pList->ItemHeight * pList->pData.size - pObject->Height));
    }
    else
    {
...
px_void PX_Object_ListRender(px_surface *psurface, PX_Object *pObject, px_uint elapsed)
{
    ...
    if (pList->Items.size != (px_int)(objHeight / (pList->ItemHeight) + 2))
    {
        ...
        for (i = 0; i < (px_int)(objHeight / pList->ItemHeight + 2); i++)
        {
            Item.pList = pObject;
            Item.pdata = PX_NULL;
            pItemObject = PX_ObjectCreateEx(pList->mp, 0, 0, 0, 0, (px_float)pObject->Width,
                                            (px_float)pList->ItemHeight, 0, PX_OBJECT_TYPE_LISTITEM, PX_NULL, PX_NULL, PX_NULL, &Item,
                                            sizeof(PX_Object_ListItem));
    ...
    if (pList->ItemHeight * pList->pData.size > objHeight)
    {
        ...
        pSliderBar->SliderButtonLength = (px_int)(pList->SliderBar->Height * objHeight /
                                                  (pList->ItemHeight * pList->pData.size));
    ...
    if ((px_int)pList->renderSurface.width != (px_int)pObject->Width ||
        (px_int)pList->renderSurface.height != (px_int)pObject->Height)
    {
        PX_SurfaceFree(&pList->renderSurface);
        if (!PX_SurfaceCreate(pList->mp, (px_int)pObject->Width, (px_int)pObject->Height,
                              &pList->renderSurface))
        {
            PX_ASSERT();
            return;
    ...
    PX_SurfaceClear(&pList->renderSurface, 0, 0, pList->renderSurface.width - 1,
                    pList->renderSurface.height - 1, pList->BackgroundColor);
    for (i = 0; i < pList->Items.size; i++)
    {
        px_int index = i + pList->offsety / pList->ItemHeight;
        pItemObject = *PX_VECTORAT(PX_Object *, &pList->Items, i);
        ...
        if (pItemObject->y + pItemObject->Height > 0 && pItemObject->y < pObject->Height && pItem->pdata)
        {
            ...
            if (pList->showCursor)
            {
                ...
                {
                    PX_GeoDrawRect(&pList->renderSurface, (px_int)pItemObject->x, (px_int)pItemObject->y,
                                   (px_int)(pItemObject->x + pItemObject->Width - 1),
                                   (px_int)(pItemObject->y + pItemObject->Height - 1), pList->CursorColor);
                }
            }
            if (index == pList->currentSelectedIndex)
            {
                PX_GeoDrawRect(&pList->renderSurface, (px_int)pItemObject->x, (px_int)pItemObject->y,
                               (px_int)(pItemObject->x + pItemObject->Width - 1),
                               (px_int)(pItemObject->y + pItemObject->Height - 1), pList->SelectColor);
            }
            PX_ObjectRender(&pList->renderSurface, pItemObject, elapsed);
        }
    }
    PX_GeoDrawBorder(&pList->renderSurface, 0, 0, pList->renderSurface.width - 1,
                     pList->renderSurface.height - 1, 1, pList->BorderColor);
...
PX_Object *PX_Object_ListCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                px_int Width, px_int Height, px_int ItemHeight, PX_Object_ListItemOnCreate _CreateFunc,
                                px_void *userptr)
{
    ...
    if (!(pListObject = PX_ObjectCreateEx(mp, Parent, (px_float)x, (px_float)y, 0, (px_float)Width,
                                          (px_float)Height, 0, PX_OBJECT_TYPE_LIST, PX_NULL, PX_Object_ListRender, PX_Object_ListFree, &List,
                                          sizeof(PX_Object_List)))) { return PX_NULL; }

    pList = PX_Object_GetList(pListObject);
    pList->SliderBar = PX_Object_SliderBarCreate(mp, pListObject,
                                                 (px_int)pListObject->Width - PX_OBJECT_LIST_SLIDERBAR_WIDTH, 0, PX_OBJECT_LIST_SLIDERBAR_WIDTH,
                                                 (px_int)pListObject->Height, PX_OBJECT_SLIDERBAR_TYPE_VERTICAL, PX_OBJECT_SLIDERBAR_STYLE_BOX);

    PX_ObjectRegisterEvent(pListObject, PX_OBJECT_EVENT_CURSORWHEEL, PX_Object_ListOnWheel,
                           pListObject);
    PX_ObjectRegisterEvent(pListObject, PX_OBJECT_EVENT_CURSORDOWN, PX_Object_ListOnCursorDown,
                           pListObject);
    PX_ObjectRegisterEvent(pListObject, PX_OBJECT_EVENT_CURSORRDOWN, PX_Object_ListOnCursorRDown,
                           pListObject);
    PX_ObjectRegisterEvent(pListObject, PX_OBJECT_EVENT_CURSORMOVE, PX_Object_ListOnCursorMove,
                           pListObject);
    PX_ObjectRegisterEvent(pList->SliderBar, PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_ListOnSliderValueChanged, pListObject);
    PX_ObjectRegisterEvent(pListObject, PX_OBJECT_EVENT_ANY, PX_Object_ListOnEvent, pListObject);
    return pListObject;
}

static px_void PX_Object_ListContentItemOnRender(px_surface *psurface, PX_Object *pObject,
                                                 px_dword elapsed)
{
    ...
    PX_FontModuleDrawText(psurface, PX_Object_GetList(pListItem->pList)->fm, (px_int)objx + 3,
                          (px_int)(objy + objHeight / 2), PX_ALIGN_LEFTMID, (const px_char *)pListItem->pdata,
                          PX_OBJECT_UI_DEFAULT_FONTCOLOR);

}

px_bool PX_Designer_ListContentItemOnCreate(px_memorypool *mp, PX_Object *ItemObject,
                                            px_void *userptr)
{
...
PX_Object *PX_Object_ListContentCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                       px_int Width, px_int Height, PX_FontModule *fm)
{
    PX_Object *pObject;
    if (fm)
    {
        pObject = PX_Object_ListCreate(mp, Parent, x, y, Width, Height, fm->max_Height + 6,
                                       PX_Designer_ListContentItemOnCreate, PX_NULL);
        if (pObject)
        {
    ...
    {
        pObject = PX_Object_ListCreate(mp, Parent, x, y, Width, Height, __PX_FONT_HEIGHT + 6,
                                       PX_Designer_ListContentItemOnCreate, PX_NULL);
    }
    return pObject;
...
px_void PX_Object_ListArrayRender(px_surface *psurface, PX_Object *pObject, px_uint elapsed)
{
    ...
    if (pList->ItemHeight * pList->ArrayDataCount > objHeight)
    {
        ...
        pList->SliderBar->Height = objHeight;
        pSliderBar->SliderButtonLength = (px_int)(pList->SliderBar->Height * objHeight /
                                                  (pList->ItemHeight * pList->ArrayDataCount));
        if (pSliderBar->SliderButtonLength < 32) { pSliderBar->SliderButtonLength = 32; }
...
px_bool PX_Designer_ListArrayItemOnCreate(px_memorypool *mp, PX_Object *ItemObject,
                                          px_void *userptr)
{
    PX_Object_List *pList = (PX_Object_List *)PX_ObjectGetDesc(PX_Object_ListItem,
                                                               ItemObject)->pList->pObjectDesc;
    ItemObject->Func_ObjectRender = pList->ArrayRender;
    return PX_TRUE;
}
PX_Object *PX_Object_ListArrayCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                     px_int Width, px_int Height, px_int ItemHeight, Function_ObjectRender render, px_void *userptr)
{
    PX_Object *pObject;

    pObject = PX_Object_ListCreate(mp, Parent, x, y, Width, Height, ItemHeight,
                                   PX_Designer_ListArrayItemOnCreate, PX_NULL);
    if (pObject)
    {
        PX_Object_List *pList = PX_ObjectGetDesc(PX_Object_List, pObject);
...
px_void PX_Object_ListArraySetData(PX_Object *pListObject, px_void *ArrayData,
...
PX_Object *PX_Designer_ListCreate(px_memorypool *mp, PX_Object *pparent, px_float x, px_float y,
