...
px_bool PX_ParticalLauncherInitialize(PX_ParticalLauncher *launcher, px_memorypool *mp,
                                      PX_ParticalLauncher_InitializeInfo Info)
{
    ...
    launcher->ParticalPool = (PX_Partical_Atom *)MP_Malloc(mp,
                                                           sizeof(PX_Partical_Atom) * launcher->LauncherInfo.maxCount);
...
px_void PX_ParticalLauncherSetPosition(PX_ParticalLauncher *launcher, px_float x, px_float y,
                                       px_float z)
{
...
px_void PX_ParticalLauncherUpdate(PX_ParticalLauncher *launcher, px_dword elapsed)
{
    ...
    if (launcher->LauncherInfo.launchCount != 0)
    {
        ...
        {
            if (launcher->elapsed / launcher->LauncherInfo.generateDuration < ((launcher->elapsed + elapsed) /
                                                                               launcher->LauncherInfo.generateDuration))
            {
                gencount = (launcher->elapsed + elapsed) / launcher->LauncherInfo.generateDuration -
                           launcher->elapsed / launcher->LauncherInfo.generateDuration;
                redTime = launcher->elapsed + elapsed - (launcher->elapsed +
                                                         launcher->LauncherInfo.generateDuration) / launcher->LauncherInfo.generateDuration *
                          launcher->LauncherInfo.generateDuration;
            }
            else
            {
            ...
            if (launcher->LauncherInfo.launchCount != 0)
            {
                for (j = 0; j < (px_int)launcher->LauncherInfo.maxCount; j++)
                {
                    if (launcher->ParticalPool[j].reg_alive == 0)
                    {
                        ...
                        launcher->ParticalPool[j].position.x += (px_float)(
                                                                    launcher->LauncherInfo.deviation_position_distanceRange * PX_cosd(randAngle));
                        launcher->ParticalPool[j].position.y += (px_float)(
                                                                    launcher->LauncherInfo.deviation_position_distanceRange * PX_sind(randAngle));
                        //velocity
                        dirAngle = (px_float)PX_RadianToAngle(PX_atan2(launcher->LauncherInfo.direction.y,
                                                                       launcher->LauncherInfo.direction.x));
                        dirAngle += (px_float)PX_randRange(-launcher->LauncherInfo.deviation_rangAngle,
                                                           launcher->LauncherInfo.deviation_rangAngle);
                        var = launcher->LauncherInfo.velocity;
                        var += (px_float)PX_randRange(launcher->LauncherInfo.deviation_velocity_min,
                                                      launcher->LauncherInfo.deviation_velocity_max);
                        ...
                        var += (px_float)PX_randRange(launcher->LauncherInfo.deviation_atomsize_min,
                                                      launcher->LauncherInfo.deviation_atomsize_max);
                        if (var < 0) { var = 0; }
                        launcher->ParticalPool[j].reg_size = var;
                        //rotation
                        var = launcher->LauncherInfo.rotation;
                        var += (px_float)PX_randRange(-launcher->LauncherInfo.deviation_rotation,
                                                      launcher->LauncherInfo.deviation_rotation);
                        launcher->ParticalPool[j].reg_rotation = var;
                        //alpha
                        var = launcher->LauncherInfo.alpha;
                        var += (px_float)PX_randRange(-launcher->LauncherInfo.deviation_alpha,
                                                      launcher->LauncherInfo.deviation_alpha);
                        if (var < 0) { var = 0; }
                        launcher->ParticalPool[j].reg_alpha = var;

                        //hdrR
                        var = launcher->LauncherInfo.hdrR;
                        var += (px_float)PX_randRange(-launcher->LauncherInfo.deviation_hdrR,
                                                      launcher->LauncherInfo.deviation_hdrR);
                        if (var < 0) { var = 0; }
                        launcher->ParticalPool[j].hdrR = var;

                        //hdrG
                        var = launcher->LauncherInfo.hdrG;
                        var += (px_float)PX_randRange(-launcher->LauncherInfo.deviation_hdrG,
                                                      launcher->LauncherInfo.deviation_hdrG);
                        if (var < 0) { var = 0; }
                        launcher->ParticalPool[j].hdrG = var;

                        //hdrB
                        var = launcher->LauncherInfo.hdrB;
                        var += (px_float)PX_randRange(-launcher->LauncherInfo.deviation_hdrB,
                                                      launcher->LauncherInfo.deviation_hdrB);
                        if (var < 0) { var = 0; }
...
px_bool PX_ParticalIsInSurfaceRegion(px_point atomPoint, px_int atomWidth, px_int atomHeight,
                                     px_float scale, px_int surfaceWidth, px_int surfaceHeight)
{
    px_rect atomRect;
    px_rect surfaceRect;
...
px_void PX_ParticalLauncherRender(px_surface *surface, PX_ParticalLauncher *launcher,
                                  px_dword elapsed)
{
    px_int m, i;
    ...
    if (!PX_isRectCrossRect(PX_RECT(0, 0, (px_float)surface->width, (px_float)surface->height), \
                            PX_RECT(launcher->lefttopX + launcher->LauncherInfo.position.x,
                                    launcher->leftTopY + launcher->LauncherInfo.position.y,
                                    launcher->rightBottomX - launcher->lefttopX + 1, launcher->rightBottomY - launcher->leftTopY + 1)))
    {
        return;
    }
    ...
    for (m = 0; m < launcher->LauncherInfo.maxCount; m++, i++)
    {
        ...
        if (launcher->ParticalPool[i].reg_alive != 0 && launcher->ParticalPool[i].reg_size != 0)
        {
            if (launcher->ParticalPool[i].reg_size != 1.0 || launcher->ParticalPool[i].reg_currentrotation != 0)
            {
                ...
                if (launcher->ParticalPool[i].reg_alpha == 1.0 && launcher->ParticalPool[i].hdrR == 1.0f &&
                    launcher->ParticalPool[i].hdrG == 1.0f && launcher->ParticalPool[i].hdrB == 1.0f)
                {
                    PX_TextureRenderEx(surface, launcher->ParticalPool[i].ptexture, (px_int)(pos.x), (px_int)(pos.y),
                                       PX_ALIGN_CENTER, PX_NULL, launcher->ParticalPool[i].reg_size,
                                       launcher->ParticalPool[i].reg_currentrotation);
                }
                else
                {
                    ...
                    PX_TextureRenderEx(surface, launcher->ParticalPool[i].ptexture, (px_int)(pos.x), (px_int)(pos.y),
                                       PX_ALIGN_CENTER, &blend, launcher->ParticalPool[i].reg_size,
                                       launcher->ParticalPool[i].reg_currentrotation);
                }
            }
            else
            {
                if (launcher->ParticalPool[i].reg_alpha == 1.0 && launcher->ParticalPool[i].hdrR == 1.0f &&
                    launcher->ParticalPool[i].hdrG == 1.0f && launcher->ParticalPool[i].hdrB == 1.0f)
                {
                    PX_TextureRender(surface, launcher->ParticalPool[i].ptexture, (px_int)(pos.x), (px_int)(pos.y),
                                     PX_ALIGN_CENTER, PX_NULL);
                }
                else
                {
                    blend.alpha = launcher->ParticalPool[i].reg_alpha;
