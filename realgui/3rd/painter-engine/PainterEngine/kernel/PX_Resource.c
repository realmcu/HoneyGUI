...
px_bool PX_ResourceLibraryLoad(PX_ResourceLibrary *lib, PX_RESOURCE_TYPE type, px_byte *data,
                               px_uint datasize, const px_char key[])
{
    PX_Resource res;
    res.Type = type;
    ...
    case PX_RESOURCE_TYPE_SOUND:
        if (PX_WaveVerify(data, datasize))
        {
            ...
            if (pcmSize != 0)
            {
                ...
                res.sound.channel = PX_WaveGetChannel(data,
    ...
    PX_MapPut(&lib->map, (const px_byte *)key, PX_strlen(key), PX_ListPush(&lib->resources, &res,
                                                                           sizeof(res)));
    return PX_TRUE;
}
...
px_bool PX_ResourceLibraryAdd(PX_ResourceLibrary *lib, PX_Resource res, const px_char key[])
{
    ...
    {
        if (PX_MapPut(&lib->map, (const px_byte *)key, PX_strlen(key), PX_ListPush(&lib->resources, &res,
                                                                                   sizeof(res))) == PX_HASHMAP_RETURN_OK)
        {
            return PX_TRUE;
        }
    }
...
px_bool PX_ResourceLibraryAddTexture(PX_ResourceLibrary *lib, const px_char key[],
                                     px_texture *pTexture)
{
    PX_Resource res;
...
px_texture *PX_ResourceLibraryCreateTexture(PX_ResourceLibrary *lib, const px_char key[],
...
PX_AnimationLibrary *PX_ResourceLibraryGetAnimationLibrary(PX_ResourceLibrary *lib,
                                                           const px_char key[])
{
    PX_Resource *pres = PX_ResourceLibraryGet(lib, key);
