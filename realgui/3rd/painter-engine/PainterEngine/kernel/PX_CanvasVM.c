...
px_void PX_CanvasVM_PenPaint(px_surface *psurface, px_float x, px_float y, px_float size,
                             px_color color, px_void *ptr)
{
    PX_GeoDrawPenCircle(psurface, x, y, size, color);
}

px_void PX_CanvasVM_PaintPaint(px_surface *psurface, px_float x, px_float y, px_float size,
                               px_color color, px_void *ptr)
{
...
px_void PX_CanvasVM_SparyPaint(px_surface *psurface, px_float x, px_float y, px_float size,
                               px_color color, px_void *ptr)
{
    PX_GeoDrawSpray(psurface, x, y, size, color);
}

px_void PX_CanvasVM_EraserPaint(px_surface *psurface, px_float x, px_float y, px_float size,
                                px_color color, px_void *ptr)
{
    PX_GeoDrawPenCircleEraser(psurface, x, y, size, PX_TRUE);
}

px_void PX_CanvasVM_HandPaint(px_surface *psurface, px_float x, px_float y, px_float size,
                              px_color color, px_void *ptr)
{}
...
px_void PX_CanvasVM_PaintDot(PX_CanvasVM *pCanvasVM, px_float x, px_float y, px_float z)
{
    if (pCanvasVM->reg_tool >= 0 && pCanvasVM->reg_tool < pCanvasVM->reg_tools_count)
    {
        ...
        {
            pTool->paint(pCanvasVM->pcache_editing_surface, x, y, pCanvasVM->reg_size * z, pCanvasVM->reg_color,
                         pTool->ptr);
        }
    }
}

px_bool PX_CanvasVMInitialize(px_memorypool *mp, PX_CanvasVM *pCanvasVM, px_int width,
                              px_int height, px_int zoom_width, px_int zoom_height)
{
    ...
    {
        pCanvasVM->min_scale = pCanvasVM->view_surface.width * 1.0f / (pCanvasVM->cache_surface.width *
                                                                       1.25f);

        if (pCanvasVM->min_scale > pCanvasVM->view_surface.height * 1.0f / (pCanvasVM->cache_surface.height
                                                                            * 1.25f))
        {
            pCanvasVM->min_scale = pCanvasVM->view_surface.height * 1.0f / (pCanvasVM->cache_surface.height *
                                                                            1.25f);
        }
    }
    pCanvasVM->scale = 1;
...
px_void PX_CanvasVMOnDrawLastInterpolate(PX_CanvasVM *pCanvasVM)
{
    ...
    {
        px_point3D v1 = PX_POINT(pCanvasVM->currentPath[pCanvasVM->currentPathCount - 1].x,
                                 pCanvasVM->currentPath[pCanvasVM->currentPathCount - 1].y,
                                 pCanvasVM->currentPath[pCanvasVM->currentPathCount - 1].z);
        px_point3D v2 = PX_POINT(pCanvasVM->currentPath[pCanvasVM->currentPathCount - 2].x,
                                 pCanvasVM->currentPath[pCanvasVM->currentPathCount - 2].y,
                                 pCanvasVM->currentPath[pCanvasVM->currentPathCount - 2].z);
        ...
        for (i = 1; i <= dots; i++)
        {
            PX_CanvasVM_PaintDot(pCanvasVM, v2.x + dir.x * i * step, v2.y + dir.y * i * step,
                                 v2.z + dir.z * i * step);
        }

        if (pCanvasVM->reg_showdot && pCanvasVM->pcache_editing_surface)
        {
            PX_GeoDrawSolidCircle(pCanvasVM->pcache_editing_surface,
                                  (px_int)pCanvasVM->currentPath[pCanvasVM->currentPathCount - 1].x,
                                  (px_int)pCanvasVM->currentPath[pCanvasVM->currentPathCount - 1].y, 2, PX_COLOR_RED);
        }
    }
    else if (pCanvasVM->currentPathCount == 1)
    {
        PX_CanvasVM_PaintDot(pCanvasVM, pCanvasVM->currentPath[0].x, pCanvasVM->currentPath[0].y,
                             pCanvasVM->currentPath[0].z);
        if (pCanvasVM->reg_showdot && pCanvasVM->pcache_editing_surface)
        {
            PX_GeoDrawSolidCircle(pCanvasVM->pcache_editing_surface, (px_int)pCanvasVM->currentPath[0].x,
                                  (px_int)pCanvasVM->currentPath[0].y, 2, PX_COLOR_RED);
        }
    }
...
px_void PX_CanvasVMOnMove(PX_CanvasVM *pCanvasVM, px_float x, px_float y, px_float z)
{
    ...
    else
    {
        ...
        do
        {
            px_float m1, m2;
            px_point2D v1, v2;
            v1 = PX_POINT2D(lastPoint.x - pCanvasVM->reg_lastCursorPos.x,
                            lastPoint.y - pCanvasVM->reg_lastCursorPos.y);
            ...
            if (m1 * m2 > 1)
            {
                ...
                if (_cosv > 0.5f)
                {
                    if (PX_CanvasVM_AddNode(pCanvasVM, pCanvasVM->reg_lastCursorPos.x, pCanvasVM->reg_lastCursorPos.y,
                                            pCanvasVM->reg_lastCursorPos.z))
                    {
                        PX_CanvasVMOnDrawLastInterpolate(pCanvasVM);
                    }
...
px_void PX_CanvasVMStorePaintShell(PX_CanvasVM *pCanvasVM)
{
    ...
    PX_MemoryLeft(&pCanvasVM->shell, pCanvasVM->reg_ip);
    if (!PX_MemoryCat(&pCanvasVM->shell, &header, sizeof(header))) { return; }
    if (!PX_MemoryCat(&pCanvasVM->shell, pCanvasVM->currentPath,
                      sizeof(PX_CanvasNode) * pCanvasVM->currentPathCount)) { return; }

    pCanvasVM->reg_ip = pCanvasVM->shell.usedsize;

}

px_void PX_CanvasVMRegisterTool(PX_CanvasVM *pCanvasVM,
                                const px_char name[PX_CANVASVM_MAX_TOOL_NAME], pfun_PX_CanvasVMOnPaint paint, px_bool store,
                                px_void *ptr)
{
    px_int i;
    for (i = 0; i < PX_COUNTOF(pCanvasVM->tools); i++)
...
{
    if (layerindex < 0 || layerindex >= PX_COUNTOF(pCanvasVM->layers) ||
        !pCanvasVM->layers[layerindex].activating)
    {
        PX_ASSERT();
        return;
    }
    PX_TextureScaleToTexture(&pCanvasVM->layers[layerindex].surface_layer,
                             &pCanvasVM->layers[layerindex].surface_preview_mini);
}
...
static px_void PX_CanvasVMRenderLayerToSurface(PX_CanvasVM *pCanvasVM, px_int index,
                                               px_texture *prendertarget)
{
    ...
    if (pCanvasVM->layers[index].attribute == PX_OBJECT_LAYER_ATTRIBUTE_CLIP)
    {
        ...
        for (j = index + 1; j < PX_COUNTOF(pCanvasVM->layers); j++)
        {
            ...
            if (pCanvasVM->layers[j].attribute == PX_OBJECT_LAYER_ATTRIBUTE_NORMAL)
            {
                ...
                {
                    PX_TextureRenderMask(prendertarget, &pCanvasVM->layers[j].surface_layer,
                                         &pCanvasVM->layers[index].surface_layer, 0, 0, PX_ALIGN_LEFTTOP, 0);
                    return;
                }
            }
        }
    ...
    {
        PX_TextureRender(prendertarget, &pCanvasVM->layers[index].surface_layer, 0, 0, PX_ALIGN_LEFTTOP,
                         0);
    }
...
px_void PX_CanvasVMRepaintCache(PX_CanvasVM *pdesc)
{
    ...
    if (pdesc->pcache_editing_surface)
    {
        ...
        {
            PX_TextureRenderMask(&pdesc->cache_surface, pdesc->pcache_mask_surface,
                                 pdesc->pcache_editing_surface, 0, 0, PX_ALIGN_LEFTTOP, 0);
        }
        else
        {
...
px_bool PX_CanvasVMExecuteShell(PX_CanvasVM *pCanvasVM)
{
    ...
    if ((px_uint)pCanvasVM->reg_ip + pHeader->size + sizeof(PX_CanvasVMShell_Header) >
        (px_uint)pCanvasVM->shell.usedsize)
    {
        return PX_FALSE;
    }
    ...
    case PX_CANVASVM_OPCODE_LAYERCREATE:
        {
            ...
            if (pCanvasVM->shell.buffer[pCanvasVM->reg_ip + sizeof(PX_CanvasVMShell_Header) + pHeader->size - 1]
                != '\0')
            {
                PX_ASSERT();
                return PX_FALSE;
            ...
            if (!PX_TextureCreate(pCanvasVM->mp, &pCanvasVM->layers[i].surface_layer, pCanvasVM->width,
                                  pCanvasVM->height))
            {
                //out of memory
                PX_ASSERTIF(1);
            ...
            PX_strcpy(pCanvasVM->layers[i].name,
                      (const px_char *)pCanvasVM->shell.buffer + pCanvasVM->reg_ip + sizeof(PX_CanvasVMShell_Header),
                      sizeof(pCanvasVM->layers[i].name));
    ...
    case PX_CANVASVM_OPCODE_PAINT:
        {
            ...
            PX_CanvasNode *pNode;
            PX_ASSERTIF((px_uint)pCanvasVM->shell.usedsize - (px_uint)pCanvasVM->reg_ip < sizeof(
                            PX_CanvasVMShell_Header) + pHeader->size);
            ...
            pNode = (PX_CanvasNode *)(pCanvasVM->shell.buffer + pCanvasVM->reg_ip + sizeof(
...
static px_void PX_CanvasVMTakeSnapshot(PX_CanvasVM *pCanvasVM)
{
    ...
    for (i = 0; i < PX_COUNTOF(pCanvasVM->snapshot_layers); i++)
    {
        ...
        if (pCanvasVM->snapshot_layers[i].activating)
        {
            PX_TextureCopy(pCanvasVM->mp, &pCanvasVM->snapshot_layers[i].surface_layer,
                           &pCanvasVM->layers[i].surface_layer);
            PX_TextureCopy(pCanvasVM->mp, &pCanvasVM->snapshot_layers[i].surface_preview_mini,
                           &pCanvasVM->layers[i].surface_preview_mini);
        }
    }
...
px_void PX_CanvasVMMoveBack(PX_CanvasVM *pCanvasVM)
{
    ...
    {
        pHeader = (PX_CanvasVMShell_Header *)(pCanvasVM->shell.buffer + pCanvasVM->reg_ip);
        if ((px_uint)pCanvasVM->reg_ip + sizeof(PX_CanvasVMShell_Header) + (px_uint)pHeader->size >=
            (px_uint)lastip)
        {
            break;
        }
