...
static px_void PX_MenuSubMenuUpdateEx(PX_Object_Menu *pMenu, PX_Object_Menu_Item *pItem,
                                      px_int startX, px_int startY)
{
    px_int index = 0;
...
static px_bool PX_Object_MenuInitialize(px_memorypool *mp, PX_Object_Menu *pMenu, px_int x,
                                        px_int y, px_int width, PX_FontModule *fontmodule)
{
...
px_bool PX_Menu_CursorAction(PX_Object *pObject, PX_Object_Menu_Item *pItem, px_float x, px_float y,
                             PX_MENU_CURSORACTION action)
{
    px_list_node *pNode = PX_NULL;
    PX_Object_Menu *pMenu = PX_Object_GetMenu(pObject);

    if (PX_isXYInRegion(x, y, (px_float)pItem->x, (px_float)pItem->y, (px_float)pItem->width,
                        (px_float)pItem->height))
    {
...
px_void PX_MenuPostEvent_OnCursorDown(PX_Object *pObject, PX_Object_Event e)
{
    ...
    if (!pMenu->activating)
    {
        ...
        {
            PX_Object_Menu_Item *pSubItem = PX_LIST_NODETDATA(PX_Object_Menu_Item, pNode);
            if (PX_isXYInRegion(x, y, (px_float)pSubItem->x, (px_float)pSubItem->y, (px_float)pSubItem->width,
                                (px_float)pSubItem->height) && pSubItem->enable)
            {
...
static px_void PX_MenuRenderItem(px_surface *pSurface, PX_Object_Menu *pMenu,
                                 PX_Object_Menu_Item *pItem, px_bool broot, px_dword elapsed)
{
    if (pItem->width && pItem->height)
    {
        if (pItem->onCursor || pItem->Activated)
        {
            PX_GeoDrawRect(pSurface, pItem->x, pItem->y, pItem->x + pItem->width - 1,
                           pItem->y + pItem->height - 1, pMenu->cursorColor);
        }
        else
        {
            PX_GeoDrawRect(pSurface, pItem->x, pItem->y, pItem->x + pItem->width - 1,
                           pItem->y + pItem->height - 1, pMenu->backgroundColor);
        }

        if (broot)
        {
            PX_FontModuleDrawText(pSurface, pMenu->fontmodule, pItem->x + pItem->width / 2,
                                  pItem->y + pItem->height / 2, PX_ALIGN_CENTER, pItem->Text, pMenu->fontColor);
        }
        else
        {
            if (!pItem->enable)
            {
                PX_FontModuleDrawText(pSurface, pMenu->fontmodule, pItem->x + 18, pItem->y + pItem->height / 2,
                                      PX_ALIGN_LEFTMID, pItem->Text, pMenu->disableColor);
            }
            else
            {
                PX_FontModuleDrawText(pSurface, pMenu->fontmodule, pItem->x + 18, pItem->y + pItem->height / 2,
                                      PX_ALIGN_LEFTMID, pItem->Text, pMenu->fontColor);
            }
...
PX_Object_Menu_Item *PX_Object_MenuAddItem(PX_Object *pObject, PX_Object_Menu_Item *parent,
                                           const px_char Text[], PX_MenuExecuteFunc _callback, px_void *ptr)
{
...
PX_Object *PX_Object_MenuCreate(px_memorypool *mp, PX_Object *Parent, px_int x, px_int y,
                                px_int width, PX_FontModule *fontmodule)
{
    PX_Object *pObject;
