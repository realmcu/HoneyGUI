...
px_void PX_SoundModuleInitialize(px_memorypool *mp, PX_SoundModule *SoundModule, px_int SampleRate,
                                 px_double pitchShift, PX_SoundModule_Formant Formant[], px_int FormantCount)
{
...
px_int PX_SoundModuleFilter(PX_SoundModule *SoundModule, _IN px_double _in[], px_int count,
                            _OUT px_double _out[])
{
    ...
    for (blk = 0; blk < blkCount - 1; blk++)
    {
        ...
        if (Pitch != 0 && !(ZCR < SoundModule->ZCR_Low || ZCR > SoundModule->ZCR_High ||
                            Pitch < SoundModule->Pitch_threshold_Low || Pitch > SoundModule->Pitch_threshold_High))
        {
            ...
            for (i = 0; i < PX_SOUNDMODULE_FORMANT_COUNT; i++)
            {
                ...
                while (PX_TRUE)
                {
                    ...
                    Frame[HZindex].re = SoundModule->Formant[i].A * PX_sind(PX_PI / 2 * (1 - PX_ABS(
                                                                                             distanceHZ - Pitch * i) / SoundModule->Formant[i].W * 2));
