                       ...
                       px_int lineWidth, px_color color)
{
    ...
    px_float p0x, p0y, p1x, p1y, p2x, p2y, p3x, p3y, vx, vy, rx, ry, xlen, ylen, S, ftemp, ryL, ryR,
             rxL, rxR;
    px_float k, recK;
    px_color clr;
...
px_void PX_GeoDrawBorder(px_surface *psurface, px_int left, px_int top, px_int right, px_int bottom,
                         px_int lineWidth, px_color color)
{
    if (lineWidth > 0)
    {
    ...
    PX_GeoDrawRect(psurface, left, top + lineWidth + 1, left + lineWidth, bottom - lineWidth - 1,
                   color);
    PX_GeoDrawRect(psurface, right - lineWidth, top + lineWidth + 1, right, bottom - lineWidth - 1,
                   color);
}



px_void PX_GeoDrawRect(px_surface *psurface, px_int left, px_int top, px_int right, px_int bottom,
                       px_color color)
{
    px_int i, j, mid;
    if (color._argb.a == 0)
...
static px_void PX_GeoDrawSolidCircle_Ex1(px_surface *psurface, px_int x, px_int y, px_int Radius,
                                         px_color color)
{
    ...
    if (x >= 0 - Radius && y >= 0 - Radius && x < psurface->width + Radius &&
        y < psurface->height + Radius)
    {
        clr = color;
        clr._argb.a >>= 1;
        dy = Radius;
...
static px_void PX_GeoDrawSolidCircle_Ex2(px_surface *psurface, px_float x, px_float y,
                                         px_float Radius, px_color color)
{
...
px_void PX_GeoDrawSolidCircleFast(px_surface *psurface, px_int x, px_int y, px_int Radius,
                                  px_color color)
{
    px_int _x, _y;
    px_int left, top, right, bottom;
...
px_void PX_GeoDrawPenCircleDecay(px_surface *psurface, px_float x, px_float y, px_float Radius,
                                 px_color color, px_float Decay)
{
    px_int left, top, right, bottom, i, j;
    px_float d;
    px_color clr;
...
px_void PX_GeoDrawPenCircle(px_surface *psurface, px_float x, px_float y, px_float Radius,
                            px_color color)
{
    PX_GeoDrawPenCircleDecay(psurface, x, y, Radius, color, 5);
}

px_void PX_GeoDrawPenRing(px_surface *psurface, px_float x, px_float y, px_float Radius,
                          px_float linewidth_inside, px_color color)
{
    px_int left, top, right, bottom, i, j;
    px_float d;
    px_color clr;
...
px_void PX_GeoDrawPenCircleEraser(px_surface *psurface, px_float x, px_float y, px_float Radius,
                                  px_bool white_background)
{
    px_int left, top, right, bottom, i, j;
    px_float d;
...
px_void PX_GeoDrawSpray(px_surface *psurface, px_float x, px_float y, px_float Radius,
                        px_color color)
{
    px_int left, top, right, bottom, i, j;
    px_float d;
...
px_void PX_GeoDrawBall(px_surface *psurface, px_float x, px_float y, px_float Radius,
                       px_color color)
{
    px_int left, top, right, bottom, i, j;
    px_float d;
...
px_void PX_GeoDrawPath(px_surface *psurface, px_point path[], px_int pathCount, px_float linewidth,
                       px_color color)
{
    px_int i;
    if (pathCount == 1)
...
px_void PX_GeoDrawPath2D(px_surface *psurface, px_point2D path[], px_int pathCount,
                         px_float linewidth, px_color color)
{
    px_int i;
    if (pathCount == 1)
    {
...
px_void PX_GeoDrawPenLine(px_surface *psurface, px_float x0, px_float y0, px_float x1, px_float y1,
                          px_float lineWidth, px_color color)
{
    px_point p[2] = {0};
    p[0].x = x0;
...
px_void PX_GeoDrawDashLine(px_surface *psurface, px_int x0, px_int y0, px_int x1, px_int y1,
                           px_int lineWidth, px_float dash_width, px_color color)
{
    px_point2D v = PX_Point2DSub(PX_POINT2D((px_float)x1, (px_float)y1), PX_POINT2D((px_float)x0,
                                                                                    (px_float)y0));
...
px_void PX_GeoDrawPenDots(px_surface *psurface, px_point _samples[], px_int samplescount,
                          px_float radius, px_color color)
{
    px_int i;
...
px_void PX_GeoDrawCircle(px_surface *psurface, px_int x, px_int y, px_int Radius, px_int lineWidth,
                         px_color color)
{
        ...
        y < psurface->height + Radius)
    {
        ...
        if (i < (x - psurface->width) * (x - psurface->width) + (y - psurface->height) *
            (y - psurface->height))
        {
        ...
        if (i < (Radius - lineWidth / 2) * (Radius - lineWidth / 2) &&
            !PX_isPointInRect(PX_POINT((px_float)x, (px_float)y, 0), PX_RECT(0, 0, (px_float)psurface->width,
                                                                             (px_float)psurface->height)))
        {
            return;
        }
    }
...
static px_void PX_GeoDrawRingPoint(px_surface *psurface, px_int x, px_int y, px_color color,
                                   px_int cx, px_int cy, px_float s_r, px_float e_r)
{
...
px_void PX_GeoDrawRing(px_surface *psurface, px_int x, px_int y, px_float Radius,
                       px_float lineWidth, px_color color, px_int start_angle, px_int end_angle)
{
    if (lineWidth <= 0)
    {
        return;
    }
    PX_GeoDrawSector(psurface, x, y, Radius + lineWidth / 2.0f, Radius - lineWidth / 2.0f, color,
                     start_angle, end_angle);
}

px_void PX_GeoDrawSector(px_surface *psurface, px_int x, px_int y, px_float Radius_outside,
                         px_float Radius_inside, px_color color, px_int start_angle, px_int end_angle)
{
    ...
    if (x >= 0 - Radius_outside && y >= 0 - Radius_outside && x < psurface->width + Radius_outside &&
        y < psurface->height + Radius_outside)
    {
        i = x * x + y * y;
        ...
        if (i < Radius_outside * Radius_outside &&
            !PX_isPointInRect(PX_POINT((px_float)x, (px_float)y, 0), PX_RECT(0, 0, (px_float)psurface->width,
                                                                             (px_float)psurface->height)))
        {
            return;
...
px_void PX_GeoDrawSolidRoundRect(px_surface *psurface, px_int left, px_int top, px_int right,
                                 px_int bottom, px_float roundRaduis, px_color color)
{
    ...
    if (Width > 2 * (px_int)roundRaduis)
    {
        ...
        {
            PX_GeoDrawRect(psurface, left + (px_int)roundRaduis, top,
                           left + (px_int)roundRaduis + drawWidth - 1, top + drawHeight - 1, color);
            PX_GeoDrawRect(psurface, left + (px_int)roundRaduis, bottom - drawHeight + 1,
                           left + (px_int)roundRaduis + drawWidth - 1, bottom, color);
        }
    }

    if (Height > 2 * (px_int)roundRaduis)
    {
        PX_GeoDrawRect(psurface, left, top + (px_int)roundRaduis, right, bottom - (px_int)roundRaduis,
                       color);
    }

}

px_void PX_GeoDrawRoundRect(px_surface *psurface, px_int left, px_int top, px_int right,
                            px_int bottom, px_float roundRaduis, px_float linewidth, px_color color)
{
    px_int y, x, Height, Width;
    px_float r_x, r_y, dis;
    px_int drawWidth;
    ...
    if (Height > 2 * (px_int)roundRaduis)
    {
        PX_GeoDrawRect(psurface, left, top + (px_int)roundRaduis, left + drawlinewidth_int - 1,
                       bottom - (px_int)roundRaduis, color);
        PX_GeoDrawRect(psurface, right - drawlinewidth_int + 1, top + (px_int)roundRaduis, right,
                       bottom - (px_int)roundRaduis, color);
    }
}

px_void PX_GeoDrawBezierCurvePoint(px_surface *rendersurface, px_point pt[], px_int pt_count,
                                   px_float t, px_float radius, px_color clr)
{
    px_int i;
...
px_void PX_GeoDrawBresenhamLine(px_surface *psurface, px_int x0, px_int y0, px_int x1, px_int y1,
                                px_color color)
{
...
px_void PX_GeoDrawTriangle(px_surface *psurface, px_point2D p0, px_point2D p1, px_point2D p2,
                           px_color color)
{
    px_int ix, iy;
...
px_void PX_GeoDrawArrow(px_surface *psurface, px_point2D p0, px_point2D p1, px_float size,
                        px_color color)
{
    ...
    PX_GeoDrawLine(psurface, (px_int)p0.x, (px_int)p0.y, (px_int)p0_5.x, (px_int)p0_5.y, (px_int)size,
                   color);
...
static px_point2D PX_GeoDeBoorCoxRecursion(px_int j,  px_int i, px_int k, px_float t,
                                           px_point2D controlPoints[], px_float knots[])
{
    px_point2D p1, p2;
...
px_void PX_GeoDrawPenSamplesLine(px_surface *psurface, px_point _samples[], px_int samplescount,
                                 px_float radius, px_color color, px_float v, px_float filter_factor)
{
    px_int i, dindex = 1;
    ...
    while (PX_TRUE)
    {
        ...
        while (step_distance > 0)
        {
            ...
            dir = PX_POINT(_samples[dindex].x - guider.x, _samples[dindex].y - guider.y,
                           _samples[dindex].z - guider.z);
