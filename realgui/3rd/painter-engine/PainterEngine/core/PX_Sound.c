...
px_void PX_SoundPlaySetUserRead(PX_SoundPlay *pSoundPlay, px_void(*userread)(px_void *userptr,
                                                                             px_byte *pBuffer, px_int readSize), px_void *ptr)
{
...
px_bool PX_SoundPlayRead(PX_SoundPlay *pSoundPlay, px_byte *pBuffer, px_int readSize)
{
    ...
    for (j = 0; j < PX_COUNTOF(pSoundPlay->Sounds); j++)
    {
        ...
        if (pSound->datasize != -1)
        {
            ...
            resBlockCount = pSound->data->channel == PX_SOUND_CHANNEL_ONE ? pSound->datasize / 2 :
                            pSound->datasize / 4;
...
px_bool PX_SoundPlayReadCurrentPlayingData(PX_SoundPlay *pSoundPlay, px_int soundIndex,
                                           px_int channel, px_int16 *out, px_int count)
{
    PX_SoundData *pData;
    px_int offset;
...
px_int PX_SoundDataReadSamples(PX_SoundData *sounddata, px_int byte_offset, px_int readcount,
                               px_float unit_out[], px_int channel)
{
...
px_bool PX_SoundDataCopy(px_memorypool *mp, PX_SoundData *resSounddata,
                         PX_SoundData *targetSounddata)
{
...
px_bool PX_SoundDataCreateFromWavFileData(PX_SoundData *sounddata, px_memorypool *mp, px_byte *data,
                                          px_int datasize)
{
    if (PX_WaveVerify(data, datasize))
    {
        ...
        if (pcmSize != 0)
        {
            ...
            sounddata->channel = PX_WaveGetChannel(data,
