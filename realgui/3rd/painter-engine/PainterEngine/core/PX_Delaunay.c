...
PX_DELAUNAY_ROUND_TEST PX_DelaunaryTest(px_point2D pt_array[], px_int testIndex,
                                        PX_Delaunay_Triangle triangle)
{
    px_float x, y, r;
    ...
    x = ((y2 - y1) * (y3 * y3 - y1 * y1 + x3 * x3 - x1 * x1) - (y3 - y1) *
         (y2 * y2 - y1 * y1 + x2 * x2 - x1 * x1)) / (2.0f * ((x3 - x1) * (y2 - y1) - (x2 - x1) * (y3 - y1)));
    y = ((x2 - x1) * (x3 * x3 - x1 * x1 + y3 * y3 - y1 * y1) - (x3 - x1) *
         (x2 * x2 - x1 * x1 + y2 * y2 - y1 * y1)) / (2.0f * ((y3 - y1) * (x2 - x1) - (y2 - y1) * (x3 - x1)));
                                ...
                                px_vector *out_triangles, PX_DELAUNAY_RETURN_TYPE type)
{
        ...
        while (index < sortTriangles.size)
        {
            ...
            for (i = index + 1; i < sortTriangles.size; i++)
            {
                ...
                if (PX_DelaunaryTriangleNeighbor(pTriangle1, pTriangle2))
                {
                    ...
                    if (pTriangle1->index1 != pTriangle2->index1 && pTriangle1->index1 != pTriangle2->index2 &&
                        pTriangle1->index1 != pTriangle2->index3)
                    {
                        pA = pTriangle1->index1;
                    }
                    else if (pTriangle1->index2 != pTriangle2->index1 && pTriangle1->index2 != pTriangle2->index2 &&
                             pTriangle1->index2 != pTriangle2->index3)
                    {
                    ...
                    if (pTriangle2->index1 != pTriangle1->index1 && pTriangle2->index1 != pTriangle1->index2 &&
                        pTriangle2->index1 != pTriangle1->index3)
                    {
                        pB = pTriangle2->index1;
                    }
                    else if (pTriangle2->index2 != pTriangle1->index1 && pTriangle2->index2 != pTriangle1->index2 &&
                             pTriangle2->index2 != pTriangle1->index3)
                    {
    ...
    {
        PX_Delaunay_Triangle *pTriangle = PX_VECTORAT(PX_Delaunay_Triangle, &DelaunaryTriangle, i);
        if (pTriangle->index1 - count >= 0 || pTriangle->index2 - count >= 0 ||
            pTriangle->index3 - count >= 0)
        {
