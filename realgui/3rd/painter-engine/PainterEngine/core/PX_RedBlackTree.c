...
PX_RBTREE_MATCH PX_RBNodeMatchCode(PX_RBNode *test, px_dword keyvalue, const px_byte code[],
                                   px_int codesize)
{
        ...
        else
        {
            ...
            while (1)
            {
                ...
                else
                {
                    if (pBrother == pParent->right)
                    {
                        if (pBrother->left && pBrother->left->color == PX_RBNODE_COLOR_RED && (pBrother->right == PX_NULL ||
                    ...
                    else
                    {
                        ...
                        else if ((pBrother->left == PX_NULL || pBrother->left->color == PX_RBNODE_COLOR_BLACK) &&
                                 pBrother->right && pBrother->right->color == PX_RBNODE_COLOR_RED)
                        {
                            pBrother->color = PX_RBNODE_COLOR_RED;
                            pBrother->right->color = PX_RBNODE_COLOR_BLACK;
