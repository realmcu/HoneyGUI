...
px_bool PX_PacketCompress(px_byte *_in, px_uint input_size, px_byte *_out, px_uint *out_size,
                          px_dword algorithm)
{
...
px_bool PX_PacketDecompress(px_byte *_in, px_uint input_size, px_byte *_out, px_uint *out_size)
{
    ...
    if (!_out)
    {
        ...
        case PX_PACKET_ALOGRITHM_ARLE:
            {
                PX_ArleDecompress(_in + sizeof(PX_Packet_Header), input_size - sizeof(PX_Packet_Header), _out,
                                  out_size);
            }
            break;
        }
