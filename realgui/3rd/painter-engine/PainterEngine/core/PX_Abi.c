...
px_bool PX_AbiWriteMemory(px_memory *pmemory, PX_ABI_TYPE type, const px_char name[],
                          px_void *buffer, px_int buffersize)
{
    px_dword size;
...
px_bool PX_AbiWrite(px_abi *pabi, PX_ABI_TYPE type, const px_char name[], px_void *buffer,
                    px_int buffersize)
{
    if (pabi->dynamic.mp)
    {
    ...
    else
    {
        px_dword size;
        size = PX_strlen(name) + 1 + buffersize;
        if (pabi->static_write_offset + sizeof(type) + sizeof(size) + PX_strlen(
                name) + 1 + buffersize > pabi->static_size)
        {
            return PX_FALSE;
        }
...
{
    return PX_AbiWriteMemory(pmem, PX_ABI_TYPE_STRING, name, (px_void *)_string,
                             PX_strlen(_string) + 1);
}
