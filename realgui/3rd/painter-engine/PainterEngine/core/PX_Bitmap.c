...
px_void PX_BitmapRender(px_surface *psurface, void *BitmapBuffer, px_int BufferSize, px_int x,
                        px_int y)
{
    px_int i, j;
    px_int Height;
...
px_bool PX_BitmapBuild(px_surface *psurface, px_char *BitmapBuffer, px_int *size)
{
    ...
    _outsize = psurface->height * psurface->width * 3 + sizeof(PX_BITMAPFILEHEADER) + sizeof(
                   PX_BITMAPINFOHEADER) + rowAppendSize * psurface->height;
    ...
    *size = _outsize;
    if (BitmapBuffer)
    {
        ...
        pdata = (PX_BITMAP_RGB24 *)(BitmapBuffer + sizeof(PX_BITMAPFILEHEADER) + sizeof(
                                        PX_BITMAPINFOHEADER));
        for (y = psurface->height - 1; y >= 0; y--)
        {
            for (x = 0; x < psurface->width; x++)
            {
                (*pdata).R = (px_char)(PX_SURFACECOLOR(psurface, x, y)._argb.r * PX_SURFACECOLOR(psurface, x,
                                       y)._argb.a / 256);
                (*pdata).G = (px_char)(PX_SURFACECOLOR(psurface, x, y)._argb.g * PX_SURFACECOLOR(psurface, x,
                                       y)._argb.a / 256);
                (*pdata).B = (px_char)(PX_SURFACECOLOR(psurface, x, y)._argb.b * PX_SURFACECOLOR(psurface, x,
                                       y)._argb.a / 256);
                pdata++;
            }
