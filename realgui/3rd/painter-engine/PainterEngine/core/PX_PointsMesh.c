...
px_bool PX_PointsMeshBuild(px_memorypool *mp, px_point2D limit_line_pt[], px_int line_pt_count,
                           px_point2D pt[], px_int pt_count, px_vector *out_triangles, PX_DELAUNAY_RETURN_TYPE type)
{
    px_int i;
    ...
    {
        PX_GeoDrawBresenhamLine(&testTexture, (px_int)(limit_line_pt[i - 1].x + offsetx),
                                (px_int)(limit_line_pt[i - 1].y + offsety), (px_int)(limit_line_pt[i].x + offsetx),
                                (px_int)(limit_line_pt[i].y + offsety), PX_COLOR(255, 0, 0, 0));
    }
        ...
        case PX_DELAUNAY_RETURN_TYPE_TRIANGLE:
            {
                ...
                testx = (px_int)((ptriangle->Vertex_1.x + ptriangle->Vertex_2.x + ptriangle->Vertex_3.x) / 3 +
                                 offsetx);
                testy = (px_int)((ptriangle->Vertex_1.y + ptriangle->Vertex_2.y + ptriangle->Vertex_3.y) / 3 +
                                 offsety);
            }
            break;
