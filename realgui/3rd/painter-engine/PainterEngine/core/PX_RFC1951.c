...
static px_bool PX_RFC1951_InflateUncompressData(const px_byte *in, px_uint insize,
                                                px_uint32 *pbit_position, px_memory *out)
{
    px_uint p;
    px_int len, nlen;
...
px_bool PX_RFC1951InflateBlock(px_byte *_in, px_uint input_size, px_uint32 *pbit_position,
                               px_uint32 type, px_memory *_out)
{
    switch (type)
    {
    //uncompress data
...
px_bool PX_RFC1951DeflateBlock(px_byte *_in, px_uint input_size, px_bool isLastBlock,
                               px_memory *_out)
{
    ...
    if (!PX_HuffmanDeflateCodeData((px_word *)mem.buffer, mem.usedsize / 2, &test_mem,
                                   PX_HUFFMAN_TREE_TYPE_DYNAMIC))
    {
    ...
    else if (fixed_size <= raw_size && fixed_size <= dynamic_size)
    {
        ...
        if (!PX_HuffmanDeflateCodeData((px_word *)mem.buffer, mem.usedsize / 2, _out,
                                       PX_HUFFMAN_TREE_TYPE_FIXED))
        {
            PX_MemoryFree(&mem);
            return PX_FALSE;
