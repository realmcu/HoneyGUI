...
px_bool PX_ANNInitialize(px_memorypool *mp, PX_ANN *pANN, px_double learningRate,
                         PX_ANN_REGULARZATION regularzation, px_double regularization_rate)
{
    pANN->mp = mp;
    pANN->LayerCount = 0;
    pANN->Layer = PX_NULL;
...
px_bool PX_ANNAddLayer(PX_ANN *pAnn, px_int Neurals, px_double bias,
                       PX_ANN_ACTIVATION_FUNCTION activation, PX_ANN_LAYER_WEIGHT_INITMODE mode, px_double weight_c)
{
    px_int i;
        ...
        for (i = 0; i < p->Neurals; i++)
        {
            for (j = 0; j < p->pPrevious->Neurals; j++)
            {
                ...
                case PX_ANN_REGULARZATION_L2:
                    delta = delta - delta * pAnn->regularization_rate / pAnn->Layer->Neurals * p->w[i *
                                                                                                      p->pPrevious->Neurals + j];
                    break;
...
px_bool PX_ANNImport(px_memorypool *mp, PX_ANN *pAnn, px_void *buffer, px_int size)
{
    ...
    if (!PX_ANNInitialize(mp, pAnn, dataheader.learningRate,
                          (PX_ANN_REGULARZATION)dataheader.regularzation, dataheader.regularization_rate))
    {
        return PX_FALSE;
    }
    ...
    for (i = 0; i < (px_int)dataheader.LayerCount; i++)
    {
        ...
        if (!PX_ANNAddLayer(pAnn, Layerheader.NeuralsCount, 0,
                            (PX_ANN_ACTIVATION_FUNCTION)Layerheader.activation, PX_ANN_LAYER_WEIGHT_INITMODE_CONST, 0))
        {
            PX_ANNFree(pAnn);
            return PX_FALSE;
        }
