...
void PX_Piano_DWGReverbSetCoeffs(PX_Piano_DWGReverb *preverb, px_float c1, px_float c3, px_float a,
                                 px_float mix, px_float Fs)
{
...
float PX_Piano_DWGReverbGo(PX_Piano_DWGReverb *preverb, float in)
{
    ...
    for (j = 0; j < 8; j++)
    {
        ...
        preverb->o[j] = (px_float)PX_LTIGo(&preverb->decay[j].LTI,
                                           PX_CircularBufferDelay(&preverb->delay[j], lengths[j]));
...
px_void PX_Piano_IntegratorInitialize(PX_Piano_Integrator *pIntegrator, px_float Fs,
                                      px_float inival /*= 0.0*/)
{
...
px_void PX_Piano_BanksHammerInitialize(PX_Piano_BanksHammer *pBankHammer, px_float f, px_float Fs,
                                       px_float m, px_float K, px_float p, px_float Z, px_float alpha)
{
    pBankHammer->Fs = Fs;
    pBankHammer->p = p;
    pBankHammer->K = K;
...
px_void PX_Piano_StulovHammerInitialize(PX_Piano_StulovHammer *pStulovHammer, px_float f,
                                        px_float Fs, px_float m, px_float K, px_float p, px_float Z, px_float alpha)
{
    pStulovHammer->Fs = Fs;
    pStulovHammer->p = p;
    pStulovHammer->K = K;
...
px_void PX_Piano_dwgInitialize(PX_Piano_dwg *pdwg, px_memorypool *mp, px_float z, px_int del1,
                               px_int del2, px_int commute, struct __PX_Piano_dwgs *parent)
{
    pdwg->parent = parent;
    pdwg->mp = mp;
    if (del1 > 1)
...
px_bool PX_Piano_dwgsInitialize(PX_Piano_dwgs *pdwgs, px_memorypool *mp, px_float f, px_float Fs,
                                px_float inpos, px_float c1, px_float c3, px_float B, px_float Z, px_float Zb, px_float Zh)
{
...
static px_float sigmoidal(px_float midi, px_float minV, px_float maxV, px_float ampL,
                          px_float ampR)
{
...
px_bool PX_PianoKeyInitialize(px_memorypool *mp, PX_PianoKey *pPianoKey,
                              PX_PianoKey_Parameters *param)
{
    ...
    for (k = 0; k < pPianoKey->nstrings; k++)
    {
        ...
        if (!PX_Piano_dwgsInitialize(&pPianoKey->string[k], mp, param->f * (1.0f + TUNE[k] * param->detune),
                                     param->Fs, pos, c1, c3, B, pPianoKey->Z, pPianoKey->Zb + (pPianoKey->nstrings - 1) * pPianoKey->Z,
                                     pPianoKey->Zh)) { return PX_FALSE; }
    }
    ...
    case 1:
        ...
        PX_Piano_StulovHammerInitialize(&pPianoKey->StulovHammer, param->f, param->Fs, m, K, p,
                                        pPianoKey->Z, alpha);
        break;
    case 2:

        PX_Piano_BanksHammerInitialize(&pPianoKey->BanksHammer, param->f, param->Fs, m, K, p, pPianoKey->Z,
                                       alpha);
        break;
    }
    return PX_TRUE;
...
#include "PX_Piano_mod.h"
    ...
    do
    {
        PX_PianoSoundboard_Parameters *param = (PX_PianoSoundboard_Parameters *)(piano_mod_default + sizeof(
                                                                                     PX_PianoKey_Parameters) * 88);
...
px_bool PX_PianoInitializeEx(px_memorypool *mp, PX_Piano *pPiano,
                             PX_PianoKey_Parameters keyparam[88], PX_PianoSoundboard_Parameters *soundboardparam)
{
    px_int i;
    PX_memset(pPiano, 0, sizeof(PX_Piano));
    for (i = 0; i < PX_COUNTOF(pPiano->keys); i++)
...
px_int PX_PianoKeyNameToIndex(const px_char keyName[])
{
    ...
    if (i <= 3) { column++; }
    if (column >= 0 && column < 8 && row + column * 12 < 88) { index = row + column * 12; }
    else { return -1; }
    return index;
}
...
px_bool PX_PianoModelInitializeNote(PX_PianoModel *pReverb, px_int i,
                                    PX_PianoKey_Parameters *keyparam, PX_PianoSoundboard_Parameters *soundboardparam)
{
    px_int j;
    px_float keyout;
...
#include "PX_Piano_mod.h"
    ...
    {
        soundboardparam = (PX_PianoSoundboard_Parameters *)(piano_mod_default + sizeof(
                                                                PX_PianoKey_Parameters) * 88);
    }
...
px_bool PX_PianoModelInitialize(PX_PianoModel *pmodel, PX_PianoKey_Parameters keyparam[88],
                                PX_PianoSoundboard_Parameters *soundboardparam)
{
    px_int i;
...
px_void PX_PianoModelGo(PX_PianoModel *pReverb, px_float out[], px_int count)
{
    ...
    for (j = 0; j < count; j++)
    {
        ...
        for (i = 0; i < PX_COUNTOF(pReverb->pulsation); i++)
        {
            if (pReverb->pulsation[i].note >= 0 && pReverb->pulsation[i].cursor >= 0 &&
                pReverb->pulsation[i].cursor < PX_COUNTOF(pReverb->note[i].pcm))
            {
