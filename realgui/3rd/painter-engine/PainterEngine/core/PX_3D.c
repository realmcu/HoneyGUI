...
px_void PX_3D_WorldInitialize(PX_3D_World *world, px_float x, px_float y, px_float z, px_float rotX,
                              px_float rotY, px_float rotZ, px_float scale)
{
...
px_bool PX_3D_CameraEulerInitialize(px_memorypool *mp, PX_3D_Camera *camera,
                                    px_point4D cameraPosition, px_float rotX, px_float rotY, px_float rotZ, px_float near, px_float far,
                                    px_float fov, px_float viewPortWidth, px_float viewPortHeight)
{
    ...
    camera->zbuffer = (px_float *)MP_Malloc(mp,
                                            (px_int)(sizeof(px_float) * camera->viewport_height * camera->viewport_width));
    return camera->zbuffer != PX_NULL;
}

px_bool PX_3D_CameraUVNInitialize(px_memorypool *mp, PX_3D_Camera *camera,
                                  px_point4D cameraPosition, px_point4D cameraTarget, px_float near, px_float far, px_float fov,
                                  px_float viewPortWidth, px_float viewPortHeight)
{
    PX_memset(camera, 0, sizeof(PX_3D_Camera));
...
px_void PX_3D_CameraSetPosition(PX_3D_Camera *camera, px_point4D cameraPosition,
                                px_point4D cameraTarget)
{
    camera->position = cameraPosition;
    camera->targetDirection = cameraTarget;
}
...
px_void PX_3D_RenderListTransform_Screen(PX_3D_RenderList *list, PX_3D_Camera *camera)
{
    ...
    for (i = 0; i < list->facestream.size; i++)
    {
        ...
        //inv y
        pface->transform_vertex[0].position.y = -pface->transform_vertex[0].position.y +
                                                camera->viewport_height;
        pface->transform_vertex[1].position.y = -pface->transform_vertex[1].position.y +
                                                camera->viewport_height;
        pface->transform_vertex[2].position.y = -pface->transform_vertex[2].position.y +
                                                camera->viewport_height;
    }
}
...
px_void PX_3D_RenderListPixelShader(px_surface *psurface, px_int x, px_int y, px_float z,
                                    px_float u, px_float v, px_point4D normal, px_texture *pTexture,
                                    px_color color/*px_float *zbuffer,px_int zw*/)
{
    //texture mapping
...
static px_void PX_3D_RenderListRasterization(px_surface *psurface, PX_3D_RenderList *pList,
                                             PX_3D_Vertex p0, PX_3D_Vertex p1, PX_3D_Vertex p2, \
                                             px_texture *ptexture, px_color color, px_int view_width, px_int view_height, px_float zbuffer[],
                                             px_int zw)
{
    px_int ix, iy;
    ...
    for (y = (px_int)(y0 + 0.5f) + 0.5f; y <= midy; y++)
    {
        ...
        for (ix = (px_int)(xleft + 0.5); ix < (px_int)(xright + 0.5f); ++ix)
        {
            ...
            if (ix > 0 && ix < view_width && iy >= 0 && iy < view_height)
            {
                ...
                {
                    PX_3D_RenderListPixelShader(psurface, ix, iy, originalZ, s, t, p0.normal, ptexture,
                                                color/*,zbuffer,zw*/);
                }
            }
...
px_void PX_3D_Scene(PX_3D_RenderList *list, PX_3D_World *world, PX_3D_Camera *camera)
{
    ...
    PX_memset(camera->zbuffer, 0, (px_int)(sizeof(px_float)*camera->viewport_height *
...
px_void PX_3D_Present(px_surface *psurface, PX_3D_RenderList *list, PX_3D_Camera *camera)
{
    ...
    if (ptex || clr._argb.ucolor)
    {
        ...
        {
            pface = PX_VECTORAT(PX_3D_Face, &list->facestream, i);
            if (!PX_isPointInRect(PX_POINT(pface->transform_vertex[0].position.x,
                                           pface->transform_vertex[0].position.y, 0), PX_RECT(0, 0, 1.f * psurface->width,
                                                   1.f * psurface->width)))
            {
                continue;
            }
            if (!PX_isPointInRect(PX_POINT(pface->transform_vertex[1].position.x,
                                           pface->transform_vertex[1].position.y, 0), PX_RECT(0, 0, 1.f * psurface->width,
                                                   1.f * psurface->width)))
            {
                continue;
            }
            if (!PX_isPointInRect(PX_POINT(pface->transform_vertex[2].position.x,
                                           pface->transform_vertex[2].position.y, 0), PX_RECT(0, 0, 1.f * psurface->width,
                                                   1.f * psurface->width)))
            {
                continue;
            }
            ...
            if (!(pface->state & PX_3D_FACESTATE_BACKFACE || pface->state & PX_3D_FACESTATE_CLIPPED))
            {
                if (list->PX_3D_PRESENTMODE & PX_3D_PRESENTMODE_PURE)
                {
                    ...
                    PX_3D_GPU_RenderListRasterization(psurface, list, pface->transform_vertex[0],
                                                      pface->transform_vertex[1], pface->transform_vertex[2]);
                }
                else
                {
                    PX_3D_RenderListRasterization(psurface, list, pface->transform_vertex[0],
                                                  pface->transform_vertex[1], pface->transform_vertex[2], ptex, PX_COLOR(0, 0, 0, 0),
                                                  (px_int)camera->viewport_width, (px_int)camera->viewport_height, camera->zbuffer,
                                                  (px_int)camera->viewport_width);
                }
    ...
    if (list->PX_3D_PRESENTMODE & PX_3D_PRESENTMODE_POINT)
    {
        ...
        {
            pface = PX_VECTORAT(PX_3D_Face, &list->facestream, i);
            PX_GeoDrawSolidCircle(psurface, (px_int)pface->transform_vertex[0].position.x,
                                  (px_int)pface->transform_vertex[0].position.y, 2, pface->transform_vertex[0].clr);
            PX_GeoDrawSolidCircle(psurface, (px_int)pface->transform_vertex[1].position.x,
                                  (px_int)pface->transform_vertex[1].position.y, 2, pface->transform_vertex[1].clr);
            PX_GeoDrawSolidCircle(psurface, (px_int)pface->transform_vertex[2].position.x,
                                  (px_int)pface->transform_vertex[2].position.y, 2, pface->transform_vertex[2].clr);
    ...
    if (list->PX_3D_PRESENTMODE & PX_3D_PRESENTMODE_LINE)
    {
        for (i = 0; i < list->facestream.size; i++)
        {
            ...
            if (!(pface->state & PX_3D_FACESTATE_BACKFACE || pface->state & PX_3D_FACESTATE_CLIPPED))
            {
                PX_GeoDrawLine(psurface, (px_int)pface->transform_vertex[0].position.x,
                               (px_int)pface->transform_vertex[0].position.y, (px_int)pface->transform_vertex[1].position.x,
                               (px_int)pface->transform_vertex[1].position.y, 1, pface->transform_vertex[0].clr);
                PX_GeoDrawLine(psurface, (px_int)pface->transform_vertex[1].position.x,
                               (px_int)pface->transform_vertex[1].position.y, (px_int)pface->transform_vertex[2].position.x,
                               (px_int)pface->transform_vertex[2].position.y, 1, pface->transform_vertex[1].clr);
                PX_GeoDrawLine(psurface, (px_int)pface->transform_vertex[2].position.x,
                               (px_int)pface->transform_vertex[2].position.y, (px_int)pface->transform_vertex[0].position.x,
                               (px_int)pface->transform_vertex[0].position.y, 1, pface->transform_vertex[2].clr);
            }
...
px_void PX_3D_RenderListCullOutSide(PX_3D_RenderList *list, PX_3D_Camera *camera)
{
    ...
    for (i = 0; i < list->facestream.size; i++)
    {
        ...
        else\ if (pface->transform_vertex[0].position.y < -camera->viewplane_height / 2 && \
                  pface->transform_vertex[1].position.y < -camera->viewplane_height / 2 && \
                  pface->transform_vertex[2].position.y < -camera->viewplane_height / 2)
        {
            //down
        ...
        else \ if (pface->transform_vertex[0].position.x < -camera->viewplane_width / 2 && \
...
px_void PX_3D_RenderListTransform(PX_3D_RenderList *list, px_matrix mat,
                                  PX_3D_RENDERLIST_TRANSFORM t)
{
    switch (t)
    {
...
px_bool PX_3D_RenderListInitialize(px_memorypool *mp, PX_3D_RenderList *list,
                                   px_dword PX_3D_PRESENTMODE, PX_3D_CULLMODE cullmode, px_texture *ptexture)
{
    list->mp = mp;
...
{
    px_int i, j;
    px_point *pmatrix = (px_point *)MP_Malloc(mp,
                                              sizeof(px_point) * pTexture->width * pTexture->height);
    px_float *gradient_x = (px_float *)MP_Malloc(mp,
                                                 sizeof(px_float) * pTexture->width * pTexture->height);
    px_float *gradient_y = (px_float *)MP_Malloc(mp,
                                                 sizeof(px_float) * pTexture->width * pTexture->height);
    ...
    for (j = 0; j < pTexture->height; j++)
    {
        ...
        {
            pmatrix[j * pTexture->width + i].x = (px_float)(-gradient_x[j * pTexture->width + i] / PX_sqrtd(
                                                                gradient_x[j * pTexture->width + i] * gradient_x[j * pTexture->width + i] + gradient_y[j *
                                                                        pTexture->width + i] * gradient_y[j * pTexture->width + i] + 1));
            pmatrix[j * pTexture->width + i].y = (px_float)(-gradient_y[j * pTexture->width + i] / PX_sqrtd(
                                                                gradient_x[j * pTexture->width + i] * gradient_x[j * pTexture->width + i] + gradient_y[j *
                                                                        pTexture->width + i] * gradient_y[j * pTexture->width + i] + 1));
            pmatrix[j * pTexture->width + i].z = (px_float)(1 / PX_sqrtd(gradient_x[j * pTexture->width + i] *
                                                                         gradient_x[j * pTexture->width + i] + gradient_y[j * pTexture->width + i] * gradient_y[j *
                                                                                 pTexture->width + i] + 1));
        }
    }
