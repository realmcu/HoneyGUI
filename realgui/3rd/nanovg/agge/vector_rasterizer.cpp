...
void vector_rasterizer::append(const vector_rasterizer &source, int dx, int dy)
{
    ...
    const cell &last = *(_cells.end() -
                         1); // Relying on a guarantee that we had at least one cell prior this call.
...
void vector_rasterizer::sort(bool was_presorted)
{
    ...
    else
    {
        ...
        phistogram_x--;
        for (cells_container::iterator i = _cells.begin(), j = _x_sorted_cells.begin(), e = _cells.end();
             i != e; ++i)
        {
