...
static struct GLNVGtexture *glnvg__allocTexture(struct GLNVGcontext *gl)
{
    ...
    if (tex == NULL)
    {
        if (gl->ntextures + 1 > gl->ctextures)
        {
            ...
            gl->textures = (struct GLNVGtexture *)BX_REALLOC(gl->allocator, gl->textures,
...
static int nvgRenderUpdateTexture(void *_userPtr, int image, int x, int y, int w, int h,
...
)
{
    ...
    else
    {
        ...
        frag->scissorScale[0] = sqrtf(scissor->xform[0] * scissor->xform[0] + scissor->xform[2] *
                                      scissor->xform[2]) / fringe;
        frag->scissorScale[1] = sqrtf(scissor->xform[1] * scissor->xform[1] + scissor->xform[3] *
...
static void nvgRenderFlush(void *_userPtr)
{
    ...
    if (gl->ncalls > 0)
    {
        ...
        for (uint32_t ii = 0, num = gl->ncalls; ii < num; ++ii)
        {
            ...
            gl->state = BGFX_STATE_BLEND_FUNC_SEPARATE(blend->srcRGB, blend->dstRGB, blend->srcAlpha,
                                                       blend->dstAlpha)
...
static struct GLNVGcall *glnvg__allocCall(struct GLNVGcontext *gl)
{
    ...
    if (gl->ncalls + 1 > gl->ccalls)
    {
        ...
        gl->calls = (struct GLNVGcall *)BX_REALLOC(gl->allocator, gl->calls,
...
NVGcontext *nvgCreateBGFX(int32_t _edgeaa, uint16_t _viewId, uint32_t _width, uint32_t _height,
                          SDL_Window *_window)
{
...
NVGLUframebuffer_bgfx *nvgluCreateFramebufferByViewId(NVGcontext *ctx, int32_t width,
                                                      int32_t height, int32_t imageFlags, uint16_t viewId)
{
...
NVGLUframebuffer_bgfx *nvgluCreateFramebuffer(NVGcontext *_ctx, int32_t _width, int32_t _height,
                                              int32_t _imageFlags)
{
