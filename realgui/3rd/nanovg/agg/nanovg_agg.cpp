...
void renderPaint(AGGNVGcontext *agg, NVGpaint *paint)
{
    ...
    if (paint->image > 0)
    {
        ...
        agg::trans_affine img_mtx(invxform[0], invxform[1], invxform[2], invxform[3], invxform[4],
                                  invxform[5]);
        ...
        case NVG_TEXTURE_RGBA:
            {
                ...
                agg::span_image_filter_rgba_bilinear_clip<agg::pixfmt_rgba32, agg::span_interpolator_linear<> > sg(
                    img_pixf, agg::rgba(0, 0, 0, 0), interpolator);
        ...
        case NVG_TEXTURE_BGRA:
            {
                ...
                agg::span_image_filter_rgba_bilinear_clip<agg::pixfmt_bgra32, agg::span_interpolator_linear<> > sg(
                    img_pixf, agg::rgba(0, 0, 0, 0), interpolator);
        ...
        case NVG_TEXTURE_RGB:
            {
                ...
                agg::span_image_filter_rgb_bilinear_clip<agg::pixfmt_rgb24, agg::span_interpolator_linear<> > sg(
                    img_pixf, agg::rgba(0, 0, 0, 0), interpolator);
        ...
        case NVG_TEXTURE_BGR:
            {
                ...
                agg::span_image_filter_rgb_bilinear_clip<agg::pixfmt_bgr24, agg::span_interpolator_linear<> > sg(
                    img_pixf, agg::rgba(0, 0, 0, 0), interpolator);
...
static void nvgInitAGG(AGGNVGcontext *agg, NVGparams *params, uint32_t w, uint32_t h,
                       uint32_t stride,
...
NVGcontext *nvgCreateAGG(uint32_t w, uint32_t h, uint32_t stride, enum NVGtexture format,
                         uint8_t *data)
{
    NVGparams params;
    NVGcontext *ctx = NULL;
