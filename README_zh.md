...
HoneyGUI 遵循四段式版本号命名规范：

**格式：** `v{主版本}.{次版本}.{补丁版本}.{构建版本}`

- **主版本号（MAJOR）**：重大架构变更或产品级别更新
- **次版本号（MINOR）**：API 变更（包括不兼容的 API 变更）
- **补丁版本号（PATCH）**：Bug 修复和小的改进
- **构建版本号（BUILD）**：构建编号（通常为 0）
...
部分版本带有 `.RVD` 后缀（例如 `v1.1.1.0.RVD`），表示该版本专门针对 **RVisualDesigner** 工具集成优化的发布版本。

### 版本信息

项目使用 `tool/git_generate_version.py` 自动生成版本头文件，包含以下信息：
- 版本标签
- 分支名称
- 提交哈希值
- 构建日期
