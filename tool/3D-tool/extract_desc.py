...
TOOL_VERSION = 4  # v4: multi-animation g3m format (animation table)
...
def extract_gltf_desc(gltf_filename: str, bin_filename: str, txt_filename: str):
    ...
    try:
        ...
        # Accumulate counts across ALL animations (v4 multi-animation format)
        channel_count = 0
        sampler_count = 0
        ...
        # Process skins
        # Blob write order must match the C exe (extract_gltf_desc.c): IBM first, then joints.
        # Absent-value sentinel is 0 (not 0xFFFFFFFF), also matching C.
        skin_data = []
        for skin in gltf.skins if gltf.skins else []:
            # Inverse bind matrices (transpose column-major -> row-major)
            ibm_offset = 0
            if skin.inverseBindMatrices is not None:
                ...
                for mat in ibm_data:
                    mat_transposed = mat.reshape((4, 4)).T
                    blob.write(mat_transposed.astype(np.float32).tobytes())

            # Joint node indices
            joints_offset = 0
            joints_count = len(skin.joints) if skin.joints else 0
            if joints_count > 0:
        ...
        # Process animations
        # Flat channels/samplers across ALL animations + animation table (v4)
        animation_data = {'channels': [], 'samplers': [], 'animations': []}
        if gltf.animations:
            for anim in gltf.animations:
                channel_start = len(animation_data['channels'])
                sampler_start = len(animation_data['samplers'])

                # Channels (sampler_index stays relative to this animation)
                ...
                # Animation table entry: [start, count) ranges into the flat arrays
                animation_data['animations'].append({
                    'channel_start': channel_start,
                    'channel_count': len(animation_data['channels']) - channel_start,
                    'sampler_start': sampler_start,
                    'sampler_count': len(animation_data['samplers']) - sampler_start
                })
        ...
        with open(bin_filename, 'wb') as f:
            ...
            f.write(struct.pack('<I', animation_count))
            f.write(struct.pack('<I', material_count))
            f.write(struct.pack('<I', texture_count))
            
            # Calculate offsets
            current_offset = 16 + 88 # file head(16) + g3m_header_t(22 * uint32)
            ...
            animations_offset = current_offset
            current_offset += animation_count * 16  # sizeof(g3m_animation_on_disk_t)
            ...
            f.write(struct.pack('<I', animations_offset))
            ...
            # Write animation table (g3m_animation_on_disk_t)
            for a in animation_data['animations']:
                f.write(struct.pack('<I', a['channel_start']))
                f.write(struct.pack('<I', a['channel_count']))
                f.write(struct.pack('<I', a['sampler_start']))
                f.write(struct.pack('<I', a['sampler_count']))
        ...
        print(f"Animation Count:     {animation_count}")
...
def create_minimal_gltf_binary(bin_filename: str):
    ...
    with open(bin_filename, 'wb') as f:
        ...
        # Empty g3m_header_t (10 counts + 10 offsets + 2 data blob = 22 uint32)
        for _ in range(22):
...
def main():
    ...
    print("=" * 60)
    print(f"HoneyGUI 3D Model Descriptor Generator v{TOOL_VERSION}")
    print("=" * 60)
