...
char *get_img_file(const char *filename)
{
    ...
    char *binFilename = (char *)malloc(len + 5);
    strncpy(binFilename, filename, len);
    strcpy(binFilename + len, ".bin");
    return binFilename;
}
...
unsigned char *read_array_from_file(const char *filename, uint32_t *length)
{
    ...
    size_t read_size = fread(array, 1, file_size, file);
    fclose(file);
...
void save_obj_desc_to_binary_file(tinyobj_description_t *desc, const char *filename)
{
    ...
    file_header.face_type = face_type;
