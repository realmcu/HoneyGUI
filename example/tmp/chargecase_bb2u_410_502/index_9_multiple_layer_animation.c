...
static uint32_t *gui_watchface_array[38] = {0};
static uint32_t *gui_a1_array[21] = {0};
static uint32_t *gui_a2_array[21] = {0};
static uint32_t *gui_a3_array[21] = {0};
static uint32_t *gui_a4_array[21] = {0};
...
static void switch_widget_play_watchface(void *p)
{
    ...
    int array_size = ARRAY_SIZE(gui_watchface_array);
...
static void switch_widget_play_watchface2(void *p)
{
    ...
    uint64_t dur = ms_cur - ms_start;
    int array_size = ARRAY_SIZE(gui_a1_array);
    int index = (array_size - 1) * dur / WATCHFACE_DUR_MS;
    index = index % array_size;
...
static void switch_widget_play_watchface3(void *p)
{
    ...
    uint64_t dur = ms_cur - ms_start;
    int array_size = ARRAY_SIZE(gui_a2_array);
    int index = (array_size - 1) * dur / WATCHFACE_DUR_MS;
    index = index % array_size;
...
static void switch_widget_play_watchface4(void *p)
{
    ...
    uint64_t dur = ms_cur - ms_start;
    int array_size = ARRAY_SIZE(gui_a3_array);
    int index = (array_size - 1) * dur / WATCHFACE_DUR_MS;
    index = index % array_size;
...
static void switch_widget_play_watchface5(void *p)
{
    ...
    uint64_t dur = ms_cur - ms_start;
    int array_size = ARRAY_SIZE(gui_a4_array);
    int index = (array_size - 1) * dur / WATCHFACE_DUR_MS;
    index = index % array_size;
...
static void multiple_layer_animation_switch_in(gui_view_t *view)
{
    ...
    if (gui_watchface_array[0] == NULL)
    {
        uint32_t *watchface_array[] =
        {
        ...
        memcpy(gui_watchface_array, watchface_array, sizeof(watchface_array));
    }
    if (gui_a1_array[0] == NULL)
    {
        uint32_t *a1_array[] =
        {
            A1FRAME_000_BIN,
        ...
        memcpy(gui_a1_array, a1_array, sizeof(a1_array));
    }
    if (gui_a2_array[0] == NULL)
    {
        uint32_t *a2_array[] =
        {
            A2FUEL_000_BIN,
        ...
        memcpy(gui_a2_array, a2_array, sizeof(a2_array));
    }
    if (gui_a3_array[0] == NULL)
    {
        uint32_t *a3_array[] =
        {
            RPM_000_BIN,
        ...
        memcpy(gui_a3_array, a3_array, sizeof(a3_array));
    }
    if (gui_a4_array[0] == NULL)
    {
        uint32_t *a4_array[] =
        {
            SPEED_000_BIN,
        ...
        memcpy(gui_a4_array, a4_array, sizeof(a4_array));
    }

    {
        gui_img_t *img =
            gui_img_create_from_mem(view,
                                    0, WATCHFACE_FRAME_0000_BIN, 0, 0, 0, 0);
