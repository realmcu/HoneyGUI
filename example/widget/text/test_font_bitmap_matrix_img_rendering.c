...
*****************************************************************************************
  * @file test_font_bitmap_matrix_img_rendering.c
 ...
 *                            Macros
 *============================================================================*/
#define CURRENT_VIEW_NAME "test_font_bitmap_matrix_img_rendering"
#define FONT_NUM 4
...
{
    left_view = gui_view_descriptor_get("test_font_bitmap_matrix_rendering");
    right_view = gui_view_descriptor_get("test_font_bitmap_img_rendering");
    return 0;
}
...
static void ui_text_font(gui_view_t *view)
{
    ...
    for (int i = 0; i < FONT_NUM; i++)
    {
        ...
        gui_text_use_matrix_by_img(text1[i], 1);
    }
    for (int i = 0; i < FONT_NUM; i++)
    {
        ...
        gui_text_use_matrix_by_img(text2[i], 1);
    }
    for (int i = 0; i < FONT_NUM; i++)
    {
    ...
    // for (int i = 0; i < FONT_NUM; i++)
    // {
    //     text8[i] = gui_text_create(view, "text8", i % 2 * dc->screen_width / 2, font_size * (i / 2) + font_size * FONT_NUM * 3 / 2, 300, font_size);
    //     gui_text_set(text8[i], text_string[i], GUI_FONT_SRC_MAT, color[i], strlen(text_string[i]), font_size);
    //     gui_text_type_set(text8[i], font32b8, FONT_SRC_MEMADDR);
    //     gui_text_use_matrix_by_img(text8[i], 1);
    // }
