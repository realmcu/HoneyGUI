...
const static gui_view_descriptor_t *watchface_add_view = NULL;
const static gui_view_descriptor_t *watchface_big_num_view = NULL;
const static gui_view_descriptor_t *watchface_flow_view = NULL;
...
static void switch_app_cb(void *obj)
{
    ...
    else if (strcmp(obj_name, "FLOW") == 0)
    {
        // gui_view_switch_direct(current_view, gui_view_descriptor_get("watchface_view"), SWITCH_OUT_ANIMATION_ZOOM,
        //                        SWITCH_IN_ANIMATION_ZOOM);
    }
...
static void note_design(gui_obj_t *obj, void *p)
{
    ...
    const char *watchface_names[] = {"Big Number", "Flow", "Video", "Add Watchface",};
    const void *watchface_icons[] = {WATCHFACE_BIG_NUM_BIN, WATCHFACE_FLOW_BIN, WATCHFACE_VIDEO_BIN, ADD_WATCHFACE_BIN,};

    uint16_t offset_x = 100;
    ...
    if (index == 1)
    {
        gui_img_t *img = gui_img_create_from_mem(obj, "FLOW", watchface_icons[index], offset_x, offset_y, 0,
                                                 0);

        gui_text_t *label = gui_text_create(obj, "watchface_flow", offset_x + 40, 80, 200, 40);
        gui_text_set(label, watchface_names[index], GUI_FONT_SRC_TTF, gui_rgb(255, 255, 255),
                     strlen(watchface_names[index]), 20);
        gui_text_type_set(label, SF_COMPACT_TEXT_BOLD_BIN, FONT_SRC_MEMADDR);
        gui_text_mode_set(label, LEFT);
...
static void watchface_select_design(gui_view_t *view)
{
    ...
    int space = 10;
    int length = 200;
    list = gui_list_create(parent, "list", 0, 0, 0, 0, length, space, HORIZONTAL, note_design, NULL,
                           false);
