    ...
    rt_size_t len)
{
    ...
    sprintf((char *)buf, "%s%c%ld", (char *) & (cctx->fpath[0]), insert_0, (long)file_buf.st_size);
...
{
    rt_err_t res;
    struct custom_ctx *ctx = rt_calloc(1, (1 + 2 + 1024 + 2) + sizeof(*ctx));
    rt_memset(ctx, 0xaa, sizeof(*ctx));
