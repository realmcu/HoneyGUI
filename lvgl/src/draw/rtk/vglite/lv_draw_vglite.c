...
static lv_res_t lv_draw_vglite_img(lv_draw_ctx_t *draw_ctx, const lv_draw_img_dsc_t *draw_dsc,
...
static void lv_draw_vglite_rect(lv_draw_ctx_t *draw_ctx, const lv_draw_rect_dsc_t *dsc,
                                const lv_area_t *coords);

static lv_res_t lv_draw_vglite_bg(lv_draw_ctx_t *draw_ctx, const lv_draw_rect_dsc_t *dsc,
                                  const lv_area_t *coords);
...
static void lv_draw_vglite_arc(lv_draw_ctx_t *draw_ctx, const lv_draw_arc_dsc_t *dsc,
...
void lv_draw_vglite_ctx_init(lv_disp_drv_t *drv, lv_draw_ctx_t *draw_ctx)
{
    ...
    vglite_draw_ctx->base_draw.draw_img = lv_draw_vglite_img;
...
static void lv_draw_vglite_init_buf(lv_draw_ctx_t *draw_ctx)
{
    lv_gpu_vglite_init_buf(draw_ctx->buf, draw_ctx->buf_area, lv_area_get_width(draw_ctx->buf_area));
}
...
static void lv_draw_vglite_blend(lv_draw_ctx_t *draw_ctx, const lv_draw_sw_blend_dsc_t *dsc)
{
    ...
    if (dsc->mask_buf == NULL && dsc->blend_mode == LV_BLEND_MODE_NORMAL &&
        lv_area_get_size(&blend_area) >= LV_GPU_VG_LITE_SIZE_LIMIT)
    {
        ...
        else
        {
            ...
            done = (lv_gpu_vglite_blit(dest_buf, &blend_area, dest_stride,
                                   ...
                                   const lv_area_t *coords, const void *src)
{
    ...
    if (img->header.cf >= LV_IMG_CF_INDEXED_1BIT && img->header.cf <= LV_IMG_CF_INDEXED_8BIT)
    {
        return lv_gpu_vglite_index(img, &blend_area, draw_dsc);
    }
    else if (img->header.cf >= LV_IMG_CF_ALPHA_1BIT && img->header.cf <= LV_IMG_CF_ALPHA_8BIT)
    {
        return lv_gpu_vglite_alpha_only(img, &blend_area, draw_dsc);
    }
    return LV_RES_INV;
}
                                ...
                                const lv_area_t *coords)
{
    ...
    //    DBG_DIRECT("bg %d, img %d, border %d, outline %d", dsc->bg_opa, dsc->bg_img_opa, dsc->border_opa, dsc->outline_opa);
    /* Draw the background */
